...
sub build_test_context
{
	...
	my $shared_ext = `uname -s`;
	chomp($shared_ext);
	$shared_ext = $shared_ext eq 'Darwin' ? 'dylib' : 'so';
	...
	for my $flag (@link_flags)
	{
		...
		$need_helper_shared = 1 if $flag =~ m/(?:\.__SHARED_EXT__|\.dylib|\.so)$/;
	}
...
sub compile_helper_inputs
{
	...
	for my $src (@{$ctx->{helper_sources}})
	{
		...
		{
			my @cmd = $ctx->{shared_ext} eq 'dylib'
				? (@{$ctx->{host_cc}}, '-dynamiclib', '-o', $sharedlib, $obj,
				   '-Wl,-install_name,@rpath/' . "lib$name.$ctx->{shared_ext}")
				: (@{$ctx->{host_cc}}, '-shared', '-o', $sharedlib, $obj);
			my $helper_status = run_command_capture(
				cmd => \@cmd,
				stdout => $ctx->{impl_stdout},
				stderr => $ctx->{impl_stderr},
...
sub finish_one_test
{
	...
	if ($impl_status != 0)
	{
		...
		my @explicit_provider_flags =
			grep { $_ =~ m/\.(?:o|a|dylib|so)$/ && -e $_ } @{$ctx->{link_flags}};
		my @nm_referencers = (
			@{$ctx->{objfiles}},
