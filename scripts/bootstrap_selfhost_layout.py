...
def source_list(repo_root: Path, frontend: str, layout: str, test_runner: bool):
    ...
    if layout == "pa37-selfhost" and test_runner:
...
def pa36_object_path(build_root: Path, relative: Path, stem: str, test_runner: bool):
    ...
    raise SystemExit(f"pa37-selfhost layout does not know how to place {relative}")
...
def object_path(repo_root: Path,
                ...
                layout: str,
                test_runner: bool):
    if layout == "pa37-selfhost":
        relative = src.relative_to(repo_root)
        return pa36_object_path(build_dir, relative, src.stem, test_runner)
