...
if (scalar(@ARGV) != 2)
{
	die "Usage: check_pa11_goto_lifetime_cleanup.pl " .
		"<cppgm++> <test-or-directory>\n";
}

my ($app, $root) = @ARGV;
my @tests = collect_tests($root, qr/goto-lifetime-cleanup\.cpp$/);
die "No PA11 goto-lifetime-cleanup properties found under $root\n" if !@tests;

for my $test (@tests)
{
	my $directory = tempdir('pa11-goto-lifetime-XXXXXX', TMPDIR => 1,
		CLEANUP => 1);
