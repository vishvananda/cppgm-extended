...
def parse_pa37_layout(path: Path) -> tuple[List[str], Dict[str, str]]:
    checkpoints: List[str] = []
    ...
    for raw_line in path.read_text().splitlines():
        ...
        if line.startswith("CHECKPOINTS = "):
            ...
            continue
        match = re.match(r"^PA37_CHECKPOINT_FOR_(pa\d+) = (\S+)$", line)
        if match:
            stage_to_checkpoint[match.group(1)] = match.group(2)
    if not checkpoints:
        raise SystemExit("failed to parse CHECKPOINTS from pa37/Makefile")
...
def infer_flavor(target: str, assignments: Dict[str, str]) -> str:
    if "PA37_COMPILER_FLAVOR" in assignments and assignments["PA37_COMPILER_FLAVOR"]:
        return assignments["PA37_COMPILER_FLAVOR"]
...
def infer_output_suffix(flavor: str, target: str, assignments: Dict[str, str]) -> str:
    if "PA37_OUTPUT_SUFFIX" in assignments and assignments["PA37_OUTPUT_SUFFIX"]:
        return assignments["PA37_OUTPUT_SUFFIX"]
...
def resolve_obj_root_base(assignments: Dict[str, str]) -> Path:
    raw = assignments.get("PA37_OBJ_ROOT_BASE", "../obj/pa37")
    return (PA37_DIR / raw).resolve()


def resolve_bin_root_base(assignments: Dict[str, str], obj_root_base: Path) -> Path:
    raw = assignments.get("PA37_BIN_ROOT_BASE")
    if raw:
        return (PA37_DIR / raw).resolve()
...
def resolve_generated_root(assignments: Dict[str, str], obj_root_base: Path) -> Path:
    raw = assignments.get("PA37_GENERATED")
    if raw:
        return (PA37_DIR / raw).resolve()
...
def infer_cxx_dep(assignments: Dict[str, str],
    ...
    if flavor == "inception" and assignments.get("PA37_COMPILER_FLAVOR") != "inception":
        return bin_root_base / "selfhost" / "cppgm++-self"
    explicit = resolve_cxx_dep(assignments.get("CXX"))
    if explicit is not None:
        return explicit
    if flavor == "selfhost":
        return resolve_pa37_path("../dev/cppgm++")
    if flavor == "inception":
        return bin_root_base / "selfhost" / "cppgm++-self"
    return None
...
def manual_build_spec(args: argparse.Namespace,
    ...
    if not args.target:
        return None
    obj_root_base = (REPO_ROOT / args.obj_root_base).resolve() if args.obj_root_base else (REPO_ROOT / "obj" / "pa37")
...
def shared_object_stem(stem: str) -> str:
    # pa37/Makefile uses $(basename $(notdir <source>)) for shared objects.
...
def resolve_pa37_path(raw: str) -> Path:
    path = Path(raw)
    if path.is_absolute():
        return path
    return (PA37_DIR / path).resolve()
...
def active_tasks_for_build(root_pid: Optional[int],
    ...
    for pid in sorted(candidate_pids):
        ...
        output_path = resolve_pa37_path(output) if output else None
...
def discover_build_processes(processes: Sequence[ProcessInfo]) -> List[ProcessInfo]:
    ...
    for process in processes:
        ...
        try:
            ...
            continue
        if c_index + 1 >= len(argv) or argv[c_index + 1] != "pa37":
            continue
...
def render_screen(views: Sequence[BuildView],
    ...
    lines = [style(f"PA37 Build Watch  {now}  refresh={interval:.1f}s",
                   Color.BOLD,
                   Color.CYAN,
                   enabled=use_color), ""]
    if stale:
        lines.append(style("No active pa37 build detected; showing last captured snapshot.",
                           Color.YELLOW,
                           enabled=use_color))
        lines.append("")
    if not views:
        lines.append("No active pa37 build detected.")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Watch pa37 build progress, object counts, and active compile files."
    )
    ...
    parser.add_argument("--pid", type=int,
                        help="watch one active top-level 'make -C pa37' pid")
    parser.add_argument("--target",
                        help="manual target to watch even if no active make process exists")
    parser.add_argument("--obj-root-base",
                        help="manual PA37_OBJ_ROOT_BASE relative to repo root")
    parser.add_argument("--generated-root",
                        help="manual PA37_GENERATED relative to repo root")
    ...
    parser.add_argument("--output-suffix",
                        help="manual PA37_OUTPUT_SUFFIX, default inferred from flavor")
...
def main() -> int:
    ...
    checkpoints, stage_to_checkpoint = parse_pa37_layout(PA37_MAKEFILE)
