...
2. Ordinary function analysis held a `FunctionInfo&` while parameter lifetime
   ...
   as a false function-try-block diagnosis in a PA17 friend-constructor case.
3. Semantic telemetry did not expose parser time, and total front-end elapsed
   ...
   slow-path audit.
4. The function-fact repair pushed `pa7_semantic.cpp` above the file audit's
...
- Moved condition analysis into `pa7_condition_semantic.cpp` and registered it
  in the compiler source set, reducing `pa7_semantic.cpp` to 2,918 lines.
- Replaced the checkpoint-only PA30 documents with this PA-wide architecture
...
- PA17 friend-constructor reproducer: passing normally and under Valgrind with
...
- PA30 final file audit: pass with 22 inherited advisory warnings.
- PA1-PA30 required report: 4,907/4,907 tests and 35/35 stages passing.
