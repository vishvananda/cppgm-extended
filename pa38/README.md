...
- The **quality bar** is what the optimized output must reach on the course
  ...
  size envelope (`x.ref.expect`), and the behaviour programs stay inside
  the performance envelope (`make test-perf`).  The Quality Bar section is
  normative.
- **One design** is how the course solution meets that bar: its reactive
  placement, its planner, and the placement decisions listed in the One
  Design section.  That section is a worked example.  A different
  allocator that meets the bar is a correct PA38; a course fixture never
...
- when `x.ref.expect` is present, the machine-IR dump meets every predicate
  ...
  10% tolerance plus one (`../scripts/make_ir_expect.pl`), and outcome
...
`make test-perf` regenerates the behaviour programs, runs each under
Cachegrind, and requires its instruction count to stay within
`PERF_TOLERANCE_PERCENT` (10) of the course solution's `x.ref.ir`.  It is skipped when valgrind is not
installed.  This is the one place the course compares your design with the
course solution's on outcome: an allocator within ten percent passes.

The rule for a new course fixture: it states the contract or the outcome it
tests.  A fixture whose only justification is the placement the course
solution's planner produces belongs in the regression lane.
...
for the contract-shape fixtures named in Quality Bar, and for the
regression lane.
