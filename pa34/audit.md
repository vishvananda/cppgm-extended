# PA34 Inception Audit

This audit treats the checked-in PA34 stage, not the prior green run, as the
...
1. Establish provenance: review the PA34 stage commit against `spec.md`, the
   PA34 contract, fixed source sets, Makefile generation graph, changed source,
...
3. Audit the PA34 build graph for a fixed source manifest, immediate-generation
   ...
   targets.
4. Search implementation and build changes for PA34/test/source-name branches,
...
6. Run the file audit, host PA1--PA33 report, self PA1--PA5 ladder, pptoken
...
1. The PA34 stage is one commit, `d994aab5` (`Implement PA34 reproducible
   inception`), on the audited PA33 parent `28bebf67`.  The canonical PA34
...
2. The production path remains source/tokens -> compact syntax and canonical
   ...
   recognizes PA34, a test filename, or compiler-source spelling.
3. The stage's `InternalIdentityClassifier` was an architecture defect.  PA10
...
4. Two other identity/order defects were present in PA34 changes.  Cached
...
5. PA5's late class-name preindex scanned nested inline bodies repeatedly.
...
6. Searches of the PA34 build/source changes found no disabled semantic work,
   whole-program retry loop, PA34-only compiler branch, source-name shortcut,
...
7. All 49 stage reducers are in the earliest assignment whose observable
   ...
   - PA5 parser/name facts: `known-function-name-with-type-marker`,
   ...
   - PA7 lexical/semantic identity, lookup, access, and calls:
   ...
   - PA10 typed lowering/symbol identity:
   ...
   - PA11 object lifetime/operator lowering and emission order:
   ...
   - PA14--PA16 templates and constant semantics:
     `local-type-namespace-template-specialization-abi` (PA14),
     `braced-constructor-function-pack` (PA15), and
     `address-of-incomplete-reference` plus
     `noexcept-in-suppressed-specialization` (PA16).
   - PA20 lambda identity/access: `cached-lambda-rebinds-automatic-capture`,
     `lambda-captures-this-for-out-of-class-member-call`,
     `lambda-implicitly-captures-this-for-member-call`, and
     `lambda-inherits-friend-class-access`; PA21 owns
     `noreturn-direct-object-fallback`.
   - PA24 register/value lowering: `branch-local-parameter-spill`,
   ...
   - PA25 native EH/import/linkage/lifetime:
     ...
     `310-anonymous-template-specialization-linkage`; PA26 owns
     `310-shared-conditional-cleanup-resume`, PA27 owns
     `200-host-extern-template-vtable-reference`, and PA32 owns
...
- Replaced PA10's lowering-time classifier with the PA14-owned
...
- Kept PA10 lowering as a direct consumer of the published binding fact and
  removed `pa10_internal_identity.{h,cpp}`.  Registered the new semantic owner
  in the compiler source manifest.
- Replaced both lambda spelling round trips with canonical one-component
  `NamePath` lookup.
- Added PA5-owned brace matching and made class-name preindexing jump over
...
brace matcher into its own PA5 translation unit for stable ownership and the
...
- `perl scripts/cppgm_file_audit.pl --stage pa34 --paths dev/src`: pass, zero
  fatal findings (23 advisory warnings).
- `make test-report-through-pa33`: pass, 5,138/5,138.
- `make -C pa34 test-through-pa5 CXX=../dev/cppgm++ CPPGM_HOST_CXX=g++`:
  pass for every self-built PA1--PA5 checkpoint.
- `make -C pa34 compare-pptoken-inception CXX=../dev/cppgm++
  CPPGM_HOST_CXX=g++`: pass with matching entry/shared objects and
  `MATCH pptoken`.
- `make -C pa34 compare-cppgm++-inception CXX=../dev/cppgm++
  CPPGM_HOST_CXX=g++`: pass with all 138 objects matching and `MATCH cppgm++`.
- Focused ownership checks also passed: PA5 157/157 plus 6/6 course, PA14
  293/293 plus 8/8 course, PA17 308/308 plus 2/2 course, and PA25 88/88 plus
  9/9 course.
