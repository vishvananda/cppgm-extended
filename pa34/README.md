## CPPGM Programming Assignment 34 (Inception)

### Overview

PA34 is the inception assignment. To complete PA34, make `cppgm++` build
...
semantic, lowering, runtime, or backend surface at a time; the PA34 ladder uses
...
PA34 does not add a new language feature, command-line mode, object format, or
...
compiler contract bug rather than adding a PA34-only side channel.

If the earlier assignment tests cover every language, lowering, runtime,
linking, and optimization feature used by your implementation, PA34 should be a
straightforward build plus a few reproducibility cleanups. In practice, the
self-build usually finds missing coverage. A failure in PA34 is usually evidence
...
Complete PA33 before starting PA34. PA34 reuses:

- the PA1-PA4 preprocessing tools and PA8 LowIR tool
- the cumulative `cppgm++` compiler
- the native, object, runtime, and hosted compile/link surfaces
- the PA32/PA33 optimization surfaces
- the earlier `pa1` through `pa33` tests for preservation checks

PA34 also needs a host C++ compiler and linker. The host compiler links staged
object files and provides the hosted configuration used by the compiler build.

### Build Variables

PA34 defaults to the course compiler under test. Use these compiler variables
...
If `../dev/cppgm++` does not exist yet, the PA34 Makefile first builds it with
`CPPGM_HOST_CXX`.

PA34 applies a per-file wall timeout to self-hosted compile steps so a single
...
During a normal in-tree inception build, PA34 also checks each newly built
...
as the PA34 result. The restored-self diagnostic target disables this object
check because it may not have a matching self-host object tree.

For both self-host and inception checkpoint binary builds, PA34 schedules
...
```sh
make through-lowir CXX=../dev/cppgm++ CPPGM_HOST_CXX=g++
...
```sh
make test-through-pa8 CXX=../dev/cppgm++ CPPGM_HOST_CXX=g++
make test-through-pa33 CXX=../dev/cppgm++ CPPGM_HOST_CXX=g++
```
...
make test-pa5 CXX=../dev/cppgm++ CPPGM_HOST_CXX=g++
make test-pa32 CXX=../dev/cppgm++ CPPGM_HOST_CXX=g++
```

The test targets reuse the earlier assignment harnesses. PA34 changes which
binary those harnesses run; it does not change the expected PA1 through PA33
outputs.

### Single-Object Probes

During PA34, changing one `dev/src/*.cpp` file and then rerunning a canonical
...
The probe uses the PA34 self-host compile flags and timeout, but writes to
`../obj/pa34/probe/selfhost/...` instead of the canonical
`../obj/pa34/selfhost/...` tree. It prints the compiler, source, object, and
...
only for diagnosis; final validation still requires the canonical PA34 targets.
...
- `ppexpr-self`: PA3
- `preproc-self`: PA4
- `cppgm++-self`: PA5-PA7, PA10-PA23, and PA25-PA31
- `lowir-self`: PA8
- `abimangle-self`: PA9
- `lowir2native-self`: PA24 and PA33
- `lowiropt-self`: PA32

Checkpoint source sets are fixed by `../dev/frontend_source_sets.mk`. Do not
replace that with a generated source scan; PA34 is checking whether the known
implementation source sets can be rebuilt reproducibly.

### Working Through Failures

Treat PA34 failures as compiler bugs until proven otherwise.
...
The best PA34 fixes usually improve an earlier assignment surface and leave a
