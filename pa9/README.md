...
In the course harness today, the default PA9 test path runs the generated host-native program
directly:

    $ make test

That path builds `cy86` and runs the PA9 suite on the current host platform, comparing only
program output and exit status.

There is also an extra Linux cross-check path:

    $ make test-docker

That path builds `cy86-linux` in Docker and reruns the suite inside Docker as an additional
validation step against the Linux-reference-compatible output format.  It is slower and is not
