...
with level 2; the current PA32 extension raises it to a distinct level 3.
Explicit `-O0` remains the PA24 baseline, but it must travel through the same
LowIR preparation and native pipeline as every other level.

PA32 is the primary owner because it introduced optimization-level plumbing,
...
PA25 for compiler-object serialization and linking, and PA33 for machine-level
selection.  PA34 remains the determinism and inception gate rather than the
...
the PA32 rule that standalone `lowiropt` requires an explicit level.  The
legacy no-level `--emit-lowir` view remains a PA10 tool surface; the PA32
...
| --- | --- | --- | --- |
| source, no `-O` | raw PA25 adapter result | skipped | level 0 |
...
`lowir_model::LowirProgram` directly to PA32 and PA33, and `.cppgm_object`
contains a binary serialization of that model.  Canonicalization exists to
satisfy the PA32 boundary: direct source compilation must be reconstructible
from serialized LowIR without private frontend facts, and PA34 requires stable
object bytes.
...
level zero uses the PA10 typed renderer, while debug or nonzero optimization
uses the PA25 adapter and PA32 model.  This is not on the production
...
| mode | real | user | max RSS | adapter/prep | PA32 optimizer | native lower | PA33 optimizer | encode | object bytes |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
...
The present default-level performance problem is instead PA32 O2.  It visits
...
- per-PA32-pass input/output sizes, visits, rewrites, and elapsed time.
...
| --- | --- | --- |
| binary object loses a derived global-address fact | PA25 | compile to `.o`, relink the payload, and inspect/run an imported address use |
| source object differs from text-reconstructed object | PA32 | one source-owned root, weak helpers in first-use order, unused declarations, and an ABI alias |
| omitted `-O` is not the maximum policy | PA32 | byte-compare no flag and `-O3`; also compare direct source with O0-text-to-default object |
| LowIR optimization behavior depends on frontend-only facts | PA32 | run the same small program from source and handwritten/serialized LowIR at each level |
| native level selection differs between driver paths | PA33 | compare structural MIR/behavior for explicit maximum and the chosen default surface |
| a frozen C++ file exposes a language/backend failure | its first language/backend PA | reduce the syntax and semantics there; do not add the first regression only to PA34 |
...
Move source-only cleanup to the PA25 adapter instead of repairing its output:
...
PA32 and PA33.  These changes both avoid full scans and guarantee that source,
...
### Phase 3: reduce the PA32 O2 cost before making it the default
...
Every removed or fused scan needs a handcrafted PA32 LowIR reducer proving the
same optimized shape and behavior.  O0 remains a deterministic no-transform
level; PA33 O0 remains the PA24 machine baseline.

The concrete performance target follows from the current measurements.  To
make maximum optimization no slower than today's O0 end-to-end compile, PA32
...
This phase must preserve PA25 same-version compile/link behavior and PA32
...
compile and link paths pass the same effective level to PA32 and PA33. No
...
The equality above is byte equality where PA32 already requires it and
...
retroactively promise LowIR LTO; the link invocation selects PA33 native work,
while the payload retains the LowIR level chosen when that object was built.

PA34 already supplies explicit `-O3` through `INCEPTION_CC_FLAGS`, so the
...
- PA32 primary, debuginfo, driver, and object-roundtrip buckets;
- repeated direct/text/binary canonicalization and byte reproducibility;
- PA24 O0 backend replay and PA33 O1/O2 structural/behavior tests;
- PA25 separate, direct-source, and mixed source/object link paths;
- file audit and the full cumulative report through PA33;
- frozen `semantic_overload.cpp` three-run omitted/O0/O2 comparison;
- version-matched optimizer-only LowIR profile;
- PA34 self/inception object and final compiler byte comparison.
...
optimization for PA32 object cases, explicit O0 shares the same preparation
...
owner-level/cumulative tests pass, and PA34 inception remains byte-identical.
