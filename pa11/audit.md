...
Independent review found no other PA11 correctness, semantic reconstruction,
...
- Added hits, misses, invalidations, dependency edges, and invalidation pushes
  to PA6/PA7 telemetry. LowIR statistics now own one nested semantic schema
...
- Replaced PA11 lowering's node-based maps with `FlatIdMap`, sorted compact
...
- `make test-pa6`: 70/70 pass.
- `make test-pa10`: 108/108 pass.
- `make test-pa11`: 291/291 pass.
- `perl scripts/cppgm_file_audit.pl --stage pa11 --paths dev/src`: pass; six
  non-blocking header-division warnings.
- `make test-report-through-pa11`: **1,436/1,436 tests and 16/16 stages pass**.
