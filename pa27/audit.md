...
Scope: PA27 full stage, from PA26 baseline `617abe30` through `a20d0480`,
plus the final audit repair. The review independently read `spec.md`, the PA27
...
are unchanged inherited header-division advisories, not PA27 failures.

## Changes

- `pa27_template_preemption_semantic.cpp`: replaced recursive per-argument
...
- `pa14_template_semantic.cpp`: made dependent-shape traversal iterative and
...
- `pa10_lowering_abi.cpp`: interned ABI type-argument facts in a geometric
...
- Added
  `pa27/tests/200-canonical-template-type-dag-scaling.t`, a
...
- `make test-pa27`: 133/133 handout and 8/8 course tests pass.
...
- `make test-report-through-pa27`: 4,291/4,291 tests and 32/32 stages
