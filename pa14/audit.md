...
5. The first structured-name integration exposed older-stage boundary
   assumptions: PA5 non-type argument lists were being reparsed as PA14 type
   trees, PA6 treated hidden name facts as expression children, and PA7 could
...
- PA5 now retains hidden structured-name facts below declarations,
...
- PA7/PA14 resolve those facts directly and build every supported template
...
- Syntax orchestration moved to `pa5_syntax_driver.cpp`; function-template
  lookup/binding/upgrade/instantiation moved to
  `pa14_function_template_instantiation.cpp`. Both modules are in the
...
- Cross-stage compatibility is explicit: structured argument retention stops
  when a PA5 template-id contains a non-type argument, PA6 skips hidden
  structured-name children in first-language-child queries, and PA7 applies
...
- `make test-pa14`: pass, 300/300 (293 handout plus 7 course tests).
- `make test-report-through-pa14`: pass, 2,013/2,013 and 19/19 stages.
