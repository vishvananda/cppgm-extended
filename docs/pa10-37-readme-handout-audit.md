...
| PA25 | edited | Host EH facts handout; replace the old private `cppeh` assignment framing with the basic `cppgm++ -c` host-EH metadata contract. | Rewrote PA25 around normalized host EH runtime-helper imports, personality/unwind/LSDA facts, typeinfo relocations, cleanup-only resume facts, and absence of private EH symbols; moved the basic host-EH fact-owner tests from PA33; kept rethrow and richer ABI/runtime interactions in later EH assignments. | `make -C pa25 test` passed `11/11`; `make -C pa33 test` passed `74/74`; root `test-report` for `pa25 pa33` passed `85/85`. |
