...
| 6 | `libs/asio/test` | frontier | `(retained decltype sidecar fix)` | The initial forced eight-job survey timed out at 1800s after recording multiple compiler families; log `/tmp/boost-frontier-v2-suite-006-initial-forced/libs__asio__test.log`. Forced focused `bind_allocator` and `bind_allocator_select` compile, link, run, and pass; log `/tmp/boost-frontier-v2-asio-bind-allocator-final.log`. The exact post-fix survey timed out at 1800.0s after 192 passes and six compile failures. All allocator and immediate-executor bound-handler variants pass. Log: `/tmp/boost-frontier-v2-suite-006-bind-allocator-final/libs__asio__test.log`. | Suite remains open at `buffer`. `buffer` and `buffer_select` are the earliest failures and share raw integer literal overload selection. Later independent pairs lose `async_accept` in `ip_tcp` and `async_write_some` in `posix_basic_stream_descriptor`. Final direct-LowIR report passes `3829/3829` with PA9 omitted. |
...
- focused targets: `libs/asio/test//buffer` and
  `libs/asio/test//buffer_select`
- failure phase: integer user-defined literal call construction
- diagnostic: `operator""_buf` is called with one cooked integer argument even
  though the viable raw literal operator is a `template<char...>` with no
  function arguments
- reduced repro: pending
- owning PA/cluster: pending reduction audit
- implementation area: typed user-defined literal overload classification and
  the existing structured `template<char...>` argument path
- performance risk: current three-run cumulative median is +0.96% instructions,
  +0.78% RSS, and +0.11% footprint; all fixed hard gates pass, and the latest
  fix adds only 0.06 instruction percentage points
- next action: reduce a raw integer literal operator template coexisting with a
  cooked overload, establish the earliest owning PA, and prove overload-shape
  selection from typed declarations before changing production code
...
env CPPGM_B2_CXX=/Users/vishvananda/cppgm-extended/dev/cppgm++ \
  ...
  JOBS=1 \
  ./run-cppgm-b2.sh -a \
  libs/asio/test//bind_allocator \
  libs/asio/test//bind_allocator_select
```
