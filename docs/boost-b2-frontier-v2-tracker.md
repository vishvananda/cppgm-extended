...
| 6 | `libs/asio/test` | frontier | `(structured template-id type-probe fix)` | The initial forced eight-job survey timed out at 1800s after recording multiple compiler families; log `/tmp/boost-frontier-v2-suite-006-initial-forced/libs__asio__test.log`. Forced focused `ip_tcp` and `ip_tcp_select` compile, link, run, and pass; log `/tmp/boost-frontier-v2-asio-ip-tcp-focused-fixed.log`. A follow-on survey was manually stopped after 54 minutes because its invocation had omitted the documented 1800-second external timeout. It had recorded 286 passing tests, the two ordered POSIX stream-descriptor compile failures, and two later independent `strand` link failures; log `/tmp/boost-frontier-v2-asio-full-after-ip-tcp.log`. | Suite remains open at `posix_stream_descriptor`. Both variants report unknown member `async_write_some` at `libs/asio/test/posix/stream_descriptor.cpp:147`; the later `strand` pair has an undefined `basic_executor_type<recycling_allocator<...>, 4ul>` destructor. Both `ip_tcp` variants also pass in the broad survey. Final direct-LowIR report passes `3831/3831` with PA9 explicitly omitted. |
...
- focused targets: `libs/asio/test//posix_stream_descriptor` and
  `libs/asio/test//posix_stream_descriptor_select`
- failure phase: direct member function-template collection or availability
- diagnostic: `basic_stream_descriptor<any_io_executor>` has collected ordinary
  `write_some` overloads but no `async_write_some` member at
  `libs/asio/test/posix/stream_descriptor.cpp:147`
- reduced repro: pending
- owning PA/cluster: pending reduction audit
- implementation area: typed class member-template collection, the
  `async_initiate` trailing return, and completion-token constraints
- performance risk: current three-run cumulative median is +0.89% instructions,
  +0.29% RSS, and +0.15% footprint; the latest fix improves instructions by
  0.03 percentage points and runs only after a cold exact-anchor mismatch
- next action: focus `async_write_some`, determine whether the declaration is
  rejected during collection or lost later, reduce it to a non-STL case, and
  establish its first owning PA before changing production code
...
env CPPGM_B2_CXX=/Users/vishvananda/cppgm-extended/dev/cppgm++ \
  ...
  JOBS=1 \
  ./run-cppgm-b2.sh -a \
  libs/asio/test//posix_stream_descriptor \
  libs/asio/test//posix_stream_descriptor_select
```
