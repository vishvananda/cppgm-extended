...
- active compiler frontier: concrete non-type template argument retention in
  Asio executor layout after the qualified nested allocator rebind alias fix
...
| 6 | `libs/asio/test` | frontier | `(qualified nested allocator rebind alias fix)` | The initial forced eight-job survey timed out at 1800s after recording multiple compiler families; log `/tmp/boost-frontier-v2-suite-006-initial-forced/libs__asio__test.log`. Forced focused `any_completion_executor` now compiles, links, runs, and passes after both the direct function-pack and nested allocator-rebind fixes; log `/tmp/boost-v2-asio-any-completion-executor-after-rebind.log`. The exact post-fix full forced survey timed out at 1800.0s after 160 passed tests and 28 compile failures; 14 failures share the next `Bits` frontier, while later independent families remain. Log: `/tmp/boost-frontier-v2-suite-006-after-rebind/libs__asio__test.log`. | Suite remains open at `append`. The prior allocator converting-constructor symptom was caused by a detector-selected nested rebind alias retaining stale qualified lookup state, not missing constructor discovery. The earliest remaining failure leaves `Bits` symbolic in `basic_executor_type<allocator<void>, Bits>` when a concrete layout is required. |
...
- focused targets: `libs/asio/test//append` and `//append_select`
- failure phase: class layout/template substitution
- diagnostic: `named type size unavailable` for
  `boost::asio::io_context::basic_executor_type<std::__1::allocator<void>, Bits>`
- reduced repro: pending
- owning PA/cluster: pending reduction audit
- implementation area: non-type template argument retention across partial
  specialization result types and concrete class layout
- performance risk: current cumulative result is +0.85% instructions, -0.53%
  RSS, and +0.06% footprint; all fixed cumulative hard gates pass and the
  accepted rebind path adds only 0.06 instruction percentage points
- next action: reduce the symbolic `Bits` layout request, prove whether the
  concrete argument is lost during typed substitution or class completion,
  and place the smallest non-STL regression in its earliest owning PA
