...
- active compiler frontier: dependent `handler_type_requirements<sizeof(...)>`
  instantiation in Asio bound completion handlers
...
| 6 | `libs/asio/test` | frontier | `(enclosing NTTP substitution fix)` | The initial forced eight-job survey timed out at 1800s after recording multiple compiler families; log `/tmp/boost-frontier-v2-suite-006-initial-forced/libs__asio__test.log`. Forced focused `append` and `append_select` compile, link, run, and pass; log `/tmp/boost-frontier-v2-asio-append-pair-enclosing-nttp-final.log`. The exact post-fix survey timed out at 1800.0s after 175 passes and 12 compile failures; every prior symbolic-`Bits` target is clear. Log: `/tmp/boost-frontier-v2-suite-006-enclosing-nttp/libs__asio__test.log`. | Suite remains open at `bind_allocator`. Six bound-handler variants share the earliest `handler_type_requirements<sizeof(...)>` failure. Two immediate-executor variants, two buffer-literal variants, and two `ip_tcp` variants expose later independent diagnostics. Final direct-LowIR report passes `3828/3828` with PA9 omitted. |
...
- focused targets: `libs/asio/test//bind_allocator` and
  `libs/asio/test//bind_allocator_select`
- failure phase: dependent class-template argument instantiation
- diagnostic: `failed class template instantiation` for
  `boost::asio::detail::handler_type_requirements<sizeof(...) + sizeof(...)>`
- reduced repro: pending
- owning PA/cluster: pending reduction audit
- implementation area: typed substitution and constant evaluation for a
  dependent non-type class-template argument containing `sizeof` and calls
- performance risk: current three-run cumulative median is +0.90% instructions,
  +1.27% RSS, and +0.10% footprint; all fixed hard gates pass, and the latest
  fix adds only 0.05 instruction percentage points
- next action: reduce the `handler_type_requirements` expression, determine
  which retained expression fact fails to substitute or evaluate, and place a
  non-STL reducer in the earliest owning PA before changing production code
