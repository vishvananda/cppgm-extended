...
|---:|---|---|---|---|---|
| 1 | `libs/accumulators/test` | frontier | `(structured GNU complex fix)` | Forced single-job `count` rebuild passed end to end and updated 72 targets; log `/tmp/boost-v2-acc-count-typed-member-pointer-fix.log`. Forced full-suite survey with 8 jobs updated 114 targets, failed 37 compile targets, and exposed the remaining target-local frontiers; log `/tmp/boost-v2-suite-001-typed-member-pointer-full.log`. Focused single-job `covariance` confirmed the libc++ parameter-clause failure in `/tmp/boost-v2-acc-covariance-focused-pre-fix.log`; the forced post-fix run advanced to the separate GNU complex ABI frontier in `/tmp/boost-v2-acc-covariance-structured-complex-fix.log`. | The qualified-friend parser, `mp_bool`, Boost.Test `format_report`, Boost.Regex `perl_matcher`, and structured `_Complex` parameter parsing frontiers are fixed. `count` passes. `covariance` now reaches typed symbol construction for `std::__1::complex<float>::__builtin(_Complex float)`. |
...
- failure phase: typed ABI IR construction for a libc++
  `std::__1::complex<float>` member function
- diagnostic: `ERROR: failed to build ABI IR function symbol for weak function
  std::__1::complex<float>::__builtin` at libc++ `complex:442`; its parameter is
  `_Complex float`
- reduced repro: `/tmp/cppgm-v2-gnu-complex-mangle.cpp`; Clang mangles complex
  float/double/long-double parameters as `Cf`, `Cd`, and `Ce`, while `cppgm++`
  currently rejects the weak symbol
- owning PA/cluster: PA30 typed ABI facts/model plus PA34 hosted GNU type coverage
- implementation area: semantic `TypePtr` to typed `abi_mangle::Type` adapter
- performance risk: constant named-builtin mapping only; use hotspot counters and
  sampling if the production fix moves instructions by more than 0.25%
- next action: add GNU complex builtin codes to the PA30 fact grammar and tests,
  map semantic GNU complex types to `abi_mangle::Type::builtin`, strengthen the
  PA34 reducer for the weak member symbol, and rerun focused `covariance`
