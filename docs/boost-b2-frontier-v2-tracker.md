...
- active compiler frontier: raw integer user-defined literal overload selection
  in Asio buffer literals
...
| 6 | `libs/asio/test` | frontier | `(typed numeric UDL selection fix)` | The initial forced eight-job survey timed out at 1800s after recording multiple compiler families; log `/tmp/boost-frontier-v2-suite-006-initial-forced/libs__asio__test.log`. Forced focused `buffer` and `buffer_select` compile, link, run, and pass; log `/tmp/boost-frontier-v2-asio-buffer-raw-numeric-literal.log`. The exact post-fix survey timed out at 1800.1s after 190 passes and four compile failures. Both buffer variants pass; the remaining failures are the `ip_tcp` and POSIX stream-descriptor pairs. Log: `/tmp/boost-frontier-v2-suite-006-buffer-literal-final/libs__asio__test.log`. | Suite remains open at `ip_tcp`. Both variants lose the one-argument `async_accept(move_accept_handler())` overload at `libs/asio/test/ip/tcp.cpp:924`. The later POSIX pair independently loses `async_write_some`. Final direct-LowIR report passes `3830/3830` with PA9 omitted. |
...
- focused targets: `libs/asio/test//ip_tcp` and
  `libs/asio/test//ip_tcp_select`
- failure phase: member function-template candidate viability for a one-argument
  forwarding-reference call
- diagnostic: `basic_socket_acceptor<ip::tcp, any_io_executor>` has 16 collected
  `async_accept` members, but none remains viable for
  `async_accept(move_accept_handler())` at `libs/asio/test/ip/tcp.cpp:924`
- reduced repro: pending
- owning PA/cluster: pending reduction audit
- implementation area: typed member function-template deduction, substituted
  trailing-return `decltype`, and SFINAE for `async_initiate`
- performance risk: current three-run cumulative median is +0.92% instructions,
  -0.54% RSS, and +0.10% footprint; the latest fix improves instructions by
  0.04 percentage points and does not run on the stable benchmark
- next action: focus the one-argument `async_accept` overload, reduce the
  candidate loss to a non-STL function-template case, and establish its first
  owning PA before changing production code
