...
| 4 | `libs/any/test` | frontier | `JOBS=12`, direct run after nested static member fix advanced past `basic_tests<Any>::copy_counter::count`; remaining failures include compile-time `any_cast` reference-return diagnostics in normal tests and runtime wrong-value/exit-139 failures in `unique_any/unique_base` and `unique_any/unique_from_any`. |
