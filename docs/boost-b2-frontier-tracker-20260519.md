...
| 4 | `libs/any/test` | frontier | `JOBS=12`, direct run after scalar reference return materialization advances past MPL-if `any_cast` LowIR failures. Current suite frontier has 13 failed targets: no-RTTI `ArrayLength` lookup failures, explicit destructor member-call rejection in `basic_any`, `unique_any` initializer-list/emplace constructor selection, and existing `any_test` / `unique_any` runtime failures. |
