...
- active perf baseline: `/tmp/cppgm-perf-baseline-boost-frontier-current-dee7966f4-20260519.json`
  recorded at `dee7966f4711b4aa0c3d25d7b0c46dfdbf991cd1` after accepting the
  rebased frontier head as the new baseline; median instructions
  `311,308,478,832`, RSS `1.08 GiB`, footprint `857.33 MiB`
...
| 4 | `libs/any/test` | frontier | `JOBS=12`, direct run after nested class injected-name fix advances past `boost::any::placeholder::clone()` and compiles direct `libs/any/test/any_test.cpp`. Current suite frontier has 15 failed targets, including `unknown storage %t7` in MPL-if any_cast paths, no-RTTI `ArrayLength` lookup failures, explicit destructor member-call rejection in `basic_any`, `unique_any` initializer-list/emplace constructor selection, and existing `unique_any` runtime wrong-value/exit-139 failures. |
