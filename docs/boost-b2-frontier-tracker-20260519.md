...
| 4 | `libs/any/test` | frontier | `JOBS=12`, direct run after nested static member fix advanced past `basic_tests<Any>::copy_counter::count`; constructor-template ordering fix advances `any_cast<std::string>(const std::string&)`/`boost::any` construction to the next `boost::any::placeholder::clone()` nested-name member-initializer failure. Later known failures still include `any_cast` reference-return diagnostics and runtime wrong-value/exit-139 failures in `unique_any/unique_base` and `unique_any/unique_from_any`. |
