...
- active perf baseline: `/tmp/cppgm-perf-baseline-boost-frontier-current-2fcb9d5f-20260519.json`
  recorded at `2fcb9d5fdfcdee3ad752aa5d14ac804b3025fec5` after accepting the
  current rebased frontier head as the new baseline; median instructions
  `311,949,299,491`, RSS `1.08 GiB`, footprint `860.64 MiB`
...
| 4 | `libs/any/test` | frontier | `JOBS=12`, direct run after const-object explicit destructor fix advances past the `basic_any` compile failures. Current suite frontier has 6 failed targets: two `unique_any` initializer-list/emplace constructor compiles and four runtime failures (`any_test`, `basic_any_test`, `unique_base`, `unique_from_any`). |
