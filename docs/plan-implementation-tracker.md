...
| 2 | pending | [machine-backend-o0-quality-plan.md](machine-backend-o0-quality-plan.md) | Improve baseline PA28 `-O0` machine-code quality before later backend optimization work. | Partway landed; remaining work is a deferred PA28 quality lane, not active PA37 `-O1`/`-O2` work. |
| 3 | pending | [template-argument-text-shape-removal-plan.md](template-argument-text-shape-removal-plan.md) | Remove function-template deduction's `deduce_template_argument_text_shape` string-shape fallback. | New focused cleanup lane discovered during Boost.Bimap frontier work; first step is audit coverage, then structured data plumbing until the helper is gone and perf returns to baseline. |
