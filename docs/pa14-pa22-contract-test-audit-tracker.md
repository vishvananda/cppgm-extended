...
| Test moves/reductions | in_progress | Mechanical move batches completed. Split/reduce and PA23/PA33 harness-rewrite items are deferred below. |
...
- PA33 owners are hosted/source-compatibility owners, not core-language owners.
...
| `lang.extended_integer` | `pa33` | 600 | N/A: GNU/Clang `__int128` extension | pending | `__int128`, unsigned 128-bit constants, truncation/canonicalization; ordinary `long long` remains part of earlier scalar support. |
...
| `class.attribute.no_unique_address` | `pa33` | 600 | N/A: post-C++11 attribute; 7.6 `[dcl.attr]` for attribute syntax | pending | `[[no_unique_address]]` layout, empty member overlap, recursive empty holder cases in hosted/source-compatibility inputs. |
...
| `support.attribute` | `pa33` | 500 | 7.6 `[dcl.attr]` | pending | standard/GNU attributes outside `no_unique_address`, including hosted/vendor declaration attributes and attributes on specialization declarations. |
...
| `template.builtin_traits` | `pa33` | 600 | N/A: compiler intrinsics/hosted trait probes | pending | builtin trait/intrinsic expressions such as `__is_*`, `__builtin_*`, and trait wrappers. |
