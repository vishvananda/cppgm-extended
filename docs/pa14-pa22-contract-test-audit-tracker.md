...
| `exception.guarded_static_cleanup` | `pa25` | 200 | 3.7.1 `[basic.stc.static]`, 6.7 `[stmt.dcl]`, 12.2 `[class.temporary]`, 15 `[except]` | pending | A function-local dynamic class static whose checked LowIR contains EH control. The initialization edge must finish initializer-temporary cleanup before joining the already-initialized edge. |
| `exception.aggregate_cleanup` | `pa25` | 200 | 8.5.1 `[dcl.init.aggr]`, 12.2 `[class.temporary]`, 15 `[except]` | pending | An aggregate return over destructible class members whose checked LowIR contains EH control. Nontrivial members use constructor actions, and cleanup tracks the constructed subobjects. |
| `exception.class_value_argument_cleanup` | `pa25` | 200 | 5.2.2 `[expr.call]`, 12.2 `[class.temporary]`, 12.4 `[class.dtor]`, 15 `[except]` | pending | A destructible class value parameter whose checked LowIR contains EH control. The caller transfers the materialized argument to the callee and must not retain a second normal-path cleanup for that object. |
| `exception.throw_operand_cleanup` | `pa25` | 200 | 12.2 `[class.temporary]`, 15.1 `[except.throw]` | pending | A class throw operand with destructible temporaries and checked LowIR EH control. Operand temporaries are destroyed after exception-object initialization and removed from later sibling unwind snapshots. |
| `exception.handler_branch_cleanup` | `pa25` | 200 | 12.4 `[class.dtor]`, 15.3 `[except.handle]` | pending | A branch inside a typed handler can reach a throwing call while an outer object remains live. The call's unwind path must finish the catch and destroy outer objects before resuming. |
| `host.eh_object` | `pa31` | 100 | N/A: host object and Itanium C++ ABI EH contract | pending | PA31 `cppgm++ -c` object/runtime coverage: EH runtime imports, personality references, unwind and LSDA facts, typed-catch references, or final host-unwinder behavior. Distinguish this layer from PA25 source-to-LowIR exception lowering. |
...
| `support.bodyless_placeholder_return` | `pa34` | 500 | N/A: C++14 placeholder-return extension | pending | Bodyless function declarations with a deduced `auto` return, including deleted overloads accepted while compiling newer hosted headers. PA24 owns only definitions with a visible deduction body. |
