# PA33/PA34 Convergence Plan

## Goal

Make the full `pa33` and `pa34` test sets work without turning hosted
compatibility into a dumping ground for earlier language bugs.

`pa33` and `pa34` are discovery gates:

- `pa33` exposes hosted header/source compatibility failures.
- `pa34` exposes hosted header-emission, link, symbol, ABI, and runtime
  failures.
...
vendor-extension failures may stay in `pa33`; emitted-code/link/runtime
failures that first require hosted linking may stay in `pa34`.
...
- Current PA33 contract: `pa33/README.md`
...
1. Generate patched-Clang witness references for C++ translation-unit tests in
   ...
   ```sh
   python3 validation/templates/materialize_pa_tests.py pa33 pa34
   ```

   The generated `.ref.witness` files are debugging references for this phase.
   They are not a requirement that `pa33`/`pa34` reach full witness parity now.
   The generator intentionally skips preprocessor-only tests and expands
   `pa34/tests/link/*.t` manifests to their real numbered translation units
   ...
   - The first pass incorrectly invoked PA34 link-test manifests as C++
   ...
   - Running the same failed inputs with regular Homebrew Clang produced the
     ...
     regressions.
   - After fixing link-test handling, the PA34 witness gap is one
     Clang-incompatible source,
     `pa34/tests/link/645-builtin-operator-new-delete-link-smoke.t.1`, which
     redeclares Clang builtin `__builtin_operator_new`.
   - PA33 preprocessor tests are intentionally skipped. They are
   ...
   - Current witness coverage after that cleanup:
     PA33 has 306 expected-success C++ translation-unit candidates, 302 with
     witness refs, and four missing Clang witness refs. PA34 has 98/98 link
     translation units covered. The four PA33 gaps are hosted/compiler-extension
     ...
     missing Clang witness files for those tests as a PA33/PA34 correctness
     blocker.
   - Invalid pre-repair versions of the six PA33 sources that needed
     source-level cleanup are preserved under
     `validation/templates/deferred_negative_inputs/pa33-invalid-witness-sources`.
     They are not active tests. After PA33/PA34 convergence, use them as seeds
...
2. Collect the active failure set:
   ...
   ```sh
   make test-report ACTIVE_TEST_REPORT_PAS='pa33 pa34' \
     CXX=/usr/local/opt/llvm/bin/clang++ \
     CPPGM_HOST_CXX=/usr/local/opt/llvm/bin/clang++
   ```

   Use `test-report` as the authoritative PA33/PA34 progress view. It should
...
3. Use witness output to classify the semantic state:
   ...
   - witness-only drift in `pa33`/`pa34` should be noted but is not a blocking
...
7. Re-run the owner strict suite and the PA33/PA34 focused report.
...
## Baseline: 2026-04-29 PA33/PA34 Report
...
- The permissive PA33 frontier moved past the original `540`/`542` failures.
...
- `pa33/tests/compile/540-reference-wrapper-smoke.t` exposed a structured
...
- After those PA18 body-check fixes, the current `540` frontier moved to
  ...
  Regression: `pa32/tests/spec/231-host-dependent-alias-template-mangling.t`.
  Validation: strict `pa18 pa19 pa21 pa22` owner suite plus focused PA19/PA22
  SFINAE checks and the new PA32 reducer.
- The current `540` frontier is now unrelated type template argument
  ...
  Validation: strict `pa18 pa19 pa21 pa22` owner suite plus focused PA19/PA22
  SFINAE checks, the PA32 mangling reducer, and the new PA22 reducer.
...
- The `540` performance tranche later recovered compile time and the refreshed
  strict PA33/PA34 baseline no longer lists `540` as a failing hosted compile
...
## Baseline: 2026-04-30 Strict PA33/PA34 Report
...
- `pa33/tests/compile/532-anonymous-enum-template-argument.t` reduced to PA19:
...
- `pa33/tests/compile/542-local-functor-std-function-assignment.t` first failed
...
- `pa33/tests/compile/601-gnu-complex-template-constructor.t` is a PA33 hosted
...
- `pa33/tests/compile/606-using-if-exists-missing-target.t` is a PA33 hosted
  ...
  Validation: focused `606` PA33 harness check and strict
  `pa18 pa19 pa21 pa22` owner suite.
...
- `pa33/tests/compile/610-random-to-address-qualified-call.t` reduced to PA19:
...
- `pa33/tests/compile/627-anonymous-allocator-traits-pointer.t` reduced to
...
- `pa33/tests/compile/644-unnamed-nested-enum-allocator-pointer.t` reduced to
...
- `pa33/tests/compile/654-string-compare-prefix-substr.t` reduced to PA18:
...
- `pa33/tests/compile/655-const-unordered-map-find.t` reduced to PA22:
...
- The follow-up `655` strict hosted frontier exposed carried dependent
  ...
  reproducer for this slice is the hosted PA33 `655` test; no smaller non-STL
...
- `pa33/tests/compile/540-reference-wrapper-smoke.t` exposed a rewritten
...
- `pa33/tests/compile/542-local-functor-std-function-assignment.t` then
...
## Baseline: 2026-05-01 Strict+Lazy PA33/PA34 Report
...
Active PA33 compile exit-status failures:

- `pa33/tests/compile/656-forward-array-string-pair.t` - unclassified compile
  exit failure after the previous carried-syntax fixes.
- `pa33/tests/compile/665-hosted-pointer-traits-pair-pointer-to.t` -
  unclassified compile exit failure.
- `pa33/tests/compile/676-hosted-algorithm-copy-n-using-directive.t` -
  unclassified compile exit failure.
- `pa33/tests/compile/721-hosted-unreachable-inline-callee-export-closure.t` -
  unclassified compile exit failure.
- `pa33/tests/compile/725-hosted-piecewise-pair-index-sequence-alias.t` -
  unclassified compile exit failure.
- `pa33/tests/compile/736-hosted-local-class-distinct-member-symbols-compile.t`
  - unclassified compile exit failure.

Active PA34 implementation/link/runtime exit-status failures:

- `pa34/tests/link/651-hosted-unordered-map-string-int-link-smoke.t` -
  implementation exit failure.
- `pa34/tests/link/652-hosted-unordered-set-pointer-link-smoke.t` -
  implementation exit failure.
- `pa34/tests/link/654-hosted-forward-as-tuple-rvalue-ref-runtime.t` -
  implementation exit failure.
- `pa34/tests/link/695-hosted-std-vector-pair-abi-link-smoke.t` -
  implementation exit failure.
- `pa34/tests/link/698-hosted-template-angle-vector-pair-substitution-link-smoke.t`
  - implementation exit failure.
- `pa34/tests/link/702-hosted-local-wide-string-return-link-smoke.t` -
  implementation exit failure.
- `pa34/tests/link/705-hosted-inline-header-odr-link-smoke.t` -
  implementation exit failure.
- `pa34/tests/link/710-hosted-set-insert-count-link-smoke.t` -
  implementation exit failure.
- `pa34/tests/link/724-hosted-vector-class-brace-init-ref-capture-runtime-smoke.t`
  - implementation exit failure.
- `pa34/tests/link/727-hosted-vector-char-assign-initlist-runtime-smoke.t` -
  implementation exit failure.
- `pa34/tests/link/729-hosted-unreachable-inline-callee-link-smoke.t` -
  implementation exit failure.
- `pa34/tests/link/733-hosted-map-find-iterator-link-smoke.t` -
  implementation exit failure.
- `pa34/tests/link/735-hosted-map-subscript-piecewise-construct-link-smoke.t` -
  implementation exit failure.
- `pa34/tests/link/741-hosted-deque-move-assign-link-smoke.t` -
  implementation exit failure.

## Earliest Owner Mapping

Use this mapping before leaving a failure in `pa33` or `pa34`.
...
- `pa33`: hosted preprocessor compatibility, GNU/Clang parser concessions,
...
- `pa34`: hosted emitted definitions, inline/template emitted-code correctness,
...
extensions or header shape, keep a focused hosted regression in `pa33` or
`pa34`.
...
make test-strict-nobuild STRICT_PAS='pa18 pa19 pa21 pa22' STRICT_SUBTEST_JOBS=8 \
  CXX=/usr/local/opt/llvm/bin/clang++ \
  CPPGM_HOST_CXX=/usr/local/opt/llvm/bin/clang++
make test-report-nobuild ACTIVE_TEST_REPORT_PAS='pa33 pa34' \
  CXX=/usr/local/opt/llvm/bin/clang++ \
  CPPGM_HOST_CXX=/usr/local/opt/llvm/bin/clang++
...
1. Generate the PA33/PA34 witness references.
2. Run the PA33/PA34 `test-report` and record the failure set.
...
6. Re-run PA33/PA34 reporting to confirm the failure set shrank before moving
   to an unrelated family.

## Progress Log

- Fixed PA33 `540-reference-wrapper-smoke` strict fallback on
...
- Fixed the next PA33 `540` frontier, unsupported
...
- Fixed the PA33 `542-local-functor-std-function-assignment` subfailure where
...
- Fixed the `542` `__member_pointer_class_type<_DecayFp>::type` fallback.
  ...
  failure.
- Fixed the PA33 `609-regex-iterator-difference-alias` frontier through the
...
- Fixed the PA33 `610-random-to-address-qualified-call` frontier through a
  ...
  owner suite.
- Fixed the PA33 `627-anonymous-allocator-traits-pointer` frontier through a
...
- Refreshed the strict PA33/PA34 baseline after the transform-alias fix:
  `233 / 273 TESTS PASSED`. The PA33 frontier moved to
...
- Fixed the final `657` `basic_string::__init` overload failure. Function
  ...
  `pa18 pa19 pa21 pa22` owner suite.
- Fixed the PA33 `660-hosted-forward-as-tuple-rvalue-ref` frontier, also
...
- Fixed the next PA33 builtin-trait frontier exposed by `658`/`660`.
  ...
  evaluation. Added PA33 regression
  `747-builtin-dependent-trait-integral-constant.t`. Validation: focused PA33
...
- Fixed the follow-on hosted builtin trait replay slice. The dependency marker
  ...
  of collapsing all trait results to `bool`. Added PA33 regression
  `748-hosted-dependent-builtin-trait-replay.t`. Validation: focused PA33
...
- Fixed the follow-on hosted tuple/reference mangling recursion exposed by
  ...
  PA32 regression `234-host-owner-pack-reference-mangling.t`. Validation:
  focused PA32 regression check passed; focused `660` now reaches the unrelated
...
- Fixed the `660` tuple forwarding constructor viability frontier through a
  ...
  `pa22` passed, and the latest PA33/PA34 report is `236 / 275 TESTS PASSED`.
  The current PA33 frontier is `635-nested-class-constructor-reentry.t`; PA34
  still has link/runtime failures starting at
  `651-hosted-unordered-map-string-int-link-smoke.t`.
- Fixed the PA33 `635-nested-class-constructor-reentry` frontier through a
  PA32 non-STL reduction. ABI mangling for a dependent non-type owner argument
  ...
  Added PA32 regression
  `235-host-dependent-bool-owner-argument-mangling.t`. Validation: focused
  PA32 host-interop check passed, focused `635` LowIR compile passed, and
  strict `pa18 pa19 pa21 pa22` owner suite passed.
- Fixed the PA33 `662-hosted-ostringstream-unsigned-int` frontier through a
  ...
  PA33 `662` LowIR compile passed, and strict `pa18 pa19 pa21 pa22` owner suite
  passed.
- Fixed the first PA33 `676-hosted-algorithm-copy-n-using-directive` frontier
  as a PA33 hosted extension reduction. libc++ uses a static member
  ...
  non-members only. Added PA33 regression
...
- Fixed the follow-on PA33 `676` `std::make_pair` lookup failure through a PA22
...
- Fixed the PA33 `678-hosted-deque-member-template-include` frontier through a
...
- Fixed the PA33
  ...
  PA33 `734` compile passed, the PA18 reducer passed, PA22 nested-class
...
- Fixed the PA33
  ...
  subfailure through a PA32 non-STL reduction. Dependent alias expansion for an
  ...
  Clang-compatible substitution indexes. Added PA32 regression
  `236-host-dependent-enable-if-alias-expression-mangling.t` with a raw-symbol
  inspect check. Validation: focused PA32 regression and hosted PA33 `736`
  compile passed.
- Fixed the PA33 `745-class-alias-dependent-destroy-compile` frontier through
  ...
  Validation: focused PA18/PA21/PA22/PA33 checks passed and strict
  `pa18 pa19 pa21 pa22` owner suite passed.
- Fixed the PA33 `610-random-to-address-qualified-call` strict-lazy frontier.
...
- Fixed strict-lazy owner-suite fallout exposed while validating the `610`
  ...
  passed.
- Fixed the PA33 `651-allocator-deallocate-included-class-layout` strict-lazy
  ...
  before falling back to the old index heuristic. Added non-STL PA33 reducer
  `750-lazy-header-body-keeps-primary-source.t`.
- Fixed the PA33 `655-const-unordered-map-find` strict-lazy frontier. The
  ...
  evaluation materializes lazy function bodies on demand. Added non-STL PA33
  reducer `751-lazy-header-constexpr-sfinae-pair.t`.
- Fixed the follow-on PA33 `676-hosted-algorithm-copy-n-using-directive`
  ...
  semantic duplicate suppression. Added non-STL PA33 reducer
  `752-lazy-header-explicit-constexpr-template-id.t`.
- Fixed the PA33 `678-hosted-deque-member-template-include` strict-lazy
...
- Fixed the first PA34
  ...
  PA18 reducer passed, focused PA34 `651` link check passed, and strict
  `pa18 pa19 pa21 pa22` owner suite passed.
- Fixed the follow-on PA34
  ...
  focused PA34 `652` link check passed, and strict `pa18 pa19 pa21 pa22` owner
  suite passed.
- Improved the PA34
  ...
  partial-specialization regressions passed, focused PA34 `651` link check
...
- `make test-report ACTIVE_TEST_REPORT_PAS='pa33 pa34'` is green, or every
  remaining failure is explicitly deferred with a root cause and owner.
- All standard-language failures found through PA33/PA34 have minimal
...
- Hosted-only failures left in `pa33`/`pa34` are documented as hosted/vendor or
