...
| `builtin_is_convertible_array_to_class_nostl.cpp` | confirmed-positive | PA34 no-STL reducer for builtin conversion traits where a non-reference array source such as `char[6]` must decay to `char const *` and then satisfy a class constructor target |
