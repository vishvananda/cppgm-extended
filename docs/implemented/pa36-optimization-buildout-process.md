# PA36 Optimization Buildout Process

## Purpose

This document defines the intended buildout approach for `PA36`.

## Completion Note

This plan is now implemented.

The landed PA36 surface in this repo is:
...
- an O2 slot-promotion pass for simple non-escaping scalar slots, followed by a
  cleanup rerun
- structural PA36 LowIR tests plus source-driven `cppgm++ --emit-lowir -O*`
  integration owners

The document remains useful as a record of the intended PA36 shape, but the
...
`PA36` now comes before self-hosting and establishes the first real
optimization layer in the compiler.

## Main Recommendation

The best `PA36` shape is:
...
secondary optimization layer or a future assignment, not the primary `PA36`
contract.
...
The primary oracle for `PA36` should be:
...
the first new boundary the assignment owns. `PA36` owns the optimization stage
itself, so the structural optimized IR is the right main contract.

## Secondary Oracles

`PA36` should also have secondary behavioral checks:
...
This should be part of the core `PA36` pipeline.
...
This is the most ambitious pass that still plausibly belongs in `PA36`, and it
...
This should likely be the hardest required `PA36` pass, and it belongs at `-O2`,
not `-O1`.
...
That is intentionally smaller than the full eventual PA36 ambition, but it
...
The README for `PA36` should make two things explicit:
...
Recommended `PA36` buildout order:
...
`PA36` should wait until the LowIR move is complete enough that:
