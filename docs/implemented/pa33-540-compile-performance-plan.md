# PA33 540 Compile Performance Plan

## Goal

Use `pa33/tests/compile/540-reference-wrapper-smoke.t` as the primary hosted
...
- Focused benchmark `pa33-reference-wrapper-smoke` now has a three-run median
...
- Keep correctness gates focused on the owning strict PA suites; do not use the
  full PA33/PA34 report as the normal iteration loop.
...
python3 scripts/run_structured_ast_perf_benchmarks.py \
  --repeat 3 \
  --benchmark pa33-reference-wrapper-smoke \
  --output-prefix /tmp/cppgm-pa33-540-baseline
```
...
/usr/bin/time -lp \
  ...
  ./dev/cppgm++ -c \
    -o /tmp/cppgm-pa33-540.o \
    pa33/tests/compile/540-reference-wrapper-smoke.t
```
...
```sh
env CPPGM_STRICT_SEMANTIC_FALLBACKS=1 ./dev/cppgm++ -E -o /tmp/540.ii pa33/tests/compile/540-reference-wrapper-smoke.t
env CPPGM_STRICT_SEMANTIC_FALLBACKS=1 ./dev/cppgm++ --emit-ast -o /tmp/540.ast pa33/tests/compile/540-reference-wrapper-smoke.t
env CPPGM_STRICT_SEMANTIC_FALLBACKS=1 ./dev/cppgm++ --emit-semantics -o /tmp/540.sem pa33/tests/compile/540-reference-wrapper-smoke.t
env CPPGM_STRICT_SEMANTIC_FALLBACKS=1 ./dev/cppgm++ --emit-lowir -O0 -o /tmp/540.lowir pa33/tests/compile/540-reference-wrapper-smoke.t
env CPPGM_STRICT_SEMANTIC_FALLBACKS=1 ./dev/cppgm++ -c -o /tmp/540.o pa33/tests/compile/540-reference-wrapper-smoke.t
```
...
env \
  ...
  ./dev/cppgm++ -c \
    -o /tmp/cppgm-pa33-540.o \
    pa33/tests/compile/540-reference-wrapper-smoke.t \
  2>/tmp/cppgm-pa33-540-hotspot.log
```
...
```sh
sample <cppgm-pid> 10 1 -file /tmp/cppgm-pa33-540.sample.txt
```
...
- Started branch `codex/pa33-540-compile-performance-20260430` from
...
- Tested a portable `PostTokenizer` source-location fast path from the older
  ...
  `7.796s` on `pa33-reference-wrapper-smoke`.
...
- Add `pa33-reference-wrapper-smoke` to
...
full PA33/PA34 report as the routine loop.

Performance gates:

- `pa33-reference-wrapper-smoke` median must improve outside noise
- existing PA33 focused controls must not materially regress
...
- Clean prefix-index candidate vs retained direct-inline baseline:
  `/tmp/cppgm-pa33-540-ab-prefix-index-clean.md`
...
- Final clean candidate vs integration baseline commit `5981fc5b`:
  `/tmp/cppgm-pa33-540-ab-final-prefix-index-vs-integration.md`
...
4. PA33 `540` focused benchmark only after the migrated slice is behaviorally
   stable
...
- Baseline before early-layer batching: `/tmp/cppgm-pa33-540-early-layer-baseline-cppgm++`
- Recognized-token batch candidate: `/tmp/cppgm-pa33-540-recog-token-batch-cppgm++`
- Post-token batch/source-location-cache candidate: `/tmp/cppgm-pa33-540-post-token-batch-cppgm++`
- Final kept binary after reverting scratch reuse: `/tmp/cppgm-pa33-540-final-early-layer-batch-cppgm++`

Results on `pa33/tests/compile/540-reference-wrapper-smoke.t`:

- Recognized-token batching vs prefix-index-clean baseline: median `-0.79%`, average `-3.13%` (`/tmp/cppgm-pa33-540-ab-recog-token-batch.md`).
- Post-token batching plus source-file-index cache vs recognized-token batching: median `-1.46%`, average `-0.99%` (`/tmp/cppgm-pa33-540-ab-post-token-batch.md`).
- Early phase check for post-token batching vs recognized-token batching:
  ...
  - Report: `/tmp/cppgm-pa33-540-ab-post-token-early-phases.md`.
...
- After caching the current file index, `SourceLocationTable::add_file` dropped out of the top stack counts (`/tmp/cppgm-pa33-540-post-token-batch.sample.txt`). The remaining early-layer heat is now mostly `RecogTokenBuffer::peek/ensure`, `RecogTokenizer::get_many`, `PostTokenizer::get_many`, `Preprocessor`, `Macroizer`, `PPTokenizer`, and parser expression descent.
...
- Tried reusing scratch vectors for `PostToken`/`RecogToken` batch fills. Full compile timing was too noisy and superficially positive, but cheaper phase checks were consistently slower:
  ...
  - Report: `/tmp/cppgm-pa33-540-ab-reused-token-early-phases.md`.
...
Additional benchmarking used back-to-back alternating A/B runs because the host was noisy. For `-E`, the benchmark source was a frozen copy of `dev/src/callsemantic.cpp` at `/tmp/cppgm-pa33-540-frozen-callsemantic.cpp`, run with `-E -Idev/src`.
...
- Macro object fast path vs early-layer batch: `/tmp/cppgm-pa33-540-ab-macro-object-fastpath.md` showed full compile median -1.04%, average -1.70%.
- Macro simple function fast path vs object-only: `/tmp/cppgm-pa33-540-ab-macro-simple-function-fastpath-large-preprocess.md` showed large `-E` median -1.42%, average -2.83%.
- Macro precomputed flags vs simple-function: `/tmp/cppgm-pa33-540-ab-macro-precomputed-flags-large-preprocess.md` showed large `-E` median -3.98%, average -2.39%; `/tmp/cppgm-pa33-540-ab-macro-precomputed-flags.md` showed full compile median -0.46%, average -0.60%.
- Rejected repeated-argument cache vs precomputed-flags: `/tmp/cppgm-pa33-540-ab-macro-param-cache-large-preprocess.md` showed large `-E` median +2.28%, average +3.95%.
- Translator ASCII direct vs macro-fastpath: `/tmp/cppgm-pa33-540-ab-translator-ascii-direct-large-preprocess.md` showed large `-E` median -6.81%, average -6.44%; `/tmp/cppgm-pa33-540-ab-translator-ascii-direct.md` showed full compile median -3.68%, average -3.46%.

Current final candidate binary: `/tmp/cppgm-pa33-540-final-translator-ascii-direct-cppgm++`.
...
- Preprocessor output buffering vs translator ASCII direct: `/tmp/cppgm-pa33-540-ab-preproc-output-buffer-large-preprocess.md` showed large `-E` median -7.05%, average -4.56%.

Current final candidate binary: `/tmp/cppgm-pa33-540-final-preproc-output-buffer-cppgm++`.

Rejected follow-up:

- Switching `stream_post_tokens` to use `IPostTokenSource::get_many` was tried after output buffering. It regressed the large `-E` benchmark, likely because the extra vector materialization outweighed fewer virtual calls in this path, so it was reverted. Report: `/tmp/cppgm-pa33-540-ab-stream-post-tokens-batch-large-preprocess.md` showed median +1.67%, average +1.10%.

Rejected follow-up:

- A `PostTokenizer::emit_identifier` prefilter to skip impossible keyword/operator unordered-map lookups was tested and reverted. Report: `/tmp/cppgm-pa33-540-ab-posttoken-keyword-prefilter-large-preprocess.md` showed large `-E` median +1.12%, average +1.90%.
...
- Empty `-E`, warm cache vs disabled cache: `/tmp/cppgm-pa33-540-ab-host-probe-cache-empty.md` showed median -89.66%, average -89.83%.
- Frozen `callsemantic.cpp -E`, warm cache vs disabled cache: `/tmp/cppgm-pa33-540-ab-host-probe-cache-large-preprocess.md` showed median -11.24%, average -12.93%.
- Full 540 compile, warm cache vs disabled cache: `/tmp/cppgm-pa33-540-ab-host-probe-cache-full.md` showed median -8.81%, average -8.85%.

Current final candidate binary: `/tmp/cppgm-pa33-540-final-host-probe-cache-cppgm++`.
...
- Empty `-E`, build-time defaults vs runtime host probe: `/tmp/cppgm-pa33-540-ab-buildtime-host-defaults-empty.md` showed median -91.78%, average -90.99%.
- Frozen `callsemantic.cpp -E`, build-time defaults vs runtime host probe: `/tmp/cppgm-pa33-540-ab-buildtime-host-defaults-large-preprocess.md` showed median -12.57%, average -12.30%.
- Full 540 compile, build-time defaults vs runtime host probe: `/tmp/cppgm-pa33-540-ab-buildtime-host-defaults-full.md` showed median -11.23%, average -8.40%.

Current final candidate binary: `/tmp/cppgm-pa33-540-final-buildtime-host-defaults-cppgm++`.
...
- Large frozen `callsemantic.cpp -E`: `/tmp/cppgm-pa33-540-ab-pptoken-identifier-fastpath-large-preprocess.md` showed median `-4.97%`, average `-1.30%` vs build-time host defaults.
- Full 540 compile: `/tmp/cppgm-pa33-540-ab-pptoken-identifier-fastpath-full.md` showed median `-0.51%`, average `-0.13%` vs build-time host defaults.

Current final candidate binary: `/tmp/cppgm-pa33-540-final-pptoken-identifier-fastpath-cppgm++`.
...
A/B reports against `/tmp/cppgm-pa33-540-final-pptoken-identifier-fastpath-cppgm++`:

- Large frozen `callsemantic.cpp -E`: `/tmp/cppgm-pa33-540-ab-buffer-inline-next-large-preprocess.md` showed median `-12.74%`, average `-9.48%`.
- Full 540 compile: `/tmp/cppgm-pa33-540-ab-buffer-inline-next-full.md` showed median `-4.45%`, average `-3.32%`.

Current final candidate binary: `/tmp/cppgm-pa33-540-final-buffer-inline-next-cppgm++`.
...
Rejected after `/tmp/cppgm-pa33-540-ab-whitespace-fastpath-large-preprocess.md`: median was `-1.52%`, but average was `+1.14%` against `/tmp/cppgm-pa33-540-final-buffer-inline-next-cppgm++`. The result was too noisy and the change carried extra token-boundary risk around whitespace/comment coalescing, so it was reverted.
...
Rejected after `/tmp/cppgm-pa33-540-ab-fulltranslator-increment-large-preprocess.md`: median was `+1.39%`, average `+2.68%` against `/tmp/cppgm-pa33-540-final-buffer-inline-next-cppgm++`.
...
Rejected after `/tmp/cppgm-pa33-540-ab-posttoken-direct-large-preprocess.md`: median was `+6.56%`, average `+6.62%` against `/tmp/cppgm-pa33-540-final-buffer-inline-next-cppgm++`. The extra branching/construction cost outweighed removing the deque push/pop path.
...
A/B reports against `/tmp/cppgm-pa33-540-final-buffer-inline-next-cppgm++`:

- Large frozen `callsemantic.cpp -E`: `/tmp/cppgm-pa33-540-ab-posttoken-op-switch-large-preprocess.md` showed median `-2.71%`, average `-2.87%`.
- Full 540 compile: `/tmp/cppgm-pa33-540-ab-posttoken-op-switch-full.md` showed median `-0.90%`, average `-9.40%`, with noisy full-compile outliers but no median regression.

Current final candidate binary: `/tmp/cppgm-pa33-540-final-posttoken-op-switch-cppgm++`.
...
Rejected after `/tmp/cppgm-pa33-540-ab-buffer-always-inline-large-preprocess.md`: median was `+9.04%`, average `+11.62%` against `/tmp/cppgm-pa33-540-final-posttoken-op-switch-cppgm++`. The likely cause is code-size / optimizer-heuristic fallout, so the methods were restored to normal definitions.
...
Rejected after `/tmp/cppgm-pa33-540-ab-posttoken-location-guard-large-preprocess.md`: median was `+0.18%`, average `+0.81%` against `/tmp/cppgm-pa33-540-final-posttoken-op-switch-cppgm++`. The compiler already handles the inline null checks well enough, so the extra outer branch was not useful.
...
Rejected after `/tmp/cppgm-pa33-540-ab-posttoken-emplace-large-preprocess.md`: median was `+2.47%`, average `+2.89%` against `/tmp/cppgm-pa33-540-final-posttoken-op-switch-cppgm++`. The existing temporary path appears to optimize better than the added overloads/emplace path.
...
Rejected after the corrected candidate passed full compilation but regressed `/tmp/cppgm-pa33-540-ab-posttoken-keyword-switch-full.md`: median `+1.46%`, average `+0.52%` against `/tmp/cppgm-pa33-540-final-posttoken-op-switch-cppgm++`. The large `-E` run was modestly positive (`/tmp/cppgm-pa33-540-ab-posttoken-keyword-switch-large-preprocess.md`, median `-1.25%`, average `-2.11%`), but the full compile path remains the controlling benchmark.
...
Rejected after `/tmp/cppgm-pa33-540-ab-whitespace-bool-large-preprocess.md`: median was `+1.25%`, average `+2.31%` against `/tmp/cppgm-pa33-540-final-posttoken-op-switch-cppgm++`. The original counter loop appears to optimize better.
...
A/B reports against `/tmp/cppgm-pa33-540-final-posttoken-op-switch-cppgm++`:

- Large frozen `callsemantic.cpp -E`: `/tmp/cppgm-pa33-540-ab-identifier-known-initial-large-preprocess.md` showed median `-1.21%`, average `+0.51%` under noisy samples.
- Full 540 compile: `/tmp/cppgm-pa33-540-ab-identifier-known-initial-full.md` showed median `-10.93%`, average `-4.64%`, also with heavy machine-noise outliers in both variants.

Current final candidate binary: `/tmp/cppgm-pa33-540-final-identifier-known-initial-cppgm++`.
...
Rejected after `/tmp/cppgm-pa33-540-ab-identifier-ascii64-large-preprocess.md`: median was `+1.76%`, average `+2.73%` against `/tmp/cppgm-pa33-540-final-identifier-known-initial-cppgm++`. The larger stack scratch likely hurt cache/register behavior more than it helped long names.
...
Rejected after `/tmp/cppgm-pa33-540-ab-normalizer-snextc-large-preprocess.md`: median was nearly flat at `-0.35%`, but average regressed `+1.42%` against `/tmp/cppgm-pa33-540-final-identifier-known-initial-cppgm++`. The existing explicit pair remains clearer and not measurably worse.
