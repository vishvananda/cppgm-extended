# PA37 Self-Host Buildout Process

Archive note: this buildout is complete. Current self-host validation is through
`pa37` `test-through-*` targets and the root `test-report-pa37` wrapper. The
...
for building out `PA37`.

`PA37` is different from the earlier assignments because the main goal is no
...
So `PA37` is a self-hosting and regression-preservation milestone, not just a
...
For `PA37`, the contract is instead:
...
## Core Contract For PA37

Before building tests, freeze the `PA37` contract explicitly.
...
The README should avoid pretending that `PA37` is just "make everything work."
...
the default PA37 build flow should require students to pass the relevant `.cpp`
...
directly in the PA37 `Makefile`. They should not depend on generated manifests
or automatic link-set discovery in the student-facing path.

## Oracle For PA37

The primary oracle for `PA37` is not a text dump. It is a staged
...
That means `PA37` should use executable/toolchain validation as the primary
contract, unlike most earlier PAs.

## Test Surface Design

`PA37` tests should be organized in layers.

### 1. Early-Frontend Self-Host Ladder

The first phase of `PA37` should be the `pa1` through `pa9` early self-host
work.
...
- the checkpoint source lists should be written directly in the PA37
  `Makefile`
...
`PA37` should then select a defined set of earlier tests that are rerun through
...
The maintainer buildout for `PA37` should still keep a larger checkpoint gate:
...
## README Guidance For PA37

`PA37` will likely need more guidance than a typical earlier assignment, but
...
`PA37` is the first assignment where students will likely hit bugs in earlier
...
- the published PA37 test surface must be achievable from the shipped contract
...
- [pa37-ladder-fix-process.md](../../legacy/pa37-ladder-fix-process.md)

The public PA37 contract can absorb the idea of the PA-fast ladder without
...
The current maintained execution surface is the `pa37/Makefile`
`test-through-*` family.

## Buildout Sequence

Recommended `PA37` buildout order:

1. freeze the PA37 README contract and required validation surface
...
3. switch the public default build contract to explicit source-file lists for
   those checkpoint binaries, written directly in the PA37 `Makefile`
...
When `PA37` self-host work finds a real earlier bug:

- put the durable regression in the earliest real owning `paN`
- keep only the self-host integration proof in `PA37`
