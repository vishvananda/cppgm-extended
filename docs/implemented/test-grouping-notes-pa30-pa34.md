...
- Missing tests to add later: focused driver-mode owners for any remaining `-c`/link edge cases after the PA31-PA34 split settles; possible future N3485-specific linkage tests should go in `tests/spec/` with a leading `// N3485 focus: ...` comment.
...
- Tests renamed: none for uniqueness. A follow-up numbering correction restored
  ...
  `tests/general/`.
- Tests kept in spec and N3485 anchors: none. PA31 tests target host object format, host link, symbol spelling, coalescing, and interoperability.
- Tests that still need N3485 comments: none because no `tests/spec/` bucket remains.
...
- Validation: `git diff --check` and `git diff --cached --check` pass.
  `make -C pa31 test CXX=/usr/local/opt/llvm/bin/clang++ CPPGM_HOST_CXX=/usr/local/opt/llvm/bin/clang++` passes `62/62`.
- Open questions: none for grouping.
...
- Tests kept in spec and N3485 anchors: none. PA32 tests target host ABI/runtime behavior, EH, RTTI/vtables, thunks, and related object facts.
- Tests that still need N3485 comments: none because no `tests/spec/` bucket remains.
...
- Validation: `git diff --check` and `git diff --cached --check` pass.
  `make -C pa32 test CXX=/usr/local/opt/llvm/bin/clang++ CPPGM_HOST_CXX=/usr/local/opt/llvm/bin/clang++` passes `77/77`.
- Open questions: none for grouping.
...
- Tests renamed: none. A follow-up numbering correction kept the existing
  ...
  numeric prefixes.
- Tests kept in spec and N3485 anchors: none. PA33 tests are hosted/vendor compatibility, reducer, sentinel, and bootstrap-facing compile cases.
...
- Validation: `git diff --check` and `git diff --cached --check` pass.
  `make -C pa33 test CXX=/usr/local/opt/llvm/bin/clang++ CPPGM_HOST_CXX=/usr/local/opt/llvm/bin/clang++`
...
- Tests moved: none across buckets; `tests/link/` is kept because link/run and optional symbol inspection are the PA34 oracle.
- Tests renamed: none. A follow-up numbering correction kept the existing
  pa1-pa9-style shared hosted-link family prefixes rather than forcing unique
  numeric prefixes.
- Tests kept in spec and N3485 anchors: none. PA34 tests are hosted link/runtime, ABI spelling, object-inspection, and regression cases.
- Tests that still need N3485 comments: none because no `tests/spec/` bucket exists.
- Missing tests to add later: no new cases added; future additions should stay in `tests/link/` unless they are hosted compile-only regressions that belong back in PA33.
