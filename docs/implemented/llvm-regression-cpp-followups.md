...
- wrapper-linked programs now route ordinary allocation/deallocation through the host runtime surface via `pa31` link smoke `669-cmake-wrapper-new-delete-link-smoke`
- `exception_spec_test.cpp`: fixed via `pa31` hosted link/runtime smoke `675-hosted-dynamic-exception-spec-runtime`; hosted `std::set_terminate` / `std::set_unexpected` now mangle to host-compatible `std` free-function symbols, dynamic exception specifications dispatch disallowed escapes through `std::unexpected` / `std::terminate`, and the final hosted link no longer needs a helper EH runtime object
