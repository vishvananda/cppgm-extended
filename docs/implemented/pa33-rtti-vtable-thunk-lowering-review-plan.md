# PA33 RTTI / Vtable / Thunk Lowering Review Plan

## Goal

Apply the same explicit lowering-review method used for PA23 to the PA33
RTTI/vtable/thunk surface, with a C++ ABI-specific oracle.

For PA33, runtime alone is not enough. The review should inspect:
...
This plan stays inside PA33's owned host-linked RTTI/vtable/thunk subset:
...
Start with the current PA33 ABI owner ladder and extend where coverage is thin:
...
A complete PA33 lowering-review tranche should leave behind:

- a checked-in tracker with every reviewed owner family listed explicitly
- added or tightened STL-free PA33 owners where current coverage is thin
...
- README language making clear that PA33 ownership includes the observable host
...
The checked-in tracker now covers the targeted ownership-heavy PA33 ladder
...
missing PA33-owned ABI surface.
