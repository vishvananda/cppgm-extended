...
## Semantic source ownership and definition-demand checkpoint, 2026-08-11

This Phase 5 checkpoint removes four inherited-constructor source rows and
repairs three other typed function-call facts without a fixture, path, line,
or rendered-name exception.

An inherited constructor wrapper is not a public function-template call
owner. Function-call publication now recognizes that status on either the
selected binding or its source template. It also recognizes constructor work
performed while an inherited wrapper is the semantic closure trigger. Both
the wrapper call and the nested synthesized-initializer call remain available
to semantic analysis but no longer publish source rows.

Suppressing those rows originally hid real base-constructor definition
demands. Closure entry contexts therefore retain the semantic trigger binding,
and each witness session carries a typed set of function-definition
dependencies. The lifecycle observer propagates membership through semantic
function edges. This preserves the required base-constructor closure facts
without using a false function-call row as an ownership side channel. The
same carrier lets a direct reference-binding conversion retain definition
demands from the selected conversion-function body while excluding target
constructor probes from its public candidate-drop set.

Conditional-expression conversion now passes each operand's structured AST
start location through `ArgumentConversionOptions`. The selected conversion
operator is consequently anchored at the converted operand rather than at a
later parser cursor. Out-of-class member-template drop suppression is also
narrowed by the selected class definition: it remains in force for a primary
class-template definition, but a selected class partial specialization keeps
the overload ranks and partial-order drops produced by its semantic operation.

The five newly exact outputs are:

- `pa23/tests/general/200-inherited-constructor-template-forwarding.t`;
- `pa23/tests/general/400-partial-specialization-inherited-constructor-template.t`;
- `pa23/tests/general/500-inherited-constructor-template-member-alias-pack.t`;
- `pa24/tests/general/400-out-of-class-partial-member-template-owner-parameter-alias.t`;
- `pa23/tests/spec/500-conversion-function-template-reference-conditional-auto-ref.t`.

Focused controls cover inherited non-constructor member templates, primary
out-of-class member-template definitions, direct reference conversion
operators, and object-result conversion operators. All 12 target and control
outputs are byte-exact, and every focused LowIR result is accepted by the
shared comparison contract.

Expanded convergence improves from 1,518 to 1,523 exact outputs. The
1,530-reference inventory has seven known mismatches, no warning output, and
no missing actual file. Function-call debt falls from 14 mismatched
occurrences across ten tests to six occurrences across five tests: one changed
row, three missing rows, and two unexpected rows. Class-use remains one
changed row in one test, and lifecycle remains the one lazy-alias
class-instantiation gap. No test enters the mismatch set or changes family.

The final Homebrew-Clang validation produced these results:

- ordinary and provenance strict runs both report PA19 279 compared with zero
  failures, PA20 158 with zero, PA22 293 with three residuals, PA23 385 with
  two, and PA24 415 with two; their expected nonzero exit is exactly the
  documented seven-test residual set;
- the PA1-PA38 direct-LowIR report passes 4,862/4,862, including PA30 runtime;
- all 1,530 ordinary and provenance witness sessions complete, and all 3,060
  ordinary/provenance witness and LowIR pairs match byte for byte;
- all 1,530 provenance sessions flush, producing 69,187 records, 4,767 source
  attempts, and 6,298 lifecycle attempts with no unknown producer attempt and
  no unexercised producer site;
- the convergence, provenance, materialization, text-reparse, path,
  performance, template-boundary, and class-audit helper suites pass 60/60;
...
The function producer now makes 1,268 attempts, inserts 838 rows, classifies
430 exact duplicates, and leaves 789 final visible rows. Relative to the
parent, four inherited-constructor rows disappear before publication and one
duplicate attempt disappears. The renderer still removes four same-location
calls, 44 source-defined calls, and one visible duplicate; its legacy
drop-order pass still rewrites 27 events. There are no rejected, replaced, or
enriched function attempts. Class consolidation is unchanged at 3,003
completed candidates, 3,206 early repeats, 358 prepublication merges, 2,645
collected occurrences, and 2,632 publications. Lifecycle attempts are
unchanged in number; 39 definition-demand events move from incidental
visibility to the typed public-source dependency route.

The ordinary convergence report is
`/tmp/cppgm-phase5-source-ownership-convergence-20260811.json`, SHA-256
`e957b572bb885f8eaa1b038606c9b671e0c491e50439a225a2225958aa082b91`.
The PA1-PA38 report is
`/tmp/cppgm-phase5-source-ownership-broad-20260811.log`, SHA-256
`1024fd1b54513eed3ab617c468a27c5f868c3abf72e815b7b1c38d6fe8909c85`.

The provenance trace directory is recorded in
`/tmp/cppgm-phase5-source-ownership-provenance-trace-path-20260811.txt`.
The provenance analysis and correlated convergence reports are
`/tmp/cppgm-phase5-source-ownership-provenance-analysis-20260811.json` and
`/tmp/cppgm-phase5-source-ownership-provenance-convergence-20260811.json`,
with SHA-256 values
`73b5c5bdf235d8e0fa67aa21ce6c71aa0de9a9221ef01f4e94447bfb11bbf84d`
and
`91bbadd5be6803e9e8a17dce1e56c0fcb43620338e2a43e15a3227c93e5219fa`.
The byte-identical ordinary and provenance manifests both have SHA-256
`e2b39c7b918fa4db0d951fcacb2c7116ba750c36ef581006aa74525c37931631`;
their empty difference has SHA-256
`e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855`.

The 60-test helper report is
`/tmp/cppgm-phase5-source-ownership-helper-tests-20260811.log`, SHA-256
`8235f773fb26b9bcb5dd618733c91e0f98fa0776579f9161d260e329d7801e5d`.
The materialization, zero-finding text-reparse, template-boundary,
semantic-boundary, and structure-size reports have SHA-256 values
`27acfb819a6872ffb36e59e33cccdec28a83ea0543b69f5b4c0a8bb3ee33e526`,
`1de948196cc856fc673897264f3b7210dab0ab768743743555644db743b7c515`,
...
The ordinary binary grows by 4,168 bytes to 17,261,728 bytes. Its Mach-O
`__TEXT` segment grows by 4,096 bytes to 13,123,584; `__DATA_CONST`, `__DATA`,
and `__LINKEDIT` remain 61,440, 442,368, and 4,071,424 bytes. It contains no
witness-provenance symbols. The frozen binary is
`/tmp/cppgm-phase5-source-ownership-ordinary-20260811`, SHA-256
`880b15f62cf506973114a39472f00657b453f2251e51e0cc386e39647b917d14`.

The three-run performance record passes both comparisons:

| Comparison | Instructions | Maximum RSS | Peak footprint | Report |
| --- | ---: | ---: | ---: | --- |
| Fixed alias-convergence baseline | -1.13% | -0.27% | -4.09% | `/tmp/cppgm-phase5-source-ownership-perf-fixed-20260811.json` |
| Typed candidate-normalization parent | +0.07% | +0.39% | -0.10% | `/tmp/cppgm-phase5-source-ownership-perf-parent-20260811.json` |

The candidate medians are 174,036,777,199 instructions, 755,052,544 bytes
maximum RSS, and 568,741,888 bytes peak footprint. Wall time remains an
informational measurement. The raw candidate record is
`/tmp/cppgm-phase5-source-ownership-raw-candidate-20260811.json`, SHA-256
`45d225271d0db0218d2ed945b93ef04c05f0026ce709c5796ad2d7e8dee7179b`.
The fixed-baseline and parent-comparison reports have SHA-256 values
`951c50fb0bd40a5a001cfd99b0f4e15b6bba0369aea2ec3099e563404f5f738a`
and
`b8ff75c44ce72ba51c293ffbc3bcaac87b5272927856d6a37ea263d31c801973`.
The candidate metadata names commit `0f40cb6b1` because the measurements cover
this uncommitted checkpoint.

This checkpoint adds 110 and removes nine production lines before this ledger
entry. Phase 5 remains open on the five function-call tests and the one
class-use test. Phase 4 remains open on the lazy-alias lifecycle oracle
divergence. Inception remains forbidden.
...
## Derived source-anchor carrier collapse checkpoint, 2026-08-12

This Phase 6 checkpoint removes the final source-table anchor wrappers whose
values were already determined by canonical locations. Every public spelling
anchor was either identical to `SemanticSourceUse::location` or absent, in
which case the renderer fell back to that same location. The final
`SourceAnchor`/`SourceAnchorKind` representation therefore disappears,
template-id occurrences retain their name location directly, and provenance
derives its schema-stable `spelling_anchor` field from the canonical use
location. The selected-declaration location remains an independent string.

The same proof deletes three upstream mirrors. All five class-use producers
set `use_anchor_present` true and `use_anchor_location` equal to `location`, so
the request pair and its always-equal preference term are gone. Function calls
assigned `decision.use_anchor` from `decision.location`, and alias calls did
the same through a helper, so both carriers and the helper disappear.
Variable-use identifier validation could only decide whether to populate the
redundant spelling anchor; a failed check still rendered the already-set use
location, so that request field and branch are also gone. The richer
`TemplateWitnessSourceAnchorKind` used by selected-declaration request policy
is retained because it still distinguishes exact declaration-name anchors
from approximate fallbacks.
...
The ordinary and provenance strict reports are
`/tmp/cppgm-phase6-anchor-collapse-ordinary-strict-20260812.log` and
`/tmp/cppgm-phase6-anchor-collapse-provenance-strict-20260812.log`. Both have
SHA-256
`bec6edbbb5a4cdeb8d9064ab5773d4194921f82d9ab63723493769e91f950a94`.
The integrated broad report is
`/tmp/cppgm-phase6-anchor-collapse-broad-20260812.log`, SHA-256
`2d79de16f5593712f6c0549f05a73618a248bda7bce7745403978687132346c3`.

The provenance trace directory is
`/tmp/cppgm-phase6-anchor-collapse-final-provenance-trace-20260812.yqQ6aW`.
The provenance analysis and correlated convergence reports are
`/tmp/cppgm-phase6-anchor-collapse-provenance-analysis-20260812.json` and
`/tmp/cppgm-phase6-anchor-collapse-provenance-convergence-20260812.json`, with
SHA-256 values
`591b760ebe9671941d0c265170cbd19a9e3d6718c5e31b40daa3f4f66c741359`
and
`6de1ceb5ac0c937a5a131015acb64a567c265ad7649efa11486d8316624588b1`.
The normalized ordinary and provenance manifests both retain SHA-256
`fd40d5ae2cbf63b17387317c614d95f61d73c4bdeb0cf7630aadf7630c53e940`.

The 60-test helper report is
`/tmp/cppgm-phase6-anchor-collapse-helper-tests-20260812.log`, SHA-256
`e2d529a83510538ed5efc20b65bd168220dd94d695b46cced04e363a3735de39`.
The materialization, zero-finding text-reparse, template-boundary,
...
`4f0a11b4f714f79a05808fd51bf8bfb2a048e9f615debd7365ba954b7c37afa8`.
The dynamic class audit is
`/tmp/cppgm-phase6-anchor-collapse-class-materialization-audit-20260812.json`,
SHA-256
`0fd5e887a77c3c1b38d330888b25679fc8c43151574ff52c19f1997718dba038`.

The ordinary binary shrinks by 4,888 bytes to 17,163,152 bytes. Its Mach-O
`__TEXT` segment shrinks by 4,096 bytes to 13,033,472; `__text` shrinks by
3,664 bytes and `__gcc_except_tab` by 36 bytes. `__DATA_CONST`, `__DATA`, and
`__LINKEDIT` remain 61,440, 442,368, and 4,063,232 bytes. The binary contains
no provenance symbols. The frozen binary is
`/tmp/cppgm-phase6-anchor-collapse-final-ordinary-20260812`, SHA-256
`4de4e331c062f4b047dc3a0198ce2b7177f4359dd93fc7aaa09c5849d61c1be2`.

The clean ordinary three-run performance record passes both comparisons:

| Comparison | Instructions | Maximum RSS | Peak footprint | Report |
| --- | ---: | ---: | ---: | --- |
| Fixed alias-convergence baseline | -0.85% | +0.19% | -4.04% | `/tmp/cppgm-phase6-anchor-collapse-perf-fixed-20260812.json` |
| Dead-payload parent | -0.15% | -0.14% | +0.08% | `/tmp/cppgm-phase6-anchor-collapse-perf-parent-20260812.json` |

The candidate medians are 174,513,989,153 instructions, 758,538,240 bytes
maximum RSS, and 569,061,376 bytes peak footprint. The raw candidate is
`/tmp/cppgm-phase6-anchor-collapse-raw-candidate-20260812.json`, SHA-256
`2de04e50ac6e2f64f722bbf50fdeb6f9d2f8d199841989f70c0ae5fc602e6337`.
The fixed-baseline and parent-comparison reports have SHA-256 values
`2cbc1239b9d7e596df7126f8d65f2c15baeb0b2643e677a506f638dedf99dfe2`
and
`835b6a6cab7329f5d7797724860683242639785e7dedcb42d81c317c5fefc27a`.
The candidate metadata names commit `da263ef7e` because the measurements cover
this uncommitted checkpoint.

This checkpoint adds 45 and removes 170 production lines, a net deletion of
...
## Linker-dead source recovery checkpoint, 2026-08-13

Commit `0ae101873eae0a48642e8d7bb247109a3da39f5c` is the parent. This
checkpoint closes the Phase 6 production-source criterion. The candidate has
413,758 lines under `dev/src`, 49 fewer than the 413,807-line Phase 0 recovery
tree.

Clang dead-strip linker maps for every production frontend and
`mobjroundtrip` supplied the initial symbol inventory. The source audit then
required each deleted root to have no C++ caller and each private helper chain
to close under that root. The audit kept functions whose standalone symbols
disappeared through inlining while live callers still used their definitions.

The deleted code falls into three groups:

- obsolete source-location and template-source recovery, including compact
  token reconstruction, current-specialization alias spelling rewrites, pack
  binding text repair, qualified-template subtree probes, and unused
  template-angle recovery;
- retired template, semantic, ABI, LowIR, MIR, CY86, assembler, preprocessor,
  and compiler-driver adapters with no source caller;
- the unused `lowir_tool_cli` translation unit and its frontend source-set
  entry.

The Homebrew-Clang validation produced these results:

- all production frontends and `mobjroundtrip` build without warnings;
- the frozen parent and candidate produce 10,884 artifacts each over all
  1,530 strict inputs. Exit status, witness output, LowIR, diagnostics, and
  stdout match byte for byte;
- ordinary and provenance strict retain the three documented cross-oracle
  rows: PA19 279/0, PA20 158/0, PA22 293/1, PA23 385/1, and PA24 415/1;
- expanded convergence remains 1,527/1,530 with no missing actual output or
  warning;
- the integrated PA1-PA38 direct-LowIR report passes 4,862/4,862. PA9 appears
  once in that report and had no separate invocation;
- ordinary and provenance compilers produce the same 10,884 artifacts. All
  1,530 provenance trace files exist, and schema 6 contains 10,587 records:
  4,289 source publications and 6,298 lifecycle publications. The analyzer
  finds no unknown producer or unexercised site;
- the helper suite passes 261 tests with one intentional skip. Ownership,
  static and dynamic materialization, and duplicate-AST audits have no
  finding. All 23 text-reparse categories remain zero;
- the dynamic class-materialization audit retains five accepted source
  occurrences, 55 rejected rows at 53 locations, and 2,631 class
  publications;
- the template-boundary inventory reduces `text_recovery_bridge` from 115 to
  114 and retains the other parent counts: 4, 2, 15, 63, 136, and 193. The
  semantic-boundary inventory remains 0, 0, 0, 5, 10, and 9.

The parent-parity and ordinary/provenance mismatch reports are
`/tmp/cppgm-phase6-source-recovery-parent-parity-20260813.txt` and
`/tmp/cppgm-phase6-source-recovery-ordinary-provenance-parity-20260813.txt`.
Both are empty and have SHA-256
`e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855`.

The ordinary strict, provenance strict, integrated, and helper reports are
`/tmp/cppgm-phase6-source-recovery-strict-20260813.log`,
`/tmp/cppgm-phase6-source-recovery-provenance-strict-20260813.log`,
`/tmp/cppgm-phase6-source-recovery-integrated-20260813.log`, and
`/tmp/cppgm-phase6-source-recovery-helper-tests-20260813.log`. Their SHA-256
values are
`bec6edbbb5a4cdeb8d9064ab5773d4194921f82d9ab63723493769e91f950a94`,
`7aa4cdb5b6a3f97b1ab2b0e92bab4ceb997a5a2c2720a2d582a03e80837ff729`,
`c1d057a8db0d00a9c2600660dbf8f997ae3c067b31ba014df418651d3fb19dce`,
and
`f7e39791fbbf2e8832c72ac58046e7e2b835706a45ec775c12bf2425176c4a37`.

The provenance trace directory is
`/tmp/cppgm-phase6-source-recovery-provenance-trace-20260813.8dnkgvRt`. The
analysis and convergence reports are
`/tmp/cppgm-phase6-source-recovery-provenance-analysis-20260813.json` and
`/tmp/cppgm-phase6-source-recovery-provenance-convergence-20260813.json`,
with SHA-256 values
`48e53fb0b96070270ff6b1ec51f84ce01c54a326081cb647618b0c391f1eb114`
and
`697daabd75ba73671dc8741a1b3454bf8ceb4d40dff759cfa17584073873c74b`.

The ownership, static materialization, dynamic class-materialization,
duplicate-AST, text-reparse, template-boundary, and semantic-boundary reports
have SHA-256 values
`140b898f29adbba7fb5b61e3e2d3515afb8224ad1bdb4c61afa491289617695f`,
`27acfb819a6872ffb36e59e33cccdec28a83ea0543b69f5b4c0a8bb3ee33e526`,
`722dcaa39ae98f81b38233f76bcc5af081fb7a30adbb32b317d8271ec451006c`,
`ba5355fab0e7bd3a74b6c085371e204afcdc7f540b7d55294ab8a23928d02332`,
`1de948196cc856fc673897264f3b7210dab0ab768743743555644db743b7c515`,
`cf00bf85435ad5abc53cead13c66deb754a6f6d6960fe7e36c638c21807d5e5b`,
and
`a8654f85de246d956481db71e121f1e8ff01fbf2e003bc2b9d968a847121dff2`.
...
`TemplateArgument`, `ClassInfo`, and `TemplateWitnessSession` remain 280, 136,
1,104, and 312 bytes. The report is
`/tmp/cppgm-phase6-source-recovery-structure-sizes-20260813.txt`, SHA-256
`1f872ff18916238323328ce75039b29c15d01f4bd37303c6b4e604112e09b00d`.

The ordinary binary shrinks by 85,928 bytes to 16,869,344 bytes and contains
no provenance symbols. `__text`, `__gcc_except_tab`, and `__unwind_info`
shrink by 48,896, 2,704, and 888 bytes. The `__cstring` section grows by 24
bytes, while all `__DATA` sections remain unchanged. The section report is
`/tmp/cppgm-phase6-source-recovery-binary-sections-20260813.txt`, SHA-256
`b942d4ed9b5973cbdfe06f0e9f2c23e3da55f3877ef1cc27a9ef9af543b72c57`.
The frozen ordinary binary is
`/tmp/cppgm-phase6-source-recovery-candidate-ordinary-20260813`, SHA-256
`80ed9c69fdd18f984f70ced7c8807e1af192fa8f766323ef00d33642003a4eed`.

The ordinary three-run performance record passes both comparisons:

| Comparison | Instructions | Maximum RSS | Peak footprint | Report |
| --- | ---: | ---: | ---: | --- |
| Fixed alias-convergence baseline | -0.82% | +0.03% | -4.10% | `/tmp/cppgm-phase6-source-recovery-perf-fixed-20260813.json` |
| Immediate parent | -0.26% | -0.10% | +0.10% | `/tmp/cppgm-phase6-source-recovery-perf-parent-20260813.json` |

The candidate medians are 174,568,495,125 instructions, 757,342,208 bytes
maximum RSS, and 568,725,504 bytes peak footprint. The raw candidate is
`/tmp/cppgm-phase6-source-recovery-raw-candidate-20260813.json`, SHA-256
`9200bf2476935947ecc58225fd6eced3dd03680389d8ea15ae548b54630dc350`.
The fixed-baseline and parent-comparison reports have SHA-256 values
`e54de43fe7beea59a44f697b7d5e8ce08ddf5ef7d25890b15bab18ea9163fc00`
and
`9cf7d3ea1d083035b0c6294c9a776521639762ba4fd2b2f94c96911345221369`.
The candidate metadata names commit `0ae101873` because the measurements cover
this uncommitted checkpoint.

Production code adds no line and removes 2,312, a net deletion of 2,312.
Since the Phase 6 entry commit
`0014913a13cd1ba59abc24136be1026bf4631c5c`, the cumulative production change
adds 1,475 and removes 9,923 lines, a net deletion of 8,448. Phase 6 is
complete. Phase 7 begins from this committed checkpoint; inception remains
forbidden until the committed-clean-worktree correctness and performance gate
passes.
