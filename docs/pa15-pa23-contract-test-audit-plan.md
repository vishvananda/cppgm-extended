# PA15-PA23 Contract And Test Audit Plan

## Purpose

PA15 through PA23 are the front-end and LowIR assignments where the language
surface grows fastest. PA15 is the procedural LowIR baseline, PA16-PA18 add the
object model, and PA19-PA23 complete the template/constant-evaluation language
...
- an exact, public assignment contract in each `pa15` through `pa23` README
...
maintainer-written course tests. For PA15-PA23 it is a current-state input only:
...
### PA15: Procedural C++ To LowIR

PA15 owns the first C++ source-to-LowIR lowering boundary over the PA12
...
- accepted scalar immediate canonicalization where the PA15 README permits it

PA15 does not own class/object semantics, synthesized class helper output,
...
- `pa15/tests/general/200-five-arg-call.t` may contradict the current "up to
  four parameters" README wording
- `cppgm.tests/course/pa15/232-const-ref-converted-float-argument.t` may
  exercise floating conversion support that the README currently places out of
  scope
- local static and array initialization tests need a precise PA15 boundary line
  so they do not blur into PA16 lifetime or PA21 constant-evaluation ownership

### PA16: Basic Object Model

PA16 owns the non-polymorphic class/object layer over the PA15 procedural LowIR
surface:

- namespace-scope and nested-namespace class/struct definitions and forward
  declarations
- nested class/struct declarations when they are needed by the PA16 object model
...
- ordinary non-template operator overloads over the PA16 object subset,
...
- non-template non-member calls found through associated-namespace lookup /
  hidden-friend ADL when arguments stay inside the PA16 object subset
...
PA16 must not own these unless the audit intentionally changes the contract:
...
- conversion-operator tests currently appear in PA16 but the README also calls
  broader conversion operators out of scope
- member-pointer tests need an explicit owner decision
- some out-of-class constructor tests may contradict the current PA16 out-of-
  scope text

### PA17: Non-Polymorphic Value Semantics

PA17 owns value transfer for non-polymorphic class objects:
...
- supported `operator new` / `operator delete` call surfaces used by PA17 value
  and cleanup tests

Known PA17 contract questions to resolve:

- lambda, range-for, `decltype`, and template syntax appear in PA17 tests as
  support fixtures and need explicit classification or rehoming
- inheriting constructors are owned by the PA16 class/using-declaration surface
  as a late cluster, so PA17 tests should be rehomed or reduced
...
PA18 owns polymorphic class semantics and LowIR lowering:
...
- covariant return checks over the supported pointer/reference class subset if
  kept in PA18 tests
- pure virtual declarations as declarations if kept in PA18 tests
- key-function / vtable binding behavior if kept in PA18 tests

Current PA18 tests also exercise a broader multiple-base subset. The audit must
choose one of these outcomes and make README/tests match:

- PA18 explicitly owns the tested limited multiple-direct-base subset, including
...
PA18 does not own full virtual inheritance, full RTTI, `dynamic_cast`, full
template-aware virtual dispatch, or general multiple-inheritance ABI behavior
unless the audit changes the contract.

### PA19: Basic Templates

PA19 owns the first usable template instantiation layer:
...
- out-of-class definitions of nested classes declared inside supported class
  templates when their bodies stay within PA16-PA18
- ordinary PA10 function declarator forms, including trailing return types, on
  supported function templates
- instantiated specialization names participating in ordinary PA16-PA18
...
PA19 does not own non-type template parameters/arguments, explicit
...
### PA20: First Metaprogramming Layer

PA20 owns the first practical compile-time template value layer:
...
PA20 does not own partial specialization, SFINAE-heavy metaprogramming, full
...
### PA21: Constant Evaluation

PA21 owns full `constexpr` / constant evaluation over the implemented language
surface inherited from PA20:
...
PA21 does not own template-language features deferred to PA22/PA23, post-C++11
constant-evaluation features, or hosted/vendor-only compatibility forms.

### PA22: Template Entities And Specialization Model

PA22 owns the template declaration graph and specialization model:
...
PA22 does not own full function-template deduction over the intended language
...
- tests with `sfinae` in the name must be audited carefully because SFINAE is
  out of PA22 scope unless the test is only a specialization-graph proxy

### PA23: Deduction, Substitution, And SFINAE Completion

PA23 owns the remaining template-language completion layer before native backend
work:
...
PA23 does not own hosted/vendor-only extensions, post-C++11 template-language
features, or backend/toolchain behavior that belongs to PA29 and later.

Known PA23 contract questions to resolve:

- PA19-PA22 tests with SFINAE, no-eager-instantiation, or full deduction
  behavior may need to move here
- PA23 itself may contain tests whose primary assertion is earlier PA19-PA22
...
Run the feature auditors over `pa15` through `pa23` after each batch of moves.
The final report must have no unexplained placement violations.

Run:

```sh
make test-report ACTIVE_TEST_REPORT_PAS='pa15 pa16 pa17 pa18 pa19 pa20 pa21 pa22 pa23'
...
- optional `--pa pa15 --pa pa23` narrowing
