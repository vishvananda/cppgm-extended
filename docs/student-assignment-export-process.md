...
PA39 is the main exception to the single-binary pattern. It is a staged
...
- exported binary name, or staged checkpoint targets for PA39-style assignments
- exported README
...
Handle each PA in order, `pa1` through `pa39`.
...
but not the desired long-term export mechanism for the full PA1-PA39 series.

## Completion Criteria

This export process is complete when:

- every cleaned PA from `pa1` through `pa39` has an export inventory entry
