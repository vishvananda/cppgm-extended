...
| allocate `Scope::named_type_access` on first use | `+0.170 / +0.058 / +1.237%` | balance score `0.788`; historical result was only a screen and RSS supplies most of the score | Run a current paired decision and an independent RSS confirmation. |
| inline `strip_top_level_cv` and return the selected pointer by reference | `+0.165 / -0.074 / +1.980%` | balance score `1.155`; historical result was only a screen and RSS supplies most of the score | Run a current paired decision and an independent RSS confirmation. Keep the value-returning API if the broad reference-return contract does not repeat. |
...
6. Recount dependent-resolution root repeats before restoring the last-result
   ...
   predicts reuse.
7. Run current paired decisions for the two balanced-lane omissions from the
   first ledger audit: lazy `Scope::named_type_access` and the combined inline,
   reference-returning `strip_top_level_cv`. Each old screen improved
   instructions and RSS, but each needs paired evidence and an independent RSS
   confirmation. Reject the broad API form if a narrower inspection-only
   helper captures the measured callers.
...
| allocate `Scope::named_type_access` on first use | 2,614 of 36,327 scopes populated the map. The candidate emitted exact output and used `124,074,856,078` instructions, a `0.17%` reduction. Against the contemporaneous checkpoint, footprint improved `0.058%` and one-run RSS improved `1.237%` | the old instruction-only rejection no longer decides the result. The historical balance score is `0.788`, but RSS supplies most of it. Run a current paired batch and independent RSS confirmation before retention | `/tmp/cppgm-lazy-named-type-access-screen.json` |
...
| inline `strip_top_level_cv` and return its selected `shared_ptr` by reference | the caller-attributed profile showed the out-of-line helper as a 183-sample leaf across 1,627 static call sites. The inline reference-returning form preserved the frozen object, reduced one-run RSS by `1.980%`, regressed footprint by `0.074%`, and used `119,375,123,872` instructions, a `0.165%` reduction | the old instruction-only rejection no longer decides the result. The historical balance score is `1.155`, but RSS supplies most of it. Run a current paired batch and independent RSS confirmation; retain the broad API only if a narrower inspection helper cannot reproduce the gain | `/tmp/cppgm-post-template-type-frame-profile-2.sample.txt` and `/tmp/cppgm-inline-strip-top-cv-screen.json` |
