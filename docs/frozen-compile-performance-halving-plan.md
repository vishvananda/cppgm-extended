...
| inline `strip_top_level_cv` across semantic translation units | the helper was the largest compiler-owned leaf in the fresh profile at 86 samples, but exposing its three-branch body in the header screened at `132,748,141,512` instructions, only about `0.03%` below the contemporaneous parent median | the sampled cost is the required shared-pointer result and kind test rather than call dispatch. Restore the out-of-line definition and avoid broad type-helper inlining | `/tmp/cppgm-inline-strip-cv-screen.json` and `/tmp/cppgm-using-directive-profile.sample.txt` |
