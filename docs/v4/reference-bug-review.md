...
| R3 | Value-initialization zeros empty complete objects and members; placement `new T()` records the preliminary zero-initialization before the selected non-user-provided constructor. The existing empty-base exception remains. Controls cover complete/member storage, empty bases, padding, bit-fields, unions, user-provided constructors, `T()`, `T{}` and default initialization. See [dcl.init]/5,7 and [dcl.init.aggr]. |
