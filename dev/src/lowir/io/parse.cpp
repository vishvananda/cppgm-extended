...
public:
  Parser(const std::vector<Token> & tokens, const std::string & source_name)
    : tokens_(tokens), at_(0), strings_(0), source_name_(source_name) {}

  Program Parse()
  {
    Program program;
    strings_ = &program.strings;
    while(!done()) { publish_position(); parse_top_level(program); }
    strings_ = 0;
    return program;
  }
...
private:
  ...
  // Publishes where the reader is, so an input error names a line without
  // every site carrying one.
  void publish_position() const
  {
    PublishLowirInputPosition(source_name_,
      at_ < tokens_.size() ? tokens_[at_].line :
...
private:
                     ...
                     const TypeIndex & slots)
  {
    ...
    // A slot argument passes the slot's contents, so its type is the
    // parameter's type, unless the parameter takes the slot's address
    // (pass=by_address, pass=indirect_result); a scalar slot where a direct
    // parameter reads a pointer would otherwise hand the callee the contents
    // as an address.
    const std::vector<Parameter> & params =
      ins.has_call_signature ? ins.call_params : *found->second.params;
    for(std::size_t i = 0; i < ins.args.size() && i < params.size(); ++i) {
      if(ins.args[i].kind != Operand::OP_SLOT ||
         params[i].metadata.passing != PPM_DIRECT) continue;
      const TypeIndex::const_iterator slot = slots.find(operand_spelling(ins.args[i]));
      if(slot == slots.end() || same_lowir_type(*slot->second, params[i].type)) continue;
      ThrowLowirInputError("call argument " + operand_spelling(ins.args[i]) +
        " is a slot of type " + lowir_type_text(*slot->second) +
        " but the parameter type is " + lowir_type_text(params[i].type));
    }
  }
                                      ...
                                      LowirEntryPolicy entry_policy)
{
  ...
  Program program = parse_tokens(tokens, entry_policy, source_name);
                                       ...
                                       LowirEntryPolicy entry_policy)
{
  ...
  Program program = parse_tokens(tokens, entry_policy,
    paths.size() == 1 ? paths[0] : std::string());
