...
// `literal_type` is the type an integer literal operand is read at; an
// `i128` literal prints its high word, a narrower one does not.
void write_operand(std::ostream & out, const Operand & operand,
                   const Program & program, const Function * function = 0,
                   const LowType * literal_type = 0)
{
  ...
  if(operand.kind == Operand::OP_FLOAT ||
    ...
    const std::string spelling = lowir_literal_text(
      operand, &program.strings,
      literal_type != 0 && literal_type->kind == LTK_I128);
    if(operand.kind == Operand::OP_FLOAT &&
                ...
                const Program & program, const Function * function)
{
  ...
  for(std::size_t i = 0; i < ins.args.size(); ++i) {
    ...
    write_operand(out, ins.args[i], program, function,
                  ins.has_call_signature && i < ins.call_params.size() ?
                  &ins.call_params[i].type : 0);
  }
  out << ')';
                       ...
                       const Program & program, const Function * function)
{
  ...
  case Instruction::IK_CONST:
    ...
    write_operand(out, ins.first, program, function, &ins.type); break;
  ...
  case Instruction::IK_STORE:
    out << (ins.volatile_access ? "store volatile " : "store ")
        << lowir_type_text(ins.type) << ' ';
    write_operand(out, ins.first, program, function);
    out << ", "; write_operand(out, ins.second, program, function, &ins.type); break;
  case Instruction::IK_ATOMIC_STORE:
    out << "atomic_store " << lowir_type_text(ins.type) << ' '; write_operand(out, ins.first, program, function);
    out << ", "; write_operand(out, ins.second, program, function, &ins.type); out << ", ";
  ...
  case Instruction::IK_ATOMIC_EXCHANGE:
    ...
    write_operand(out, ins.first, program, function); out << ", "; write_operand(out, ins.second, program, function, &ins.type);
  ...
  case Instruction::IK_CMP:
    ...
    write_operand(out, ins.first, program, function, &ins.type); out << ", "; write_operand(out, ins.second, program, function, &ins.type); break;
  ...
  case Instruction::IK_ATOMIC_COMPARE_EXCHANGE:
    ...
    write_operand(out, ins.first, program, function); out << ", "; write_operand(out, ins.second, program, function, &ins.type);
    out << ", "; write_operand(out, ins.third, program, function, &ins.type); out << ", ";
  ...
  case Instruction::IK_RETURN:
    ...
    if(ins.type.kind != LTK_VOID) { out << ' '; write_operand(out, ins.first, program, function, &ins.type); }
    break;
  ...
  case Instruction::IK_PHI:
    ...
    for(std::size_t i = 0; i + 1 < ins.args.size(); i += 2) {
      ...
      write_operand(out, ins.args[i], program, function);
      out << ": ";
      write_operand(out, ins.args[i + 1], program, function, &ins.type);
    }
    out << ']';
    break;
  }
                  ...
                  const Program & program)
{
  ...
  if(item.structured) {
    ...
    for(std::size_t i = 0; i < item.data_items.size(); ++i) {
      ...
      if(data.kind == GlobalDefinition::DataItem::ITEM_ZERO)
        ...
        write_operand(out, data.literal_operand, program, 0, &data.type);
      }
      out << '\n';
    }
    out << "}\n";
  ...
  if(item.init_kind == GlobalDefinition::INIT_ZERO) out << "zero";
    ...
    write_operand(out, item.init_operand, program, 0, &item.type);
  else {
  ...
  for(std::size_t i = 0; i < function.blocks.size(); ++i) {
    ...
    for(std::size_t j = 0; j < function.blocks[i].instructions.size(); ++j) {
      ...
      try {
        ...
        // Name the function: an operand without an identity is a lowering
        // defect, and the program has thousands of functions to search.
        ThrowLowirInternalError(std::string(error.what()) + " in function @" +
                                lowir_symbol_name(program, function.symbol) +
                                " block ^" +
                                lowir_block_label(program.strings, function, function.blocks[i].id));
      }
      out << '\n';
    }
  }
