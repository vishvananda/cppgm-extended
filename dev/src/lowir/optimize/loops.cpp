...
// What a loop writes: through unknown pointers, calls, atomics or bulk
// operations (`unknown`), which globals and which slots, and whether it
// writes memory at all.  A load is invariant only against this.
struct LoopMemoryWrites
{
  bool unknown;
  bool any;
  std::vector<lowir_model::SymbolId> globals;
  std::vector<unsigned char> slots;
};

LoopMemoryWrites summarize_loop_memory_writes(
    const Function & function, const lowir_analysis::NaturalLoop & loop)
{
  LoopMemoryWrites writes;
  writes.unknown = false;
  writes.slots.assign(function.slot_names.size(), 0);
  for(std::size_t member = 0; member < loop.blocks.size(); ++member) {
    const std::vector<Instruction> & instructions =
      function.blocks[loop.blocks[member]].instructions;
    for(std::size_t instruction = 0;
      ...
      if(ins.kind == Instruction::IK_STORE) {
        if(ins.second.kind == Operand::OP_GLOBAL)
          writes.globals.push_back(ins.second.symbol);
        else if(ins.second.kind == Operand::OP_SLOT)
          writes.slots[ins.second.slot] = 1;
        else
          writes.unknown = true;
  ...
  writes.any = writes.unknown || !writes.globals.empty();
  for(std::size_t slot = 0; slot < writes.slots.size() && !writes.any; ++slot)
    if(writes.slots[slot]) writes.any = true;
  std::sort(writes.globals.begin(), writes.globals.end());
  writes.globals.erase(
    std::unique(writes.globals.begin(), writes.globals.end()),
    writes.globals.end());
  return writes;
}
                           ...
                           int optimization_level, Stats * stats)
{
  ...
  for(std::size_t order_index = 0;
    ...
    const LoopMemoryWrites writes = summarize_loop_memory_writes(*function, loop);
    ...
    for(std::size_t member = 0; member < loop.blocks.size(); ++member) {
      ...
      for(std::size_t instruction = 0;
        ...
        if(optimization_level >= 2 && ins.kind == Instruction::IK_LOAD) {
          ...
          else if(ins.first.kind == Operand::OP_GLOBAL && !writes.unknown)
            eligible = !std::binary_search(
              writes.globals.begin(), writes.globals.end(),
              ins.first.symbol);
          else if(ins.first.kind == Operand::OP_SLOT &&
                  !escaped_slots[ins.first.slot])
            eligible = !writes.slots[ins.first.slot];
          // A loop that writes no memory at all leaves every location as
          // it found it, so a load through any invariant pointer hoists
          // when the loop reads it on every iteration -- one behind a
          // branch could fault where the loop would not have gone.  A
          // byte hash over a string reads the string's long/short flag
          // once instead of per byte.
          else if(ins.first.kind == Operand::OP_TEMP && !writes.any) {
            const lowir_analysis::DominatorTree & dominators =
              analysis->dominator_tree();
            eligible = !loop.latches.empty();
            for(std::size_t latch = 0;
                eligible && latch < loop.latches.size(); ++latch)
              eligible = dominators.dominates(block, loop.latches[latch]);
          }
