...
const std::size_t kAddressPhiNoBlock = static_cast<std::size_t>(-1);

// The storage an address operand names, through copies and `addr`: a slot,
// a global, or the value that computed it.
Operand address_root(const Function & function,
                     const lowir_analysis::ValueIndex & values,
                     Operand operand)
{
  for(int depth = 0; depth < 16 && operand.kind == Operand::OP_TEMP; ++depth) {
    const lowir_analysis::ValueDefinition definition =
      values.definition(operand.value);
    if(definition.kind != lowir_analysis::ValueDefinition::INSTRUCTION) break;
    const Instruction & ins =
      function.blocks[definition.block].instructions[definition.instruction];
    if(ins.kind == Instruction::IK_COPY) operand = ins.first;
    else if(ins.kind == Instruction::IK_ADDR &&
            (ins.first.kind == Operand::OP_SLOT ||
             ins.first.kind == Operand::OP_GLOBAL)) return ins.first;
    else break;
  }
  return operand;
}

// Whether `ins` may write the storage `root` names: a store to another slot
// cannot, anything else that writes memory may.
bool may_clobber_root(const Function & function,
                      const lowir_analysis::ValueIndex & values,
                      const Instruction & ins, const Operand & root)
{
  if(keeps_memory(ins)) return false;
  if(ins.kind == Instruction::IK_LOAD) return ins.volatile_access;
  if(ins.kind == Instruction::IK_STORE && !ins.volatile_access &&
     root.kind == Operand::OP_SLOT) {
    const Operand target = address_root(function, values, ins.second);
    return !(target.kind == Operand::OP_SLOT && target.slot != root.slot);
  }
  return true;
}

// The value of `type` that `root` holds at the end of `block`: the last
// load or store of it with nothing writing memory after, found in the block
// or, through a chain of single predecessors, a few blocks above it.
bool value_at_block_end(const Function & function,
                        const lowir_analysis::Graph & graph,
                        const lowir_analysis::ValueIndex & values,
                        std::size_t block, const Operand & root,
                        const LowType & type, Operand * value, int depth)
{
  const std::vector<Instruction> & instructions = function.blocks[block].instructions;
  for(std::size_t i = instructions.size(); i-- > 0;) {
    ...
    if(ins.kind == Instruction::IK_LOAD && !ins.volatile_access &&
       optimizer_support::same_storage_location(
         address_root(function, values, ins.first), root)) {
      if(!lowir_model::same_lowir_type(ins.type, type) || !ins.dest.valid())
        return false;
      value->kind = Operand::OP_TEMP;
      value->value = ins.dest;
      return true;
    }
    if(may_clobber_root(function, values, ins, root)) return false;
  }
  if(depth >= 4 || graph.predecessors[block].size() != 1) return false;
  return value_at_block_end(function, graph, values,
                            graph.predecessors[block][0], root, type, value,
                            depth + 1);
}

// One rewrite per scan: the value index describes instruction positions,
// and a rewrite moves them.
bool forward_one_load_through_address_phi(
  Function * function, lowir_analysis::FunctionAnalysis * analysis, Stats * stats)
{
  ...
  for(std::size_t block = 0; block < function->blocks.size(); ++block) {
    ...
    while(phi_end < instructions.size() &&
          instructions[phi_end].kind == Instruction::IK_PHI) ++phi_end;
    for(std::size_t index = phi_end; index < instructions.size(); ++index) {
      const Instruction & load = instructions[index];
      if(load.kind != Instruction::IK_LOAD) {
        if(!keeps_memory(load)) break;
        continue;
      }
      if(load.volatile_access || !load.dest.valid()) continue;
      // A phi carries a register-sized scalar; an object, an i128 or an f80
      // load stays a load.
      if(load.type.kind == lowir_model::LTK_OBJECT ||
         load.type.kind == lowir_model::LTK_I128 ||
         load.type.kind == lowir_model::LTK_F80) continue;
      const Operand root = address_root(*function, values, load.first);
      if(root.kind != Operand::OP_TEMP) continue;
      const lowir_analysis::ValueDefinition definition =
        values.definition(root.value);
      if(definition.kind != lowir_analysis::ValueDefinition::INSTRUCTION ||
         definition.block != block) continue;
      const Instruction & phi = instructions[definition.instruction];
      if(phi.kind != Instruction::IK_PHI ||
         phi.type.kind != lowir_model::LTK_PTR || phi.args.size() < 4) continue;
      std::vector<Operand> incoming;
      bool available = true;
      for(std::size_t arg = 0; available && arg + 1 < phi.args.size(); arg += 2) {
        const std::size_t predecessor = graph.find(phi.args[arg].block);
        if(predecessor == kAddressPhiNoBlock ||
           predecessor >= function->blocks.size()) { available = false; break; }
        Operand value;
        if(!value_at_block_end(*function, graph, values, predecessor,
                               address_root(*function, values, phi.args[arg + 1]),
                               load.type, &value, 0)) { available = false; break; }
        incoming.push_back(value);
      }
      if(!available) continue;
      Instruction replacement;
      replacement.kind = Instruction::IK_PHI;
      replacement.type = load.type;
      replacement.dest = load.dest;
      for(std::size_t k = 0; k < incoming.size(); ++k) {
        replacement.args.push_back(phi.args[2 * k]);
        replacement.args.push_back(incoming[k]);
      }
      instructions.erase(instructions.begin() + index);
      instructions.insert(instructions.begin() + phi_end, replacement);
      if(stats) { ++stats->phi_address_loads_forwarded; ++stats->rewrites; }
      return true;
    }
  }
  return false;
...
// A load or store whose address is a slot's, reached through copies, names
// the slot directly, so that promotion sees a plain slot use once the copies
// are dead.
bool name_slots_directly(Function * function,
                         const lowir_analysis::ValueIndex & values)
{
  bool changed = false;
  for(std::size_t block = 0; block < function->blocks.size(); ++block) {
    ...
    for(std::size_t index = 0; index < instructions.size(); ++index) {
      ...
      Operand * address = 0;
      if(ins.kind == Instruction::IK_LOAD) address = &ins.first;
      else if(ins.kind == Instruction::IK_STORE) address = &ins.second;
      else continue;
      if(address->kind != Operand::OP_TEMP) continue;
      const Operand root = address_root(*function, values, *address);
      if(root.kind != Operand::OP_SLOT) continue;
      *address = root;
      changed = true;
    }
  }
...
bool forward_loads_through_address_phis(Function * function,
    lowir_analysis::FunctionAnalysis * analysis, Stats * stats)
{
  bool changed = false;
  while(forward_one_load_through_address_phi(function, analysis, stats)) {
    changed = true;
    analysis->invalidate_values();
  }
  if(changed && name_slots_directly(function, analysis->value_index()))
    analysis->invalidate_values();
  return changed;
}

// Within one block, a load repeats an earlier load of the same storage, or
// reads what an earlier store put there, when nothing that may write memory
// lies between.  Addresses are compared through copies and `addr`, and a
// store's value is tracked like a load's result: memory GVN, which forwards
// across blocks, skips a function with exception handling, and nearly every
// libc++ string and vector member can throw -- its `__set_short_size`
// stored the flag byte and reloaded it in the next instruction.
    ...
    for(std::size_t index = 0;
      ...
      if(ins.kind == Instruction::IK_LOAD && !ins.volatile_access) {
        ...
        if(!matched && ins.dest.valid()) {
          ...
          tracked.value.kind = Operand::OP_TEMP;
          tracked.value.value = ins.dest;
      ...
      if(ins.kind == Instruction::IK_STORE && !ins.volatile_access &&
         (ins.first.kind == Operand::OP_TEMP ||
          ins.first.kind == Operand::OP_INTEGER)) {
        // A store to a slot or a global leaves every other slot and global
        // as it was; through a pointer it may reach anything.
        const Operand address = address_root(*function, values, ins.second);
        const bool named_storage = address.kind == Operand::OP_SLOT ||
          address.kind == Operand::OP_GLOBAL;
        std::size_t kept = 0;
        for(std::size_t i = 0; i < live.size(); ++i) {
          const bool distinct = named_storage &&
            (live[i].address.kind == Operand::OP_SLOT ||
             live[i].address.kind == Operand::OP_GLOBAL) &&
            !optimizer_support::same_storage_location(live[i].address, address);
          if(distinct) live[kept++] = live[i];
        }
        live.resize(kept);
        TrackedValue tracked;
        tracked.address = address;
        tracked.type = ins.type;
        tracked.value = ins.first;
        live.push_back(tracked);
        continue;
      }
