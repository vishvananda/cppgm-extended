...
// Replace a load through a phi of addresses by a phi of the values those
// addresses hold on each incoming edge, when a load or store of the same type
// reaches the edge with nothing writing memory between.  `std::min(a, b)` on
// references lowers to a conditional address and a reload through it; clang
// folds that to a select, and this leaves a phi of the two values already
// loaded for the comparison, with the slots behind the addresses dead.
bool forward_loads_through_address_phis(lowir_model::Function * function,
