    ...
    lowir_analysis::FunctionAnalysis * analysis)
{
  ...
  // Where each value is defined, so a planned incoming can be checked against
  // the edge it arrives on.
  std::vector<std::size_t> value_definition_block(
  ...
  for(std::size_t block = 0; block < function->blocks.size(); ++block)
    for(std::size_t i = 0; i < function->blocks[block].instructions.size(); ++i) {
      const lowir_model::ValueId dest =
        function->blocks[block].instructions[i].dest;
      if(dest.valid() &&
         static_cast<std::size_t>(dest) < value_definition_block.size())
        value_definition_block[static_cast<std::size_t>(dest)] = block;
    }
  for(std::size_t phi = 0; phi < planned_phis.size(); ++phi) {
    ...
    for(std::size_t predecessor = 0;
      ...
      // The state says the slot holds this value when control leaves `source`,
      // which is a statement about execution, not about where the value is
      // defined.  An exceptional edge can carry a state whose value is defined
      // on a path that does not reach `source` at all, and a phi built from it
      // names a value the backend cannot have in hand on that edge.  Take the
      // incoming only when its definition dominates the edge.
      if(value.kind == Operand::OP_TEMP) {
        const std::size_t defined =
          static_cast<std::size_t>(value.value) < value_definition_block.size() ?
            value_definition_block[static_cast<std::size_t>(value.value)] :
            kNoBlockIndex;
        if(defined != kNoBlockIndex &&
           !promotion_dominators.dominates(defined, source)) {
          planned.complete = false;
          break;
        }
      }
                              ...
                              void * opaque)
{
  ...
  if(object_slots_changed) {
    emptied = timed_dce(function, *context.boundaries, stats,
                        context.dce_scratch) || emptied;
  ...
  // Dead-code removal after the last CFG cleanup can empty a diamond's
  // arms -- libc++'s inlined annotation calls leave a branch on the
  // short-string flag with nothing on either side, three times in a row
  // in the string copy constructor -- or an inlined callee's entry, so
  // the cleanup runs once more when anything was removed.
  if(emptied && timed_cfg(function, stats, context.cfg_scratch))
...
// Both loop rewrites want a pointer phi; a function without one has nothing
// for them and does not need the CFG analysis they would build.
bool function_has_pointer_phi(const Function & function)
{
  for(std::size_t b = 0; b < function.blocks.size(); ++b) {
    const std::vector<Instruction> & instructions =
      function.blocks[b].instructions;
    for(std::size_t i = 0; i < instructions.size(); ++i) {
      if(instructions[i].kind != Instruction::IK_PHI) break;
      if(instructions[i].type.kind == lowir_model::LTK_PTR) return true;
    }
  }
  return false;
}

void promote_after_late_inlining(LowirProgram * program, Function * function,
                                 const FunctionBoundaries & boundaries,
                                 Stats * stats,
                                 SimplifyScratch * simplify_arena,
                                 DceScratch * dce_scratch,
                                 CleanupCfgScratch * cfg_scratch)
{
  const bool promoted = timed_function_pass(promote_slots, function, stats,
  ...
  if(!promoted && !function_has_pointer_phi(*function)) return;
  ...
  if(promoted) {
    ...
    if(timed_cfg(function, stats, cfg_scratch, &analysis)) {
      timed_function_pass(simplify_values, function, stats,
        &Stats::simplify_runs, &Stats::simplify_nanoseconds, &analysis,
        simplify_arena);
      if(timed_dce(function, boundaries, stats, dce_scratch) &&
  ...
  // A conditional address inlined late -- std::min on references -- takes
  // its shape here too.
  if(forward_loads_through_address_phis(function, &analysis, stats)) {
    timed_dce(function, boundaries, stats, dce_scratch);
    timed_function_pass(promote_slots, function, stats,
      &Stats::slot_runs, &Stats::slot_nanoseconds);
    timed_function_pass(simplify_values, function, stats,
      &Stats::simplify_runs, &Stats::simplify_nanoseconds, &analysis,
      simplify_arena);
  ...
  // A loop inlined late, or one whose object was only now promoted, takes
  // its final shape here, after the body phase ran the loop rewrites; libc++
  // hands vector's construct-at-end transaction to exactly this point.
  if(!function_has_pointer_phi(*function)) return;
  const bool loops_deleted = delete_effect_free_loops(function, &analysis, stats);
  const bool fills_recognized =
    recognize_fill_loops(program, function, &analysis, stats);
  if(loops_deleted || fills_recognized) {
    timed_dce(function, boundaries, stats, dce_scratch);
    timed_cfg(function, stats, cfg_scratch, &analysis);
  }
}
    ...
    CleanupCfgScratch & cfg_scratch)
{
  ...
  for(std::size_t i = 0; i < program.functions.size(); ++i) {
    ...
    if(level >= 1 &&
       forward_loads_through_address_phis(&function, &analysis, stats)) {
      timed_dce(&function, boundaries, stats, &dce_scratch);
      // The slots behind the addresses are plain slots now.
      timed_function_pass(promote_slots, &function, stats,
        &Stats::slot_runs, &Stats::slot_nanoseconds);
      timed_function_pass(simplify_values, &function, stats,
        &Stats::simplify_runs, &Stats::simplify_nanoseconds, &analysis,
    ...
    if(level >= 1 && recognize_fill_loops(&program, &function, &analysis, stats)) {
      timed_dce(&function, boundaries, stats, &dce_scratch);
      timed_cfg(&function, stats, &cfg_scratch, &analysis);
    }
...
// Native lowering walks blocks in array order and needs a value's definition to
// come before every use.  That is stronger than SSA, which only asks the
// definition to dominate the use: a pass can leave a dominating block sitting
// after the block it dominates, and the result is valid LowIR the backend
// cannot walk.  Put such a function back into reverse postorder, and leave a
// function whose order is already good exactly as it is, so nothing that
// compiles today changes shape.
bool repair_block_order(Function * function, Stats * stats)
{
  const std::size_t n = function->blocks.size();
  if(n < 2) return false;
  const Graph graph = build_graph(*function, stats);
  std::vector<std::size_t> definition(function->value_names.size(), kNoBlockIndex);
  for(std::size_t b = 0; b < n; ++b)
    for(std::size_t i = 0; i < function->blocks[b].instructions.size(); ++i) {
      const lowir_model::ValueId dest = function->blocks[b].instructions[i].dest;
      if(dest.valid() && static_cast<std::size_t>(dest) < definition.size())
        definition[static_cast<std::size_t>(dest)] = b;
    }
  bool broken = false;
  for(std::size_t b = 0; b < n && !broken; ++b)
    for(std::size_t i = 0;
        i < function->blocks[b].instructions.size() && !broken; ++i) {
      const Instruction & ins = function->blocks[b].instructions[i];
      if(ins.kind == Instruction::IK_PHI) {
        for(std::size_t a = 0; a + 1 < ins.args.size(); a += 2) {
          if(ins.args[a].kind != Operand::OP_LABEL) continue;
          if(ins.args[a + 1].kind != Operand::OP_TEMP) continue;
          const std::size_t use = graph.find(ins.args[a].block);
          const std::size_t value = static_cast<std::size_t>(ins.args[a + 1].value);
          if(use == kNoBlockIndex || value >= definition.size()) continue;
          if(definition[value] != kNoBlockIndex && definition[value] > use)
            broken = true;
        }
        continue;
      }
      const Operand * operands[3] = { &ins.first, &ins.second, &ins.third };
      for(std::size_t k = 0; k < 3 && !broken; ++k)
        if(operands[k]->kind == Operand::OP_TEMP) {
          const std::size_t value = static_cast<std::size_t>(operands[k]->value);
          if(value < definition.size() && definition[value] != kNoBlockIndex &&
             definition[value] > b)
            broken = true;
        }
      for(std::size_t a = 0; a < ins.args.size() && !broken; ++a)
        if(ins.args[a].kind == Operand::OP_TEMP) {
          const std::size_t value = static_cast<std::size_t>(ins.args[a].value);
          if(value < definition.size() && definition[value] != kNoBlockIndex &&
             definition[value] > b)
            broken = true;
        }
  ...
  stack.push_back(std::make_pair(static_cast<std::size_t>(0), 0));
  seen[0] = 1;
  while(!stack.empty()) {
    const std::size_t block = stack.back().first;
    std::size_t & edge = stack.back().second;
    if(edge < graph.successors[block].size()) {
      const std::size_t next = graph.successors[block][edge++];
      if(!seen[next]) { seen[next] = 1; stack.push_back(std::make_pair(next, 0)); }
      continue;
    }
  ...
  for(std::size_t i = postorder.size(); i-- > 0; )
    ordered.push_back(std::move(function->blocks[postorder[i]]));
  for(std::size_t b = 0; b < n; ++b)
    if(!seen[b]) ordered.push_back(std::move(function->blocks[b]));
    ...
    const std::chrono::steady_clock::time_point & started)
{
  if(level >= 1) {
    ...
    for(std::size_t i = 0; i < program.functions.size(); ++i)
      if(late_rewritten_symbols[program.functions[i].symbol]) {
        ...
        promote_after_late_inlining(&program, &program.functions[i], boundaries, stats,
    ...
    if(post_prune_rewrites) {
      ...
      for(std::size_t i = 0; i < program.functions.size(); ++i)
        if(post_prune_rewritten_symbols[program.functions[i].symbol]) {
          ...
          promote_after_late_inlining(&program, &program.functions[i], boundaries, stats, &simplify_arena,
            &dce_scratch, &cfg_scratch);
        }
  ...
  for(std::size_t i = 0; i < program.functions.size(); ++i)
    repair_block_order(&program.functions[i], stats);
  // A constant formed by the last simplification of a phase can leave a
  // branch on it behind; one closing CFG pass folds what remains.
  for(std::size_t i = 0; i < program.functions.size(); ++i) {
    // An object slot whose address was stored into a by-reference argument
    // slot, or passed to a callee the late wave inlined afterwards, failed
    // the aggregate split while those uses stood; both are gone by now.
    // libc++'s bit iterators (a pointer and a bit offset) in vector<bool>'s
    // constructor reach the backend as 16-byte slots that way.
    const bool split = scalar_replace_aggregate_slots(
      &program, &program.functions[i], stats);
    const bool objects = promote_small_objects(&program.functions[i], stats);
    if(split || objects) {
      timed_dce(&program.functions[i], boundaries, stats, &dce_scratch);
      remove_dead_slots(&program.functions[i], stats);
      if(promote_slots(&program.functions[i], stats)) {
        timed_function_pass(simplify_values, &program.functions[i], stats,
          &Stats::simplify_runs, &Stats::simplify_nanoseconds, 0,
          &simplify_arena);
        timed_dce(&program.functions[i], boundaries, stats, &dce_scratch);
    ...
    if(timed_cfg(&program.functions[i], stats, &cfg_scratch)) {
      timed_function_pass(simplify_values, &program.functions[i], stats,
        &Stats::simplify_runs, &Stats::simplify_nanoseconds, 0,
        &simplify_arena);
      timed_dce(&program.functions[i], boundaries, stats, &dce_scratch);
    }
    // Every conditional value is a diamond here, and libc++ recomputes the
    // same ones (a string's long/short choice for its size and its data)
    // in every function that touches a string: a diamond on a dominating
    // diamond's condition with the same arms is that diamond's phi, a phi
    // choosing between its comparison's operands is often one of them
    // (`std::min(n, npos)`), and an equality's edge fixes the compared
    // value in what it dominates.  Each collapse exposes the next, so they
    // run together until nothing folds.
    if(level >= 3) {
      // A dead store into a slot nothing loads (an inlined by-reference
      // argument) would otherwise read as a change of memory between two
      // diamonds.
      timed_function_pass(remove_dead_slots, &program.functions[i], stats,
        &Stats::slot_runs, &Stats::slot_nanoseconds);
      for(std::size_t round = 0; round < 4; ++round) {
        bool collapsed = value_number_diamonds(&program.functions[i], stats);
        collapsed = fold_diamond_select_identities(
          &program.functions[i], stats) || collapsed;
        collapsed = propagate_edge_integer_equalities(
          &program.functions[i], stats) || collapsed;
        if(!collapsed) break;
        timed_function_pass(simplify_values, &program.functions[i], stats,
          &Stats::simplify_runs, &Stats::simplify_nanoseconds, 0,
    ...
    // Value reuse across a block that a fold made a dominator of blocks
    // listed before it can leave a use ahead of its definition in block
    // order, which the native lowering cannot take.
    if(restore_definition_order(&program.functions[i]) && stats)
      ++stats->block_orders_restored;
    // A slot whose last load was forwarded away keeps its stores until
    // the dead-slot pass runs again; inlined by-reference arguments and
    // libc++'s max_size temporaries end that way.
    timed_function_pass(remove_dead_slots, &program.functions[i], stats,
      &Stats::slot_runs, &Stats::slot_nanoseconds);
  }
...
bool simplify_function_values(Function * function, Stats * stats)
{
  return simplify_values(function, stats);
}
