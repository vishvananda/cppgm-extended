...
private:
                          ...
                          lowir_model::SlotId result_slot)
  {
    ...
    if(tail.empty()) {
      // The call does not return, so its block ended with the call itself and
      // there is nothing to continue with.  The block still has to exist and
      // still has to end in a terminator, because the callee's returns -- if
      // its lowering gave it any -- name this block as their target.  Control
      // cannot actually arrive here, and `unreachable` says exactly that.
      Instruction stop;
      stop.kind = Instruction::IK_UNREACHABLE;
      stop.debug_location = call.debug_location;
      result.instructions.push_back(stop);
    }
                  ...
                  InlineNames * names)
  {
    ...
    // With no tail there is no terminator moving out of this block, so no phi
    // edge changes predecessor.
    if(!tail.empty())
