...
// A callee kept out of line only by its exception region is retried from a
// copy with the call's constant arguments substituted (see the folded-copy
// section of the inliner).  The copy is bounded by the callee's size and by
// how many times folding has failed for that callee.
const std::size_t kEhFoldMaximumCalleeInstructions = 256;
const std::size_t kEhFoldMaximumFailuresPerTarget = 16;
// The folded copy of a hinted callee is admitted up to this size in either
// wave.  Every conditional value in LowIR is a diamond of a branch, two
// jumps and a phi, so libc++'s string equality against a literal folds to
// 66 instructions in a unit where `compare` was not inlined into it (its
// size diamond twice, three minimum diamonds and the result merge) where
// clang's select form counts twenty; the cap admits it, as clang's hinted
// threshold does.
const std::size_t kEhFoldHintSizeCap = 80;
// Folded copies are charged to their own whole-caller budget: a string
// switch compares one name against dozens of literals, and each failing
// comparison is cheapest when its size check is inline.
const std::size_t kEhFoldCallerBudget = 2048;
// A call inside the copy that still can unwind and takes constants is folded
// the same way and spliced first, to this depth: libc++'s equality reaches
// its throw through `compare`.
const std::size_t kEhFoldMaximumDepth = 2;

// The first constant argument of a call whose folded copy did not become
// exception-free; the same constant at another site will not either.
struct EhFoldKey
{
  std::size_t target;
...
public:
  Inliner(LowirProgram * program,
    ...
    : program_(*program), rewritten_symbols_(rewritten_symbols),
      ...
      rewrites_(0), address_origins_caller_(kNoFunction)
  {
    ...
    eh_fold_failures_.resize(program_.functions.size(), 0);
    remaining_eh_fold_budget_.resize(program_.functions.size(),
                                     kEhFoldCallerBudget);
...
private:
  ...
  // Folded-copy state: the unit's readonly byte strings (built on first
  // use), the caller whose direct global addresses are indexed, that index,
  // and the failures recorded per callee and per first constant argument.
  std::vector<ReadonlyByteStringIndex> byte_strings_;
  std::size_t address_origins_caller_;
  std::vector<Operand> address_origins_;
  std::vector<std::size_t> eh_fold_failures_;
  std::set<EhFoldKey> eh_fold_failed_;
  std::vector<std::size_t> remaining_eh_fold_budget_;
  ...
  void inline_call(Function & caller, std::size_t block_index,
  ...
  // ---- Folded copies of exception-bearing callees ----
  //
  // libc++ writes string equality against a literal as strlen, a length
  // check and a compare whose out-of-range throw sits under the noexcept
  // terminate wrapper.  The throw is dead once the literal's length is
  // known, but only the call site knows it, and the exception region keeps
  // the callee out of every caller.  A call that passes a constant is
  // retried from a copy of the callee with the constants substituted: the
  // copy is folded, its dead regions are stripped, and it is spliced in
  // place of the call when nothing exceptional remains.  A copy that keeps
  // its region is the ordinary rejection, remembered per callee and
  // constant so the attempt is not repeated.

  static std::vector<Operand> address_origins(const Function & function)
  {
    std::vector<Operand> origins(function.value_names.size(), Operand());
    for(std::size_t b = 0; b < function.blocks.size(); ++b)
      for(std::size_t j = 0; j < function.blocks[b].instructions.size(); ++j) {
        const Instruction & ins = function.blocks[b].instructions[j];
        if(ins.kind == Instruction::IK_ADDR && ins.dest.valid() &&
           ins.first.kind == Operand::OP_GLOBAL &&
           static_cast<std::uint32_t>(ins.dest) < origins.size())
          origins[ins.dest] = ins.first;
      }
    return origins;
  }

  const std::vector<Operand> & caller_address_origins(std::size_t caller)
  {
    if(address_origins_caller_ != caller) {
      address_origins_caller_ = caller;
      address_origins_ = address_origins(program_.functions[caller]);
    }
    return address_origins_;
  }

  const ReadonlyByteStringIndex & byte_strings()
  {
    if(byte_strings_.empty())
      byte_strings_.push_back(ReadonlyByteStringIndex(program_));
    return byte_strings_[0];
  }

  // The readonly byte string of this unit whose direct address the
  // argument carries, if any.
  bool literal_argument(const std::vector<Operand> & origins,
                        const Operand & argument, Operand * global)
  {
    if(argument.kind != Operand::OP_TEMP) return false;
    const std::uint32_t value = argument.value;
    if(value >= origins.size() || origins[value].kind != Operand::OP_GLOBAL)
      return false;
    const ReadonlyByteStringIndex & strings = byte_strings();
    const std::uint32_t symbol = origins[value].symbol;
    if(symbol >= strings.known.size() || !strings.known[symbol]) return false;
    *global = origins[value];
    return true;
  }

  static bool integer_argument(const Operand & argument)
  {
    return argument.kind == Operand::OP_INTEGER && argument.has_int_value;
  }

  bool constant_argument(const std::vector<Operand> & origins,
                         const Operand & argument)
  {
    Operand global;
    return integer_argument(argument) ||
      literal_argument(origins, argument, &global);
  }

  // The structural conditions a call must meet before its callee is copied,
  // shared by the site test and the nested splice.
  bool eh_fold_shape(std::size_t caller, std::size_t target,
                     const Instruction & call) const
  {
    if(target == kNoFunction) return false;
    if(call.copy_elision_candidate) return false;
    if(call_graph_.recursive[target] || target == caller) return false;
    const Function & callee_function = program_.functions[target];
    if(callee_function.metadata.no_inline) return false;
    if(loop_inline_shapes_[target] &&
       !callee_function.metadata.inline_hint &&
       !callee_function.metadata.force_inline) return false;
    if(callee_function.params.size() != call.args.size()) return false;
    if(callee_function.boundary.arity == lowir_model::CAM_VARIADIC)
      return false;
    return instruction_counts_[target] <= kEhFoldMaximumCalleeInstructions;
  }

  bool eh_fold_candidate(std::size_t caller, std::size_t target,
                         const Instruction & call)
  {
    if(target == kNoFunction || !contains_eh_[target]) return false;
    if(loop_priority_only_ || definition_removing_only_) return false;
    if(!eh_fold_shape(caller, target, call) ||
       eh_fold_failures_[target] >= kEhFoldMaximumFailuresPerTarget)
      return false;
    const std::vector<Operand> & origins = caller_address_origins(caller);
    for(std::size_t i = 0; i < call.args.size(); ++i)
      if(constant_argument(origins, call.args[i])) return true;
    return false;
  }

  static bool replace_parameter_uses(Function * function,
                                     lowir_model::ValueId parameter,
                                     const Operand & replacement)
  {
    const std::uint32_t id = parameter;
    bool changed = false;
    for(std::size_t b = 0; b < function->blocks.size(); ++b)
      for(std::size_t j = 0; j < function->blocks[b].instructions.size(); ++j) {
        ...
        for(std::size_t k = 0; k < 3; ++k)
          if(fixed[k]->kind == Operand::OP_TEMP &&
             static_cast<std::uint32_t>(fixed[k]->value) == id) {
            *fixed[k] = replacement;
            changed = true;
          }
        for(std::size_t k = 0; k < ins.args.size(); ++k)
          if(ins.args[k].kind == Operand::OP_TEMP &&
             static_cast<std::uint32_t>(ins.args[k].value) == id) {
  ...
  void fold_copy_values(Function * copy)
  {
    fold_readonly_byte_string_lengths(copy, byte_strings(), 0);
    for(int round = 0; round < 4; ++round) {
      bool changed = simplify_function_values(copy, 0);
      changed = cleanup_cfg(copy, 0) || changed;
      if(!changed) break;
    }
  }

  // Copy `target` with the call's constant arguments substituted (the
  // call's own origins resolve its literals) and fold it; nested calls that
  // still can unwind are folded and spliced to the depth limit.  False when
  // no argument was a constant.
  bool fold_copy_for_constant_arguments(
      const std::vector<Operand> & origins, std::size_t target,
      const Instruction & call, Function * copy, std::size_t depth)
  {
    *copy = program_.functions[target];
    if(copy->blocks.empty()) return false;
    std::size_t insert = 0;
    while(insert < copy->blocks[0].instructions.size() &&
          copy->blocks[0].instructions[insert].kind == Instruction::IK_PHI)
      ++insert;
    bool substituted = false;
    for(std::size_t i = 0; i < call.args.size(); ++i) {
      Operand global;
      Operand replacement;
      if(integer_argument(call.args[i])) replacement = call.args[i];
      else if(literal_argument(origins, call.args[i], &global)) {
        // The literal's address is defined at the entry so the length fold
        // sees the ordinary addr form.
        const LowType & type = copy->params[i].type;
        const lowir_model::ValueId address = lowir_model::append_lowir_value(
          *copy, program_.strings.intern("__ehfold" + std::to_string(i)),
          type);
        Instruction addr;
        addr.kind = Instruction::IK_ADDR;
        addr.dest = address;
        addr.type = type;
        addr.first = global;
        copy->blocks[0].instructions.insert(
          copy->blocks[0].instructions.begin() + insert, addr);
        ++insert;
        replacement = value_operand(address, type);
      } else continue;
      substituted = replace_parameter_uses(copy, copy->params[i].value,
                                           replacement) || substituted;
    }
    if(!substituted) return false;
    fold_copy_values(copy);
    if(depth + 1 < kEhFoldMaximumDepth && splice_folded_callees(copy, target, depth + 1))
      fold_copy_values(copy);
    remove_unreachable_blocks(copy);
    Stats * const saved = stats_;
    stats_ = 0;
    strip_dead_eh_regions(copy);
    stats_ = saved;
    return true;
  }

  // Fold and splice the copy's calls that keep it unwinding and take
  // constants.
  bool splice_folded_callees(Function * copy, std::size_t target,
                             std::size_t depth)
  {
    std::vector<Operand> origins = address_origins(*copy);
    Names names(program_, *copy);
    ValueMap replacements;
    std::vector<unsigned char> block_eh(copy->next_block_id, 0);
    bool spliced = false;
    for(std::size_t b = 0; b < copy->blocks.size(); ++b) {
      std::size_t j = 0;
      while(j < copy->blocks[b].instructions.size()) {
        const Instruction & ins = copy->blocks[b].instructions[j];
        const std::size_t nested_target = callee(ins);
        if(nested_target == kNoFunction || !instruction_can_unwind(ins) ||
           !eh_fold_shape(target, nested_target, ins)) {
          ++j;
          continue;
        }
        bool constant = false;
        for(std::size_t i = 0; i < ins.args.size() && !constant; ++i)
          constant = constant_argument(origins, ins.args[i]);
        if(!constant) {
          ++j;
          continue;
        }
        Function nested;
        if(!fold_copy_for_constant_arguments(
             origins, nested_target, ins, &nested, depth) ||
           contains_eh(nested) || !body_is_no_unwind(nested)) {
          ++j;
          continue;
        }
        inline_call(*copy, b, j, nested, true, &names, &replacements,
                    &block_eh, true);
        spliced = true;
        origins = address_origins(*copy);
      }
    }
    if(spliced) replace_values(copy, replacements);
    return spliced;
  }

  static bool pure_value_kind(Instruction::Kind kind)
  {
    return kind == Instruction::IK_CONST || kind == Instruction::IK_COPY ||
      ...
      kind == Instruction::IK_PHI || kind == Instruction::IK_LOAD;
  }

  static void prune_unused_pure_values(Function * function)
  {
    for(;;) {
      std::vector<unsigned char> used(function->value_names.size(), 0);
      for(std::size_t b = 0; b < function->blocks.size(); ++b)
        for(std::size_t j = 0; j < function->blocks[b].instructions.size(); ++j) {
          const Instruction & ins = function->blocks[b].instructions[j];
          ...
          for(std::size_t k = 0; k < 3; ++k)
            if(fixed[k]->kind == Operand::OP_TEMP &&
               static_cast<std::uint32_t>(fixed[k]->value) < used.size())
              used[fixed[k]->value] = 1;
          for(std::size_t k = 0; k < ins.args.size(); ++k)
            if(ins.args[k].kind == Operand::OP_TEMP &&
               static_cast<std::uint32_t>(ins.args[k].value) < used.size())
              used[ins.args[k].value] = 1;
        }
      bool changed = false;
      for(std::size_t b = 0; b < function->blocks.size(); ++b) {
        ...
        for(std::size_t j = 0; j < instructions.size(); ++j) {
          const Instruction & ins = instructions[j];
          if(pure_value_kind(ins.kind) && !ins.volatile_access &&
             ins.dest.valid() &&
             static_cast<std::uint32_t>(ins.dest) < used.size() &&
             !used[ins.dest]) {
            changed = true;
            continue;
          }
          kept.push_back(instructions[j]);
        }
        if(kept.size() != instructions.size()) instructions.swap(kept);
      }
      if(!changed) return;
    }
  }

  bool inline_eh_folded_call(std::size_t caller, std::size_t block,
                             std::size_t index, std::size_t target,
                             bool landing, Names * names,
                             ValueMap * replacements,
                             std::vector<unsigned char> * block_eh,
                             bool inside_eh)
  {
    const Instruction call =
      program_.functions[caller].blocks[block].instructions[index];
    const std::vector<Operand> & origins = caller_address_origins(caller);
    EhFoldKey key = {target, 0, false, 0};
    bool keyed = false;
    for(std::size_t i = 0; i < call.args.size() && !keyed; ++i) {
      Operand global;
      if(integer_argument(call.args[i])) {
        key.parameter = i;
        key.literal = false;
        key.payload = static_cast<std::uint64_t>(call.args[i].int_value);
        keyed = true;
      } else if(literal_argument(origins, call.args[i], &global)) {
        key.parameter = i;
        key.literal = true;
        key.payload = static_cast<std::uint32_t>(global.symbol);
        keyed = true;
      }
    }
    if(!keyed || eh_fold_failed_.count(key)) return false;
    if(stats_) ++stats_->inline_eh_fold_attempts;
    Function copy;
    if(!fold_copy_for_constant_arguments(origins, target, call, &copy, 0) ||
       contains_eh(copy)) {
      eh_fold_failed_.insert(key);
      ++eh_fold_failures_[target];
      return false;
    }
    // Cost the copy as the caller will keep it: the wave's cleanup where
    // there is one, otherwise a sweep of the values nothing uses.
    if(cleanup_ && cleanup_->run) cleanup_->run(&copy, 0, cleanup_->context);
    else prune_unused_pure_values(&copy);
    const Function & callee_function = program_.functions[target];
    const std::size_t count = instruction_count(copy);
    const bool leaf = leaf_inline_shape(copy);
    const std::size_t size_cap = callee_function.metadata.inline_hint ?
      std::max(limits_.hint_size_cap, kEhFoldHintSizeCap) : kOrdinarySizeCap;
    const std::size_t late_nonleaf_limit =
      callee_function.metadata.inline_hint ?
      std::max(limits_.hint_late_nonleaf_limit, kEhFoldHintSizeCap) :
      kLateNonleafInstructionLimit;
    if(count > size_cap ||
       (optimized_late_wave_ && count > late_nonleaf_limit && !leaf) ||
       (landing && !body_is_no_unwind(copy))) {
      eh_fold_failed_.insert(key);
      ++eh_fold_failures_[target];
      return false;
    }
    std::size_t & budget = remaining_eh_fold_budget_[caller];
    if(count > budget) {
      if(!(count <= kTrivialLeafInstructionLimit && leaf)) {
        if(stats_) ++stats_->budget_skips;
        return false;
      }
    } else budget -= count;
    inline_call(program_.functions[caller], block, index, copy, true, names,
                replacements, block_eh, inside_eh);
    ++rewrites_;
    if(stats_) {
      ++stats_->inline_calls;
      ++stats_->inline_eh_fold_calls;
      if(callee_function.metadata.inline_hint) ++stats_->inline_hint_calls;
      stats_->inline_cloned_instructions += count;
      if(optimized_late_wave_) {
        ++stats_->late_inline_calls;
        stats_->late_inline_cloned_instructions += count;
      }
    }
    return true;
  }
    ...
    for(std::size_t b = 0; !has_candidate && b < original.blocks.size(); ++b)
      for(std::size_t j = 0; j < original.blocks[b].instructions.size(); ++j) {
        ...
        if(candidate(function_index, target, original.blocks[b].instructions[j],
           ...
           eh_fold_candidate(function_index, target,
                             original.blocks[b].instructions[j])) {
          has_candidate = true;
          break;
        }
      }
    ...
    for(std::size_t b = 0;
      ...
      while(j < program_.functions[function_index].blocks[b].instructions.size()) {
        ...
        if(target != kNoFunction) {
          ...
          if(stats_ && optimized_late_wave_)
            ++stats_->late_inline_call_visits;
          const bool landing = static_cast<std::uint32_t>(
              program_.functions[function_index].blocks[b].id) <
              eh.landing_blocks.size() &&
            eh.landing_blocks[
              program_.functions[function_index].blocks[b].id] != 0;
          if(candidate(function_index, target, ins, landing, true,
                       caller_loop)) {
            bool used_single_call = false;
            if(!consume_inline_budget(target, &inline_budget,
                 &remaining_single_call_budget_[function_index],
            ...
            inline_call(program_.functions[function_index], b, j,
              program_.functions[target], used_single_call, &names,
              &replacements, &block_eh, active);
            finish_successful_inline(target, used_single_call);
            changed = true;
            continue;
          }
          if(eh_fold_candidate(function_index, target, ins) &&
             inline_eh_folded_call(function_index, b, j, target, landing,
                                   &names, &replacements, &block_eh,
                                   active)) {
            changed = true;
            continue;
          }
