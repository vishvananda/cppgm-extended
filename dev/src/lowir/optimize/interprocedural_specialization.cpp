...
bool thread_fast_split_phi_comparison(Function * function)
{
  ...
  // The comparison and branch either follow the phi in a continuation
  // block of their own, or, once the CFG cleanup merged that block into
  // the join, in the join itself.
  for(std::size_t join = 0; join < function->blocks.size(); ++join) {
    ...
    if((join_instructions.size() != 2 && join_instructions.size() != 3) ||
    ...
    if(uses[phi.dest] != 1) continue;
    std::size_t continuation = join;
    std::size_t decision_start = 1;
    if(join_instructions.size() == 2) {
      if(join_instructions[1].kind != Instruction::IK_JUMP ||
         join_instructions[1].first.kind != Operand::OP_LABEL)
        continue;
      const std::size_t continuation_id = join_instructions[1].first.block;
      continuation = continuation_id < block_by_id.size() ?
        block_by_id[continuation_id] : function->blocks.size();
      if(continuation >= function->blocks.size() || continuation == join ||
    ...
    if(continuation_instructions.size() != decision_start + 2 ||
       continuation_instructions[decision_start].kind != Instruction::IK_CMP ||
       continuation_instructions[decision_start + 1].kind !=
         Instruction::IK_BRANCH ||
       continuation_instructions[decision_start + 1].first.kind !=
         Operand::OP_TEMP ||
       continuation_instructions[decision_start + 1].first.value !=
         continuation_instructions[decision_start].dest ||
       uses[continuation_instructions[decision_start].dest] != 1)
      continue;
    const Instruction & comparison =
      continuation_instructions[decision_start];
    ...
    const Instruction & branch =
      continuation_instructions[decision_start + 1];
