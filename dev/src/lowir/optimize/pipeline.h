...
struct Stats
{
  ...
  std::size_t effect_free_loops_removed = 0;
  std::size_t fill_loops_recognized = 0;
  std::size_t phi_address_loads_forwarded = 0;
  ...
  std::size_t inline_eh_fold_attempts = 0;
  std::size_t inline_eh_fold_calls = 0;
  ...
  std::size_t diamond_value_numberings = 0;
  std::size_t diamond_select_identities = 0;
  std::size_t diamond_condition_reuses = 0;
  ...
  std::size_t cfg_phi_bypasses = 0;
  std::size_t cfg_phi_merges = 0;
  std::size_t block_orders_restored = 0;
...
// Local value simplification (constant folding, copy propagation and the
// scalar rules) for a body outside the ordinary pipeline walk, such as a
// copy the inliner folds before deciding whether to splice it.
bool simplify_function_values(lowir_model::Function * function,
                              Stats * stats);
