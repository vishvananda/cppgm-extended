...
{
  // `inf` and `INF` end in the float suffix letter and carry none: the
  // spellings the grammar names are returned whole.
  std::string lowered;
  for(std::size_t i = text.empty() ? 0 : (text[0] == '+' || text[0] == '-' ? 1 : 0);
      i < text.size(); ++i)
...
// The decimal spelling of a 128-bit two's-complement value, used for an
// `i128` operand, whose high word is part of the value: the powers of ten
// above 2^63 in libc++'s `__pow10_128` table need it.  Narrower operands
// keep the 64-bit spelling, because their high word is not maintained --
// several producers leave it zero beside a negative low word.
std::string wide_integer_text(long long low, std::uint64_t high)
{
  typedef unsigned __int128 WideUnsigned;
  WideUnsigned magnitude = (static_cast<WideUnsigned>(high) << 64) |
    static_cast<std::uint64_t>(low);
  const bool negative = (high >> 63) != 0;
  if(negative) magnitude = WideUnsigned(0) - magnitude;
  std::string digits;
  do {
    digits.push_back(static_cast<char>('0' + static_cast<unsigned>(magnitude % 10)));
    magnitude /= 10;
  } while(magnitude != 0);
...
std::string generated_lowir_literal_text(const Operand & operand, bool wide)
{
  if(operand.kind == Operand::OP_INTEGER) {
    if(!wide) return std::to_string(operand.int_value);
    return wide_integer_text(operand.int_value, operand.int_high);
  }
...
std::string lowir_literal_text(const Operand & operand,
                               const StringPool * strings, bool wide)
{
  ...
  return generated_lowir_literal_text(operand, wide);
}

std::string lowir_literal_text(const Operand & operand,
                               const SealedStringPool & strings, bool wide)
{
