...
bool resolve_type_pack_element_argument_type(SemanticContext & ctx,
                                             ...
                                             bool prefer_bound_pack,
                                             TypePtr & out)
{
  const std::string trimmed_arg = trim_space(text);
  if(has_invalid_top_level_qualified_owner_syntax(trimmed_arg)) {
    return false;
  }

  (void)prefer_bound_pack;
  out = ctx.lookup_type(scope, trimmed_arg, true);
  if(out) {
    return true;
  }
...
bool append_type_pack_element_argument_types(SemanticContext & ctx,
                                             Scope & scope,
                                             const std::string & text,
                                             std::vector<TypePtr> & out)
{
  ...
  if(trimmed.size() > 3 && trimmed.substr(trimmed.size() - 3) == "...") {
    const std::string pattern = trim_space(trimmed.substr(0, trimmed.size() - 3));
    if(const std::vector<TypePtr> * bound_pack = ctx.lookup_type_pack(scope, pattern)) {
      out.insert(out.end(), bound_pack->begin(), bound_pack->end());
      return true;
    }

    const std::vector<std::string> expanded_texts =
        ctx.expand_bound_type_pack_texts(scope, std::vector<std::string>(1, text));
    const bool expanded =
        expanded_texts.size() != 1 ||
        trim_space(expanded_texts[0]) != trimmed;
    for(size_t i = 0; i < expanded_texts.size(); ++i) {
      TypePtr arg_type;
      if(!resolve_type_pack_element_argument_type(
             ctx, scope, expanded_texts[i], expanded, arg_type)) {
        return false;
      }
      out.push_back(arg_type);
    }
    return true;
  }

  TypePtr arg_type;
  if(!resolve_type_pack_element_argument_type(ctx, scope, trimmed, false, arg_type)) {
    return false;
  }
  out.push_back(arg_type);
  return true;
...
bool parse_integer_literal_index_text(const std::string & text, long long & out)
{
  const std::string trimmed = trim_space(text);
  if(trimmed.empty() ||
...
bool lookup_pack_size(Scope & scope, const std::string & name, size_t & out)
{
...
bool parse_sizeof_pack_index_text(Scope & scope,
                                  const std::string & text,
                                  long long & out)
{
  const std::string trimmed = trim_space(text);
  const std::string prefix = "sizeof...(";
...
bool lookup_integral_constant_index_text(Scope & scope,
                                         const std::string & text,
                                         long long & out)
{
  const std::string name = trim_space(text);
...
bool evaluate_type_pack_element_index_text(Scope & scope,
                                           const std::string & text,
                                           long long & out)
{
  return parse_integer_literal_index_text(text, out) ||
         parse_sizeof_pack_index_text(scope, text, out) ||
         lookup_integral_constant_index_text(scope, text, out);
}
                                     ...
                                     std::string & arg_text)
{
  ...
  if(open == std::string::npos || open == 0 || text.empty() || text.back() != ')') {
    return false;
  }
  ...
  name = trim_space(text.substr(0, open));
  arg_text = trim_space(text.substr(open + 1, text.size() - open - 2));
  return !name.empty() && !arg_text.empty();
}

bool parse_angle_type_transform_text(const std::string & text,
                                     std::string & name,
                                     std::vector<std::string> & arg_texts)
{
  ...
  const std::string trimmed = trim_space(text);
  const size_t langle = trimmed.find('<');
  if(langle == std::string::npos || langle == 0 || trimmed.empty() || trimmed.back() != '>') {
    return false;
  }

  int angle_depth = 0;
  for(size_t i = langle; i < trimmed.size(); ++i) {
    if(trimmed[i] == '<') {
      ++angle_depth;
    } else if(trimmed[i] == '>') {
      --angle_depth;
      if(angle_depth == 0 && i + 1 != trimmed.size()) {
        return false;
      }
      if(angle_depth < 0) {
        return false;
  ...
  name = trim_space(trimmed.substr(0, langle));
  const std::string args_text = trimmed.substr(langle + 1, trimmed.size() - langle - 2);
  if(name.empty() || args_text.empty()) {
    return false;
  }
  ...
  for(size_t i = 0; i < args_text.size(); ++i) {
    ...
    if(c == ',' && angle_depth == 0 && paren_depth == 0 && square_depth == 0) {
      ...
      arg_texts.push_back(trim_space(current));
      current.clear();
      continue;
    }
    current += c;
                                       ...
                                       std::vector<TypePtr> & types)
{
  ...
  if(lparen == std::string::npos || lparen == 0 || trimmed.empty() || trimmed.back() != ')') {
    return false;
  }
  ...
  std::string args_text = trimmed.substr(lparen + 1, trimmed.size() - lparen - 2);
  std::vector<std::string> arg_texts;
  if(args_text.empty()) {
    return true;
  }
  ...
  const auto resolve_trait_arg_type =
      [&](const std::string & text, TypePtr & out) -> bool
      {
        const std::string trimmed_arg = trim_space(text);
        if(has_invalid_top_level_qualified_owner_syntax(trimmed_arg)) {
          return false;
  ...
  const std::vector<std::string> expanded_arg_texts =
      ctx.expand_bound_type_pack_texts(scope, arg_texts);

  for(size_t i = 0; i < expanded_arg_texts.size(); ++i) {
    TypePtr type;
    if(!resolve_trait_arg_type(expanded_arg_texts[i], type)) {
      return false;
    }
    types.push_back(type);
  }
...
{
  std::string builtin_name;
  std::vector<std::string> angle_arg_texts;
  if(parse_angle_type_transform_text(text, builtin_name, angle_arg_texts) &&
     builtin_name == "__type_pack_element" &&
     angle_arg_texts.size() >= 2) {
    long long index = -1;
    if(!evaluate_type_pack_element_index_text(scope, angle_arg_texts[0], index) ||
       index < 0) {
      return false;
    }

    std::vector<TypePtr> expanded_types;
    for(size_t i = 1; i < angle_arg_texts.size(); ++i) {
      if(!append_type_pack_element_argument_types(
             ctx, scope, angle_arg_texts[i], expanded_types)) {
        return false;
      }
    }

    if(static_cast<size_t>(index) >= expanded_types.size()) {
      return false;
    }
    out = expanded_types[static_cast<size_t>(index)];
    return static_cast<bool>(out);
  }

  std::string arg_text;
  if(!parse_unary_type_transform_text(text, builtin_name, arg_text)) {
    return false;
  }

  TypePtr arg_type = ctx.lookup_type(scope, trim_space(arg_text), true);
  if(!arg_type) {
    return false;
  }
