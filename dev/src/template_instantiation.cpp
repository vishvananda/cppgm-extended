...
using callsemantic::retained_template_type_lookup_anchor;
...
void store_class_instantiation(
    std::map<std::string, ClassInfo *> & storage,
    ...
    ClassInfo * info)
{
  ...
  std::pair<std::map<std::string, ClassInfo *>::iterator, bool> inserted =
      storage.insert(std::make_pair(key, info));
  if(!inserted.second) {
    inserted.first->second = info;
  }
  borrow_class_instantiation_key(*info, inserted.first->first);
}
...
bool type_contains_invalid_member_pointer_owner(const TypePtr & type)
{
  TypePtr base = strip_top_level_cv(type);
  if(!base) {
    return false;
  ...
  switch(base->kind) {
  case Type::TK_MEMBER_POINTER:
    return type_is_definitely_not_class(base->owner) ||
           type_contains_invalid_member_pointer_owner(base->owner) ||
           type_contains_invalid_member_pointer_owner(base->inner);
  case Type::TK_ATOMIC:
  case Type::TK_POINTER:
  case Type::TK_BLOCK_POINTER:
  case Type::TK_LVALUE_REFERENCE:
  case Type::TK_RVALUE_REFERENCE:
  case Type::TK_ARRAY:
    return type_contains_invalid_member_pointer_owner(base->inner);
  case Type::TK_FUNCTION:
    if(type_contains_invalid_member_pointer_owner(base->inner)) {
      return true;
    }
    for(std::size_t i = 0; i < base->params.size(); ++i) {
      if(type_contains_invalid_member_pointer_owner(base->params[i])) {
        return true;
      }
    }
    return false;
                                                        ...
                                                        const TypePtr & type)
{
  ...
  if(type_contains_invalid_member_pointer_owner(type)) {
    std::ostringstream out;
    out << "instantiated function template parameter has non-class member-pointer owner";
    out << " [template " << decl.name << "]";
    out << " [type " << describe_type(type) << "]";
    throw TemplateSubstitutionFailure(out.str());
  }
    ...
    const ClassInfo & instantiation_owner)
{
  ...
  if(ClassInfo * owner_info = ctx.class_info_for_type(base_owner)) {
    ...
    return owner_info->source_template &&
           ...
           class_instantiation_key(*owner_info) ==
               class_instantiation_key(instantiation_owner);
  }
  ...
  case Type::TK_NAMED:
  {
    ...
    for(std::size_t i = 0;
      ...
      validate_instantiated_result_template_arguments(
          ctx,
          scope,
          rare.named_dependent_class_arguments[i].type);
    }
    for(std::size_t i = 0;
      ...
      validate_instantiated_result_template_arguments(
          ctx,
          scope,
          rare.named_dependent_alias_arguments[i].type);
    }
    ...
    const TemplateArgument & parsed)
{
  ...
  if(current.kind == TemplateArgument::TA_VALUE) {
    const TemplateArgument::RareData & current_rare = current.rare();
    const TemplateArgument::RareData & parsed_rare = parsed.rare();
    ...
    if(current_rare.function_value || parsed_rare.function_value) {
      return current_rare.function_value == parsed_rare.function_value;
    }
    if(!current_rare.function_internal_symbol.empty() ||
       !parsed_rare.function_internal_symbol.empty()) {
      return current_rare.function_internal_symbol ==
          parsed_rare.function_internal_symbol;
    }
    if(current_rare.value_binding || parsed_rare.value_binding) {
                                                ...
                                                TemplateArgument & argument)
{
  ...
  FunctionBinding * function =
      ctx.first_function_by_internal_symbol(
          argument.rare().function_internal_symbol);
  argument.mutable_rare().function_value = function;
...
bool type_has_dependent_alias_pack_expansion(const TypePtr & type)
{
  ...
  if(type->kind == Type::TK_NAMED) {
    ...
    if(dependent_alias_argument_list_has_pack_expansion(
           rare.named_dependent_alias_arguments) ||
    ...
    const std::vector<TemplateParameterInfo> & parameters)
{
  ...
  if(type->kind == Type::TK_NAMED) {
    ...
    if(type_mentions_template_parameter_name(rare.named_dependent_qualified_owner,
                                             parameters)) {
      return true;
    }
  }
    ...
    const std::vector<TemplateParameterInfo> & parameters)
{
  ...
  if(cppast_conversion_type_id_syntax_storage(node) &&
     ast_mentions_template_parameter_name(*cppast_conversion_type_id_syntax_storage(node),
                                          parameters)) {
    return true;
  }
  if(cppast_base_type_syntax_storage(node) &&
     ast_mentions_template_parameter_name(*cppast_base_type_syntax_storage(node), parameters)) {
    return true;
  }
                                      ...
                                      bool clear_resolved_argument_types)
{
  ...
  if(cppast_conversion_type_id_syntax_storage(node)) {
    clear_cached_semantic_types_impl(*cppast_conversion_type_id_syntax_storage(node),
                                     clear_resolved_argument_types);
  }
  if(cppast_base_type_syntax_storage(node)) {
    clear_cached_semantic_types_impl(*cppast_base_type_syntax_storage(node),
                                     clear_resolved_argument_types);
  }
  for(std::size_t i = 0; i < node.qualifier_template_id_syntaxes.size(); ++i) {
    ...
    const std::vector<TemplateParameterInfo> & parameters)
{
  ...
  if(cppast_conversion_type_id_syntax_storage(node) &&
     ast_has_dependent_non_type_template_argument(
         *cppast_conversion_type_id_syntax_storage(node), parameters)) {
    return true;
  }
  if(cppast_base_type_syntax_storage(node) &&
     ast_has_dependent_non_type_template_argument(*cppast_base_type_syntax_storage(node),
                                                  parameters)) {
    return true;
  }
  for(std::size_t i = 0; i < node.qualifier_template_id_syntaxes.size(); ++i) {
    ...
    const std::vector<TemplateParameterInfo> & parameters)
{
  ...
  if(cppast_conversion_type_id_syntax_storage(node)) {
    clear_dependent_cached_semantic_types_impl(*cppast_conversion_type_id_syntax_storage(node),
                                               parameters);
  }
  if(cppast_base_type_syntax_storage(node)) {
    clear_dependent_cached_semantic_types_impl(*cppast_base_type_syntax_storage(node),
                                               parameters);
  }
  for(std::size_t i = 0; i < node.qualifier_template_id_syntaxes.size(); ++i) {
    ...
    const TypePtr & target_class_type)
{
  ...
  const Type::NamedRareMetadata & unqualified_rare = unqualified->named_rare();
  if(current_qualified.name.empty() &&
     unqualified_rare.named_member_owner_type &&
     !unqualified_rare.named_member_name.empty()) {
    TypePtr owner =
        strip_top_level_cv(unqualified_rare.named_member_owner_type);
    if(owner && owner->kind == Type::TK_NAMED) {
      ...
      if(!current_qualified.name.empty()) {
        ...
        current_qualified.name = unqualified_rare.named_member_name;
      }
    }
  }
  if(current_qualified.name.empty()) {
  ...
  ClassInfo * nested_owner_info =
      nested_info && nested_info->enclosing_scope ?
          ...
          nullptr;
  if(!nested_owner_info && unqualified_rare.named_member_owner_type) {
    nested_owner_info =
  ...
  if(!stored.qualified_name_syntax.qualifiers.empty()) {
    const std::string stored_owner_name =
        semantic_utils::unqualified_member_name(
            strip_template_id_suffix(
                stored.qualified_name_syntax.qualifiers.back()));
    if(!stored_owner_name.empty() && stored_owner_name != info.name) {
      return false;
    }
  }
...
{
  const QualifiedName & symbol_name =
      class_symbol_qualified_name_syntax(info);
    ...
    bool bind_stored_parameters)
{
  ...
  for(std::map<std::string, std::vector<OutOfClassMemberFunctionDecl> >::const_iterator it =
    ...
    for(std::size_t i = 0; i < it->second.size(); ++i) {
      ...
      const ExactTemplateTypeLookupAnchor syntax_anchor =
          retained_template_type_lookup_anchor(stored.declarator);
...
bool ast_node_contains_this_expression(const CppAstNode & node)
{
  ...
  if((cppast_conversion_type_id_syntax_storage(node) &&
      ast_node_contains_this_expression(*cppast_conversion_type_id_syntax_storage(node))) ||
     (cppast_base_type_syntax_storage(node) &&
      ast_node_contains_this_expression(*cppast_base_type_syntax_storage(node)))) {
    return true;
  }
  for(std::size_t i = 0; i < node.qualifier_template_id_syntaxes.size(); ++i) {
    if(template_id_syntax_contains_this_expression(
    ...
    unsigned depth = 0)
{
  ...
  std::shared_ptr<const ClassTemplateSpecializationMangleInfo> mangle_info =
      named_type_class_template_specialization_mangle_info_const(type);
  const std::vector<TemplateParameterInfo> * mangle_parameters =
      mangle_info ? class_template_mangle_parameters(*mangle_info) : nullptr;
  if(!mangle_info ||
...
std::string class_instantiation_key_for_metadata(
    ...
    const ClassInfo & info)
{
  if(!class_instantiation_key(info).empty()) {
    return class_instantiation_key(info);
  }
    ...
    const TemplateArgument & argument)
{
  ...
  if(argument.kind == TemplateArgument::TA_CLASS_TEMPLATE ||
     argument.kind == TemplateArgument::TA_ALIAS_TEMPLATE) {
    return template_template_argument_identity_text(ctx, argument);
  }
  return template_model::template_argument_text(
      argument,
      [&ctx](const TypePtr & type)
    ...
    const std::map<std::string, std::size_t> * pack_sizes = nullptr)
{
  ...
  const QualifiedName * retained_name_syntax =
      &class_symbol_qualified_name_syntax(info);
  if(retained_name_syntax->name.empty() && info.type) {
    retained_name_syntax = &info.type->named_qualified_name_syntax();
  }
  // The specialization name can be very long, but it is replaced immediately
  // by the source template name. Copy only the syntax components that survive.
  mangle_info->template_name_syntax.rooted = retained_name_syntax->rooted;
  mangle_info->template_name_syntax.qualifiers =
      retained_name_syntax->qualifiers;
  ...
  mangle_info->template_parameters.share(
      retained_class_template_mangle_parameters(*info.source_template));
    ...
    const std::map<std::string, std::size_t> * pack_sizes = nullptr)
{
  ...
  set_class_instantiation_key(info, key);
    ...
    const std::vector<TemplateArgument> & source)
{
  ...
  for(std::size_t i = 0; i < count; ++i) {
    const std::vector<TemplateValueDependency> & source_dependencies =
        source[i].rare().value_dependencies;
    if(source_dependencies.empty()) {
      continue;
    }
    std::vector<TemplateValueDependency> & target_dependencies =
        target[i].mutable_rare().value_dependencies;
    for(std::size_t j = 0; j < source_dependencies.size(); ++j) {
      append_template_value_dependency_if_new(target_dependencies,
                                             source_dependencies[j]);
    }
  }
}
                                       ...
                                       bool select_specialization)
{
  ...
  out.has_stored_key = !class_instantiation_key(*info).empty();
  ...
  const auto candidate_matches_declared_owner =
      [declared_owner](const ClassInfo * candidate) -> bool
      {
        ...
        if(declared_owner->source_template) {
          ...
          if(!class_instantiation_key(*declared_owner).empty()) {
            return class_instantiation_key(*candidate) ==
                   class_instantiation_key(*declared_owner);
          }
          return true;
        }
                                                ...
                                                const std::string & instantiation_use_location_override)
{
    ...
    const auto candidate_matches_declared_owner =
        [declared_owner](const ClassInfo * candidate) -> bool
        {
          ...
          if(!declared_owner->dependent_instantiation &&
             !class_instantiation_key(*declared_owner).empty()) {
            return class_instantiation_key(*candidate) ==
                   class_instantiation_key(*declared_owner);
          }
          return true;
  ...
  if(source_decl->specifiers &&
     source_decl->declarator &&
     effective_template_body(*source_decl) &&
     decl_spec_contains_token(*source_decl->specifiers, KW_AUTO) &&
     !cpp_decl::find_child(*source_decl->declarator,
                          CppAstKind::trailing_return_type)) {
    bool is_typedef = false;
    TypePtr deduced_result;
    if(!ctx.parse_function_definition_base(inst_scope,
                                           *source_decl->specifiers,
                                           *source_decl->declarator,
                                           *effective_template_body(*source_decl),
                                           source_decl->is_const_method,
                                           source_decl->is_volatile_method,
       ...
       !deduced_result) {
      throw_substitution_failure(
          source_decl->is_lambda_call_operator_template ?
              "failed generic lambda implicit return type deduction" :
              "failed function template implicit return type deduction",
          std::string(),
          "template-instantiation");
    }
    TypePtr function_pattern = strip_top_level_cv(type);
    std::vector<TypePtr> resolved_param_types;
    resolved_param_types.reserve(params.size());
    for(std::size_t i = 0; i < params.size(); ++i) {
      resolved_param_types.push_back(params[i].second);
    }
    type = make_function(
        deduced_result,
        resolved_param_types,
        function_pattern && function_pattern->kind == Type::TK_FUNCTION ?
            function_pattern->variadic : false,
        function_pattern && function_pattern->kind == Type::TK_FUNCTION ?
            function_pattern->function_const : false,
        function_pattern && function_pattern->kind == Type::TK_FUNCTION ?
            function_pattern->function_volatile : false,
        function_pattern && function_pattern->kind == Type::TK_FUNCTION ?
            function_pattern->prototype_relaxed : false,
        function_pattern && function_pattern->kind == Type::TK_FUNCTION ?
            function_pattern->function_ref_qualifier : FTRQ_NONE);
  }
  ...
  if(!source_decl->is_constructor && !source_decl->is_destructor) {
    ...
    if(function_base && function_base->kind == Type::TK_FUNCTION) {
      ...
      const bool source_result_mentions_template_parameter =
          ...
          ast_mentions_template_parameter_name(source_decl->result_type_pattern,
                                               source_decl->parameters);
      const bool source_result_has_template_parameter_pack =
          any_of(source_decl->parameters.begin(),
                 source_decl->parameters.end(),
                 [](const TemplateParameterInfo & parameter)
                 {
                   return parameter.parameter_pack;
                 });
      const bool can_rebind_result_pattern_after_recovery_failure =
          source_result_mentions_template_parameter &&
          source_result_has_template_parameter_pack;
      const auto recover_result_without_preempting_pattern_rebind =
          [&](Scope & recovery_scope, TypePtr & recovered) -> bool
          {
            ...
            try {
              ...
              if(!can_rebind_result_pattern_after_recovery_failure) {
                throw;
              }
              recovered.reset();
              return false;
      ...
      if(result_type_still_dependent &&
        ...
        if(has_member_trailing_return_scope &&
          ...
          if(configure_member_trailing_return_scope(result_scope,
                                                    synthetic_function)) {
            recovered_result =
                recover_result_without_preempting_pattern_rebind(
                    result_scope,
      ...
      if(result_type_still_dependent &&
        ...
        if(recover_result_without_preempting_pattern_rebind(
               *instantiation_owner->member_scope,
               owner_scope_result) &&
           owner_scope_result) {
      ...
      if(result_type_still_dependent &&
        ...
        bool recovered_owner_member = false;
        try {
          recovered_owner_member =
              recover_instantiation_owner_member_type(ctx,
                                                      instantiation_owner,
                                                      result_type,
                                                      owner_member_result);
        } catch(const std::logic_error &) {
          ...
          owner_member_result.reset();
        }
        if(recovered_owner_member &&
           owner_member_result) {
        ...
        const bool parsed_result_type =
            template_api::with_template_services(
                ...
                [&](template_api::TemplateServices & services)
                {
                  ...
                  clear_cached_semantic_types(parse_pattern);
                  CppAstNode substituted_pattern;
                  const bool substituted_result_pattern =
