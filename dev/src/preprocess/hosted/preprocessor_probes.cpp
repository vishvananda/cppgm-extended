...
{
	// Ask the registries that hold the answer.  A builtin we implement but do
	// not admit to is worse than one we lack: the header takes its fallback,
	// and libc++'s fallbacks are written in later-standard machinery that we
	// then have to support instead of the builtin we already had.
