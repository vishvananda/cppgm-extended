...
// The standard library was chosen when this compiler was built, and its
// include paths were baked in then.  A `-stdlib=` on the command line can
// therefore only agree or disagree with that choice; it cannot change it.
const char* HostedCompilerStdlibFlags()
{
	return cppgm_builtin_host_config::kStdlibFlags;
}
