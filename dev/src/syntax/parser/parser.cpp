...
#include "syntax/parser/special_members.h"
...
class SyntaxParser : private ParserCursor<SyntaxParser>,
	...
	private SpecialMemberSyntax<SyntaxParser>
{
...
friend class syntax::TemplateSyntax<SyntaxParser>; friend class syntax::ControlFlowSyntax<SyntaxParser>;
friend class syntax::SpecialMemberSyntax<SyntaxParser>; public:
...
private:
	// N3485 14.2/3 ends a template-argument-list at the first non-nested
	// `>`.  Inside parentheses, brackets or braces the token is the
	// greater-than operator again, whatever depth of argument lists
	// encloses them, so the stop is suspended for the nested expression
	// and restored afterwards, on every exit.
	struct AngleStopSuspension
	{
		explicit AngleStopSuspension(std::size_t* depth)
			: depth_(depth), saved_(*depth) { *depth = 0; }
		~AngleStopSuspension() { *depth_ = saved_; }
		std::size_t* depth_;
		std::size_t saved_;
	};
	struct Mark
	...
	// A GNU `aligned` argument was consumed as raw tokens because the attribute
	// scanner has no grammar.  Re-parse that token range here, where the
	// grammar is, and hand back the same `alignment-specifier` node `alignas`
	// produces so both spellings evaluate through one path.
	NodeId ParseGnuAlignedArgument(std::size_t begin, std::size_t end)
	{
		if (begin >= end || end > tokens_.size()) return kNoNode;
		const std::size_t resume = position_;
		position_ = begin;
		const NodeId alignment = arena_.Make("alignment-specifier");
		arena_.AddFlags(alignment, SYNTAX_FLAG_SEMANTIC_ONLY);
		const Mark type_mark = Checkpoint();
		if (!ParseTypeId(alignment) || position_ != end)
		{
			Rollback(type_mark);
			position_ = begin;
			const NodeId value = ParseExpression(2);
			if (value == kNoNode || position_ != end)
			{
				position_ = resume;
				return kNoNode;
			}
...
NodeId SyntaxParser::ParseDeclSpecifierSeq(bool for_type_id, std::string* first_type)
{
	...
	while (true)
	{
		...
		if (!saw_type && At(KW_DECLTYPE))
		{
			...
			{
				AngleStopSuspension nested(&angle_stop_depth_);
...
NodeId SyntaxParser::ParseTypeTemplateParameter()
{
	...
	if (AtIdentifier())
	{
		...
		// Facts are keyed by spelling, so a template template-parameter named
		// the same thing in an earlier declaration leaves its own fact behind.
		// libc++ names one `_Sp` and then reuses `_Sp` as an ordinary type
		// parameter, and the stale fact stopped `_Sp (` from starting a
		// declarator, so the parenthesis parsed as a parameter clause.  Say
		// which kind this parameter is rather than only adding to it.
		SetNameFact(name, kKnownTemplate, template_template);
	}
	if (Match(OP_ASS))
	{
...
{
	std::vector<syntax::GnuAttributeExpressionArgument> aligned_arguments;
	if (syntax::ConsumeLeadingGnuObjectAttribute(
		tokens_, strings_, arena_, &position_, attributes, &aligned_arguments))
	{
		for (std::size_t i = 0; i < aligned_arguments.size(); ++i)
		{
			const NodeId parsed = ParseGnuAlignedArgument(
				aligned_arguments[i].begin, aligned_arguments[i].end);
			if (parsed != kNoNode)
				arena_.Add(aligned_arguments[i].attribute, parsed);
		}
		return true;
	}
	return syntax::ConsumeLeadingStandardObjectAttribute(
