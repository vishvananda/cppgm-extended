...
protected:
	...
	void PredeclareClassTypeNames()
	{
		...
		while (scan < parser.tokens_.size())
		{
			...
			// N3485 3.3.7/1: a member function body is a complete-class
			// context, so a member type declared later in the class is
			// visible in it.  The class-key names below are predeclared for
			// that reason; the alias names have to be too, or a body that
			// uses one parses the name as an expression.
			if (kind == static_cast<std::uint16_t>(KW_TYPEDEF))
			{
				scan = PredeclareTypedefNames(scan + 1);
				continue;
			}
			if (kind == static_cast<std::uint16_t>(KW_USING))
			{
				const std::size_t name = scan + 1;
				if (name + 1 < parser.tokens_.size() &&
					parser.tokens_[name].Kind() == kIdentifierToken &&
					parser.tokens_[name + 1].Kind() ==
						static_cast<std::uint16_t>(OP_ASS))
					parser.SetNameFact(
						parser.tokens_[name].spelling, parser.kKnownType);
				++scan;
				continue;
			}
	...
	// A typedef declares its names in the declarator positions, so take the
	// identifier that ends each one: the one before the terminating semicolon
	// and before each comma that separates declarators.  Parentheses and
	// brackets are tracked so a function or array typedef does not offer an
	// inner name.  Returns the position after the declaration.
	std::size_t PredeclareTypedefNames(std::size_t scan)
	{
		Derived& parser = static_cast<Derived&>(*this);
		std::size_t depth = 0;
		std::size_t previous_identifier = parser.tokens_.size();
		for (; scan < parser.tokens_.size(); ++scan)
		{
			const std::uint16_t kind = parser.tokens_[scan].Kind();
			if (kind == static_cast<std::uint16_t>(OP_LPAREN) ||
				kind == static_cast<std::uint16_t>(OP_LSQUARE)) { ++depth; continue; }
			...
			if (kind == static_cast<std::uint16_t>(OP_LBRACE) ||
				kind == static_cast<std::uint16_t>(OP_RBRACE)) return scan;
			if (kind == kIdentifierToken)
			{
				previous_identifier = scan;
				continue;
			}
			const bool ends_declarator = depth == 0 &&
				(kind == static_cast<std::uint16_t>(OP_SEMICOLON) ||
				 kind == static_cast<std::uint16_t>(OP_COMMA));
			if (ends_declarator && previous_identifier < parser.tokens_.size())
				parser.SetNameFact(
					parser.tokens_[previous_identifier].spelling,
					parser.kKnownType);
			if (kind == static_cast<std::uint16_t>(OP_SEMICOLON)) return scan + 1;
			if (ends_declarator) previous_identifier = parser.tokens_.size();
		}
		return scan;
	}
