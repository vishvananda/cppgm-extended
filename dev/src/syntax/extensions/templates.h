...
protected:
	...
	std::vector<NodeId> ParseSpecialMemberSpecifiers()
	{
		...
		// Attributes interleave with these specifiers rather than preceding
		// them all: libc++ writes an explicit-specifier, then its visibility
		// attribute macro, then constexpr on one declaration.  Stopping at the
		// attribute leaves the declaration half-parsed.  Only look for one
		// after a specifier has been taken, so a declaration with none of
		// these costs what it did before: this runs for every member.
		bool taken = false;
		while (true)
		{
			if (taken) parser.SkipAttributes();
			if (!(parser.At(KW_INLINE) || parser.At(KW_VIRTUAL) ||
				parser.At(KW_EXPLICIT) || parser.At(KW_CONSTEXPR) ||
				parser.At(KW_FRIEND) || parser.At(KW_STATIC))) break;
			taken = true;
