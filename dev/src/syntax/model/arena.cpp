...
// A node whose range was never set has token_first zero, which would otherwise
// read the first token of the translation unit and report a confident location
// in whichever header happened to come first.  A wrong location costs more than
// an absent one, so say nothing instead.
bool SyntaxArena::HasSourceLocation(NodeId node) const
{
	const std::size_t token = nodes_[node].token_first;
	return token != 0 && token < tokens_.size();
}

const std::string& SyntaxArena::SourceFile(NodeId node) const
{
	return strings_.Get(HasSourceLocation(node) ?
		tokens_[nodes_[node].token_first].source_file : 0);
}

std::size_t SyntaxArena::SourceLine(NodeId node) const
{
	return HasSourceLocation(node) ?
		tokens_[nodes_[node].token_first].source_line : 0;
}

std::size_t SyntaxArena::SourceColumn(NodeId node) const
{
	return HasSourceLocation(node) ?
		tokens_[nodes_[node].token_first].source_column : 0;
}
