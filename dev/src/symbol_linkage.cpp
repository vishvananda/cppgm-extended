...
struct MangleSubstitutionState
{
  ...
  struct SubstitutionKeyPtrHash
  {
    size_t operator()(const abi_mangle::SubstitutionKey * key) const
    {
      return key ? SubstitutionKeyHash()(*key) : 0;
    }
  };

  struct SubstitutionKeyPtrEqual
  {
    bool operator()(const abi_mangle::SubstitutionKey * lhs,
                    const abi_mangle::SubstitutionKey * rhs) const
    {
  ...
  unordered_map<const abi_mangle::SubstitutionKey *,
                size_t,
                SubstitutionKeyPtrHash,
                SubstitutionKeyPtrEqual> ir_substitution_index;
  unordered_map<string, size_t> ir_named_alias_index;
  vector<string> substitution_keys;
  deque<abi_mangle::SubstitutionKey> substitution_ir_keys;
...
struct MangleSubstitutionCheckpoint
{
  ...
  void rollback()
  {
    ...
    state->ir_named_alias_index.clear();
...
static bool try_build_type_ir(const TypePtr & type,
                              const TypeMangleContext * mangle_ctx,
                              abi_mangle::Type & out);
static bool try_build_type_ir_cached(const TypePtr & type,
                                     const TypeMangleContext * mangle_ctx,
                                     abi_mangle::Type & out);
static bool type_ir_operation_cache_enabled();
...
struct TypeMangleContext
{
  ...
  bool prefer_concrete_owner_non_type_expression_values = false;
...
static void materialize_ir_substitution_index(MangleSubstitutionState * state)
{
  ...
  state->ir_named_alias_index.reserve(state->substitution_ir_keys.size());
  for(size_t i = 0; i < state->substitution_ir_keys.size(); ++i) {
    ...
    if(key.kind ==
           abi_mangle::SubstitutionKey::SK_CLASS_TEMPLATE_SPECIALIZATION &&
       !key.payload.empty()) {
      state->ir_named_alias_index.insert(make_pair(key.payload, global_index));
    }
  }
    ...
    size_t & index)
{
  ...
  unordered_map<const abi_mangle::SubstitutionKey *,
                size_t,
                MangleSubstitutionState::SubstitutionKeyPtrHash,
                MangleSubstitutionState::SubstitutionKeyPtrEqual>::
      const_iterator found = state->ir_substitution_index.find(&key);
  bool have_index = false;
  if(found != state->ir_substitution_index.end()) {
    index = found->second;
    have_index = true;
  }
  if(key.kind == abi_mangle::SubstitutionKey::SK_NAMED) {
    unordered_map<string, size_t>::const_iterator alias_found =
        state->ir_named_alias_index.find(key.payload);
    if(alias_found != state->ir_named_alias_index.end() &&
       (!have_index || alias_found->second < index)) {
      index = alias_found->second;
      have_index = true;
    }
  }
  return have_index;
...
static void index_registered_ir_substitution(
    MangleSubstitutionState * state,
    const abi_mangle::SubstitutionKey & key,
    size_t index)
{
  ...
  const bool was_materialized = state->ir_substitution_index_materialized;
  materialize_ir_substitution_index(state);
  if(!was_materialized) {
    return;
  }
  ...
  if(key.kind ==
         abi_mangle::SubstitutionKey::SK_CLASS_TEMPLATE_SPECIALIZATION &&
     !key.payload.empty()) {
    state->ir_named_alias_index.insert(make_pair(key.payload, index));
  }
}
    ...
    const abi_mangle::SubstitutionKey & key)
{
  ...
  if(find_registered_ir_substitution_index(state, key, existing_index)) {
    return false;
  }
  return true;
}
    ...
    const abi_mangle::SubstitutionKey & key)
{
  if(prepare_ir_substitution_key_registration(state, key)) {
    ...
    index_registered_ir_substitution(
        state, state->substitution_ir_keys.back(), index);
  }
}
    ...
    QualifiedName & out)
{
  ...
  for(const semantic_model::Scope * current = scope; current; current = current->parent) {
    if(current->class_info &&
       current->class_info->member_scope.get() == current) {
      out = class_symbol_qualified_name_syntax(*current->class_info);
      if(out.name.empty()) {
        return false;
      }
      reverse(parts.begin(), parts.end());
...
static bool type_is_std_class_template_specialization(
    ...
    const ClassTemplateSpecializationMangleInfo ** info_out = nullptr)
{
    ...
    const TemplateArgument & argument)
{
  ...
  const ClassTemplateSpecializationMangleInfo * info = nullptr;
    ...
    const TypeMangleContext * mangle_ctx)
{
  ...
  if(cppast_conversion_type_id_syntax_storage(node) &&
     ast_node_mentions_template_parameter(*cppast_conversion_type_id_syntax_storage(node),
                                          mangle_ctx)) {
    return true;
  }
                                                 ...
                                                 const TypeMangleContext * mangle_ctx)
{
  ...
  if(cppast_conversion_type_id_syntax_storage(node) &&
     ast_node_mentions_function_parameter(*cppast_conversion_type_id_syntax_storage(node),
                                          mangle_ctx)) {
    return true;
  }
  for(size_t i = 0; i < node.qualifier_type_syntaxes.size(); ++i) {
...
static CppAstNode clone_ast_node_for_mangling(const CppAstNode & source)
{
  ...
  if(cppast_base_type_syntax_storage(source)) {
    mutable_cppast_base_type_syntax_storage(out).reset(
        new CppAstNode(clone_ast_node_for_mangling(*cppast_base_type_syntax_storage(source))));
  }
  out.qualifier_template_id_syntaxes.reserve(
      source.qualifier_template_id_syntaxes.size());
  for(size_t i = 0; i < source.qualifier_template_id_syntaxes.size(); ++i) {
...
{
  const string & builtin_name = cppast_builtin_type_transform_name(node);
  ...
  for(size_t i = 0; i < count; ++i) {
    if(!type_id_ast_is_template_type_parameter(*cppast_base_type_syntax_storage(node),
                                               parameters[i])) {
      continue;
    }
    TypePtr arg_type =
    ...
    if(arg_type && type_has_dependent_mangle_state(arg_type)) {
      ...
      mutable_cppast_builtin_type_transform_name(node) = builtin_name;
...
static bool ast_contains_decltype_specifier(const CppAstNode & node)
{
  ...
  if(cppast_conversion_type_id_syntax_storage(node) &&
     ast_contains_decltype_specifier(*cppast_conversion_type_id_syntax_storage(node))) {
    return true;
  }
  for(size_t i = 0; i < node.qualifier_type_syntaxes.size(); ++i) {
    ...
    const TemplateParameterInfo * parameter)
{
  ...
  if(parameter &&
     ...
     argument.syntax.pack_expansion) {
    syntax.pack_expansion = true;
  }
  return syntax;
}
                                            ...
                                            string & out)
{
  if(!cppast_builtin_type_transform_name(node).empty() &&
    ...
    out = cppast_builtin_type_transform_name(node);
    return true;
  }
    ...
    ExternalEntityArgumentIrPayload & payload)
{
  ...
  if(rare.value_binding && base && base->kind == Type::TK_MEMBER_POINTER) {
    const semantic_model::ValueBinding * binding = rare.value_binding;
    if(binding->kind == semantic_model::ValueBinding::VK_FIELD) {
...
static bool try_build_template_argument_type_ir(
    ...
    abi_mangle::Type & out)
{
  return try_build_type_ir_cached(
      type,
    ...
    abi_mangle::DependentExpression & out)
{
  ...
  if((mangle_ctx->prefer_concrete_non_type_values_for_dependent_parameter_types ||
      mangle_ctx->prefer_concrete_owner_non_type_expression_values) &&
     mangle_ctx->owner_template_parameters &&
     mangle_ctx->owner_template_arguments) {
    const vector<TemplateParameterInfo> & parameters =
        *mangle_ctx->owner_template_parameters;
    ...
    for(size_t i = 0; i < parameters.size() && i < arguments.size(); ++i) {
      ...
      abi_mangle::Type ir_type;
      if(!value_type || !try_build_type_ir(value_type, mangle_ctx, ir_type)) {
        return false;
      }
      out = abi_mangle::DependentExpression::typed_integral_value(
          std::move(ir_type), argument.value);
      return true;
    }
  }
...
static const semantic_model::ValueBinding *
lookup_direct_value_for_mangling(const semantic_model::Scope & scope,
                                 const string & name)
{
  map<string, semantic_model::ValueBinding>::const_iterator found =
      scope.values.find(name);
  if(found != scope.values.end()) {
    return &found->second;
  }
  for(size_t i = 0; i < scope.namespace_children.size(); ++i) {
    ...
    if(const semantic_model::ValueBinding * binding =
           lookup_direct_value_for_mangling(child, name)) {
      return binding;
    }
  }
  return nullptr;
...
static bool try_build_unqualified_variable_expression_ir(
    const CppAstNode & node,
    const TypeMangleContext * mangle_ctx,
    abi_mangle::DependentExpression & out)
{
  ...
  const string name = qualified && !qualified->name.empty() ?
      qualified->name : trim_space(node.value);
  if(name.empty()) {
    return false;
  }
  ...
  for(const semantic_model::Scope * current = mangle_ctx->lookup_scope;
      current;
      current = current->parent) {
    binding = lookup_direct_value_for_mangling(*current, name);
    if(binding) {
      break;
    }
  }
  if(!binding || binding->kind != semantic_model::ValueBinding::VK_VARIABLE) {
    return false;
  }
  ...
  if(binding->declaration_node &&
    ...
    abi_mangle::Type value_type;
    if(!binding->type ||
       !try_build_type_ir(strip_top_level_cv(binding->type),
                          mangle_ctx,
                          value_type)) {
      return false;
    }
    out = abi_mangle::DependentExpression::typed_integral_value(
        std::move(value_type), binding->constant_value);
    return true;
  }

  string symbol;
  if(!try_build_value_binding_object_symbol_ir_payload(*binding,
                                                       mangle_ctx,
                                                       symbol)) {
    return false;
  }
  out = abi_mangle::DependentExpression::external_entity(std::move(symbol),
                                                          false);
  return true;
}
...
static bool ast_contains_pack_expansion_for_mangling(const CppAstNode & node)
{
  ...
  for(size_t i = 0; i < node.children.size(); ++i) {
    if(ast_contains_pack_expansion_for_mangling(node.children[i])) {
      return true;
    }
  }
  return false;
...
static bool ast_contains_function_parameter_pack_for_mangling(
    const CppAstNode & node)
{
  if(node.kind == CppAstKind::parameter_clause &&
     ast_contains_pack_expansion_for_mangling(node)) {
    return true;
  }
  for(size_t i = 0; i < node.children.size(); ++i) {
    if(ast_contains_function_parameter_pack_for_mangling(node.children[i])) {
      return true;
    }
  }
  return false;
    ...
    abi_mangle::Type::ClassTemplateArgument & out)
{
  if(parameter.kind != TemplateParameterInfo::TP_TYPE ||
     ...
     (argument.syntax.type_id &&
      ast_contains_function_parameter_pack_for_mangling(
          *argument.syntax.type_id)) ||
    ...
    vector<abi_mangle::Type::ClassTemplateArgument> & out)
{
  ...
  for(size_t i = 0; i < arguments.size(); ++i) {
    ...
    if(!try_build_template_argument_syntax_ir(
           effective_dependent_argument_syntax(arguments[i], nullptr),
           nullptr,
           mangle_ctx,
           ir_argument)) {
      if(parser_trace::enabled("symbol.linkage")) {
                              ...
                              vector<TemplateParameterInfo> & explicit_parameter_storage)
{
  ...
  const size_t parameter_count = syntax.argument_syntaxes.size();
  ...
  const vector<TemplateParameterInfo> * raw_parameters =
      class_template ? &class_template->parameters :
                       lookup_template_parameters_for_template_id_syntax(
                           syntax, mangle_ctx);
  if(!suppress_current_pack_grouping &&
     raw_parameters &&
    ...
    abi_mangle::Type & out)
{
  ...
  if(!qualified.rooted) {
    ...
    if(qualified_type_text_lexical_lookup_prefix(qualified,
      ...
      qualifiers.insert(qualifiers.end(),
                        namespace_template_id.name.qualifiers.begin(),
                        namespace_template_id.name.qualifiers.end());
      namespace_template_id.name.qualifiers.swap(qualifiers);
      if(lookup_class_template_for_template_id_syntax(namespace_template_id,
                                                      mangle_ctx)) {
        return try_build_template_id_type_ir(namespace_template_id,
                                             mangle_ctx,
                                             out);
      }
    }
    ...
    abi_mangle::Type & out)
{
  ...
  unique_ptr<abi_mangle::Type> owner_ir(new abi_mangle::Type());
  ...
  unique_ptr<abi_mangle::Type> current = std::move(owner_ir);
  for(size_t i = 0; i < members.size(); ++i) {
    ...
    if(i < member_template_ids.size() &&
      ...
      unique_ptr<abi_mangle::Type> member_template(new abi_mangle::Type());
...
static bool try_build_concrete_owner_pack_parameter_types_ir(
    const CppAstNode & node,
    const TypeMangleContext * mangle_ctx,
    vector<abi_mangle::Type> & out)
{
  ...
  bool cv_volatile = false;
  for(size_t i = 0; i < specifiers->children.size(); ++i) {
    cv_const = cv_const || specifiers->children[i].value == "const";
    cv_volatile = cv_volatile || specifiers->children[i].value == "volatile";
  }
  ...
  for(size_t i = 0; i < parameters.size(); ++i) {
    ...
    size_t pack_count = arguments.size() >= argument_index + trailing_nonpack ?
        arguments.size() - argument_index - trailing_nonpack :
        static_cast<size_t>(-1);
    if(mangle_ctx->template_argument_pack_sizes) {
      map<string, size_t>::const_iterator found =
          !parameter.name.empty() ?
              mangle_ctx->template_argument_pack_sizes->find(parameter.name) :
              mangle_ctx->template_argument_pack_sizes->end();
      if(found == mangle_ctx->template_argument_pack_sizes->end() &&
         !parameter.placeholder_key.empty()) {
        found = mangle_ctx->template_argument_pack_sizes->find(
            parameter.placeholder_key);
      }
      if(found != mangle_ctx->template_argument_pack_sizes->end()) {
        pack_count = found->second;
    ...
    if(pack_count == static_cast<size_t>(-1) ||
       argument_index + pack_count > arguments.size()) {
      return false;
    }

    if(!type_specifier_seq_is_template_type_parameter(*specifiers, parameter)) {
    ...
    vector<abi_mangle::Type> expanded;
    expanded.reserve(pack_count);
    for(size_t j = 0; j < pack_count; ++j) {
      const TemplateArgument & argument = arguments[argument_index + j];
      if(argument.kind != TemplateArgument::TA_TYPE ||
         !argument.type ||
         template_argument_is_self_type_parameter(argument,
                                                  parameter,
      ...
      TypeMangleContext argument_ctx_storage;
      const TypeMangleContext * argument_ctx =
          suppress_owner_template_argument_index(mangle_ctx,
                                                 i,
                                                 argument_ctx_storage);
      abi_mangle::Type concrete;
      const auto build_base =
          [&argument,
           argument_ctx,
           cv_const,
           cv_volatile](abi_mangle::Type & type) -> bool
          {
            if(!try_build_template_argument_type_ir(argument.type,
                                                    argument_ctx,
                                                    type)) {
              return false;
            }
            if(cv_const || cv_volatile) {
              type = abi_mangle::Type::cv(cv_const,
                                          cv_volatile,
                                          std::move(type));
            }
            return true;
      ...
      if(declarator) {
        if(!try_build_declarator_ast_type_ir(
               *declarator,
               build_base,
               argument_ctx,
               concrete)) {
    ...
    abi_mangle::Type & out)
{
  ...
  for(size_t i = 0; i < clause.children.size(); ++i) {
    ...
    vector<abi_mangle::Type> expanded_params;
    if(try_build_concrete_owner_pack_parameter_types_ir(child,
                                                        mangle_ctx,
                                                        expanded_params)) {
    ...
    abi_mangle::Type & out)
{
  ...
  const function<bool(abi_mangle::Type &)> build_pointer_base =
      [&ptr_operators, &build_base, mangle_ctx](abi_mangle::Type & type) -> bool
      {
        if(!build_base(type)) {
          return false;
        }
        for(vector<const CppAstNode *>::const_reverse_iterator it =
          ...
          if(op.has_token && op.simple_type == OP_STAR) {
          ...
          } else if(!op.has_token) {
            const QualifiedName * qualified =
                cppast_qualified_name_syntax(op);
            ...
            abi_mangle::Type owner;
            const TemplateIdSyntax * template_id =
                cppast_template_id_syntax(op);
            if(template_id) {
              if(qualified->qualifiers.empty()) {
                if(!try_build_template_id_type_ir(*template_id,
                                                  mangle_ctx,
                                                  owner)) {
                  return false;
                }
              } else if(!try_build_qualified_template_id_type_ast_ir(
                            op,
                            *template_id,
                            *qualified,
                            mangle_ctx,
            ...
            } else if(qualified->qualifiers.empty()) {
              if(!try_build_unqualified_component_type_ir(
                     qualified->name,
                     mangle_ctx,
                     true,
                     owner)) {
              ...
              abi_mangle::Type prefix;
              if(!try_build_qualified_owner_type_ir(
                     *qualified,
                     &op.qualifier_template_id_syntaxes.as_vector(),
                     mangle_ctx,
                     prefix) ||
                 !try_build_qualified_component_type_ir(
                     qualified->name,
                     true,
                     prefix,
                     mangle_ctx,
            ...
            type = abi_mangle::Type::member_pointer(std::move(owner),
                                                    std::move(type));
          } else {
            return false;
          }
    ...
    abi_mangle::Type & out)
{
  ...
  for(size_t i = 0; i < node.children.size(); ++i) {
    ...
    if(child.kind == CppAstKind::decl_specifier_seq ||
      ...
      declarator = &child;
      pack_expansion =
          pack_expansion || ast_contains_pack_expansion_for_mangling(child);
    ...
    abi_mangle::Type::ClassTemplateArgument & out)
{
  ...
  const bool unknown_expression_argument =
      ...
      !syntax.template_id;
  if(known_type_parameter &&
     syntax.type_id &&
     ast_contains_function_parameter_pack_for_mangling(*syntax.type_id)) {
    abi_mangle::Type type;
    if(try_build_type_id_ast_ir(*syntax.type_id, mangle_ctx, type)) {
    ...
    abi_mangle::DependentExpression & out)
{
  ...
  if((node.kind == CppAstKind::id_expression ||
      node.kind == CppAstKind::identifier) &&
     try_build_unqualified_variable_expression_ir(node, mangle_ctx, out)) {
    return true;
  }
...
static bool try_build_class_template_type_argument_ir(
    const TemplateArgument & argument,
    const TypeMangleContext * mangle_ctx,
    abi_mangle::Type::ClassTemplateArgument & out)
{
  ...
  if(argument.dependent &&
    ...
    wrap_pack_expansion_type_ir_if_needed(true, type);
  }
  out = abi_mangle::Type::ClassTemplateArgument::type_arg(std::move(type));
  return true;
}

static bool try_build_non_type_or_template_class_template_argument_ir(
    const TemplateArgument & argument,
    const TemplateParameterInfo * parameter,
    const TypeMangleContext * mangle_ctx,
    abi_mangle::Type::ClassTemplateArgument & out)
    ...
    abi_mangle::Type::ClassTemplateArgument & out)
{
  return argument.kind == TemplateArgument::TA_TYPE ?
      try_build_class_template_type_argument_ir(argument, mangle_ctx, out) :
      try_build_non_type_or_template_class_template_argument_ir(
          argument, parameter, mangle_ctx, out);
}
    ...
    abi_mangle::Type & out)
{
  ...
  const ClassTemplateSpecializationMangleInfo * specialization =
      named_type_class_template_specialization_mangle_info_ptr(base);
  ...
  const vector<TemplateParameterInfo> * parameters =
      semantic_model::class_template_mangle_parameters(*specialization);
  const map<string, size_t> * pack_sizes = nullptr;
  if(!specialization->pack_sizes.empty() &&
     (specialization->mangle_parameters.empty() ||
      template_parameter_lists_match(parameters,
                                     &specialization->mangle_parameters))) {
    pack_sizes = &specialization->pack_sizes;
  }
  vector<abi_mangle::Type::ClassTemplateArgument> arguments;
  if(!try_build_class_template_arguments_ir(specialization->arguments,
                                            ...
                                            arguments) &&
     (!pack_sizes ||
      !try_build_class_template_arguments_ir(specialization->arguments,
                                             parameters,
                                             nullptr,
                                             specialization_ctx,
                                             arguments))) {
  ...
  if(has_typed_member_owner) {
    unique_ptr<abi_mangle::Type> owner(new abi_mangle::Type());
    ...
    const string owner_text =
	    ...
	    unique_ptr<abi_mangle::Type> ir_type(
	        new abi_mangle::Type(
	            abi_mangle::Type::member_class_template_specialization(
	                std::move(*owner),
	                rare.named_member_name,
                                    ...
                                    abi_mangle::Type & out)
{
  ...
  if(rare.named_member_owner_type && !rare.named_member_name.empty()) {
    ...
    const string owner_text =
        preferred_named_type_text(
            strip_top_level_cv(rare.named_member_owner_type),
            mangle_ctx);
...
static bool try_build_type_ir_uncached(const TypePtr & type,
                                       const TypeMangleContext * mangle_ctx,
                                       abi_mangle::Type & out)
{
...
struct TypeIrOperationCacheKey
{
  const Type * type = nullptr;
  const TypeMangleContext * context = nullptr;

  bool operator==(const TypeIrOperationCacheKey & rhs) const
  {
    return type == rhs.type && context == rhs.context;
  }
};

struct TypeIrOperationCacheKeyHash
{
  size_t operator()(const TypeIrOperationCacheKey & key) const
  {
    size_t seed = std::hash<const Type *>()(key.type);
    seed ^= std::hash<const TypeMangleContext *>()(key.context) +
...
struct TypeIrOperationCacheState
{
  unordered_map<TypeIrOperationCacheKey,
                abi_mangle::Type,
                TypeIrOperationCacheKeyHash> values;
  size_t depth = 0;
  size_t hits = 0;
  size_t misses = 0;
  ...
  TypeIrOperationCacheState()
      : diagnostic(
            std::getenv("CPPGM_DIAGNOSTIC_TYPE_IR_OPERATION_CACHE") != nullptr)
  {
    values.reserve(128);
  }
  ...
  ~TypeIrOperationCacheState()
  {
    if(diagnostic) {
      std::fprintf(stderr,
                   "type-ir-operation-cache hits=%zu misses=%zu "
                   "max-entries=%zu\n",
                   hits,
                   misses,
                   max_entries);
...
struct TypeIrOperationCacheFrame
{
  explicit TypeIrOperationCacheFrame(TypeIrOperationCacheState & state)
      : state(state),
        root(state.depth++ == 0)
  {
    if(root) {
  ...
  ~TypeIrOperationCacheFrame()
  {
    if(--state.depth == 0) {
      state.max_entries = max(state.max_entries, state.values.size());
      state.values.clear();
    }
  }

  TypeIrOperationCacheState & state;
  bool root;
};
                              ...
                              abi_mangle::Type & out)
{
  ...
  TypeIrOperationCacheFrame frame(state);
  TypeIrOperationCacheKey key;
  key.type = type.get();
  key.context = mangle_ctx;
  unordered_map<TypeIrOperationCacheKey,
                abi_mangle::Type,
                TypeIrOperationCacheKeyHash>::const_iterator found =
      state.values.find(key);
  ...
  pair<unordered_map<TypeIrOperationCacheKey,
       ...
       bool> inserted = state.values.emplace(key, std::move(candidate));
    ...
    abi_mangle::Type & out)
{
  ...
  struct TypeIrCacheState
  {
    unordered_map<TypePtr, abi_mangle::Type> default_cache;
    unordered_map<TypePtr, abi_mangle::Type> ir_only_cache;
    unordered_map<const Type *, size_t> hit_counts;
    size_t default_hits = 0;
    size_t default_misses = 0;
    size_t ir_only_hits = 0;
    size_t ir_only_misses = 0;
    bool diagnostic = false;

    TypeIrCacheState()
        : diagnostic(std::getenv("CPPGM_DIAGNOSTIC_TYPE_IR_CACHE") != nullptr)
    {
    }
    ...
    ~TypeIrCacheState()
    {
      ...
      std::fprintf(stderr,
                   "type-ir-cache default-hits=%zu default-misses=%zu "
                   "default-entries=%zu ir-only-hits=%zu ir-only-misses=%zu "
                   "ir-only-entries=%zu distinct-hit-types=%zu\n",
                   default_hits,
                   default_misses,
                   default_cache.size(),
                   ir_only_hits,
                   ...
                   ir_only_cache.size(),
                   hit_counts.size());
      vector<pair<size_t, const Type *> > ranked;
      ranked.reserve(hit_counts.size());
      for(unordered_map<const Type *, size_t>::const_iterator it =
              hit_counts.begin();
          it != hit_counts.end();
          ++it) {
      ...
      sort(ranked.begin(), ranked.end(),
           [](const pair<size_t, const Type *> & lhs,
              const pair<size_t, const Type *> & rhs)
           {
      ...
      for(size_t i = 0; i < limit; ++i) {
        ...
        std::fprintf(stderr,
                     ...
                     "key-bytes=%zu display-bytes=%zu text=%.*s\n",
                     i + 1,
                     ranked[i].first,
  ...
  static thread_local TypeIrCacheState state;
  unordered_map<TypePtr, abi_mangle::Type> & cache =
      mangle_ctx ? state.ir_only_cache : state.default_cache;
  ...
  if(state.diagnostic) {
    if(mangle_ctx) {
      ++state.ir_only_misses;
    } else {
      ++state.default_misses;
    }
  }
  abi_mangle::Type candidate;
    ...
    abi_mangle::Type * captured_type = nullptr)
{
  ...
  return emit_type_ir(ir_type, state, out);
}
    ...
    const TypePtr & actual_type)
{
  ...
  if(pattern_info) {
    ...
    const vector<TemplateParameterInfo> * pattern_parameters =
        semantic_model::class_template_mangle_parameters(*pattern_info);
    if(pattern_parameters) {
      hybrid_info->template_parameters = *pattern_parameters;
    }
    ...
    DependentMangleTypeVisitState & visited)
{
  ...
  case TemplateArgument::TA_ALIAS_TEMPLATE:
    return argument.template_decl == nullptr ||
           type_has_dependent_mangle_state_impl(
               argument.template_owner_type, visited);
  }
    ...
    const vector<TemplateArgument> & arguments)
{
  for(size_t i = 0; i < arguments.size(); ++i) {
    if(arguments[i].kind == TemplateArgument::TA_VALUE &&
       (arguments[i].rare().function_value ||
        arguments[i].rare().value_binding)) {
      return true;
    }
  }
  return false;
  ...
  if(type->kind == Type::TK_NAMED) {
    ...
    if(specialization) {
      const vector<TemplateArgument> * argument_sets[] = {
          &specialization->arguments.const_values(),
                                        ...
                                        const TypeMangleContext * mangle_ctx)
{
  ...
  semantic_model::ClassInfo * class_info =
      type->named_rare().named_class_info;
  if(class_info &&
     (class_info->source_template ||
      type->named_rare().named_member_owner_type)) {
    const string output_name = class_output_qualified_name(*class_info);
    ...
    abi_mangle::SubstitutionKey & out)
{
  ...
  const vector<TemplateParameterInfo> * parameters =
      semantic_model::class_template_mangle_parameters(*info);
  vector<abi_mangle::Type::ClassTemplateArgument> arguments;
    ...
    TypeMangleContext & template_argument_mangle_ctx)
{
  ...
  template_argument_mangle_ctx
      .prefer_concrete_owner_non_type_expression_values =
                                         ...
                                         abi_mangle::FunctionEncoding * captured_function = nullptr)
{
  ...
  if(captured_function) {
    ...
    *captured_function = function;
  } else if(!emit_function_encoding_ir(function, state, candidate)) {
    return false;
  }
  out.swap(candidate);
  return true;
                                                       ...
                                                       string * qualified_name)
{
  ...
  vector<abi_mangle::Type::NameComponent> components;
  components.reserve(parts.size());
  for(size_t i = parts.size(); i > 0; --i) {
    components.push_back(i == parts.size() && parts[i - 1] == "std" ?
        abi_mangle::Type::NameComponent::std_namespace() :
        abi_mangle::Type::NameComponent::source(parts[i - 1], string()));
  }
  MangleSubstitutionState state;
  MangleIrSubstitutionSink sink(&state);
  string candidate;
  const bool has_anonymous_namespace =
      find(parts.begin() + 1, parts.end(), "_GLOBAL__N_1") != parts.end();
  if(!abi_mangle::emit_variable_symbol_from_name_components(
         components,
         linkage == SL_INTERNAL && !has_anonymous_namespace,
         candidate,
         &sink)) {
    return false;
  }
    ...
    vector<const Type *> & active_types)
{
  ...
  case TemplateArgument::TA_VALUE:
    return argument.rare().function_value ||
           argument.rare().value_binding ||
           !argument.rare().value_dependencies.empty();
...
SymbolIdentity make_static_member_variable_symbol_identity(
    ...
    SymbolLinkage linkage)
{
  const QualifiedName owner_syntax =
      class_output_qualified_name_syntax(owner_class);
  const string owner_name =
      owner_syntax.name.empty() ?
          class_internal_output_qualified_name(owner_class) :
          qualified_name_syntax_key_text(owner_syntax);
  const string qualified_name =
      owner_name.empty() ?
          member_name :
    ...
    SymbolLinkage linkage)
{
  ...
  const string qualified_name =
      owner_name.empty() ?
          ...
          owner_name + "::" + internal_member_name;
  if(is_c_linkage) {
