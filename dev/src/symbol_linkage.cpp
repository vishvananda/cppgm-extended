...
static bool try_mangle_type(const TypePtr & type,
                            string & out,
                            const TypeMangleContext * mangle_ctx);
static bool try_mangle_special_type(const TypePtr & type, string & out);
static bool try_mangle_named_text(const string & text, string & out);
...
{
  string mangled;
  if(!try_mangle_special_type(type, mangled)) {
    return string();
  }
...
static string thread_local_wrapper_object_symbol(const string & qualified_name)
{
  ...
  string out = "_ZTW";
  if(!try_mangle_named_text(qualified_name, out)) {
    return string();
  }
  return out;
}
                                         ...
                                         const semantic_model::ClassInfo & base_class)
{
  ...
  if(!try_mangle_type(dynamic_class.type, dynamic_encoding, nullptr) ||
     !try_mangle_type(base_class.type, base_encoding, nullptr)) {
    return string();
  }
...
static vector<string> split_qualified_name(const string & qualified_name)
{
  vector<string> parts;
  size_t start = 0;
  while(start <= qualified_name.size()) {
    const size_t split = qualified_name.find("::", start);
    if(split == string::npos) {
      parts.push_back(qualified_name.substr(start));
      break;
    }
    parts.push_back(qualified_name.substr(start, split - start));
    start = split + 2;
  }
  if(parts.empty()) {
    parts.push_back(qualified_name);
  }
  return parts;
}
...
struct MangleSubstitutionState
{
  ...
  vector<pair<size_t, itanium_mangle_ir::SubstitutionKey> >
      substitution_ir_key_update_log;
...
struct MangleSubstitutionCheckpoint
{
  explicit MangleSubstitutionCheckpoint(MangleSubstitutionState * state)
      : state(state),
        ...
        substitution_ir_key_update_log_size(
            state ? state->substitution_ir_key_update_log.size() : 0),
  ...
  void rollback()
  {
    ...
    for(size_t i = state->substitution_ir_key_update_log.size();
        i > substitution_ir_key_update_log_size;
        --i) {
      const pair<size_t, itanium_mangle_ir::SubstitutionKey> & entry =
          state->substitution_ir_key_update_log[i - 1];
      if(entry.first < state->substitution_ir_keys.size()) {
        state->substitution_ir_keys[entry.first] = entry.second;
      }
    }
    state->substitution_ir_key_update_log.resize(
        substitution_ir_key_update_log_size);
  ...
  size_t substitution_ir_key_update_log_size;
  bool active;
...
static bool try_mangle_named_text_impl(const string & text,
                                       string & out,
                                       const TypeMangleContext * mangle_ctx,
                                       MangleSubstitutionState * state);
...
static bool try_mangle_special_type(const TypePtr & type, string & out)
{
  ...
  return try_mangle_type(type, out, &mangle_ctx);
}
...
static bool try_mangle_type_impl(const TypePtr & type,
                                 string & out,
                                 const TypeMangleContext * mangle_ctx,
                                 MangleSubstitutionState * state);
static bool try_mangle_itanium_function_name_syntax(
    const QualifiedName & qualified,
...
static bool mangle_itanium_function_encoding_with_substitutions(
...
static bool emit_template_component_with_structured_arguments(
    ...
    const TypeMangleContext * mangle_ctx,
    MangleSubstitutionState * state,
    string * canonical_full_text);
static bool emit_component_with_substitution_keys(const string & text,
                                                  const string & canonical_prefix,
                                                  string & out,
                                                  const TypeMangleContext * mangle_ctx,
                                                  MangleSubstitutionState * state,
...
static bool insert_substitution_index(MangleSubstitutionState * state,
                                      const string & key,
                                      size_t index);
static bool try_build_ir_key_from_legacy_substitution_key(
    const string & key,
    itanium_mangle_ir::SubstitutionKey & out);
                                         ...
                                         string & out)
{
  ...
  if(found == state->substitution_index.end()) {
    itanium_mangle_ir::SubstitutionKey ir_key;
    if(try_build_ir_key_from_legacy_substitution_key(key, ir_key)) {
      unordered_map<itanium_mangle_ir::SubstitutionKey,
                    size_t,
                    MangleSubstitutionState::SubstitutionKeyHash>::const_iterator ir_found =
          state->ir_substitution_index.find(ir_key);
      if(ir_found != state->ir_substitution_index.end()) {
        insert_substitution_index(state, key, ir_found->second);
        out += encode_substitution_sequence_id(ir_found->second);
        return true;
      }
    }
    return false;
...
static bool insert_ir_substitution_index(
    ...
    size_t index);
static void assign_substitution_ir_key(
    MangleSubstitutionState * state,
    size_t index,
    const itanium_mangle_ir::SubstitutionKey & key);
    ...
    const itanium_mangle_ir::SubstitutionKey & key)
{
  ...
  if(index >= state->substitution_ir_keys.size()) {
    return;
  }
  if(state->substitution_ir_keys[index].empty()) {
    if(state->checkpoint_depth != 0) {
      state->substitution_ir_key_update_log.push_back(
          make_pair(index, state->substitution_ir_keys[index]));
    }
    state->substitution_ir_keys[index] = key;
  }
  insert_ir_substitution_index(state, key, index);
}
...
substitution_slots_from_state(MangleSubstitutionState * state)
{
  ...
  for(size_t i = 0; i < state->substitution_keys.size(); ++i) {
    if(!state->substitution_keys[i].empty()) {
      slots.push_back(itanium_mangle_ir::SubstitutionSlot::legacy(
          state->substitution_keys[i]));
      continue;
    }
...
static bool find_top_level_separator(const string & text,
                                     char separator,
                                     size_t & out)
{
  int paren_depth = 0;
  ...
  for(size_t i = 0; i < text.size(); ++i) {
    ...
    } else if(ch == separator && paren_depth == 0 && bracket_depth == 0) {
      out = i;
      return true;
    }
  }
...
static bool parenthesized_payload_after_prefix(const string & text,
                                               const string & prefix,
                                               string & out)
{
  ...
  out = text.substr(prefix.size(), text.size() - prefix.size() - 1);
  return true;
}

static bool try_build_ir_type_key_from_legacy_payload(
...
static bool try_build_ir_key_from_legacy_substitution_key(
    ...
    itanium_mangle_ir::SubstitutionKey & out)
{
  static const string name_prefix = "name:";
  static const string type_prefix = "type:";
  static const string template_prefix = "template-prefix:";
  static const string function_template_prefix = "function-template-prefix:";
  if(key.compare(0, name_prefix.size(), name_prefix) == 0 &&
     key.size() > name_prefix.size()) {
    out = itanium_mangle_ir::SubstitutionKey::named(
        key.substr(name_prefix.size()));
    return true;
  }
  if(key.compare(0, template_prefix.size(), template_prefix) == 0 &&
     key.size() > template_prefix.size()) {
    out = itanium_mangle_ir::SubstitutionKey::prefix(
        key.substr(template_prefix.size()));
    return true;
  }
  if(key.compare(0,
                 function_template_prefix.size(),
                 function_template_prefix) == 0 &&
     key.size() > function_template_prefix.size()) {
    out = itanium_mangle_ir::SubstitutionKey::function_template_prefix(
        key.substr(function_template_prefix.size()));
    return true;
  }
  if(key.compare(0, type_prefix.size(), type_prefix) == 0 &&
     key.size() > type_prefix.size()) {
    return try_build_ir_type_key_from_legacy_payload(
        key.substr(type_prefix.size()),
        out);
  }
  return false;
}
...
{
  string inner;
  if(payload == "builtin(v)") {
    out = itanium_mangle_ir::SubstitutionKey::type_builtin("v");
    return true;
  }
  if(parenthesized_payload_after_prefix(payload, "builtin(", inner) &&
     !inner.empty()) {
    out = itanium_mangle_ir::SubstitutionKey::type_builtin(inner);
    return true;
  }
  if(parenthesized_payload_after_prefix(payload, "tparam(", inner)) {
    const string index_prefix = "index:";
    if(inner.compare(0, index_prefix.size(), index_prefix) == 0) {
      size_t end = inner.find(';', index_prefix.size());
      const string index_text = inner.substr(
          index_prefix.size(),
          end == string::npos ? string::npos : end - index_prefix.size());
      if(!index_text.empty() &&
         index_text.find_first_not_of("0123456789") == string::npos) {
        out = itanium_mangle_ir::SubstitutionKey::type_template_parameter(
            static_cast<size_t>(strtoull(index_text.c_str(), nullptr, 10)));
        return true;
      }
    }
    return false;
  }
  if(parenthesized_payload_after_prefix(payload, "ptr(", inner)) {
    itanium_mangle_ir::SubstitutionKey inner_key;
    if(!try_build_ir_key_from_legacy_substitution_key(inner, inner_key)) {
      return false;
    }
    out = itanium_mangle_ir::SubstitutionKey::type_pointer(inner_key);
    return true;
  }
  if(parenthesized_payload_after_prefix(payload, "lref(", inner)) {
    itanium_mangle_ir::SubstitutionKey inner_key;
    if(!try_build_ir_key_from_legacy_substitution_key(inner, inner_key)) {
      return false;
    }
  ...
  if(parenthesized_payload_after_prefix(payload, "rref(", inner)) {
    itanium_mangle_ir::SubstitutionKey inner_key;
    if(!try_build_ir_key_from_legacy_substitution_key(inner, inner_key)) {
      return false;
    }
  ...
  if(parenthesized_payload_after_prefix(payload, "cv(", inner)) {
    size_t comma = string::npos;
    if(!find_top_level_separator(inner, ',', comma)) {
      return false;
    }
    const string cv = inner.substr(0, comma);
    itanium_mangle_ir::SubstitutionKey inner_key;
    if(!try_build_ir_key_from_legacy_substitution_key(
           inner.substr(comma + 1),
           inner_key)) {
      return false;
    }
    out = itanium_mangle_ir::SubstitutionKey::type_cv(
        cv.find('K') != string::npos,
        cv.find('V') != string::npos,
        inner_key);
    return true;
  }
  if(parenthesized_payload_after_prefix(payload, "array(", inner)) {
    size_t comma = string::npos;
    if(!find_top_level_separator(inner, ',', comma)) {
      return false;
    }
    ...
    out = itanium_mangle_ir::SubstitutionKey::type_array(
        inner.substr(0, comma),
        inner_key);
    return true;
  }
  if(parenthesized_payload_after_prefix(payload, "pack-expansion(", inner)) {
    return false;
  }
  out = itanium_mangle_ir::SubstitutionKey::type(payload);
  return !out.empty();
}
...
static void register_ir_substitution_key_without_legacy(
    MangleSubstitutionState * state,
    const itanium_mangle_ir::SubstitutionKey & key)
{
  if(!state || key.empty()) {
                                      ...
                                      const string & key)
{
  if(!state || key.empty() || state->substitution_index.count(key) != 0) {
    if(state && !key.empty()) {
      ...
      if(found != state->substitution_index.end()) {
        itanium_mangle_ir::SubstitutionKey ir_key;
        if(try_build_ir_key_from_legacy_substitution_key(key, ir_key)) {
          assign_substitution_ir_key(state, found->second, ir_key);
        }
      }
    }
    return;
  ...
  if(try_build_ir_key_from_legacy_substitution_key(key, ir_key)) {
    ...
    if(ir_found != state->ir_substitution_index.end()) {
      insert_substitution_index(state, key, ir_found->second);
      assign_substitution_ir_key(state, ir_found->second, ir_key);
      return;
    }
  }
...
static void register_ir_substitution_key(
    ...
    const itanium_mangle_ir::SubstitutionKey & key)
{
  register_ir_substitution_key_without_legacy(state, key);
}

static void register_standard_substitution_slot(MangleSubstitutionState * state,
                                                const string & key)
{
  (void)state;
  (void)key;
    ...
    const itanium_mangle_ir::SubstitutionKey & key)
{
  ...
  if(key.kind == itanium_mangle_ir::SubstitutionKey::SK_LEGACY) {
    return is_template_parameter_substitution_key(key.payload);
  }
  return false;
}
    ...
    const itanium_mangle_ir::SubstitutionKey & ir_key)
{
  ...
  if(!ir_key.empty()) {
    ...
    unordered_map<itanium_mangle_ir::SubstitutionKey,
                  ...
                  MangleSubstitutionState::SubstitutionKeyHash>::const_iterator found =
        state->ir_substitution_index.find(ir_key);
    if(found != state->ir_substitution_index.end() && !key.empty()) {
      unordered_map<string, size_t>::const_iterator string_found =
          state->substitution_index.find(key);
      if(string_found == state->substitution_index.end()) {
        insert_substitution_index(state, key, found->second);
      }
    }
    return;
  }
...
struct MangleIrSubstitutionSink : public itanium_mangle_ir::SubstitutionSink
{
  ...
  {
    if(key.kind == itanium_mangle_ir::SubstitutionKey::SK_LEGACY) {
      return emit_registered_substitution(state, key.legacy_text(), out);
    }
  ...
  {
    if(key.kind == itanium_mangle_ir::SubstitutionKey::SK_LEGACY) {
      register_substitution_key(state, key.legacy_text());
      return;
    }
    ...
    const TypeMangleContext * mangle_ctx)
{
  ...
  const string base_name = trim_space(syntax.name.name);
  if(base_name.empty()) {
    return nullptr;
  }
  ...
  map<string, semantic_model::ClassTemplateDecl *>::const_iterator found =
      scope->class_templates.find(base_name);
    ...
    QualifiedName & out)
{
  ...
  if(syntax_name_matches_template_parameter(syntax.name.name, mangle_ctx)) {
    return false;
  }
  string prefix;
  ...
  out.name = trim_space(syntax.name.name);
...
static bool try_emit_direct_std_standard_substitution(const string & part,
                                                      const string & full_key,
                                                      string & out,
                                                      const TypeMangleContext * mangle_ctx,
                                                      MangleSubstitutionState * state)
{
  TemplateComponent standard_component;
  if(!parse_template_component(part, standard_component) ||
     trim_space(standard_component.base_name).empty()) {
    return false;
  }

  const string base_name = trim_space(standard_component.base_name);
  if(!direct_std_standard_substitution_component_enabled(standard_component, mangle_ctx)) {
    return false;
  }
  if(base_name == "allocator") {
    if(standard_component.has_template_id) {
      return false;
    }
    out += "Sa";
    register_standard_substitution_slot(state, "Sa");
    return true;
  }

  if(template_component_matches_char_stream(standard_component, "basic_istream") ||
     (base_name == "istream" && !standard_component.has_template_id)) {
    out += "Si";
    register_standard_substitution_slot(state, "Si");
    return true;
  }

  if(template_component_matches_char_stream(standard_component, "basic_ostream") ||
     (base_name == "ostream" && !standard_component.has_template_id)) {
    out += "So";
    register_standard_substitution_slot(state, "So");
    return true;
  }

  if(template_component_matches_char_stream(standard_component, "basic_iostream") ||
     (base_name == "iostream" && !standard_component.has_template_id)) {
    out += "Sd";
    register_standard_substitution_slot(state, "Sd");
    return true;
  }

  if(base_name == "basic_string" &&
     standard_component.arg_texts.size() == 3 &&
     is_char_type_text(standard_component.arg_texts[0]) &&
     is_std_char_traits_char_text(standard_component.arg_texts[1]) &&
     is_std_allocator_char_text(standard_component.arg_texts[2])) {
    out += "Ss";
    register_standard_substitution_slot(state, "Ss");
    return true;
  }
...
static bool can_emit_direct_std_standard_substitution(const vector<string> & parts,
                                                      const vector<string> & canonical_parts,
                                                      const TypeMangleContext * mangle_ctx)
{
  if(parts.size() != 2 || canonical_parts.size() != 2 || canonical_parts[0] != "std") {
    return false;
  }
...
static bool structured_template_arguments_are_simple_parameter_refs(
    const vector<TemplateArgument> & arguments,
    const vector<TemplateParameterInfo> * parameters)
{
  if(!parameters || parameters->empty() || arguments.empty()) {
    return false;
  }
  ...
  for(size_t i = 0; i < parameters->size(); ++i) {
    ...
    if(parameter.parameter_pack) {
      ...
      const TemplateArgument & argument = arguments[arg_index++];
      const string argument_text = trim_elaborated_type_prefix(argument.text);
      if(parameter.name.empty() ||
    ...
    if(arg_index >= arguments.size() ||
       !template_argument_is_self_type_parameter(arguments[arg_index],
                                                parameter,
                                                TypePtr(),
                                                parameter.name)) {
      return false;
...
static void throw_legacy_dependent_text_mangling(const string & kind,
                                                 const string & text)
{
  throw logic_error("legacy dependent " + kind +
...
static string qualified_name_syntax_key_text(const QualifiedName & name)
{
  ...
  for(size_t i = 0; i < name.qualifiers.size(); ++i) {
    ...
    out += trim_space(name.qualifiers[i]);
  }
  if(!out.empty() &&
     (out.size() < 2 || out.compare(out.size() - 2, 2, "::") != 0)) {
    out += "::";
  }
  out += trim_space(name.name);
  return out;
}
    ...
    const QualifiedName & qualified)
{
  ...
  template_name.name = trim_space(syntax.name.name);
...
static bool emit_qualified_prefix_text_components(
    const string & prefix,
    string & out,
    const TypeMangleContext * mangle_ctx,
    MangleSubstitutionState * state,
    string & canonical_prefix)
{
  ...
  vector<string> prefix_parts = qualified_prefix.qualifiers;
  prefix_parts.push_back(qualified_prefix.name);
  vector<string> canonical_parts;
  canonical_parts.reserve(prefix_parts.size());
  for(size_t i = 0; i < prefix_parts.size(); ++i) {
    canonical_parts.push_back(canonical_component_text(prefix_parts[i]));
  }
  if(prefix_parts.size() == 1 &&
     canonical_parts[0] == "std") {
    out += "St";
    register_standard_substitution_slot(state, "St");
    canonical_prefix = "std";
    return true;
  }

  size_t start = 0;
  for(size_t len = prefix_parts.size(); len > 0; --len) {
    const string prefix_key =
        named_substitution_key(join_canonical_qualified_parts(canonical_parts, len));
    if(emit_registered_substitution(state, prefix_key, out)) {
      canonical_prefix = join_canonical_qualified_parts(canonical_parts, len);
      start = len;
      break;
    }
  }
  if(start == 0 && !prefix_parts.empty() && canonical_parts[0] == "std") {
    out += "St";
    register_standard_substitution_slot(state, "St");
    canonical_prefix = "std";
    start = 1;
  }
  for(size_t i = start; i < prefix_parts.size(); ++i) {
    string canonical_full_text;
    if(!emit_component_with_substitution_keys(prefix_parts[i],
                                              canonical_prefix,
                                              out,
                                              mangle_ctx,
                                              state,
    ...
    canonical_prefix = canonical_full_text;
  }
  return true;
}

static bool emit_structured_class_scope_prefix(
...
static bool emit_template_decl_scope_prefix_components(
    const semantic_model::Scope * scope,
    string & out,
    const TypeMangleContext * mangle_ctx,
    MangleSubstitutionState * state,
    string & canonical_prefix)
{
  canonical_prefix.clear();
  string prefix;
  scope_prefix_text_for_template_decl(scope, prefix);
  if(prefix.empty()) {
    return true;
  }

  const semantic_model::ClassInfo * owner = nearest_member_class_scope(scope);
  if(owner) {
    const size_t begin = out.size();
    MangleSubstitutionCheckpoint checkpoint(state);
    if(emit_structured_class_scope_prefix(
           *owner, out, mangle_ctx, state, canonical_prefix)) {
      checkpoint.commit();
      return true;
    }
    out.resize(begin);
  ...
  return emit_qualified_prefix_text_components(prefix,
                                               out,
                                               mangle_ctx,
                                               state,
                                               canonical_prefix);
    ...
    string & canonical_prefix)
{
  ...
  if(!specialization || !info.source_template) {
    return false;
  }
  ...
  if(!class_template || class_template != info.source_template) {
    return false;
  }

  if(!emit_template_decl_scope_prefix_components(
  ...
  if(!specialization->pack_sizes.empty() ||
    ...
    if(!specialization->pack_sizes.empty()) {
      specialization_ctx_storage.template_argument_pack_sizes =
          &specialization->pack_sizes;
    }
    specialization_ctx_storage.prefer_concrete_non_type_values_for_dependent_parameter_types = true;
    specialization_ctx = &specialization_ctx_storage;
  }

  string canonical_full_text;
  if(!emit_template_component_with_structured_arguments(
         class_template->name,
  ...
  if(!canonical_full_text.empty()) {
    string output_name;
    if(canonicalize_named_substitution_text(class_output_qualified_name(info),
                                            output_name)) {
      canonical_full_text = output_name;
      register_substitution_key(state, named_substitution_key(canonical_full_text));
    }
  }
  canonical_prefix = canonical_full_text;
  return !canonical_prefix.empty();
}
    ...
    const TypeMangleContext * mangle_ctx)
{
  ...
  for(map<string, semantic_model::ClassInfo *>::const_iterator it =
    ...
    if(try_mangle_type_impl(it->second->type,
                            candidate_mangle,
                            mangle_ctx,
                            &local_state) &&
                                                 ...
                                                 string & out)
{
  ...
  if(final_template_id) {
    ...
    if(qualified && !qualified->qualifiers.empty()) {
      ...
      template_name.rooted = qualified->rooted;
      template_name.qualifiers = qualified->qualifiers;
      template_name.name = trim_space(final_template_id->name.name);
                                                  ...
                                                  string * canonical_full_text)
{
  ...
  const string canonical_base = trim_space(component.base_name);
  const string full_base_text =
      append_qualified_component_text(canonical_prefix, canonical_base);
  const string base_key = named_substitution_key(full_base_text);
  if(canonical_base.find("__anonymous_enum") == 0) {
    out += "Ut_";
  } else {
    out += source_name_fragment(component.base_name);
  }
  const string full_text =
      append_qualified_component_text(canonical_prefix, canonical_component_text(text));
  register_substitution_key(state, named_substitution_key(full_text));
  if(canonical_full_text) {
...
static bool should_prefer_unqualified_lexical_named_type(
    const TypePtr & type,
    const TypeMangleContext * mangle_ctx);
static bool structural_substitution_key_context_is_stable(
    const TypeMangleContext * mangle_ctx);
...
{
  return mangle_ctx &&
         mangle_ctx->suppress_legacy_type_substitution_keys &&
         !mangle_ctx->template_parameters &&
         !mangle_ctx->owner_template_parameters &&
         !mangle_ctx->owner_template_arguments &&
...
static bool attach_member_type_ir_substitution(
    itanium_mangle_ir::Type & ir_type,
    const string & legacy_text)
{
  string flat_text = trim_elaborated_type_prefix(legacy_text);
    ...
    bool & address_of)
{
  ...
  if(argument.function_value && base) {
    ...
    if(is_function_pointer || is_function_reference) {
      symbol = argument.function_value->symbol.object_symbol;
      address_of = is_function_pointer;
      return !symbol.empty();
    }
  }
  ...
  if(argument.value_binding && base) {
    ...
    if(is_object_reference) {
      symbol = argument.value_binding->symbol.object_symbol;
      return !symbol.empty();
    }
  }
...
static bool try_build_dependent_expression_ir(
    ...
    const TypeMangleContext * mangle_ctx,
    itanium_mangle_ir::DependentExpression & out);
static bool try_build_dependent_expression_text_ir(
    const string & text,
    const TypeMangleContext * mangle_ctx,
    itanium_mangle_ir::DependentExpression & out);
static bool try_build_template_argument_syntax_ir(
...
static bool try_build_class_template_argument_ir(
    ...
    const TypeMangleContext * mangle_ctx,
    itanium_mangle_ir::Type::ClassTemplateArgument & out);
static bool try_mangle_itanium_static_member_object_name(
    ...
    bool register_substitution)
{
  ...
  if(register_substitution) {
    itanium_mangle_ir::set_substitution(
        out,
        itanium_mangle_ir::SubstitutionKey::type_template_parameter(index));
  }
  return out;
}
    ...
    itanium_mangle_ir::Type & out)
{
  ...
  const string display_text =
      trim_elaborated_type_prefix(type->named_display);
  const string key_text = trim_elaborated_type_prefix(type->named_key);
  return try_build_template_parameter_type_text_ir(display_text,
                                                   mangle_ctx,
                                                   true,
                                                   out) ||
         try_build_template_parameter_type_text_ir(key_text,
                                                   mangle_ctx,
                                                   true,
                                                   out);
}
    ...
    itanium_mangle_ir::DependentExpression & out)
{
  ...
  if(found->second.owner_class &&
    ...
    string symbol;
    if(try_mangle_itanium_static_member_object_name(*info,
                                                    qualified->name,
                                                    symbol)) {
      out = itanium_mangle_ir::DependentExpression::external_entity(symbol,
                                                                    false);
...
static bool try_build_resolved_constant_member_expression_text_ir(
    const string & raw_text,
    const TypeMangleContext * mangle_ctx,
    itanium_mangle_ir::DependentExpression & out)
{
  QualifiedName qualified;
  if(!semantic_utils::split_qualified_name_text(trim_space(raw_text), qualified) ||
     qualified.qualifiers.empty() ||
     qualified.name.empty()) {
    return false;
  }
  ...
  TemplateIdSyntax qualifier_template_id;
  size_t template_qualifier_index = string::npos;
  for(size_t i = qualified.qualifiers.size(); i > 0; --i) {
    if(template_id_syntax_from_component_text(qualified.qualifiers[i - 1],
                                              qualifier_template_id)) {
      template_qualifier_index = i - 1;
      break;
    }
...
static bool ir_type_has_template_parameter(
    const itanium_mangle_ir::Type & type);

static bool ir_expression_has_template_parameter(
    const itanium_mangle_ir::DependentExpression & expression)
{
  if(expression.kind ==
     itanium_mangle_ir::DependentExpression::EK_TEMPLATE_PARAMETER) {
    return true;
  }
  if(expression.owner_type &&
     ir_type_has_template_parameter(*expression.owner_type)) {
    return true;
  }
  if(expression.inner &&
     ir_expression_has_template_parameter(*expression.inner)) {
    return true;
  }
  for(size_t i = 0; i < expression.arguments.size(); ++i) {
    if(ir_expression_has_template_parameter(expression.arguments[i])) {
      return true;
    }
  }
  for(size_t i = 0; i < expression.type_arguments.size(); ++i) {
    if(ir_type_has_template_parameter(expression.type_arguments[i])) {
      return true;
    }
  }
  return false;
...
static bool ir_class_template_argument_has_template_parameter(
    const itanium_mangle_ir::Type::ClassTemplateArgument & argument)
{
  if(argument.type && ir_type_has_template_parameter(*argument.type)) {
    return true;
  }
  if(argument.parameter_type &&
     ir_type_has_template_parameter(*argument.parameter_type)) {
    return true;
  }
  if(argument.expression &&
     ir_expression_has_template_parameter(*argument.expression)) {
    return true;
  }
  if(argument.metadata) {
    for(size_t i = 0; i < argument.metadata->pack_arguments.size(); ++i) {
      if(ir_class_template_argument_has_template_parameter(
...
static bool ir_type_has_template_parameter(const itanium_mangle_ir::Type & type)
{
  if(type.kind == itanium_mangle_ir::Type::TK_TEMPLATE_PARAMETER) {
    return true;
  }
  if(type.inner && ir_type_has_template_parameter(*type.inner)) {
    return true;
  }
  if(type.owner && ir_type_has_template_parameter(*type.owner)) {
    return true;
  }
  if(type.name_owner && ir_type_has_template_parameter(*type.name_owner)) {
    return true;
  }
  if(type.expression &&
     ir_expression_has_template_parameter(*type.expression)) {
    return true;
  }
  for(size_t i = 0; i < type.params.size(); ++i) {
    if(ir_type_has_template_parameter(type.params[i])) {
      return true;
    }
  }
  if(type.name) {
    for(size_t i = 0; i < type.name->template_arguments.size(); ++i) {
      if(ir_class_template_argument_has_template_parameter(
             type.name->template_arguments[i])) {
        return true;
      }
    }
  }
...
static bool try_evaluate_sizeof_ir_type(
    const itanium_mangle_ir::Type & type,
    long long & out)
{
  if(ir_type_has_template_parameter(type)) {
    return false;
  }

  switch(type.kind) {
  case itanium_mangle_ir::Type::TK_BUILTIN: {
    const string code(type.builtin_code);
    if(code == "a" || code == "b" || code == "c" || code == "h") {
      out = 1;
      return true;
    }
    if(code == "s" || code == "t" || code == "Ds") {
      out = 2;
      return true;
    }
    if(code == "f" || code == "i" || code == "j" || code == "w" ||
       code == "Di") {
      out = 4;
      return true;
    }
    if(code == "d" || code == "l" || code == "m" || code == "x" ||
       code == "y" || code == "Dn") {
      out = 8;
      return true;
    }
  ...
  case itanium_mangle_ir::Type::TK_CV:
    return type.inner && try_evaluate_sizeof_ir_type(*type.inner, out);
  ...
  case itanium_mangle_ir::Type::TK_ARRAY: {
    if(!type.inner || !is_signed_decimal_integer_text(type.array_bound)) {
      return false;
    }
    long long element_size = 0;
    if(!try_evaluate_sizeof_ir_type(*type.inner, element_size)) {
      return false;
    }
    out = parse_signed_decimal_integer_value(type.array_bound) * element_size;
    return true;
  }

  case itanium_mangle_ir::Type::TK_INVALID:
  case itanium_mangle_ir::Type::TK_FUNCTION:
  case itanium_mangle_ir::Type::TK_MEMBER_POINTER:
  case itanium_mangle_ir::Type::TK_BUILTIN_TYPE_TRANSFORM:
  case itanium_mangle_ir::Type::TK_PACK_EXPANSION:
  case itanium_mangle_ir::Type::TK_TEMPLATE_PARAMETER:
  case itanium_mangle_ir::Type::TK_NAMED:
  case itanium_mangle_ir::Type::TK_CLASS_TEMPLATE_SPECIALIZATION:
  case itanium_mangle_ir::Type::TK_DECLTYPE_EXPRESSION:
  case itanium_mangle_ir::Type::TK_LAMBDA_CLOSURE:
    return false;
  }
...
static bool try_build_sizeof_type_expression_text_ir(
    const string & raw_text,
    const TypeMangleContext * mangle_ctx,
    itanium_mangle_ir::DependentExpression & out)
{
  const string text = trim_space(raw_text);
  static const char prefix[] = "sizeof";
  if(text.compare(0, sizeof(prefix) - 1, prefix) != 0) {
    return false;
  }
  string payload = trim_space(text.substr(sizeof(prefix) - 1));
  if(payload.size() < 2 ||
     payload[0] != '(' ||
     payload[payload.size() - 1] != ')' ||
     !semantic_utils::is_wrapped_in_balanced_parens(payload)) {
    return false;
  }
  payload = trim_space(payload.substr(1, payload.size() - 2));
  if(payload.empty()) {
    return false;
  }

  itanium_mangle_ir::Type operand_type;
  if(!try_build_direct_type_syntax_text_ir(payload, mangle_ctx, operand_type)) {
    return false;
  }

  long long value = 0;
  if(try_evaluate_sizeof_ir_type(operand_type, value)) {
    out = itanium_mangle_ir::DependentExpression::typed_integral_value(
  ...
  out = itanium_mangle_ir::DependentExpression::sizeof_type(operand_type);
  return true;
}
                              ...
                              vector<TemplateParameterInfo> & explicit_parameter_storage)
{
  ...
  explicit_parameter_storage.assign(
      parameters->begin(),
      parameters->begin() + syntax.argument_syntaxes.size());
                                          ...
                                          bool suppress_current_pack_grouping)
{
  ...
  if(class_template) {
    ...
    if(member_owner && member_owner->type && owner_info) {
      ...
      if(have_owner) {
        ...
        itanium_mangle_ir::set_substitution(
            ir_type,
            itanium_mangle_ir::SubstitutionKey::legacy(
                named_substitution_key(
                    dependent_class_template_name_key_text(
                        prefix,
                        base_name,
                        dependent_arguments))));
        out = ir_type;
  ...
  vector<TemplateArgument> legacy_arguments =
  ...
  itanium_mangle_ir::set_substitution(
      ir_type,
      itanium_mangle_ir::SubstitutionKey::legacy(
          named_substitution_key(dependent_class_template_name_key_text(
              canonical_prefix,
              base_name,
              dependent_arguments))));
  out = ir_type;
    ...
    vector<itanium_mangle_ir::Type::ClassTemplateArgument> & out)
{
  ...
  const vector<TemplateParameterInfo> * raw_parameters =
      lookup_template_parameters_for_template_id_syntax(syntax, mangle_ctx);
    ...
    bool register_template_parameter_substitution)
{
  ...
  for(size_t i = 0; i < qualified.qualifiers.size(); ++i) {
    ...
    if(has_current && qualifier_template_id) {
    ...
    bool register_template_parameter_substitution)
{
  ...
  if(semantic_utils::split_qualified_name_text(text, qualified) &&
     !qualified.qualifiers.empty()) {
    const bool mentions_direct_template_parameter =
        text_mentions_direct_template_mangle_parameter(text, mangle_ctx);
    TypePtr lookup_type = mentions_direct_template_parameter ?
        TypePtr() :
    ...
    if(mentions_direct_template_parameter) {
  ...
  if(is_identifier_text_for_mangling(text) &&
    ...
    if(lookup_type && try_build_type_ir(lookup_type, mangle_ctx, out)) {
      itanium_mangle_ir::add_preregister_legacy_key(
          out,
          named_substitution_key(canonical_component_text(text)));
      return true;
    }
  }
    ...
    itanium_mangle_ir::Type & out)
{
  ...
  for(size_t i = 0; i < members.size(); ++i) {
    ...
    if(i < member_template_ids.size() &&
      ...
      if(!try_build_template_id_type_ir(member_template_ids[i],
                                        mangle_ctx,
                                        member_template)) {
        return false;
      }
    ...
    vector<itanium_mangle_ir::SubstitutionSlot> & substitution_slots)
{
  ...
  if(!mangle_itanium_function_encoding_with_substitutions(
    ...
    itanium_mangle_ir::Type & out)
{
  ...
  const string source_name =
      lambda_context_uses_clang_local_source_name(*context_options) ?
    ...
    itanium_mangle_ir::Type::ClassTemplateArgument & out)
{
  ...
  if(known_non_type_parameter &&
     ...
     !syntax.template_id) {
    itanium_mangle_ir::DependentExpression expression;
    if(try_build_dependent_expression_text_ir(non_type_expression_text,
                                              mangle_ctx,
                                              expression)) {
      if(syntax.pack_expansion) {
        expression =
...
static bool split_top_level_dependent_binary_expression_text(const string & text,
                                                            string & lhs,
                                                            string & op,
                                                            string & rhs)
{
  ...
  static const OperatorCandidate operators[] = {
    {"||"},
    {"&&"},
    {"=="},
    {"!="},
  ...
  for(size_t op_index = 0;
      op_index < sizeof(operators) / sizeof(operators[0]);
      ++op_index) {
    const string current_op = operators[op_index].text;
    int angle_depth = 0;
    int paren_depth = 0;
    int bracket_depth = 0;
    int brace_depth = 0;
    for(size_t i = 0; i + current_op.size() <= text.size(); ++i) {
      const char ch = text[i];
      if(angle_depth == 0 &&
         paren_depth == 0 &&
         bracket_depth == 0 &&
         brace_depth == 0 &&
         text.compare(i, current_op.size(), current_op) == 0 &&
         (current_op[0] != '<' ||
          relational_less_operator_at_for_mangling(text, i))) {
        lhs = trim_space(text.substr(0, i));
        op = current_op;
        rhs = trim_space(text.substr(i + current_op.size()));
    ...
    itanium_mangle_ir::DependentExpression & out)
{
  ...
  if(node.kind == CppAstKind::id_expression ||
    ...
    if(!node.value.empty() &&
       try_build_dependent_expression_text_ir(node.value, mangle_ctx, out)) {
      return true;
    }
    ...
    itanium_mangle_ir::DependentExpression & out)
{
  ...
  if(text.size() >= 2 &&
     text[0] == '(' &&
     text[text.size() - 1] == ')' &&
     semantic_utils::is_wrapped_in_balanced_parens(text)) {
    return try_build_dependent_expression_text_ir(
  ...
  string op;
  string rhs;
  if(split_top_level_dependent_binary_expression_text(text, lhs, op, rhs)) {
    string op_code;
    itanium_mangle_ir::DependentExpression left;
    itanium_mangle_ir::DependentExpression right;
    if(!dependent_binary_operator_mangle_code(op, op_code) ||
       !try_build_dependent_expression_text_ir(lhs, mangle_ctx, left) ||
       !try_build_dependent_expression_text_ir(rhs, mangle_ctx, right)) {
      return false;
    }
    out = itanium_mangle_ir::DependentExpression::binary(op_code, left, right);
    return true;
  }
  {
    ...
    if(text.size() > prefix.size() &&
      ...
      const string pack_name =
          trim_space(text.substr(prefix.size(),
                                 text.size() - prefix.size() - 1));
      if(!pack_name.empty() &&
         try_build_sizeof_pack_expression_ir(pack_name, mangle_ctx, out)) {
        return true;
      }
    }
  }
  if(is_signed_decimal_integer_text(text)) {
    out = itanium_mangle_ir::DependentExpression::literal(text);
    return true;
  }
  if(try_build_sizeof_type_expression_text_ir(text, mangle_ctx, out)) {
    return true;
  }
  if(try_build_template_parameter_value_expression_ir(text, mangle_ctx, out)) {
    return true;
  }
  if(text.compare(0, 2, "__") == 0 &&
     text.size() > 3 &&
     text[text.size() - 1] == ')') {
    const size_t open = text.find('(');
    if(open != string::npos && open > 0) {
      const string trait_name = trim_space(text.substr(0, open));
      vector<string> argument_texts;
      if(!trait_name.empty() &&
         split_template_arguments(text.substr(open + 1,
                                              text.size() - open - 2),
                                  argument_texts)) {
        vector<itanium_mangle_ir::Type> type_arguments;
        type_arguments.reserve(argument_texts.size());
        bool ok = true;
        for(size_t i = 0; i < argument_texts.size(); ++i) {
          itanium_mangle_ir::Type type_argument;
          if(!try_build_direct_type_syntax_text_ir(argument_texts[i],
                                                   mangle_ctx,
                                                   type_argument)) {
            ok = false;
            break;
        ...
        if(ok) {
          out = itanium_mangle_ir::DependentExpression::type_trait(
              trait_name,
              type_arguments);
          return true;
        }
  ...
  QualifiedName qualified;
  if(try_build_resolved_constant_member_expression_text_ir(text,
                                                          mangle_ctx,
                                                          out)) {
    return true;
  }
  if(semantic_utils::split_qualified_name_text(text, qualified) &&
    ...
    bool close_owner = false;
    vector<TemplateIdSyntax> qualifier_template_id_syntaxes =
        qualifier_template_id_syntaxes_from_text(qualified);
    if(!try_build_qualified_owner_type_ir(qualified,
                                          &qualifier_template_id_syntaxes,
                                          mangle_ctx,
                                          owner,
    ...
    out = itanium_mangle_ir::DependentExpression::member(
        owner,
        close_owner,
        qualified.name);
    return true;
  ...
  if(template_id_syntax_from_component_text(text, template_id)) {
    ...
    vector<itanium_mangle_ir::Type::ClassTemplateArgument> class_arguments;
    if(!build_dependent_template_arguments_ir(
           dependent_arguments,
           nullptr,
           mangle_ctx,
           class_arguments)) {
    ...
    itanium_mangle_ir::TemplateArgument & out)
{
  ...
  if(parser_trace::enabled("symbol.linkage")) {
    ostringstream trace;
    trace << "function-dependent-integral-check"
          << " has-parameter=" << (parameter ? "yes" : "no")
          << " parameter=" << (parameter ? parameter->name : string())
          << " prefer=" << (prefer_concrete ? "yes" : "no")
          << " dependent=" << (dependent_parameter ? "yes" : "no")
          << " argument-dependent=" << (argument.dependent ? "yes" : "no")
          << " text=" << argument.text;
    parser_trace::note("symbol.linkage", string(), trace.str());
  }
  ...
  if(parser_trace::enabled("symbol.linkage")) {
    const itanium_mangle_ir::Type::NameMetadata * parameter_name =
        parameter_type.name.get();
    const itanium_mangle_ir::Type::NameMetadata * owner_name =
        parameter_type.name_owner ? parameter_type.name_owner->name.get() :
                                    nullptr;
    ostringstream trace;
    trace << "function-dependent-integral-ir parameter-type-kind="
          << static_cast<int>(parameter_type.kind)
          << " parameter=" << parameter->name
          << " template-name=" << (parameter_name ?
                                    parameter_name->template_name :
                                    string())
          << " has-owner=" << (parameter_type.name_owner ? "yes" : "no")
          << " owner-kind="
          << (parameter_type.name_owner ?
              to_string(static_cast<int>(parameter_type.name_owner->kind)) :
              string("none"))
          << " owner-template="
          << (owner_name ? owner_name->template_name : string())
          << " owner-std="
          << (owner_name ? owner_name->standard_substitution : string())
          << " owner-args="
          << (owner_name ?
              to_string(owner_name->template_arguments.size()) :
              string("0"));
    if(owner_name) {
      for(size_t trace_i = 0;
          trace_i < owner_name->template_arguments.size();
          ++trace_i) {
        trace << " arg" << trace_i << "="
              << static_cast<int>(
                     owner_name->template_arguments[trace_i].kind);
        if(owner_name->template_arguments[trace_i].expression) {
          trace << "/expr"
                << static_cast<int>(
                       owner_name->template_arguments[trace_i]
                           .expression->kind);
        }
        if(owner_name->template_arguments[trace_i].type) {
          trace << "/type"
                << static_cast<int>(
                       owner_name->template_arguments[trace_i]
                           .type->kind);
    ...
    itanium_mangle_ir::Type::ClassTemplateArgument & out)
{
  ...
  case TemplateArgument::TA_VALUE: {
    ...
    bool entity_address_of = false;
    if(try_build_external_entity_argument_ir_payload(argument,
                                                    entity_symbol,
                                                    entity_address_of)) {
      out = itanium_mangle_ir::Type::ClassTemplateArgument::external_entity_arg(
          entity_symbol,
          entity_address_of);
      return true;
    }
    ...
    itanium_mangle_ir::Type & out)
{
  ...
  if(!specialization->pack_sizes.empty() ||
    ...
    specialization_ctx_storage = mangle_ctx ? *mangle_ctx : TypeMangleContext();
    specialization_ctx_storage.template_argument_pack_sizes =
        specialization->pack_sizes.empty() ? nullptr : &specialization->pack_sizes;
  ...
  if(has_typed_member_owner) {
    ...
    if(!build_type_substitution_key(base, specialization_ctx, legacy_key) ||
       legacy_key.empty()) {
      return false;
    }
    if(!build_class_template_specialization_structural_key(base,
                                                           specialization_ctx,
                                                           structural_key) &&
       !try_build_ir_key_from_legacy_substitution_key(legacy_key,
                                                      structural_key)) {
      structural_key = itanium_mangle_ir::SubstitutionKey::legacy(legacy_key);
    }
    itanium_mangle_ir::set_substitution(ir_type, structural_key);
    out = ir_type;
    return true;
  ...
  if(!standard_substitution_includes_arguments) {
    string legacy_key;
    itanium_mangle_ir::SubstitutionKey structural_key;
    if(!build_type_substitution_key(base, mangle_ctx, legacy_key) ||
       legacy_key.empty()) {
      return false;
    }
    if(!build_class_template_specialization_structural_key(base,
    ...
    itanium_mangle_ir::Type & out)
{
  ...
  static thread_local unordered_map<TypePtr, itanium_mangle_ir::Type> legacy_cache;
  ...
  unordered_map<TypePtr, itanium_mangle_ir::Type> & cache =
      mangle_ctx ? ir_only_cache : legacy_cache;
...
{
  const string stripped = trim_elaborated_type_prefix(text);
  QualifiedName unqualified_candidate;
  const bool prefer_local_component_emission =
      mangle_ctx &&
      !mangle_ctx->lexical_scope.empty() &&
      lexical_scope_supports_local_component_emission(mangle_ctx->lexical_scope) &&
      parse_external_named_text_candidate(stripped, unqualified_candidate) &&
      unqualified_candidate.qualifiers.empty();

  QualifiedName qualified;
  const string scoped_text = qualify_named_text_with_lexical_scope(text, mangle_ctx);
  if(!parse_external_named_text_candidate(scoped_text, qualified)) {
    return false;
  }
  ...
  const string full_key =
      named_substitution_key(join_canonical_qualified_parts(canonical_parts, canonical_parts.size()));
  if(!prefer_local_component_emission &&
     emit_registered_substitution(state, full_key, out)) {
    return true;
  }

  if(prefer_local_component_emission) {
    string canonical_prefix;
    if(!canonicalize_named_substitution_text(mangle_ctx->lexical_scope, canonical_prefix)) {
      canonical_prefix = mangle_ctx->lexical_scope;
    }
    string canonical_full_text;
    return emit_component_with_substitution_keys(stripped,
                                                 canonical_prefix,
                                                 out,
                                                 mangle_ctx,
                                                 state,
  ...
  if(parts.size() == 1) {
    string canonical_full_text;
    return emit_component_with_substitution_keys(parts[0],
                                                 string(),
                                                 out,
                                                 mangle_ctx,
                                                 state,
  ...
  if(can_emit_direct_std_standard_substitution(parts, canonical_parts, mangle_ctx) &&
     try_emit_direct_std_standard_substitution(parts[1], full_key, out, mangle_ctx, state)) {
    return true;
  }

  if(parts.size() == 2 && canonical_parts[0] == "std") {
    out += "St";
    register_standard_substitution_slot(state, "St");
    string canonical_full_text;
    return emit_component_with_substitution_keys(parts[1],
                                                 "std",
                                                 out,
                                                 mangle_ctx,
                                                 state,
  ...
  string canonical_prefix;
  size_t start = 0;
  for(size_t len = parts.size() - 1; len > 0; --len) {
    const string prefix_key =
        named_substitution_key(join_canonical_qualified_parts(canonical_parts, len));
    if(emit_registered_substitution(state, prefix_key, out)) {
      canonical_prefix = join_canonical_qualified_parts(canonical_parts, len);
  ...
  if(start == 0) {
    if(canonical_parts.size() >= 2 && canonical_parts[0] == "std" &&
       canonical_parts[1] == "__1") {
      out += "St3__1";
      register_standard_substitution_slot(state, "St");
      canonical_prefix = "std::__1";
      register_substitution_key(state, named_substitution_key(canonical_prefix));
      start = 2;
    } else if(canonical_parts.size() >= 2 &&
              can_emit_direct_std_standard_substitution(vector<string>(parts.begin(),
                                                                      parts.begin() + 2),
                                                        vector<string>(canonical_parts.begin(),
                                                                       canonical_parts.begin() + 2),
                                                        mangle_ctx) &&
              try_emit_direct_std_standard_substitution(
                  parts[1],
                  named_substitution_key(join_canonical_qualified_parts(canonical_parts, 2)),
                  out,
                  mangle_ctx,
                  state)) {
      canonical_prefix = join_canonical_qualified_parts(canonical_parts, 2);
      start = 2;
    } else if(canonical_parts[0] == "std") {
      out += "St";
      canonical_prefix = "std";
      register_standard_substitution_slot(state, "St");
      start = 1;
  ...
  for(size_t i = start; i < parts.size(); ++i) {
    string canonical_full_text;
    if(!emit_component_with_substitution_keys(parts[i],
                                              canonical_prefix,
                                              out,
                                              mangle_ctx,
                                              state,
...
{
  MangleSubstitutionState state;
  return try_mangle_named_text_impl(text, out, nullptr, &state);
}
                                                    ...
                                                    MangleSubstitutionState * state)
{
  ...
  out += 'L';
  if(base && try_mangle_type_impl(base, out, nullptr, state)) {
    out += to_string(value);
    out += 'E';
    return true;
  }
    ...
    size_t parameter_index)
{
  ...
  if(!try_mangle_non_type_template_parameter_type(
    ...
    throw_legacy_dependent_text_mangling(
...
{
  MangleSubstitutionState state;
  return try_mangle_type_impl(type, out, mangle_ctx, &state);
}
...
static bool build_hybrid_named_pattern_text(const TypePtr & pattern_type,
                                            const TypePtr & actual_type,
                                            const TypeMangleContext * mangle_ctx,
                                            string & out)
{
  out.clear();
  if(!pattern_type || pattern_type->kind != Type::TK_NAMED ||
     !actual_type || actual_type->kind != Type::TK_NAMED) {
    return false;
  }

  const string scoped_pattern_text =
      qualify_named_text_with_lexical_scope(selected_named_type_text(pattern_type), mangle_ctx);
  const string actual_text = selected_named_type_text(actual_type);
  if(scoped_pattern_text.empty() || actual_text.empty()) {
    return false;
  }

  QualifiedName pattern_qualified;
  QualifiedName actual_qualified;
  if(!parse_external_named_text_candidate(scoped_pattern_text, pattern_qualified) ||
     !parse_external_named_text_candidate(actual_text, actual_qualified)) {
    return false;
  }

  TemplateComponent pattern_component;
  TemplateComponent actual_component;
  if(!parse_template_component(pattern_qualified.name, pattern_component) ||
     !parse_template_component(actual_qualified.name, actual_component) ||
     trim_space(pattern_component.base_name).empty() ||
     trim_space(actual_component.base_name).empty() ||
     trim_space(pattern_component.base_name) != trim_space(actual_component.base_name) ||
     pattern_component.has_template_id != actual_component.has_template_id ||
     pattern_component.arg_texts.size() != actual_component.arg_texts.size()) {
    return false;
  }

  string final_component = trim_space(actual_component.base_name);
  if(pattern_component.has_template_id) {
    final_component += '<';
    for(size_t i = 0; i < pattern_component.arg_texts.size(); ++i) {
      if(i != 0) {
        final_component += ", ";
      }
      final_component += trim_space(pattern_component.arg_texts[i]);
    }
    final_component += '>';
  }

  vector<string> parts = actual_qualified.qualifiers;
  parts.push_back(final_component);
  out = join_canonical_qualified_parts(parts, parts.size());
  return !out.empty();
}
    ...
    const TypePtr & actual_type)
{
  ...
  vector<DependentAliasTemplateArgumentSyntax> dependent_arguments;
  if(!named_type_dependent_class_template(hybrid, pattern_decl, dependent_arguments)) {
    return;
  }
  ...
  if(!actual_info->class_template_decl) {
    return;
  }
  ...
  case Type::TK_NAMED: {
    ...
    if(actual_type &&
       ...
       !text_mentions_direct_template_mangle_parameter(
           selected_named_type_text(pattern_type), mangle_ctx)) {
      return actual_type;
    }

    string hybrid_text;
    if(build_hybrid_named_pattern_text(pattern_type, actual_type, mangle_ctx, hybrid_text)) {
      TypePtr hybrid(new Type(*pattern_type));
      hybrid->named_display = hybrid_text;
      hybrid->named_key = hybrid_text;
      adopt_actual_class_template_owner_for_hybrid_named_type(hybrid, actual_type);
      return hybrid;
    }
    return pattern_type;
  }
                                              ...
                                              size_t parameter_index)
{
  ...
  case TemplateArgument::TA_TYPE: {
    ...
    } else if(!try_mangle_type_impl(arg.type, out, mangle_ctx, state)) {
      return false;
    }
  ...
  case TemplateArgument::TA_VALUE:
  {
    ...
    if(try_build_external_entity_argument_ir_payload(arg,
                                                     ...
                                                     entity_address_of)) {
      return itanium_mangle_ir::emit_template_argument(
          itanium_mangle_ir::TemplateArgument::external_entity_arg(
              entity_symbol,
              entity_address_of),
          out,
          nullptr);
  ...
  case TemplateArgument::TA_ALIAS_TEMPLATE:
  {
    ...
    return try_mangle_named_text_impl(arg.text, out, mangle_ctx, state);
  }
  }
  return false;
}
...
static bool emit_template_component_with_structured_arguments(
    ...
    string * canonical_full_text)
{
  const string component = trim_space(base_name);
  if(component.empty()) {
    return false;
  }
  ...
  const string full_base_text =
      append_qualified_component_text(canonical_prefix,
                                      canonical_component_text(component));
  string standard_substitution;
  bool standard_substitution_includes_arguments = false;
  if(structured_std_standard_substitution_for_template_component(
         component,
         arguments,
         canonical_prefix,
         mangle_ctx,
         standard_substitution,
         standard_substitution_includes_arguments)) {
    out += standard_substitution;
    register_standard_substitution_slot(state, standard_substitution);
    if(standard_substitution_includes_arguments) {
      if(canonical_full_text) {
        *canonical_full_text = full_base_text;
      }
      return true;
    }
    if(!emit_grouped_template_arguments(
           arguments.size(),
           parameters,
           out,
           [&](size_t i, const TemplateParameterInfo * parameter) -> bool
           {
             const size_t parameter_index =
                 parameters && parameter ? static_cast<size_t>(parameter - &(*parameters)[0]) :
                                           static_cast<size_t>(-1);
             return try_mangle_template_argument_impl(
                 arguments[i], parameter, out, mangle_ctx, state, parameter_index);
           })) {
      return false;
    }
    if(canonical_full_text) {
  ...
  const string base_key = named_substitution_key(full_base_text);
  const bool suppress_simple_dependent_member_template_name =
      !canonical_prefix.empty() &&
      mangle_ctx &&
      mangle_ctx->suppress_member_template_name_component_substitution &&
      structured_template_arguments_are_simple_parameter_refs(arguments, parameters);
  const bool suppress_template_name_component_substitution =
      !canonical_prefix.empty() &&
      ((mangle_ctx &&
        mangle_ctx->suppress_member_template_name_component_substitution) ||
       suppress_simple_dependent_member_template_name);
  if(!suppress_template_name_component_substitution &&
     emit_registered_substitution(state, base_key, out)) {
    // The template-name substitution replaces the source-name component only.
  } else {
    out += source_name_fragment(component);
  }
  if(!suppress_template_name_component_substitution) {
    register_substitution_key(state, base_key);
  }
  if(arguments.empty()) {
    if(canonical_full_text) {
      *canonical_full_text = full_base_text;
...
static bool build_template_argument_structural_key(
    const TemplateArgument & argument,
    const TypeMangleContext * mangle_ctx,
    itanium_mangle_ir::SubstitutionKey & out)
{
  switch(argument.kind) {
  case TemplateArgument::TA_TYPE: {
    itanium_mangle_ir::SubstitutionKey type_key;
    if(!build_structural_type_substitution_key(argument.type,
                                               mangle_ctx,
                                               type_key,
                                               true)) {
      return false;
    }
    out = itanium_mangle_ir::SubstitutionKey::template_argument_type(type_key);
    return true;
  }
  ...
  case TemplateArgument::TA_VALUE: {
    ...
    if(try_build_external_entity_argument_ir_payload(argument,
                                                    entity_symbol,
                                                    entity_address_of)) {
      payload = string("entity:") + (entity_address_of ? "addr:" : "ref:") +
                entity_symbol;
    } else if(argument.dependent || !argument.text.empty()) {
      payload = string("text:") + trim_space(argument.text);
    } else {
      payload = string("value:") + to_string(argument.value);
    }
    out = itanium_mangle_ir::SubstitutionKey::template_argument_value(payload);
    return !out.empty();
  }

  case TemplateArgument::TA_CLASS_TEMPLATE:
  case TemplateArgument::TA_ALIAS_TEMPLATE:
    out = itanium_mangle_ir::SubstitutionKey::template_argument_template(
        reinterpret_cast<uintptr_t>(argument.template_decl),
...
static bool build_class_template_specialization_structural_key(
    ...
    itanium_mangle_ir::SubstitutionKey & out)
{
  if(!structural_substitution_key_context_is_stable(mangle_ctx)) {
    return false;
  }
  shared_ptr<const ClassTemplateSpecializationMangleInfo> info =
      named_type_class_template_specialization_mangle_info_const(type);
  ...
  string named_key;
  if(build_type_substitution_key(type, mangle_ctx, named_key)) {
    static const char name_prefix[] = "name:";
    if(named_key.compare(0, sizeof(name_prefix) - 1, name_prefix) == 0 &&
       named_key.size() > sizeof(name_prefix) - 1) {
      out = itanium_mangle_ir::SubstitutionKey::named(
          named_key.substr(sizeof(name_prefix) - 1));
      return !out.empty();
    }
  }

  vector<itanium_mangle_ir::SubstitutionKey> argument_keys;
  argument_keys.reserve(info->arguments.size());
  for(size_t i = 0; i < info->arguments.size(); ++i) {
    itanium_mangle_ir::SubstitutionKey argument_key;
    if(!build_template_argument_structural_key(info->arguments[i],
                                               mangle_ctx,
                                               argument_key)) {
      return false;
    }
    ...
    MangleSubstitutionState * state)
{
  ...
  string context_fragment;
  vector<itanium_mangle_ir::SubstitutionSlot> context_substitution_slots;
  if(!build_itanium_lambda_context_fragment_from_type(options.lambda_closure_type,
                                                      context_fragment,
                                                      context_substitution_slots)) {
    return false;
  }
  shared_ptr<FunctionSymbolOptions> context_options;
  if(!itanium_lambda_context_options_from_type(options.lambda_closure_type,
                                              context_options)) {
    return false;
  }
  vector<itanium_mangle_ir::Type> signature_parameter_types;
  const Type::LambdaMangleMetadata * metadata =
      named_type_lambda_mangle_metadata(options.lambda_closure_type);
  if(!metadata) {
    return false;
  }
  const string source_name =
                                         ...
                                         bool suppress_legacy_bridge)
{
  ...
  ir_only_type_ctx.suppress_legacy_type_substitution_keys =
      suppress_legacy_bridge;
  const TypeMangleContext * parameter_type_ctx =
      suppress_legacy_bridge ? &ir_only_type_ctx : nullptr;
    ...
    const FunctionSymbolOptions & options)
{
  ...
  if(options.lambda_closure_type) {
    return display_name == "operator()" && options.is_member_function ?
        FNIP_LAMBDA_CALL :
        FNIP_NONE;
  }
    ...
    MangleSubstitutionState * captured_state)
{
  ...
  if(options.template_parameters &&
      ...
      if(has_result_type_pattern &&
        ...
        if(!mangled_return) {
          ...
          if(actual_function_type->inner &&
             ...
             try_mangle_type_impl(actual_function_type->inner,
                                  candidate,
                                  &result_ctx,
                                  state)) {
            mangled_return = true;
        ...
        if(!mangled_return) {
          throw logic_error(
              string("dependent function result type reached legacy ABI mangler: ") +
      ...
      if(!mangled_return) {
        const TypePtr hybrid_return_type =
            hybridize_pattern_type_with_actual(pattern_type->inner,
                                               actual_function_type->inner,
                                               &mangle_ctx);
        if(!try_mangle_type_impl(hybrid_return_type, candidate, &mangle_ctx, state)) {
          return false;
        }
      }
    }
    ...
    for(size_t i = 0; i < options.parameter_pattern->size(); ++i) {
      ...
      if(type_has_structured_dependent_qualified_member(hybrid_param_type)) {
        ...
        mangled_param = try_mangle_type_impl(hybrid_param_type,
                                             candidate,
                                             &mangle_ctx,
                                             state);
        if(!mangled_param) {
      ...
      if(!mangled_param &&
        ...
        if(!mangled_param) {
          ...
          if(actual_param && !type_has_dependent_mangle_state(actual_param) &&
             try_mangle_type_impl(actual_param, candidate, &mangle_ctx, state)) {
            mangled_param = true;
          } else {
            throw logic_error(
                string("dependent function parameter declaration reached legacy ABI mangler: ") +
                ast_node_diagnostic_label(*parameter_decl_pattern));
          }
        }
      }
      if(!mangled_param &&
         !try_mangle_type_impl(hybrid_param_type, candidate, &mangle_ctx, state)) {
        return false;
      }
  ...
  for(size_t i = param_start; i < type->params.size(); ++i) {
    if(!try_mangle_type_impl(type->params[i], candidate, &mangle_ctx, state)) {
      return false;
    }
  }
  if(type->variadic) {
    ...
    string & out)
{
  ...
  if(!try_mangle_type_impl(owner_class.type, owner, &owner_ctx, &state) ||
     owner.empty()) {
    return false;
  }
...
static string scoped_symbol_qualified_name_legacy(const semantic_model::Scope & scope,
                                                  const string & name)
{
  vector<string> parts;
...
static bool try_mangle_scoped_variable_object_name(const semantic_model::Scope & scope,
                                                   const string & name,
                                                   string & out,
                                                   string * qualified_name)
{
  ...
  string candidate = "_Z";
  if(parts.size() == 2 && parts[1] == "std") {
    candidate += "St";
    candidate += source_name_fragment(parts[0]);
    out.swap(candidate);
    return true;
  }
  if(parts.size() > 1) {
  ...
  size_t begin = parts.size();
  if(parts.size() > 2 && parts.back() == "std") {
    candidate += "St";
    --begin;
  }
  for(size_t i = begin; i > 0; --i) {
    candidate += source_name_fragment(parts[i - 1]);
  }
  if(parts.size() > 1) {
    candidate += 'E';
  }
...
bool mangle_itanium_type_encoding(const TypePtr & type, string & out)
{
  string candidate;
  MangleSubstitutionState state;
  if(!try_mangle_type_impl(type, candidate, nullptr, &state)) {
    return false;
  }
  out.swap(candidate);
  return true;
    ...
    vector<itanium_mangle_ir::SubstitutionSlot> * substitution_slots)
{
  ...
  if(!try_mangle_itanium_function_name_syntax(qualified_name,
...
static string weak_object_symbol_from_function_signature(const string & qualified_name,
                                                         const string & symbol_key)
{
  ...
  for(size_t i = 0; i < parts.size(); ++i) {
    out << source_name_fragment(parts[i]);
  }
  if(parts.size() > 1) {
    out << 'E';
  }
  if(symbol_key.empty()) {
    ...
    out << 'U' << source_name_fragment(symbol_key);
  }
  return out.str();
}
                                             ...
                                             SymbolLinkage linkage)
{
  ...
  if(linkage == SL_INTERNAL) {
    if(!uses_builtin_runtime_override(display_name)) {
      try_mangle_itanium_function_name_syntax(qualified,
                                              qualified_name,
                                              display_name,
                                              type,
                                              options,
    ...
    if(!try_mangle_itanium_function_name_syntax(qualified,
                                                qualified_name,
                                                display_name,
                                                type,
                                                options,
       ...
       linkage == SL_WEAK) {
      out.object_symbol =
          weak_object_symbol_from_function_signature(qualified_name, symbol_key);
    }
  }
...
SymbolIdentity make_scoped_variable_symbol_identity(const semantic_model::Scope & scope,
                                                    ...
                                                    SymbolLinkage linkage)
{
  string qualified_name = scoped_symbol_qualified_name_legacy(scope, name);
  vector<string> parts;
  if(scoped_variable_symbol_parts(scope, name, parts)) {
    qualified_name = scoped_symbol_qualified_name_from_parts(parts);
  }
  ...
  out.internal_symbol = internal_symbol_from_name(qualified_name);
  out.linkage = linkage;
  if(!try_mangle_scoped_variable_object_name(scope, name, out.object_symbol, &qualified_name)) {
    ...
    SymbolLinkage linkage)
{
  ...
  out.internal_symbol = internal_symbol_from_name(qualified_name);
  out.linkage = linkage;
  if(!try_mangle_itanium_static_member_object_name(owner_class,
                                                   member_name,
                                                   out.object_symbol)) {
