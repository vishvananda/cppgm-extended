...
// The ELF64 executable image: a header and one loadable segment at a fixed
// address, followed by the encoded content; and its write to `path` as
// an executable file, with the encode and write timings recorded.
...
void finish_native_executable(
    const std::string & path, elf_detail::CodeBuffer & content, Stats * stats,
