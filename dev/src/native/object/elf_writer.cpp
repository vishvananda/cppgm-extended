                               ...
                               const mir_model::MirFunction * function)
{
  ...
  if(instruction.opcode == mir_model::MirInstruction::MI_COPY_BYTES) {
    ...
    return true;
  } else if(instruction.opcode == mir_model::MirInstruction::MI_FILL_BYTES) {
    // rdi = destination, esi = byte, rdx = count, as the call left them.
    require_operands(instruction, 0);
    emit_register_move(out, XR_RAX, XR_RSI);
    emit_register_move(out, XR_RCX, XR_RDX);
    out.byte(0xf3); out.byte(0xaa);
    return true;
  } else if(instruction.opcode == mir_model::MirInstruction::MI_FILL_UNITS) {
    // rdi = destination, rsi = value, rdx = unit count; rep stosw/d/q.
    require_operands(instruction, 0);
    emit_register_move(out, XR_RAX, XR_RSI);
    emit_register_move(out, XR_RCX, XR_RDX);
    if(instruction.byte_count == 2) out.byte(0x66);
    out.byte(0xf3);
    if(instruction.byte_count == 8) out.byte(0x48);
    out.byte(0xab);
    return true;
                            ...
                            Stats * stats)
{
  ...
  object_elf_detail::finish_native_executable(path, content, stats, 0, encode_start);
}
                            ...
                            Stats * stats)
{
  ...
  if(stats) encode_started = std::chrono::steady_clock::now();
  emit_program_tail(content, program, objects);
  object_elf_detail::finish_native_executable(path, content, stats, encode_nanoseconds,
                           encode_started);
}
