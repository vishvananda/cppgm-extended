...
// The local caller-saved pool a bypassable loop phi may activate in, and
// whether a register is clobbered anywhere over an exact interval; the walk
// consults both when a planned local-phi register turns out busy.
...
protected:
  ...
  {
    Derived & lowerer = static_cast<Derived &>(*this);
    pending_local_phi_promotions_.clear();
    ...
    while(planned_promotion_cursor_ < planned_promotion_schedule_.size() &&
      ...
      if(!lowerer.registers_.try_reserve(reg)) {
        ...
        // The register is often held by the phi's own incoming value,
        // which the transfer at this position consumes; retry after it.
        if(segment->kind == PLK_LOCAL_PHI_GPR) {
          PendingLocalPhi pending;
          pending.value = value;
          pending.reg = reg;
          pending.begin = segment->begin;
          pending.end = segment->end;
          pending_local_phi_promotions_.push_back(pending);
        }
        continue;
      }
  ...
  // After the predecessor's phi transfers: a local phi whose register was
  // busy at its transfer position takes a register now.  The planned one
  // may have been freed by the transfer that consumed the phi's incoming
  // value; otherwise any other register of the local pool that is free now
  // and clobbered nowhere over the phi's interval serves (the reactive
  // occupant of the planned register is often a value live only in layout
  // order, through a region the loop never reaches).  The value the
  // transfer wrote to the frame home moves into the register on this entry
  // edge alone; every later use and the back-edge transfer read and write
  // the register.
  void retry_pending_local_phi_promotions(
      std::vector<mir_model::MirInstruction> & out)
  {
    Derived & lowerer = static_cast<Derived &>(*this);
    const X64Register * pool = 0;
    const std::size_t pool_size = local_caller_pool(&pool);
    for(std::size_t i = 0; i < pending_local_phi_promotions_.size(); ++i) {
      const PendingLocalPhi & pending = pending_local_phi_promotions_[i];
      const lowir_model::ValueId value = pending.value;
      if(!lowerer.value_known_[value] ||
         lowerer.values_[value].location.kind !=
           mir_model::MirOperand::OP_FRAME)
        continue;
      X64Register chosen = pending.reg;
      bool found = false;
      for(std::size_t attempt = 0; attempt <= pool_size && !found; ++attempt) {
        const X64Register reg = attempt == 0 ? pending.reg : pool[attempt - 1];
        if(attempt != 0 && reg == pending.reg) continue;
        if(!lowerer.registers_.try_reserve(reg)) continue;
        if(register_clobbered_in_interval(
             lowerer.facts_, reg, pending.begin, pending.end)) {
          lowerer.registers_.discard_unused_reservation(reg);
          continue;
        }
        chosen = reg;
        found = true;
      ...
      if(!found) continue;
      lowerer.move_value_to_register(
        out, chosen, lowerer.values_[value].location,
        lowerer.values_[value].type);
      mir_model::MirOperand replacement;
      replacement.kind = mir_model::MirOperand::OP_REG;
      replacement.reg = chosen;
      lowerer.set_value_location(value, replacement);
      if(lowerer.stats_) {
        ++lowerer.stats_->planned_register_grants;
        ++lowerer.stats_->planned_local_phi_deferred_promotions;
      }
    }
    pending_local_phi_promotions_.clear();
  }
  ...
  // Local phis whose planned register was busy at their transfer position,
  // retried once the predecessor's transfers have consumed their sources.
  struct PendingLocalPhi
  {
  ...
  std::vector<PendingLocalPhi> pending_local_phi_promotions_;
  std::vector<std::pair<std::size_t, std::size_t> >
