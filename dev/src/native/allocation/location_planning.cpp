    ...
    Stats * stats)
{
  ...
  static const X64Register reversed_pool[] =
    {XR_R15, XR_R14, XR_RBX, XR_R12, XR_R13};
  const X64Register * const pool =
    cppgm_variant::selected("pool-reverse") ? reversed_pool : kPool;
  ...
  for(std::size_t i = 0; i < candidates.size(); ++i) {
    ...
    for(std::size_t reg = sizeof(kPool) / sizeof(kPool[0]); reg-- > 0;) {
      ...
      if(clobbered_in_interval(facts, pool[reg],
      ...
      assign_candidate_location(candidate, pool[reg], timeline);
      if(stats) {
  ...
  for(std::size_t i = 0; i < region_order.size(); ++i) {
    ...
    for(std::size_t reg = sizeof(kPool) / sizeof(kPool[0]); reg-- > 0;) {
      ...
      cyclic_region_register_spans[
        static_cast<unsigned>(pool[reg])].push_back(span);
      assign_candidate_location(candidate, pool[reg], timeline);
      if(stats) {
        ++stats->planned_value_registers;
  ...
  for(std::size_t reg = 0; reg < sizeof(kPool) / sizeof(kPool[0]); ++reg)
    register_spans[static_cast<unsigned>(pool[reg])] = claimed[reg];
}

// ---- A second design at the same seam: graph colouring ----
//
// The course solution above assigns registers by a claim-driven linear scan
// with weighted priorities and four special classes.  This assignment takes
// the same candidates, the same pools and the same clobber facts and
// decides by Chaitin-style graph colouring: candidates whose intervals
// overlap interfere; nodes with fewer neighbours than registers are removed
// first; colours are chosen on the way back.  It plans no local phis and no
// cyclic-region values, leaving those to the reactive walk.  It exists so
// that the course suites are run under an allocator that is not the course
// solution's (`CPPGM_BACKEND_VARIANT=colouring`); a course fixture it fails
// encodes the solution's placement, not the contract.
void assign_candidate_registers_by_colouring(
    const std::vector<Candidate> & candidates,
    const analysis::FunctionFacts & facts,
    FunctionLocationTimeline * timeline,
    std::vector<std::pair<std::size_t, std::size_t> > * register_spans,
    Stats * stats)
{
  static const X64Register kPreserved[] =
    {XR_RBX, XR_R12, XR_R13, XR_R14, XR_R15};
  static const X64Register kCaller[] = {XR_R8, XR_R9};
  struct Node
  {
    std::size_t candidate;
    std::size_t begin;
    std::size_t end;
    std::vector<X64Register> allowed;
  ...
  for(std::size_t i = 0; i < candidates.size(); ++i) {
    const Candidate & candidate = candidates[i];
    if(candidate.is_local_phi || candidate.is_cyclic_region) continue;
    Node node;
    node.candidate = i;
    node.begin = candidate.is_phi ? 0 : candidate.definition;
    ...
    const unsigned crossed = facts.live_across_clobbers[
      static_cast<std::uint32_t>(candidate.value)];
    const bool preserved_pool =
      candidate.is_phi || candidate.is_invariant || candidate.crossing;
    const X64Register * pool = preserved_pool ? kPreserved : kCaller;
    const std::size_t pool_size = preserved_pool ?
      sizeof(kPreserved) / sizeof(kPreserved[0]) :
      sizeof(kCaller) / sizeof(kCaller[0]);
    for(std::size_t r = 0; r < pool_size; ++r) {
      if(facts.has_i128_atomic && pool[r] == XR_RBX) continue;
      if(crossed & analysis::register_mask(pool[r])) continue;
      if(clobbered_in_interval(facts, pool[r], node.begin, node.end))
        continue;
      node.allowed.push_back(pool[r]);
    }
    if(node.allowed.empty()) continue;
    nodes.push_back(node);
  }
  for(std::size_t a = 0; a < nodes.size(); ++a)
    for(std::size_t b = a + 1; b < nodes.size(); ++b)
      if(nodes[a].begin <= nodes[b].end && nodes[b].begin <= nodes[a].end) {
        nodes[a].neighbours.push_back(b);
        nodes[b].neighbours.push_back(a);
      }
  // Simplify: remove nodes whose remaining degree is below their register
  // count; when none remains, spill the node with the longest interval.
  std::vector<std::size_t> order;
  std::vector<std::size_t> degree(nodes.size(), 0);
  for(std::size_t i = 0; i < nodes.size(); ++i)
    degree[i] = nodes[i].neighbours.size();
  std::size_t remaining = nodes.size();
  while(remaining) {
    std::size_t chosen = nodes.size();
    for(std::size_t i = 0; i < nodes.size(); ++i)
      if(!nodes[i].removed && degree[i] < nodes[i].allowed.size()) {
        chosen = i;
        break;
      }
    if(chosen == nodes.size()) {
      ...
      for(std::size_t i = 0; i < nodes.size(); ++i)
        if(!nodes[i].removed &&
           (chosen == nodes.size() ||
            nodes[i].end - nodes[i].begin > longest)) {
          chosen = i;
          longest = nodes[i].end - nodes[i].begin;
        }
      nodes[chosen].spilled = true;
    }
    nodes[chosen].removed = true;
    --remaining;
    order.push_back(chosen);
    for(std::size_t n = 0; n < nodes[chosen].neighbours.size(); ++n)
      if(degree[nodes[chosen].neighbours[n]]) --degree[nodes[chosen].neighbours[n]];
  }
  // Select: colour in reverse removal order with the first allowed register
  // no coloured neighbour holds.
  std::vector<unsigned char> coloured(nodes.size(), 0);
  for(std::size_t k = order.size(); k-- > 0;) {
    Node & node = nodes[order[k]];
    if(node.spilled) continue;
    for(std::size_t r = 0; r < node.allowed.size() && !coloured[order[k]]; ++r) {
      bool free = true;
      for(std::size_t n = 0; n < node.neighbours.size() && free; ++n) {
        const std::size_t other = node.neighbours[n];
        if(coloured[other] && nodes[other].reg == node.allowed[r]) free = false;
      }
      if(!free) continue;
      node.reg = node.allowed[r];
      coloured[order[k]] = 1;
    }
  }
  std::vector<std::pair<std::size_t, std::size_t> > claimed[16];
  for(std::size_t i = 0; i < nodes.size(); ++i) {
    const Candidate & candidate = candidates[nodes[i].candidate];
    if(!coloured[i]) {
      if(stats) ++stats->planner_assign_failures;
      continue;
    }
    assign_candidate_location(candidate, nodes[i].reg, timeline);
    if(allocation::is_callee_saved(nodes[i].reg))
      claimed[static_cast<unsigned>(nodes[i].reg)].push_back(
        std::make_pair(nodes[i].begin, nodes[i].end));
    if(stats) {
      ++stats->planned_value_registers;
      if(candidate.is_phi) ++stats->planned_phi_registers;
      if(candidate.is_invariant) ++stats->planned_invariant_registers;
    }
  }
  for(std::size_t reg = 0; reg < 16; ++reg)
    register_spans[reg] = claimed[reg];
}
    ...
    Stats * stats)
{
  ...
  *extension_spans = scan.spans;
  ...
  if(cppgm_variant::selected("linear-scan"))
    return plan_value_locations_linear_scan(function, facts, scan,
  ...
  if(cppgm_variant::selected("colouring"))
    assign_candidate_registers_by_colouring(candidates, facts, &timeline,
                                            register_spans, stats);
  else
...
std::size_t local_caller_pool(const X64Register ** pool)
{
  static const X64Register kPool[] = {XR_R9, XR_R8, XR_RDI, XR_RSI};
  *pool = kPool;
  return sizeof(kPool) / sizeof(kPool[0]);
}

bool register_clobbered_in_interval(const analysis::FunctionFacts & facts,
                                    X64Register reg,
                                    std::size_t begin, std::size_t end)
{
  return clobbered_in_interval(facts, reg, begin, end);
}
