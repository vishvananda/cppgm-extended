  ...
  for(std::size_t block = function.blocks.size(); block-- > 0; ) {
    ...
    for(std::size_t index = instructions.size(); index-- > 0; ) {
      ...
      if(facts.last_use[source] != FunctionFacts::missing_position() &&
        ...
        // A replayed index is rebuilt from its base at every consumer, so
        // the base is in use until the last of them: it must keep its home
        // and count as crossing whatever those consumers cross.  Recording
        // that only beside last_use left the base retired at its last direct
        // use and replayed from a register a call had since clobbered.
        if(replayed_index) facts.last_use[source] = end;
      }
    }
  }
}
    ...
    int optimization_level)
{
  ...
  // Bases of replayed indexes live until their last replay; extend them
  // before the clobber and call facts below are read off the ranges.
                               ...
                               lowir_model::SymbolId memcpy_symbol)
{
  ...
  extend_loop_liveness(facts, function, definition_blocks,
    block_uses, block_first_position, block_last_position, clobber_positions, optimization_level);
