...
// Which landing pads clean up, so that entering one consumes the protected
// region's frame: every `eh_push` target whose kind operand says so, or
// whose pad has a cleanup clause or no catch clause.  Returns the number of
// `resume` instructions met on the way.
std::size_t mark_cleanup_landing_blocks(
    const mir_model::MirFunction & function,
    const std::vector<std::size_t> & block_index,
    std::vector<bool> * cleanup_landing_blocks)
{
  const std::size_t unknown = static_cast<std::size_t>(-1);
  ...
  for(std::size_t i = 0; i < function.blocks.size(); ++i)
    for(std::size_t j = 0; j < function.blocks[i].instructions.size(); ++j) {
      ...
      if(instruction.opcode != mir_model::MirInstruction::MI_EH_PUSH ||
         ...
         instruction.operands[0].kind != mir_model::MirOperand::OP_LABEL)
        continue;
      const lowir_model::BlockId label = instruction.operands[0].block;
      const std::uint32_t index = label;
      const std::size_t landing =
        label.valid() && index < block_index.size() ? block_index[index] : unknown;
    ...
    const std::string & function_name)
{
  ...
  result.resume_instruction_count = mark_cleanup_landing_blocks(
    function, block_index, &cleanup_landing_blocks);
  ...
  const auto merge_entry = [&](std::size_t block, std::size_t state,
    ...
    if(entries[block] == unknown) {
      ...
      // The mirror of PreferIncoming.  That case takes the incoming state
      // when it has consumed the landing frame and the recorded one has not;
      // this is the same merge reached from the other side, with the block
      // already recorded in the consumed frame and this edge arriving before
      // the pad consumed it.  Both agree on the active region, and the
      // consumed frame is the one that ownership is already expressed in, so
      // keep it rather than reopening a region a later EH_END would close
      // twice.
      if(existing_active == incoming_active &&
         states.Consumed(entries[block]) && !states.Consumed(state))
        return;
