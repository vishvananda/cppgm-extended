...
struct Instruction
{
  ...
  enum Opcode
  {
    ...
    MI_COPY_BYTES,
    MI_ZERO_BYTES,
    // A dynamic fill has no operands either: rdi/rsi/rdx carry the
    // destination, the byte, and the count, as for the fill builtin's call.
    MI_FILL_BYTES,
    // The unit fill: the same registers, byte_count the unit (2, 4 or 8).
    MI_FILL_UNITS,
    MI_EH_PUSH,
    MI_EH_POP,
    MI_EH_CATCH,
