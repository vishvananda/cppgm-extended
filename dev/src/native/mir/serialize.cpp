// The machine-IR text dump: the serialized form of the backend model that
// native emission consumes.  Kept apart from the model so a consumer of
// model.h links the model without the printer.
                             ...
                             const Function * function)
{
  ...
  if(instruction.opcode == Instruction::MI_FILL_BYTES) {
    out << "fill_bytes_dynamic";
    render_operands(out, instruction, program, function, true);
    return out.str();
  }
  if(instruction.opcode == Instruction::MI_FILL_UNITS) {
    out << "fill_units_dynamic " << instruction.byte_count;
    render_operands(out, instruction, program, function, true);
    return out.str();
  }
