...
bool preserves_condition_flags(mir_model::MirInstruction::Opcode opcode)
{
  ...
  case MirInstruction::MI_FILL_BYTES:
  case MirInstruction::MI_FILL_UNITS:
    ...
    std::size_t start)
{
  ...
  // A zero-extension looks past ALU operations no wider than itself on the
  // same register: an 8- or 16-bit `and`, `or`, `xor`, `add`, `sub`, `not` or
  // `neg` writes only its own low bits, so whatever cleared the bits above
  // the width still holds.  libc++ tests its short-string flag as
  // `load u8; and 1; zext`, once per string operation.  A sign extension
  // cannot look past them: the operation may have changed the sign bit.
  std::size_t producer_index = start - 1;
  if(!sign_extend)
    while(producer_index > 0) {
      const MirInstruction & between = instructions[producer_index];
      const bool low_bits_only =
        (between.opcode == MirInstruction::MI_AND ||
         between.opcode == MirInstruction::MI_OR ||
         between.opcode == MirInstruction::MI_XOR ||
         between.opcode == MirInstruction::MI_ADD ||
         between.opcode == MirInstruction::MI_SUB ||
         between.opcode == MirInstruction::MI_NOT ||
         between.opcode == MirInstruction::MI_NEG) &&
        !between.operands.empty() &&
        between.operands[0].kind == MirOperand::OP_REG &&
        between.operands[0].reg == reg &&
        data_layout::type_width(between.type) <= width &&
        data_layout::type_width(between.type) <= 16;
      if(!low_bits_only) break;
      --producer_index;
    }
  const MirInstruction & producer = instructions[producer_index];
