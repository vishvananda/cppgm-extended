...
#include "native/lowering/branches.h"
...
class FunctionLowerer : private IntrinsicLowering<FunctionLowerer>,
                        ...
                        private branch_detail::BranchLowering<FunctionLowerer>,
                        ...
                        private spill_detail::SpillSelection<FunctionLowerer>
{
  ...
  friend class branch_detail::BranchLowering<FunctionLowerer>;
...
public:
  FunctionLowerer(const lowir_model::LowirProgram & program,
                  ...
                  const session_detail::LoweringBuiltinSymbols & builtins,
    ...
    : program_(program), source_(source), pointer_globals_(pointer_globals),
      tls_wrappers_(tls_wrappers), strlen_symbol_(builtins.strlen),
      memcpy_symbol_(builtins.memcpy), fill_symbol_(builtins.fill),
      fill_units_symbol_(builtins.fill_units),
    ...
    for(std::size_t i = 0; i < source_.blocks.size(); ++i) {
      ...
      for(std::size_t j = 0; j < source_.blocks[i].instructions.size(); ++j, ++position_) {
        ...
        if(source_instruction.kind == Instruction::IK_JUMP ||
          ...
          retry_pending_local_phi_promotions(block.instructions);
        }
...
private:
  ...
  const lowir_model::SymbolId strlen_symbol_, memcpy_symbol_, fill_symbol_,
    fill_units_symbol_;
  ...
  bool try_reserve_initial_call_result(lowir_model::ValueId value,
    ...
    static const X64Register ordinary_default[] = {XR_R8, XR_R9, XR_RBX, XR_R12, XR_R13, XR_R14, XR_R15};
    static const X64Register preserved_default[] = {XR_RBX, XR_R12, XR_R13, XR_R14, XR_R15};
    static const X64Register ordinary_reversed[] = {XR_R8, XR_R9, XR_R15, XR_R14, XR_R13, XR_R12, XR_RBX};
    static const X64Register preserved_reversed[] = {XR_R15, XR_R14, XR_R13, XR_R12, XR_RBX};
    const bool reversed = cppgm_variant::selected("pool-reverse");
    const X64Register * const ordinary = reversed ? ordinary_reversed : ordinary_default;
    const X64Register * const preserved = reversed ? preserved_reversed : preserved_default;
    const std::size_t ordinary_count = sizeof(ordinary_default) / sizeof(ordinary_default[0]);
    const std::size_t preserved_count = sizeof(preserved_default) / sizeof(preserved_default[0]);
                                ...
                                const LowType * pressure_type = 0)
  {
    ...
    // Only a register can be operated on in place: the duplicate shortcut
    // above does not look at the home, and a frame-resident `v op v` would
    // hand the ALU encoder memory on both sides.
    const bool safe_reuse = left.kind == MirOperand::OP_REG &&
      (can_reuse(instruction.first) || duplicate_last_use) &&
                 ...
                 std::vector<MirInstruction> & out)
  {
    ...
    const bool inline_unused_fill = fill_detail::is_inline_unused_call(instruction, facts_.uses, fill_symbol_);
    const std::size_t inline_unused_fill_units =
      fill_detail::inline_unused_units_call_width(instruction, facts_.uses, fill_units_symbol_);
    MirOperand indirect_target = reg_operand(XR_R10);
    if(!direct) {
    ...
    if(inline_unused_memcpy) out.push_back(machine_instruction(MirInstruction::MI_COPY_BYTES));
    else if(inline_unused_fill) out.push_back(machine_instruction(MirInstruction::MI_FILL_BYTES));
    else if(inline_unused_fill_units) {
      MirInstruction fill = machine_instruction(MirInstruction::MI_FILL_UNITS);
      fill.byte_count = inline_unused_fill_units;
      out.push_back(fill);
    }
    else if(direct) {
    ...
    lowir_native::Stats * stats, allocation::AllocationDecisionLog * decisions)
{
  return FunctionLowerer(program, function, pointer_globals, tls_wrappers,
                         signatures, builtins, optimization_level, stats,
                         decisions).lower();
}
