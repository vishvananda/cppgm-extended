...
// These budgets stay exact, but not as absolute byte counts: libstdc++ spells
// a std::string in 32 bytes and libc++ in 24, and each structure below embeds
// several, so a fixed number would assert a property of one standard library
// rather than of our own storage.  Subtract instead the shrinkage the current
// library implies, scaled by the std::string subobjects each structure holds.
// Under libstdc++ the subtraction is zero and these are the same counts they
// have always been.  A library with a wider string would need recalibration
// rather than silent wraparound, so require the calibration point to be the
// widest.
static_assert(sizeof(std::string) <= 32,
              "ABI storage budgets are calibrated against a 32-byte std::string");
const std::size_t kAbiStringShrink = 32 - sizeof(std::string);

static_assert(sizeof(AbiTypePresentationNames) == 32,
              "ABI presentation ranges must occupy one vector");
static_assert(sizeof(AbiType) == 400 - 7 * kAbiStringShrink,
              "typed ABI presentation storage must shrink types");
static_assert(sizeof(AbiTemplateArgument) == 1912 - 34 * kAbiStringShrink,
              "typed ABI terminal must not widen template arguments");
static_assert(sizeof(AbiDependentExpression) == 1024 - 17 * kAbiStringShrink,
              "typed ABI operation must not widen dependent expressions");
static_assert(sizeof(AbiFunctionTarget) == 1072 - 19 * kAbiStringShrink,
              "typed ABI terminal must not widen function targets");
static_assert(sizeof(AbiFunctionRecord) == 824 - 16 * kAbiStringShrink,
