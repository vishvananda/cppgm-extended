	...
	std::string* literal_suffix)
{
	...
	if (operation == "~") return OPERATOR_COMPLEMENT;
			...
			edge = arena_->NextEdge(edge))
		{
				...
				arena_->IsTag(member, ::cppgm::syntax::STAG_CLASS_FORWARD_DECLARATION))
			{
					 ...
					 program_->entities[nested].flavor == NAMED_STRUCT))
				{
					...
					// The members being injected are the anonymous class's
					// own, so it has to be complete before they can be read.
					// At namespace scope it already is by this point; nested in
					// a class template it is not, and reading an empty member
					// list here injects nothing at all and says nothing --
					// libc++'s `basic_string` puts an anonymous struct inside
					// `__long`, and every use of `__is_long_` then fails to
					// find it.
					EnsureClassDefinition(nested_type);
...
bool Analyzer::ClassDestructorIsDefaulted(const EntityRecord& owner)
{
	if (!owner.has_user_declared_destructor) return true;
	const BindingId destructor = DestructorForType(owner.type);
	return destructor != kNoBinding &&
...
void Analyzer::CompleteClassLayout(EntityId entity)
{
		...
		i < entity_layout_members_[entity].size(); ++i)
	{
		...
		// A shape-only completion lays out nothing: its members' types are the
		// template's own parameters standing in, and a stand-in has no size.
		// Asking for one turns a specialization that only had to exist into an
		// incomplete-type error.  Every concrete specialization is completed
		// separately and is laid out for real.
...
void Analyzer::AnalyzeClassMemberFunctionDefinition(NodeId node,
	ScopeId scope, TypeId owner_type, const SpecInfo& spec,
	AccessKind access)
{
	...
	TypeId owner_type, AccessKind access)
{
	...
	{
		AnalyzeClassMemberFunctionDefinition(node, scope, owner_type, spec,
			access);
		return;
	}
	...
	const SpecInfo& spec, bool local)
{
	...
	// same object, so the name has to stay external and merge weakly.
	if (!local && record.storage_class == STORAGE_CLASS_NONE &&
		top_type.kind == TYPE_QUALIFIED && (top_type.cv & CV_CONST) != 0 &&
		!previously_external && !spec.inline_specifier)
		record.storage_class = STORAGE_CLASS_STATIC;
	if (!local && spec.inline_specifier &&
		record.storage_class != STORAGE_CLASS_STATIC &&
		!record.unnamed_namespace_linkage)
		record.weak_odr = program_->bindings[record.canonical].weak_odr = true;
...
// An explicit-specifier may carry a condition, and ignoring it is worse than
// rejecting the form: a constructor declared explicit(false) would silently
// stop taking part in copy initialization.  A bare `explicit` is the same as a
// true condition.  A dependent condition cannot be answered here and is left to
// the instantiation, which knows the arguments.
bool Analyzer::EvaluateExplicitSpecifier(NodeId specifier, ScopeId scope)
{
	const NodeId condition_syntax = FirstSemanticChild(specifier);
	if (condition_syntax == kNoNode) return true;
	...
	TypeId owner_type, AccessKind access)
{
	...
	if (special_name == "~" + class_name)
	{
		...
		if (specifiers != kNoNode)
			for (std::uint32_t edge = arena_->FirstEdge(specifiers);
				...
				{
					binding.inline_function = true;
					GetMutableFunction(destructor).inline_specified = true;
				}
	...
	const DeclaratorInfo parsed = BuildDeclarator(declarator,
		program_->types.Fundamental(FUND_VOID), scope, false, true);
	if (parsed.type == kNoType || !program_->types.IsFunction(parsed.type))
	...
	if (specifiers != kNoNode)
			...
			edge = arena_->NextEdge(edge))
		{
			...
			const std::string value = PayloadSource(specifier);
			if (value == "explicit")
				info.explicit_constructor =
					EvaluateExplicitSpecifier(specifier, scope);
			else if (value == "constexpr") info.constexpr_function = true;
			else if (value == "inline")
			{
				binding.inline_function = true;
				info.inline_specified = true;
			}
		}
