	...
	const std::unordered_set<NameId>* template_parameter_names)
{
	...
	// Two traversals reach a member declaration without wanting its layout:
	// completing a specialization whose arguments are still dependent, and
	// building a type for identity only, which suppresses class completion.
	// In both, the template's own parameters stand in for types, so a bound
	// over them is meaningless rather than wrong -- `sizeof(T) - 1` folds to
	// -1 because the stand-in has no size, and libc++'s compressed-pair
	// padding writes `sizeof(_ToPad) - __datasizeof_v<_ToPad>`, which folds to
	// zero the same way.  Neither traversal produces a layout: every concrete
	// specialization is completed separately and computes its own bound.  This
	// only stands in where the bound would otherwise be rejected, so `T[N]`
	// still reaches the dependent-array representation below.
	const bool bound_needs_no_layout =
	...
	const bool usable_parameter_bound =
		...
		program_->KindOfScope(program_->bindings[expression.binding].owner) ==
			SCOPE_TEMPLATE_PARAMETERS &&
		program_->bindings[expression.binding].value >= 0 &&
		static_cast<std::uint64_t>(
			program_->bindings[expression.binding].value) <
			kNoTemplateParameter;
	if (!usable_parameter_bound)
	{
		if (bound_needs_no_layout)
			return CandidateTypeFormation(
				program_->types.TryZeroLengthArray(element),
