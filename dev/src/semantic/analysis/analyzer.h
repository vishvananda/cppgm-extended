...
private:
	...
	void MergeFunctionTemplatePatternIntoPrior(std::size_t prior_index,
		FunctionTemplatePattern* pattern, EntityId friend_owner,
		bool qualified_friend, bool definition,
		bool explicit_member_definition);
	void RegisterFunctionTemplatePattern(NodeId declaration, NodeId target,
		ScopeId scope,
	...
	void ResolveCallFunctionTemplatePatterns(NodeId callee, ScopeId scope,
	...
	void FinishLocalVariableInitializer(ScopeId scope, std::uint32_t owner,
		...
		std::uint32_t variable, bool control_dependent);
	...
	// True when the expression's value waits on the enclosing template's
	...
	bool IsTemplateNondeducedShapeType(TypeId type) const
	{
		return type != kNoType &&
			(type == function_template_nondeduced_type_shape_ ||
			 type == class_template_nondeduced_type_shape_);
	}
	...
	ClassTemplatePartialSelection& EnsureClassTemplatePartialSelection(
		BindingId binding);
	bool NamesTemplateParameterShape(ScopeId scope, NameId name);
	...
	BindingId InstantiateFunctionTemplate(std::size_t pattern,
		const std::vector<TemplateArgument>& arguments,
		const std::vector<std::uint32_t>& parameter_offsets);
	void BindFunctionTemplateSpecializationMember(BindingId binding,
		EntityId member_owner, const FunctionTemplatePattern& pattern,
		const SpecInfo& spec);
	...
	bool ExplicitInstantiationSuppressesDefinition(BindingId binding) const;
	...
	// Volatile only, ignoring _Atomic; see semantic/object_model/class_layout.cpp.
	bool IsVolatileOnlySubobjectType(TypeId type) const;
	// A stand-in type used while a template body has no arguments yet.
	bool IsDependentParameterShapeType(TypeId type) const;
	...
	bool AnalyzeExplicitDestructorCall(NodeId callee, ScopeId scope,
		const std::vector<NodeId>& argument_syntax, TypeId target,
		ExpressionInfo* result);
	void RestoreAssociatedLookup(std::vector<EntityId>* entities,
	...
	TypeId BuiltinAdditiveResultType(int op, const ExpressionInfo& left,
		const ExpressionInfo& right, TypeId* operand_type,
		bool* substitution_failed);
	ExpressionInfo AnalyzeAssignment(NodeId node, ScopeId scope);
	ExpressionInfo AnalyzeIndirectCall(ExpressionInfo callee, ScopeId scope,
		const std::vector<NodeId>& argument_syntax,
		const std::vector<ExpressionInfo>& analyzed_arguments, TypeId target);
	...
	std::uint32_t AggregateHelperIdentity(EntityId entity, TypeId type,
		std::size_t parameter_member_count,
		const std::vector<TypeId>& parameter_types,
		const std::vector<BindingId>& members,
		const std::vector<BindingId>& member_constructors,
		const std::vector<BindingId>& member_destructors,
		const std::vector<std::uint8_t>& trivial_member_constructors);
	...
	bool ClassDestructorIsDefaulted(const EntityRecord& owner);
	void SetBindingRequestedAlignment(BindingRecord& binding,
		std::size_t alignment);
	std::size_t RequestedAlignment(NodeId node, ScopeId scope);
	std::uint64_t AlignmentSpecifierValue(NodeId alignment, ScopeId scope);
	// True when the type's size can be computed now.  A retained template
	// parameter shape cannot, which only matters during a shape-only
	// completion; see semantic/declarations/analysis.cpp.
	bool TypeHasKnownSize(TypeId type) const;
	...
	bool ExplicitInstantiationCoversMember(BindingId binding) const;
	bool EnclosingExplicitInstantiationSuppressed(BindingId binding) const;
	...
	std::size_t DeclarationStorageBytes() const;
	std::size_t TemplateStorageBytes() const;
	std::size_t LifetimeStorageBytes() const;
	std::size_t ConstantStorageBytes() const;
	std::size_t FunctionRecordStorageBytes() const;
	std::size_t ClassRecordStorageBytes() const;
	std::size_t TemplateRecordStorageBytes() const;
	...
	// Nonzero while completing a class template specialization whose
	// arguments are still dependent; see semantic/templates/classes.cpp.
	std::size_t dependent_shape_completion_depth_;
