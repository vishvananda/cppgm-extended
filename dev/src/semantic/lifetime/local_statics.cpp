...
bool Analyzer::IsConstexprLiteralType(TypeId type) const
{
	...
	if (top.entity < entity_data_members_.size())
		for (std::size_t i = 0; i < entity_data_members_[top.entity].size(); ++i)
		{
			...
			if (IsVolatileOnlySubobjectType(member.type) ||
...
bool Analyzer::IsConstexprConstructorOwnerType(EntityId entity) const
{
	...
	if (entity < entity_data_members_.size())
		for (std::size_t i = 0; i < entity_data_members_[entity].size(); ++i)
		{
			...
			if (IsVolatileOnlySubobjectType(type) ||
				!IsConstexprLiteralType(type)) return false;
		}
	return true;
}
		...
		for (std::size_t i = 0; i < entity_constructors_[entity].size(); ++i)
		{
			...
			if (constructor.constexpr_function)
			{
				...
				if (!constructor_owner_suitable)
					ThrowSemanticError(
						"constexpr constructor has non-literal subobjects in " +
						program_->RenderType(owner_type));
			}
		}
