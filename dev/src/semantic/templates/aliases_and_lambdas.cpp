	...
	ScopeId scope, TypeId target)
{
	...
	else
	{
		...
		if (generic_call)
		{
			...
			// A generic lambda's call template has no enclosing template
			// declaration, so the lambda node is the best location it has.
			RegisterFunctionTemplatePattern(node, node, member_scope, ACCESS_PUBLIC,
	...
	NodeId target, ScopeId scope, bool definition)
{
	...
	// An explicit instantiation names the member after its class, and the rest
	// of the declarator is then in that class's scope: libc++'s extern template
	// list writes `basic_string<char>::__init(const value_type*, size_type)`,
	// whose parameter types are members of the class named just before them.
	// The out-of-class definition already resolves them that way; build the
	// declarator in the same scope so this spelling agrees.
	ScopeId declarator_scope = scope;
	if (structure != kNoNode)
	{
		std::vector<NodeId> components;
		NamePath structured_path;
		structured_path.global = FindChild(
		...
		{
			const NodeId list = FindChild(components[component],
			...
			std::vector<NodeId> owner_argument_syntax;
			for (std::uint32_t edge = arena_->FirstEdge(list);
				edge != kNoEdge; edge = arena_->NextEdge(edge))
				owner_argument_syntax.push_back(arena_->EdgeChild(edge));
			std::vector<TemplateArgument> owner_arguments;
			if (!BuildTemplateArguments(owner_pattern.parameters,
				owner_argument_syntax, scope, owner_pattern.lexical_scope,
				&owner_arguments)) continue;
			const BindingId owner =
				InstantiateClassTemplate(pattern_index, owner_arguments);
			// The owner has to be complete before its members are visible; an
			// explicit instantiation is often the first thing to name it.
			if (owner != kNoBinding)
				EnsureClassDefinition(program_->bindings[owner].type);
			const EntityId owner_entity = owner == kNoBinding ? kNoEntity :
				EntityOf(program_->bindings[owner].type);
			if (owner_entity != kNoEntity &&
				program_->entities[owner_entity].member_scope != kNoScope)
				declarator_scope =
					program_->entities[owner_entity].member_scope;
		}
		if (declarator_scope == scope && structured_path.Size() > 1)
		{
			const ScopeId owner = ResolveOwner(scope, structured_path);
			if (owner != kNoScope &&
				program_->KindOfScope(owner) == SCOPE_CLASS)
				declarator_scope = owner;
		}
	}
	// The declared type is named with the member's access too, so enter the
	// class before building it:  libc++ declares
	// `__time_get_storage<char>::string_type
	// __time_get_storage<char>::__analyze(...)`, whose return type names a
	// protected member of the class named just before it.
	const EntityId declarator_class =
		program_->EntityForScope(declarator_scope);
	const ScopedValueRestore<EntityId> class_context(
		&current_class_context_,
		declarator_class != kNoEntity ? declarator_class :
			current_class_context_);
	SpecInfo spec;
	...
	else
	{
		...
		spec = BuildSpecifiers(specifiers, declarator_scope,
			std::string(), true);
	}
	DeclaratorInfo parsed =
		BuildDeclarator(declarator, spec.type, declarator_scope);
