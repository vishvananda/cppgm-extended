...
// `name(args)` in a parenthesized initializer is a parameter-declaration only
// if `name` is a type.  When it is not one and does name something callable,
	...
	ScopeId scope, ExpressionInfo* result)
{
	...
	if (FunctionCallCandidates(scope, spelling, 0, name_node).empty() &&
		(structured.Empty() ?
		...
		edge = arena_->NextEdge(edge))
	{
		...
		if (!arena_->IsTag(parameter, ::cppgm::syntax::STAG_PARAMETER_DECLARATION))
			return false;
		if (FindChild(parameter, ::cppgm::syntax::STAG_DECLARATOR) != kNoNode ||
			FindChild(parameter, ::cppgm::syntax::STAG_ABSTRACT_DECLARATOR) != kNoNode)
			return false;
		const NodeId inner_specifiers =
			FindChild(parameter, ::cppgm::syntax::STAG_DECL_SPECIFIER_SEQ);
		const NodeId inner = inner_specifiers == kNoNode ? kNoNode :
			FirstSemanticChild(inner_specifiers);
		if (inner == kNoNode) return false;
		if (LookupSyntaxName(inner, scope, LOOKUP_ORDINARY).ordinary == kNoBinding)
			return false;
		argument_syntax.push_back(inner);
		arguments.push_back(
			AnalyzeNamedValue(PayloadSource(inner), scope, kNoType, inner));
	}
	if (arguments.empty()) return false;
	return AnalyzeRetainedNamedCall(name_node, spelling, scope,
		argument_syntax, arguments, kNoType, result);
}
	...
	NodeId clause, NameId variable_name)
{
		...
		edge = arena_->NextEdge(edge))
	{
		...
		if (declarator != kNoNode)
		{
			// `name(args)` is the same evidence a bare value name is, when
			// the name turns out not to be a type.  Take the shape here and
			// leave the lookup to the second pass, which has to do it anyway
			// and gives up if the name is a type -- this loop runs for every
			// local declaration and the lookup is not free.
			const NodeId inner_clause = FindChild(declarator,
				::cppgm::syntax::STAG_PARAMETER_CLAUSE);
		...
		edge = arena_->NextEdge(edge))
	{
		...
		if (empty_clause == kNoNode) return false;
		if (FirstSemanticChild(empty_clause) != kNoNode)
		{
			ExpressionInfo call;
