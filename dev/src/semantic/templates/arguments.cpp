	...
	NodeId target, ScopeId scope, AccessKind)
{
		...
		component + 1 < components.size(); ++component)
	{
		...
		// A member's declared type is named with that member's access, just as
		// its declarator is: libc++ declares
		// `__time_get_storage<char>::string_type
		// __time_get_storage<char>::__analyze(...)`, whose return type names a
		// protected member of the very class being specialized.  Enter the
		// class before building the specifiers, not just before the
		// declarator, or that access is judged from no class at all.
		...
		if (member_spec.inline_specifier)
			GetMutableFunction(selected).inline_specified = true;
		return true;
	}
		...
		arena_->IsTag(target, ::cppgm::syntax::STAG_CLASS_FORWARD_DECLARATION))
	{
		...
		else
		{
			...
			// Mark it specialized before analysing the body, not after.
			// Analysing a member can look up this very class and reach
			// EnsureClassDefinition re-entrantly, and a completion that runs
			// then would add every member a second time; it needs to see the
			// mark that is otherwise only set once the body is finished.
	...
	NodeId syntax, ScopeId scope)
{
	...
	if (!dependent)
	{
		...
		// The specialization is memoized, so whatever this initializer folds
		// to is what every later reader sees.  A caller that short-circuited
		// past the name suppressed constant evaluation for its own operand;
		// letting that suppression reach here would cache a specialization
		// with no value and make the next reader call it nonconstant.
		ExpressionInfo initializer;
		{
			ScopedValueRestore<std::size_t> evaluated(
				&constant_evaluation_suppressed_depth_, 0);
			initializer = AnalyzeConstantAwareVariableInitializer(
				selected.initializer, substitution_scope, parsed.type,
				false, spec.is_constexpr, true);
		}
...
bool Analyzer::IsDependentValueExpression(
	const ExpressionInfo& expression) const
{
	if (expression.dependent_value) return true;
	if (expression.binding == kNoBinding ||
		expression.binding >= program_->bindings.size()) return false;
	const BindingRecord& binding = program_->bindings[expression.binding];
	return binding.variable_template_specialization && !binding.constant;
}
	...
	std::vector<TemplateArgument>* arguments)
{
	...
	else
	{
		...
		// A variable template specialization only carries a value once its
		// arguments are known, so one still without a value is a dependent
		// read rather than a non-constant expression.  The dependence is
		// semantic, not lexical -- the argument names a member typedef, not a
		// template parameter -- so the syntactic retention above cannot see
		// it.  Retain it the same way, which is what the enclosing template-id
		// needs in order to stay dependent.
		else if (!expression.constant &&
			IsDependentValueExpression(expression))
		{
			argument.type = ClassTemplateNondeducedTypeShape();
			argument.dependent_parameter = kNondeducedTemplateParameter;
			argument.source_value_type = RetainedIntegralLiteralType(
