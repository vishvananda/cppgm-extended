...
// An explicit instantiation of a class template instantiates its members, but
// not every member is one of them: a member template, a member the declaration
// excluded from the instantiation, and a member the class does not write at all
// are each still the using translation unit's own to produce.
bool Analyzer::ExplicitInstantiationCoversMember(BindingId binding) const
{
	binding = program_->bindings[binding].canonical;
	if (program_->bindings[binding].excluded_from_explicit_instantiation)
		return false;
	if (program_->bindings[binding].kind != BIND_FUNCTION) return true;
	const FunctionInfo& function = GetFunction(binding);
	// An explicit instantiation covers only ordinary members; a member
	// template's specializations must still be produced by each translation
	// unit that uses them.
	if (function.template_pattern != kNoDumpEdge) return false;
	if (function.definition_in_class ||
		(program_->bindings[binding].inline_function &&
		 (function.defaulted_constructor || function.defaulted_destructor ||
		  function.defaulted_special_member))) return false;
	// An implicitly declared member is not written in the class template, so
	// an explicit instantiation of that template does not name it either:
	// whoever uses it declares and defines its own.
	return !function.implicit_constructor && !function.implicit_destructor &&
		!function.implicit_special_member;
}

// The members of a nested class are members of the enclosing class too, so an
// explicit instantiation of the enclosing template reaches them.  They are
// instantiated long after that instantiation was seen -- the nested class has
// its own definition, pulled in by whoever names it -- so the answer has to be
// asked here rather than published when the instantiation was analyzed.
bool Analyzer::EnclosingExplicitInstantiationSuppressed(BindingId binding) const
{
	EntityId owner = program_->bindings[binding].member_owner;
	if (owner == kNoEntity) return false;
	for (;;)
	{
		const ScopeId lexical = program_->entities[owner].owner;
		const EntityId enclosing = program_->EntityForScope(lexical);
		if (enclosing == kNoEntity || enclosing == owner) return false;
		if (program_->entities[enclosing].explicit_instantiation_suppressed)
			return true;
		owner = enclosing;
	}
}
	...
	EntityId entity, bool suppressed)
{
	...
	const auto mark = [this, suppressed](BindingId binding) {
		...
		binding = program_->bindings[binding].canonical;
		// An explicit instantiation does not reach a member the declaration
		// excluded from it, so that member keeps the implicit instantiation
		// and the weak definition every user would otherwise get.
		if (program_->bindings[binding].excluded_from_explicit_instantiation)
			return;
		if (suppressed && !ExplicitInstantiationCoversMember(binding)) return;
