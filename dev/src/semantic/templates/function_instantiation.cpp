	...
	const Program& program, NodeId specifiers, FunctionTemplatePattern* pattern)
{
		...
		edge = arena.NextEdge(edge))
	{
		...
		{
			// `explicit(cond)` is not `explicit`: taking the keyword alone
			// makes every conditional constructor explicit, which drops it
			// from copy initialization.  libc++'s pair converting constructor
			// is spelled that way, so nothing could convert between two pair
			// specializations.  Keep the syntax and let the instantiation
			// answer it.
			if (arena.FirstEdge(child) != kNoEdge)
				pattern->explicit_specifier_syntax = child;
			else pattern->explicit_specifier = true;
		}
	...
	const FunctionTemplatePattern& incoming)
{
	...
	if (retained->explicit_specifier_syntax == kNoNode)
		retained->explicit_specifier_syntax = incoming.explicit_specifier_syntax;
...
// A redeclaration of a function template already recorded at `prior_index`
// merges into it: result lookups and parameter metadata a definition brings,
// specifier facts and defaults, the exception specification (which must
// agree), friend visibility, and the definition itself.
void Analyzer::MergeFunctionTemplatePatternIntoPrior(std::size_t prior_index,
	FunctionTemplatePattern* pattern, EntityId friend_owner,
	bool qualified_friend, bool definition, bool explicit_member_definition)
{
...
void Analyzer::RegisterFunctionTemplatePattern(NodeId declaration, NodeId target,
	...
	TypeId dependent_result_shape, bool dependent_exception_specification)
{
	...
	// which answers with an internal error and loses the diagnostic this check
	// exists to give.  Ask about the absence first, and say where.
	if (shape_declarator.type == kNoType ||
		!program_->types.IsFunction(shape_declarator.type))
	{
		// The declarator carries no token range; the template declaration
		// that encloses it does, so report that rather than nothing.
		const NodeId located =
			arena_->HasSourceLocation(declarator) ? declarator :
			arena_->HasSourceLocation(declaration) ? declaration : kNoNode;
		ThrowSemanticError("function template has non-function declaration" +
			(located == kNoNode ? std::string() :
				" at " + arena_->SourceFile(located) + ":" +
				std::to_string(arena_->SourceLine(located)) + ":" +
				std::to_string(arena_->SourceColumn(located))));
	}
	...
	{
		MergeFunctionTemplatePatternIntoPrior(prior_index, &pattern,
			friend_owner, qualified_friend, definition,
			explicit_member_definition);
		return;
	}
	...
	std::size_t index)
{
		...
		specialization < specializations.size(); ++specialization)
	{
		...
		function.inline_specified =
			function.inline_specified || spec.inline_specifier;
...
// A specialization of a member function template is a member of the class
// that owns the pattern: it takes the pattern's access, is static when the
// pattern or its specifiers say so (allocation operators always are), and
// joins the class's member functions.
void Analyzer::BindFunctionTemplateSpecializationMember(BindingId binding,
	EntityId member_owner, const FunctionTemplatePattern& pattern,
	const SpecInfo& spec)
{
	if (member_owner == kNoEntity) return;
