	...
	FunctionTemplatePattern* pattern)
{
	...
	build = [this, pattern, &root_parameter, &collect_arguments,
		...
		{
			// This is identity, not analysis: it asks whether the syntax
			// happens to name a concrete type, and falls through when it does
			// not.  `dependent_names` holds this template's own parameters, so
			// a member template whose syntax mentions the *enclosing class's*
			// parameter reaches here -- libc++ writes
			// `enable_if_t<__is_allocator<_Allocator>::value, int>` -- and
			// building `__is_allocator<_Allocator>::value` as a type then
			// fails on the `::value`.  Ask in the mode that reports a failure
			// rather than throwing one; kNoType is already handled.
			TypeId type;
			{
				ScopedContainerPush<std::vector<std::uint8_t> > substitution(
					&candidate_substitution_failures_, 0);
				type = BuildCanonicalTemplateTypeArgument(
