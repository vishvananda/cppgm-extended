...
{
	// The right operand of a short-circuited `&&` or `||` and the discarded
	// arm of a constant conditional have their constant evaluation
	// suppressed, but they are still potentially evaluated (N3485 3.2/2)
	// and the lowering emits them, so their callees stay demanded.  Only
	// an unevaluated operand, such as the operand of `noexcept`, demands
	// nothing.
	if (constant_evaluation_suppressed_depth_ != 0 &&
		unevaluated_depth_ != 0) return false;
...
{
	bool valid = true;
	// By value, not by reference.  Evaluating a member or base initializer
	// below re-enters analysis, which can complete a class this translation
	// unit has not laid out yet and grow entity_data_members_.  That
	// reallocates, and a reference to one of its elements is left dangling:
	// the later reads see a stale size, and the member loop then produces the
	// wrong number of elements while still reporting success.
	const std::vector<BindingId> members = entity_data_members_[entity];
	const std::size_t base_count = plan.base_initializers.size();
