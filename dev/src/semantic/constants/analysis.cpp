...
std::size_t Analyzer::RequestedAlignment(NodeId node, ScopeId scope)
{
		...
		edge = arena_->NextEdge(edge))
	{
		...
		if (arena_->IsTag(alignment, ::cppgm::syntax::STAG_GNU_ATTRIBUTE))
		{
			...
			// The parser re-parses a non-literal `aligned` argument into the
			// same node `alignas` produces, so prefer that over the token
			// spelling: the argument is a constant expression, and inside a
			// template it is only evaluable once the arguments are known.
			const NodeId specifier = FindChild(
				alignment, ::cppgm::syntax::STAG_ALIGNMENT_SPECIFIER);
			if (specifier != kNoNode)
			{
				const std::uint64_t parsed_value =
					AlignmentSpecifierValue(specifier, scope);
				if (parsed_value != 0 &&
					((parsed_value & (parsed_value - 1)) != 0 ||
		...
		const std::uint64_t value = AlignmentSpecifierValue(alignment, scope);
		if (value == 0) continue;
		if ((value & (value - 1)) != 0 ||
			value > std::numeric_limits<std::size_t>::max())
			ThrowSemanticError("invalid requested alignment");
...
// Evaluate one `alignment-specifier` operand.  Returns 0 for "no alignment
// requested", which also covers an argument that is still dependent and so
// is not knowable until instantiation.
std::uint64_t Analyzer::AlignmentSpecifierValue(NodeId alignment, ScopeId scope)
{
	...
	if (arena_->IsTag(operand, ::cppgm::syntax::STAG_TYPE_ID))
	{
		...
		if (constant.ordinary != kNoBinding)
		{
			...
			// Inside a template definition the argument reads a
			// variable template specialization whose own arguments are
			// still dependent, so the alignment is not knowable yet.
			// Layout that matters runs again at instantiation, where the
			// same attribute is evaluated with concrete arguments; a
			// dependent read is deferred rather than diagnosed, while a
			// genuinely non-constant one is still an error.
			...
			if (!expression.constant || expression.value < 0)
				ThrowSemanticError(
					...
					StaticAssertLocation(*arena_, name));
	...
	else
	{
		...
		if (!expression.constant || expression.value < 0)
			ThrowSemanticError("nonconstant alignment specifier" +
				StaticAssertLocation(*arena_, operand));
		value = static_cast<std::uint64_t>(expression.value);
	}
	return value;
}
