	...
	hosted_builtin::FloatingIntrinsicKind kind)
{
	...
	else if (intrinsic.operation == FLOATING_OPERATION_EXTERNAL_LIBM)
		// This family takes its own result type in every operand, so the arity
		// is the whole signature.
		for (std::size_t i = 0; i < intrinsic.arity; ++i)
			parameter_types.push_back(result);
	else if (IsExternalLibmOperation(intrinsic.operation))
	{
		// The rest of the family names its own shape.  `result` still holds
		// the floating type here; the entries that return an integer replace
		// it after their operand has been recorded.
		const TypeId floating = result;
		parameter_types.push_back(floating);
		switch (intrinsic.operation)
		{
		case FLOATING_OPERATION_EXTERNAL_LIBM_EXPONENT_OUT:
			parameter_types.push_back(program_->types.Pointer(integer)); break;
		case FLOATING_OPERATION_EXTERNAL_LIBM_EXPONENT_IN:
			parameter_types.push_back(integer); break;
		case FLOATING_OPERATION_EXTERNAL_LIBM_LONG_EXPONENT_IN:
			parameter_types.push_back(
				program_->types.Fundamental(FUND_LONG_INT)); break;
		case FLOATING_OPERATION_EXTERNAL_LIBM_PART_OUT:
			parameter_types.push_back(program_->types.Pointer(floating)); break;
		case FLOATING_OPERATION_EXTERNAL_LIBM_QUOTIENT_OUT:
			parameter_types.push_back(floating);
			parameter_types.push_back(program_->types.Pointer(integer)); break;
		case FLOATING_OPERATION_EXTERNAL_LIBM_WIDE_SECOND:
			parameter_types.push_back(long_double); break;
		case FLOATING_OPERATION_EXTERNAL_LIBM_TO_INT:
			result = integer; break;
		case FLOATING_OPERATION_EXTERNAL_LIBM_TO_LONG:
			result = program_->types.Fundamental(FUND_LONG_INT); break;
		case FLOATING_OPERATION_EXTERNAL_LIBM_TO_LONG_LONG:
			result = program_->types.Fundamental(FUND_LONG_LONG_INT); break;
		default: break;
		}
	}
	...
	if (IsExternalLibmOperation(intrinsic.operation))
	...
	TypeId target)
{
	...
	ExpressionInfo result = BuildBoundIntrinsicCallShell(
		binding, arguments, result_type);
	const hosted_builtin::FloatingIntrinsicOperation operation =
		hosted_builtin::GetFloatingIntrinsic(kind).operation;
	if (hosted_builtin::IsExternalLibmOperation(operation)) DemandFunction(binding);
	++expression_count_;
	return ApplyTarget(result, target);
}
	...
	ExpressionInfo* result)
{
	...
	for (std::size_t i = 0; i < argument_syntax.size(); ++i)
	{
		...
		// The declared signature already says which operands are floating, and
		// it has to: an entry that takes an exponent destination is floating
		// in one position and a pointer in the next.
		const bool floating_operand = IsFloating(parameter_types[i]);
		if (floating_operand)
		{
			if (!IsFloating(argument.type))
				ThrowSemanticError(
					"floating intrinsic operand is not floating");
			if (intrinsic->operation == FLOATING_OPERATION_EXTERNAL_LIBM)
			{
	...
	const std::string& spelling, NodeId syntax)
{
	...
	// keywords, so they never reach the table above, but they introduce a
	// functional cast the same way: libc++'s charconv tables build their
	// 128-bit constants as `__uint128_t(...)`.  Ask the one place that already
	// maps those spellings instead of restating them here.
	else return HostedSpecifierType(spelling);
...
ExpressionInfo Analyzer::AnalyzeMember(NodeId node, ScopeId scope)
{
	...
	if (found.ordinary == kNoBinding)
	{
		...
		ThrowSemanticError("unknown class member " +
			program_->names.Get(name) + " in " +
			(entity == kNoEntity ? std::string("<none>") :
			 program_->RenderType(program_->entities[entity].type)) +
			(entity != kNoEntity && !program_->entities[entity].complete ?
				" (incomplete)" : ""));
	}
