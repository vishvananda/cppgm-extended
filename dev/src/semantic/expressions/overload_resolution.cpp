	...
	std::vector<BindingId>* candidates)
{
	...
	// Collection is reentrant -- a candidate's constraint can probe an
	// expression that collects candidates of its own -- and the generation
	// these marks are stamped with is shared, so a nested collection leaves
	// the outer one's marks looking unset.  Confirm against the list itself
	// before adding, or the same function is added twice and then ties with
	// itself as an ambiguity.  The list is the overload set, so this stays
	// small.
	for (std::size_t i = 0; i < candidates->size(); ++i)
		if (program_->bindings[(*candidates)[i]].canonical == canonical)
		{
...
void Analyzer::RestoreAssociatedLookup(
	std::vector<EntityId>* entities, std::vector<ScopeId>* scopes)
{
	associated_entities_.swap(*entities);
	associated_scopes_.swap(*scopes);
	// The nested lookup stamped marks with its own generation, so take one no
	// nested lookup has used and re-stamp what was saved.  Reusing the outer
	// generation would let a later lookup collide with a value the nested one
	// already wrote.
	++associated_generation_;
	if (associated_generation_ == 0)
	{
	...
	for (std::size_t i = 0; i < associated_entities_.size(); ++i)
		if (associated_entities_[i] < associated_entity_marks_.size())
			associated_entity_marks_[associated_entities_[i]] =
				associated_generation_;
	for (std::size_t i = 0; i < associated_scopes_.size(); ++i)
		if (associated_scopes_[i] < associated_scope_marks_.size())
			associated_scope_marks_[associated_scopes_[i]] =
				associated_generation_;
}
	...
	const std::vector<NodeId>* argument_syntax)
{
	...
	const std::vector<NodeId>* deduction_syntax =
		untyped_argument ? argument_syntax : 0;
	// This lookup is reentrant.  Completing an argument's class or deducing a
	// candidate can run another argument-dependent lookup -- libc++'s
	// __is_ostreamable probes `<<` while the enclosing `<<` is still
	// collecting -- and the associated sets are shared scratch that the nested
	// call clears.  Without this the outer call resumes holding the nested
	// call's leftovers and loses the very class whose hidden friend it was
	// looking for.
	std::vector<EntityId> outer_entities;
	std::vector<ScopeId> outer_scopes;
	outer_entities.swap(associated_entities_);
	outer_scopes.swap(associated_scopes_);
	Analyzer* const analyzer = this;
	const auto restore_associated = [analyzer, &outer_entities, &outer_scopes]()
	{ analyzer->RestoreAssociatedLookup(&outer_entities, &outer_scopes); };
	ScopedCleanup<decltype(restore_associated)> restore(restore_associated);
	...
	std::vector<CallConversionFact>* argument_conversions, bool quiet)
{
	...
	for (std::size_t other = 0; other < candidates.size(); ++other)
		if (other != champion && viable[other] && !better(champion, other))
		{
			...
			// An ambiguity inside a substitution makes that expression
			// ill-formed, which discards the candidate rather than failing the
			// program.  libc++'s __is_ostreamable depends on it: the partial
			// specialization is keyed on
			// `decltype(declval<_Stream>() << declval<_Tp>(), void())`, and an
			// ambiguous `<<` is exactly how that answers false.
