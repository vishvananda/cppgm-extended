...
struct ExpressionInfo
{
	...
	// Set when the value is not yet known because it reads a variable
	// template specialization whose own arguments are still dependent.
	bool dependent_value;
...
struct FunctionInfo
{
	...
	// The `inline` specifier was written on a declaration: with an in-class
	// definition and constexpr, what makes an inline function (N3485 7.1.2).
...
enum FunctionControlAttribute : std::uint8_t
{
	...
	// A member carrying this attribute is not covered by an explicit
	// instantiation of its class, so every translation unit that uses it
	// instantiates and emits its own copy.
	FUNCTION_CONTROL_EXCLUDE_FROM_EXPLICIT_INSTANTIATION = 64
};
...
struct FunctionTemplatePattern
{
	...
	// An `explicit(...)` condition can depend on the template's own parameters,
	...
	FunctionTemplatePattern()
		: owner(kNoScope), lexical_scope(kNoScope), name(0),
		  ...
		  explicit_specifier_syntax(kNoNode),
		  inline_specifier(false),
