...
// The 512-entry fast cache serves nearly all repeated resolutions.  Keep the
// backing cache bounded so long-running batch compilations do not retain every
// distinct template-argument result from every translation unit.
const std::size_t resolve_template_arguments_cache_max_entries = 4096;
...
std::string non_type_argument_text_for_value(template_api::TemplateServices & services,
                                             const TypePtr & target_type,
...
bool integral_value_fits_non_type_target(const TypePtr & target, long long value)
{
...
bool integral_value_binding_compatible_with_non_type_target(
    const TypePtr & target_base,
...
bool bind_data_member_pointer_non_type_template_argument(
    ...
    const ValueBinding ** out_binding)
{
  const ValueBinding * effective_binding = &binding;
  ClassInfo * binding_owner = binding.owner_class;
  if(binding.kind == ValueBinding::VK_FIELD &&
     binding_owner &&
     !binding_owner->complete) {
    const std::string member_name = binding.name;
    Scope * completed_scope = nullptr;
    if(!binding_owner->type ||
       !services.type_system.complete_named_type_member_scope(
    ...
    std::map<std::string, ValueBinding>::const_iterator completed_member =
        completed_scope->values.find(member_name);
    if(completed_member == completed_scope->values.end()) {
      return false;
    }
    effective_binding = &completed_member->second;
  }

  const ValueBinding & resolved_binding = *effective_binding;
  if(!target_base ||
     target_base->kind != Type::TK_MEMBER_POINTER ||
     !target_base->owner ||
     !target_base->inner ||
     is_function_type(target_base->inner) ||
     resolved_binding.kind != ValueBinding::VK_FIELD ||
     resolved_binding.is_bit_field ||
     !resolved_binding.owner_class ||
     !resolved_binding.owner_class->type ||
     !resolved_binding.type ||
     !type_equals(strip_top_level_cv(resolved_binding.type),
                  strip_top_level_cv(target_base->inner)) ||
     !type_equals(strip_top_level_cv(resolved_binding.owner_class->type),
  ...
  if(!encode_data_member_pointer_template_argument_offset(
         resolved_binding.field_offset,
         encoded_offset)) {
    return false;
  }

  if(out_binding) {
    *out_binding = effective_binding;
  }
  out.kind = TemplateArgument::TA_VALUE;
  out.type = target_type;
  TemplateArgument::RareData & rare = out.mutable_rare();
  rare.function_value = nullptr;
  rare.function_internal_symbol.clear();
  rare.value_binding = effective_binding;
                                          ...
                                          TypePtr & out_function_type)
{
  ...
  out_owner_type = target_base->owner;
  out_function_type = target_base->inner;
  return true;
}
...
bool non_type_function_template_target_type_for_deduction(
    const FunctionTemplateDecl & decl,
    const TypePtr & function_type,
    TypePtr & out)
{
  out.reset();
  TypePtr target_base = strip_top_level_cv(function_type);
...
FunctionBinding * select_non_type_function_template_argument(
    template_api::TemplateServices & services,
    Scope & scope,
    ...
    const TypePtr & function_type)
{
  ...
  for(std::size_t i = 0; i < templates.size(); ++i) {
    ...
    if(!non_type_function_template_target_type_for_deduction(*decl,
                                                             function_type,
                                                             deduction_target_type)) {
      continue;
    }

    template_api::TemplateFunctionDeductionRequest deduction_request;
    deduction_request.decl = decl;
    deduction_request.target_type = deduction_target_type;
    deduction_request.use_scope = &scope;
    deduction_request.resolution_scope = &scope;
    template_api::TemplateFunctionDeductionResult deduction;
    ...
    instantiation_request.decl = decl;
    instantiation_request.arguments = deduction.arguments;
    instantiation_request.use_scope = template_api::make_template_environment(scope);
    instantiation_request.include_body = false;
    instantiation_request.pack_sizes = deduction.pack_sizes;
    instantiation_request.has_pack_sizes = !deduction.pack_sizes.empty();
    FunctionBinding * binding =
        template_api::acquire_function_instantiation(*services.semantic_context,
...
bool member_function_binding_matches_target(FunctionBinding * binding,
                                            const TypePtr & owner_type,
                                            const TypePtr & function_type)
{
  if(!binding ||
    ...
    const TypePtr & function_type)
{
  ...
  for(std::size_t i = 0; i < functions.size(); ++i) {
    ...
    if(!member_function_binding_matches_target(binding, owner_type, function_type)) {
      continue;
    }
    if(selected) {
      return nullptr;
...
bool member_function_template_target_type_for_deduction(
    const FunctionTemplateDecl & decl,
    const TypePtr & function_type,
    TypePtr & out)
{
...
FunctionBinding * select_non_type_member_function_template_argument(
    template_api::TemplateServices & services,
    Scope & scope,
    const std::vector<FunctionTemplateDecl *> & templates,
    const TypePtr & owner_type,
    const TypePtr & function_type)
{
  ...
  for(std::size_t i = 0; i < templates.size(); ++i) {
    ...
    TypePtr deduction_target_type;
    if(!member_function_template_target_type_for_deduction(*decl,
                                                           function_type,
                                                           deduction_target_type)) {
      continue;
    }
    ...
    std::vector<FunctionTemplateDecl *> & templates)
{
  ...
  if(target->class_info) {
    ...
    if(!target_class->reference_members_collected &&
       !target_class->reference_member_collection_in_progress &&
       !target_class->full_member_collection_in_progress) {
      services.semantic_context->ensure_class_reference_members(*target_class);
    }
...
bool lookup_member_pointer_callable_candidates_node(
    template_api::TemplateServices & services,
    Scope & scope,
    const QualifiedName & qualified,
    const CppAstNode & node,
    ...
    TemplateArgument & out)
{
  ...
  if(qualified && (qualified->rooted || !qualified->qualifiers.empty())) {
    ...
    if(non_type_member_function_target_type(target_type,
      ...
      std::vector<FunctionTemplateDecl *> templates;
      try {
        if(lookup_member_pointer_callable_candidates_node(services,
                                                          scope,
                                                          *qualified,
                                                          *operand,
                                                          owner_type,
          ...
          if(!selected) {
            selected =
                select_non_type_member_function_template_argument(services,
                                                                  scope,
                                                                  templates,
                                                                  owner_type,
                                                                  member_function_type);
  ...
  if(qualified && (qualified->rooted || !qualified->qualifiers.empty())) {
    ...
    if(non_type_function_target_type(target_type, explicit_address, function_type)) {
      ...
      std::vector<FunctionTemplateDecl *> templates;
      try {
        if(lookup_qualified_function_callable_candidates_node(services,
                                                              scope,
                                                              *qualified,
                                                              *operand,
                                                              functions,
          ...
          if(!selected) {
            selected =
                select_non_type_function_template_argument(services,
                                                           scope,
                                                           templates,
                                                           function_type);
          }
                                                  ...
                                                  const ValueBinding ** out_binding)
{
  ...
  if(is_identifier_text(trimmed)) {
    ...
    if(binding &&
       bind_value_binding_non_type_template_argument(services,
                                                     scope,
                                                     trimmed,
                                                     target_type,
                                                     target_base,
    ...
    if(explicit_address &&
      ...
      const ValueBinding * binding =
          services.semantic_context ?
              ...
              nullptr;
      if(binding &&
         bind_data_member_pointer_non_type_template_argument(services,
                                                             scope,
                                                             target_type,
                                                             target_base,
                                                             *binding,
...
bool try_resolve_structured_named_non_type_template_argument(
    template_api::TemplateServices & services,
    Scope & scope,
    const TemplateArgumentSyntax * syntax,
    const TypePtr & target_type,
    ...
    const ValueBinding ** out_binding = nullptr)
{
  ...
  if(!qualified ||
     (!qualified->rooted && qualified->qualifiers.empty()) ||
     (expr.qualifier_template_id_syntaxes.empty() &&
      expr.qualifier_type_syntaxes.empty())) {
    return false;
  }

  const std::string fallback_text =
      syntax->text.empty() ? expr.value : syntax->text;
  ...
  try {
    binding =
        semantic_lookup::lookup_qualified_value_binding_node(*services.semantic_context,
                                                             scope,
                                                             *qualified,
                                                             expr);
  } catch(const TemplateSubstitutionFailure &) {
  ...
  TypePtr target_base = strip_top_level_cv(remove_reference_type(target_type));
  return bind_value_binding_non_type_template_argument(
      services,
      scope,
      fallback_text,
      target_type,
                                            ...
                                            const TemplateArgument & argument)
{
  ...
  hash_combine(seed, argument.partial_order_placeholder ? 1 : 0);
                                                   ...
                                                   std::string & pack_name)
{
  ...
  for(Scope * current = &scope; current; current = current->parent) {
    if(current->named_type_packs.count(pack_name) != 0) {
      return false;
    }
    if(current->template_bound_type_pack_names.count(pack_name) != 0) {
      return true;
    }
    if(current->namespace_scope || current->parent == nullptr) {
      break;
...
bool make_shallow_bound_alias_template_id_argument(
    template_api::TemplateServices & services,
    template_api::TemplateEnvironmentHandle scope,
    const TemplateIdSyntax & syntax,
    TypePtr & out)
{
  ...
  dependent_arguments.reserve(arg_texts.size());
  for(std::size_t i = 0; i < arg_texts.size(); ++i) {
    const TemplateArgumentSyntax & source_syntax = syntax.argument_syntaxes[i];
    DependentAliasTemplateArgumentSyntax argument;
    argument.text = trim_space(arg_texts[i]);
    argument.syntax = source_syntax;
    if(argument.syntax.text.empty()) {
      argument.syntax.text = argument.text;
    }
    if(argument.syntax.source_text.empty()) {
      argument.syntax.source_text = argument.text;
    }
    argument.type = source_syntax.resolved_type;
    if(!argument.type && source_syntax.template_id) {
      make_shallow_bound_alias_template_id_argument(
          services, scope, *source_syntax.template_id, argument.type);
    }
    if(!argument.type &&
       is_identifier_text(
           strip_elaborated_type_prefix(trim_space(argument.text)))) {
      lookup_direct_bound_type_argument(raw_scope, argument.text, argument.type);
    }
    if(argument.type && !argument.syntax.resolved_type) {
      argument.syntax.resolved_type = argument.type;
    }
    const bool argument_is_dependent =
        source_syntax.dependent ||
        template_argument_syntax_mentions_template_bound_type_name(
            scope, source_syntax) ||
        (argument.type &&
         template_argument_semantics::type_depends_on_template_parameter(
             services.type_system, argument.type));
    argument.syntax.dependent = argument_is_dependent;
    if(argument.type) {
      // The structured type carries the same nested alias application without
      // duplicating its AST at every later substitution level.  This fast path
      // is disabled for witness capture and tracing above.
      argument.syntax.template_id.reset();
      argument.syntax.type_id.reset();
      argument.syntax.source_type_id.reset();
      argument.syntax.expression.reset();
    }
    has_dependent_argument = has_dependent_argument || argument_is_dependent;
  ...
  const std::string display =
      template_id_text_from_head_and_args(alias_template->name, arg_texts);
  out = make_dependent_alias_type(
      display,
    ...
    std::vector<TemplateArgument> & out)
{
  ...
  for(std::size_t index = 0; index < parameters.size(); ++index) {
    ...
    if(!type && syntax && syntax->template_id &&
       scope_has_template_template_parameter_head(
           raw_scope, syntax->template_id->name.name)) {
      return PERTA_UNSUPPORTED;
    }
...
bool unresolved_non_type_parameter_type_can_remain_dependent(
    ...
    bool parameter_type_can_remain_dependent)
{
  ...
  return parameter_type_can_remain_dependent;
}
  ...
  for(std::size_t i = 0; i < parameters.size(); ++i) {
    ...
    if(i >= inputs.texts.size()) {
      ...
      if(!structured_function_resolved &&
        ...
        bool structured_named_resolved = false;
        long long value = 0;
        std::string eval_error;
        ...
        if(status == template_api::NT_ARG_PARSE_FAILED &&
          ...
          structured_named_resolved =
              try_resolve_structured_named_non_type_template_argument(
                  services,
                  raw_scope,
                  syntax,
                  bound_value_type,
        ...
        if(structured_named_resolved) {
          // arg was resolved from structured qualified value syntax after the
          // normal evaluator failed.
        } else if(status == template_api::NT_ARG_EVALUATED) {
...
struct DeductionContextOps
{
  ...
  bool resolve_type_argument_syntax(Scope & scope,
                                    const TemplateArgumentSyntax & syntax,
                                    TypePtr & out) const
  {
    if(services) {
                                      ...
                                      DecomposedTemplateInstantiation & out)
{
  ...
  if(have_dependent_class_template) {
    ...
    if(source_template) {
      ...
      {
        std::size_t current_argument = 0;
        const std::size_t argument_count = dependent_class_args.size();
        for(std::size_t param_i = 0;
            param_i < source_template->parameters.size();
            ++param_i) {
          const TemplateParameterInfo & parameter =
              source_template->parameters[param_i];
          if(parameter.parameter_pack) {
            std::size_t trailing_non_pack = 0;
            for(std::size_t j = param_i + 1;
                j < source_template->parameters.size();
                ++j) {
              if(!source_template->parameters[j].parameter_pack) {
                ++trailing_non_pack;
              }
            }
            if(argument_count < current_argument + trailing_non_pack) {
      ...
      for(std::size_t i = 0; i < dependent_class_args.size(); ++i) {
        ...
        out.argument_texts.push_back(
            use_normalized_arg_texts ? trim_space(parsed_arg_texts[i]) :
                                       dependent_class_arg_texts[i]);
        const TemplateParameterInfo * parameter =
            parameter_for_argument_index(i);
        if((!parameter ||
            parameter->kind == TemplateParameterInfo::TP_TYPE) &&
           dependent_arg.type) {
          TemplateArgument arg;
...
bool is_partial_order_dependent_non_type_argument(const TemplateArgument & argument)
{
...
bool name_matches_parameter_or_pack_spelling(const std::string & candidate,
                                             const std::string & name)
{
  if(candidate == name) {
    return true;
  }
  return candidate.size() == name.size() + 3 &&
         candidate.compare(0, name.size(), name) == 0 &&
         candidate.compare(name.size(), 3, "...") == 0;
}

bool qualified_name_mentions_parameter_name(const QualifiedName & qualified,
                                            const std::string & name)
{
  if(name.empty()) {
    return false;
  }
  if(name_matches_parameter_or_pack_spelling(qualified.name, name)) {
    return true;
  }
  for(std::size_t i = 0; i < qualified.qualifiers.size(); ++i) {
    if(name_matches_parameter_or_pack_spelling(qualified.qualifiers[i], name)) {
      return true;
    }
  }
  return false;
...
bool template_id_syntax_mentions_parameter_name(const TemplateIdSyntax & syntax,
                                                const std::string & name);

bool ast_node_mentions_parameter_name(const CppAstNode & node,
                                      const std::string & name)
{
  if(name.empty()) {
    return false;
  }
  if(name_matches_parameter_or_pack_spelling(node.value, name)) {
    return true;
  }
  if(node.qualified_name_syntax &&
     qualified_name_mentions_parameter_name(*node.qualified_name_syntax, name)) {
    return true;
  }
  if(node.template_id_syntax &&
  ...
  for(std::size_t i = 0; i < node.qualifier_template_id_syntaxes.size(); ++i) {
    if(template_id_syntax_mentions_parameter_name(
           node.qualifier_template_id_syntaxes[i], name)) {
      return true;
    }
  }
  for(std::size_t i = 0; i < node.qualifier_type_syntaxes.size(); ++i) {
    if(ast_node_mentions_parameter_name(node.qualifier_type_syntaxes[i], name)) {
      return true;
    }
  }
  for(std::size_t i = 0; i < node.exception_type_id_syntaxes.size(); ++i) {
    if(ast_node_mentions_parameter_name(node.exception_type_id_syntaxes[i], name)) {
      return true;
    }
  }
  for(std::size_t i = 0; i < node.alignment_specifier_nodes.size(); ++i) {
    if(ast_node_mentions_parameter_name(node.alignment_specifier_nodes[i], name)) {
      return true;
    }
  }
  for(std::size_t i = 0; i < node.children.size(); ++i) {
    if(ast_node_mentions_parameter_name(node.children[i], name)) {
      return true;
    }
  }
  return false;
...
bool template_argument_syntax_mentions_parameter_name(
    const TemplateArgumentSyntax & syntax,
    const std::string & name)
{
  if(name.empty()) {
  ...
  if(syntax.template_id &&
     template_id_syntax_mentions_parameter_name(*syntax.template_id, name)) {
    return true;
  }
  if(syntax.type_id &&
     ast_node_mentions_parameter_name(*syntax.type_id, name)) {
    return true;
  }
  if(syntax.source_type_id &&
     ast_node_mentions_parameter_name(*syntax.source_type_id, name)) {
    return true;
  }
  if(syntax.expression &&
     ast_node_mentions_parameter_name(*syntax.expression, name)) {
    return true;
  }
  return false;
}
                                                ...
                                                const std::string & name)
{
  ...
  if(qualified_name_mentions_parameter_name(syntax.name, name)) {
    return true;
  }
  for(std::size_t i = 0; i < syntax.qualifier_template_id_syntaxes.size(); ++i) {
    if(template_id_syntax_mentions_parameter_name(
           syntax.qualifier_template_id_syntaxes[i], name)) {
      return true;
    }
  }
  for(std::size_t i = 0; i < syntax.argument_syntaxes.size(); ++i) {
    if(template_argument_syntax_mentions_parameter_name(
           syntax.argument_syntaxes[i], name)) {
      return true;
    }
  }
  return false;
...
bool dependent_argument_syntax_mentions_parameter_name(
    const DependentAliasTemplateArgumentSyntax & argument,
    const std::string & name)
{
  if(name.empty()) {
  ...
  if(template_argument_syntax_mentions_parameter_name(argument.syntax, name)) {
    return true;
  }
...
bool dependent_argument_source_syntax_may_identify_unbound_parameter(
    SemanticContext & ctx,
    const DependentAliasTemplateArgumentSyntax & argument)
{
  if(argument.partial_order_placeholder || argument.dependent_value ||
  ...
  if(argument.has_non_type_value && !argument.dependent_value) {
    return false;
  }
  return true;
}

bool dependent_argument_syntax_mentions_unbound_function_template_parameter(
    SemanticContext & ctx,
    const std::vector<TemplateParameterInfo> & parameters,
    Scope & scope,
    const DependentAliasTemplateArgumentSyntax & argument)
{
  const bool allow_source_syntax =
      dependent_argument_source_syntax_may_identify_unbound_parameter(ctx,
                                                                      argument);
  if(!allow_source_syntax && !argument.value_binding) {
    return false;
  }
  for(std::size_t i = 0; i < parameters.size(); ++i) {
    if(function_template_parameter_has_non_dependent_binding(ctx, scope, parameters[i])) {
      continue;
    }
    if(allow_source_syntax &&
       dependent_argument_syntax_mentions_parameter_name(argument,
                                                        parameters[i].name)) {
      return true;
    }
    for(std::size_t j = 0; j < parameters[i].alternate_names.size(); ++j) {
      if(allow_source_syntax &&
         dependent_argument_syntax_mentions_parameter_name(
             argument, parameters[i].alternate_names[j])) {
        return true;
      }
    }
  }
...
bool template_argument_structured_mentions_parameter_name(
    const TemplateArgument & argument,
    const std::string & name)
{
  if(name.empty()) {
  ...
  if(argument.source_syntax &&
     template_argument_syntax_mentions_parameter_name(*argument.source_syntax, name)) {
    return true;
  }
  if(argument.expression &&
...
bool template_argument_source_syntax_may_identify_unbound_parameter(
    SemanticContext & ctx,
    const TemplateArgument & argument)
{
  if(argument.partial_order_placeholder || argument.dependent) {
    return true;
  }
  if(argument.kind == TemplateArgument::TA_TYPE && argument.type) {
    return template_argument_semantics::type_depends_on_template_parameter(
...
bool template_argument_structured_mentions_unbound_function_template_parameter(
    SemanticContext & ctx,
    const std::vector<TemplateParameterInfo> & parameters,
    Scope & scope,
    const TemplateArgument & argument)
{
  ...
  for(std::size_t i = 0; i < parameters.size(); ++i) {
    ...
    if(allow_source_syntax &&
       template_argument_structured_mentions_parameter_name(argument,
                                                           parameters[i].name)) {
      return true;
    }
    for(std::size_t j = 0; j < parameters[i].alternate_names.size(); ++j) {
      if(allow_source_syntax &&
         template_argument_structured_mentions_parameter_name(
             argument, parameters[i].alternate_names[j])) {
        return true;
      }
    }
    ...
    const TemplateArgument & argument)
{
  ...
  if(!argument.text.empty()) {
    return template_argument_text_mentions_function_template_parameter(
    ...
    const TypePtr & type)
{
  ...
  for(std::size_t i = 0; i < arguments.size(); ++i) {
    ...
    if(arguments[i].has_non_type_value &&
       !arguments[i].dependent_value &&
       !arguments[i].partial_order_placeholder &&
       !arguments[i].value_binding &&
       !arguments[i].function_value &&
       arguments[i].function_internal_symbol.empty()) {
      continue;
    }
    if(!arguments[i].text.empty()) {
      if(template_argument_text_mentions_function_template_parameter(
...
bool template_argument_mentions_unbound_function_template_parameter(
    ...
    const TemplateArgument & argument)
{
  if(template_argument_structured_mentions_unbound_function_template_parameter(
         ctx, parameters, scope, argument)) {
    return true;
  }
  if(argument.kind == TemplateArgument::TA_TYPE && argument.type) {
                                  ...
                                  DeducedPackArgumentMap & out)
{
  ...
  if(parameter.kind == TemplateParameterInfo::TP_TYPE) {
    ...
    TypePtr bound_value_type;
    bool parameter_type_can_remain_dependent = false;
    if(!try_resolve_non_type_template_parameter_type(
           ctx,
           bound_scope,
           parameter,
           bound_value_type,
           &parameter_type_can_remain_dependent)) {
      if(!parameter_type_can_remain_dependent) {
        return false;
      }
                                   ...
                                   const TemplateArgument & rhs)
{
  if(lhs.kind != rhs.kind ||
     ...
     lhs.partial_order_placeholder != rhs.partial_order_placeholder) {
    return false;
  }
...
bool scalarized_ext_vector_argument_names_parameter(
    const DependentAliasTemplateArgumentSyntax & argument,
    const std::string & parameter_name)
{
  const CppAstNode * expression = argument.syntax.expression.get();
  if(expression &&
     expression->kind == CppAstKind::id_expression &&
     expression->value == parameter_name) {
    return true;
  }
  return argument.value_binding &&
         argument.value_binding->name == parameter_name;
}

bool function_template_parameter_is_scalarized_ext_vector_lane_count(
    const FunctionTemplateDecl & decl,
    const std::string & parameter_name)
{
  for(std::size_t i = 0; i < decl.params_pattern.size(); ++i) {
    TypePtr pattern = strip_top_level_cv(decl.params_pattern[i].second);
    if(pattern &&
       (pattern->kind == Type::TK_LVALUE_REFERENCE ||
        pattern->kind == Type::TK_RVALUE_REFERENCE)) {
      pattern = strip_top_level_cv(pattern->inner);
    }
    void * alias_template_decl = nullptr;
    ...
    const AliasTemplateDecl * alias_decl =
        static_cast<const AliasTemplateDecl *>(alias_template_decl);
    const std::size_t lane_index =
        alias_decl->gnu_ext_vector_type_parameter_index;
    if(lane_index >= alias_decl->parameters.size() ||
       lane_index >= alias_arguments.size()) {
      continue;
    }
    if(scalarized_ext_vector_argument_names_parameter(
           alias_arguments[lane_index], parameter_name)) {
      return true;
    }
  }
  return false;
    ...
    std::map<std::string, std::size_t> * pack_sizes_out)
{
  ...
  for(std::size_t i = 0; i < decl.parameters.size(); ++i) {
    ...
    if(explicit_arguments && i < explicit_arguments->fixed_arguments.size()) {
      ...
      } else if(function_template_parameter_is_scalarized_ext_vector_lane_count(
                    decl, decl.parameters[i].name)) {
        // GNU vector attributes are represented as their scalar element type.
        // The supported single-lane form therefore has no semantic lane-count
        // surface from which ordinary template deduction can recover this
        // non-type argument.
        arg.kind = TemplateArgument::TA_VALUE;
        arg.type = bound_value_type;
        arg.value = 1;
        arg.text = typed_non_type_template_argument_text(ctx,
                                                         bound_value_type,
                                                         arg.value);
      } else {
        if(!decl.parameters[i].default_argument ||
  ...
  return template_api::with_template_services(
      ...
      [&](template_api::TemplateServices & services)
      {
        return try_resolve_non_type_template_parameter_type(
            ...
            parameter,
            out,
            can_remain_dependent);
      });
}

TypePtr adjusted_non_type_template_parameter_type_for_resolution(const TypePtr & type)
{
  TypePtr base = strip_top_level_cv(type);
  if(!base) {
    return type;
    ...
    bool * can_remain_dependent)
{
  ...
  if(!template_decl_ast::parse_type_specifier_seq(
    ...
    if(can_remain_dependent) {
      *can_remain_dependent =
          template_argument_semantics::ast_node_syntax_has_template_dependency(
  ...
  if(can_remain_dependent) {
    *can_remain_dependent = remains_dependent;
  }
  if(!resolved_type || remains_dependent) {
    return false;
  }
    ...
    bool * can_remain_dependent)
{
  ...
  try {
    ...
    if(parameter.kind != TemplateParameterInfo::TP_NON_TYPE) {
      ...
      if(!resolved) {
        bool syntax_can_remain_dependent = false;
...
bool make_shallow_bound_alias_template_id_type(
    ...
    TypePtr & out)
{
  return make_shallow_bound_alias_template_id_argument(
...
bool resolve_default_type_template_argument_from_syntax(
    template_api::TemplateServices & services,
    template_api::TemplateTypeSystem & type_system,
    template_api::TemplateEnvironmentHandle scope,
    Scope & default_argument_scope,
    const std::vector<TemplateParameterInfo> & parameters,
    const std::vector<TemplateArgument> & prefix_arguments,
    std::size_t parameter_index,
    ...
    TemplateArgument & out)
{
  ...
  const std::string default_text =
      default_type_argument_text_from_ast(parameter, default_node);
  TemplateArgumentSyntax default_syntax =
      make_default_template_argument_syntax(parameter, default_node, default_text);
  TemplateArgumentSyntax substituted_syntax;
  const TemplateArgumentSyntax * effective_syntax = &default_syntax;
  if(parameter_index != 0) {
    std::vector<TemplateParameterInfo> prefix_parameters(parameters.begin(),
                                                         parameters.begin() +
                                                             parameter_index);
    if(make_substituted_default_template_argument_syntax(services,
                                                         default_argument_scope,
                                                         parameter,
                                                         default_node,
                                                         default_text,
                                                         ...
                                                         prefix_arguments,
                                                         substituted_syntax)) {
      effective_syntax = &substituted_syntax;
    }
  }

  TypePtr type;
  if(!template_argument_semantics::resolve_type_argument_syntax_type(
         services,
         scope,
         *effective_syntax,
         true,
  ...
  template_argument_semantics::resolve_instantiated_dependent_type_if_needed(
      services, scope, type);
  set_resolved_type_template_argument(type_system,
                                      type,
                                      effective_syntax->text,
                                      out);
  attach_template_argument_source_syntax(effective_syntax, out);
  out.source_defaulted = true;
  return true;
}

bool resolve_default_non_type_template_argument_from_syntax(
    template_api::TemplateServices & services,
    template_api::TemplateTypeSystem & type_system,
    template_api::TemplateEnvironmentHandle scope,
    Scope & default_argument_scope,
    ...
    TemplateArgument & out)
{
  ...
  TypePtr bound_value_type;
  if(!try_resolve_non_type_template_parameter_type(
         services, scope, parameter, bound_value_type)) {
    return false;
  }

  std::string default_text = default_argument_expression_text(default_node);
  TemplateArgumentSyntax default_syntax =
      make_default_template_argument_syntax(parameter, default_node, default_text);
  TemplateArgumentSyntax substituted_syntax;
  const TemplateArgumentSyntax * effective_syntax = &default_syntax;
  ...
  if(parameter_index != 0) {
    ...
    if(template_argument_semantics::substitute_expression_node_for_template_arguments(
           ...
           prefix_arguments,
           substituted_default_expression)) {
      default_text = default_argument_expression_text(substituted_default_expression);
      substituted_syntax =
          make_default_template_argument_syntax(parameter,
                                                substituted_default_expression,
                                                default_text);
  ...
  template_argument_semantics::NonTypeArgumentStatus value_status =
      template_argument_semantics::NT_ARG_EVAL_FAILED;
  try {
    value_status = template_argument_semantics::evaluate_non_type_argument_syntax(
        services,
        scope,
        *effective_syntax,
        value,
  ...
  if(value_status == template_argument_semantics::NT_ARG_EVALUATED) {
    out.value = value;
    out.text = typed_non_type_template_argument_text(type_system,
                                                     bound_value_type,
                                                     value);
  } else if(value_status == template_argument_semantics::NT_ARG_DEPENDENT) {
    out.text = effective_syntax->text;
    out.dependent = true;
    if(effective_syntax->expression) {
      out.expression.reset(new CppAstNode(*effective_syntax->expression));
    }
  } else {
    return false;
  }
    ...
    template_api::TemplateEnvironmentHandle default_argument_declaring_scope)
{
  out.clear();
  if(!scope.valid() ||
     !trailing_pack_accepts_argument_count(parameters, arguments.size())) {
    return false;
  }
  ...
  out.reserve(parameters.size() > arguments.size() ?
                  parameters.size() :
                  arguments.size());
  for(std::size_t parameter_index = 0;
      parameter_index < parameters.size();
    ...
    if(parameter.parameter_pack) {
      ...
      if(arguments.size() < argument_index + trailing_non_pack) {
        out.clear();
        return false;
      }
      const std::size_t pack_end = arguments.size() - trailing_non_pack;
      while(argument_index < pack_end) {
        out.push_back(arguments[argument_index]);
        ++argument_index;
      }
      continue;
    }
    ...
    const CppAstNode & default_node = parameter.default_argument->children[0];
    TemplateArgument argument;
    const template_argument_semantics::ScopedDefaultTemplateArgumentEvaluation
        default_argument_evaluation;
    bool resolved = false;
    if(parameter.kind == TemplateParameterInfo::TP_TYPE) {
      resolved = resolve_default_type_template_argument_from_syntax(
          services,
          type_system,
          default_argument_env,
          *default_argument_scope,
      ...
      resolved = resolve_default_non_type_template_argument_from_syntax(
          services,
          type_system,
          default_argument_env,
          *default_argument_scope,
  ...
  if(argument_index != arguments.size() ||
     !template_arguments_fully_bind_parameters(parameters, out)) {
    out.clear();
    return false;
  }
  return true;
                               ...
                               TemplateArgument & out)
{
  ...
  if(parameter.kind == TemplateParameterInfo::TP_NON_TYPE) {
    ...
    const auto try_resolve_structured_named_non_type =
        [&]() -> bool
    {
      const ValueBinding * named_binding = nullptr;
      if(!try_resolve_structured_named_non_type_template_argument(
             services,
             raw_argument_scope,
             syntax,
             bound_value_type,
    ...
    const auto prefer_structured_function_non_type =
        [&]() -> bool
    {
      if(!syntax || !syntax->expression) {
        return false;
    ...
    const bool structured_function_non_type_first =
        prefer_structured_function_non_type();
    if(structured_function_non_type_first &&
       try_resolve_function_non_type_template_argument_syntax(
    ...
    if(value_status != template_api::NT_ARG_EVALUATED &&
       value_status != template_api::NT_ARG_DEPENDENT &&
       try_resolve_structured_named_non_type()) {
      return true;
    }
    if(value_status != template_api::NT_ARG_EVALUATED &&
  ...
  if(parameter.kind == TemplateParameterInfo::TP_TYPE &&
    ...
    out.type = make_named(unbound_pack_name,
                          "dependent type " + unbound_pack_name,
                          true);
    out.text = trimmed;
    out.dependent = true;
    attach_template_argument_source_syntax(syntax, out);
    ...
    template_api::TemplateEnvironmentHandle default_argument_declaring_scope)
{
      ...
      [&]() -> void
  {
    if(cache_enabled) {
      ...
      if(cache->size() < resolve_template_arguments_cache_max_entries) {
                                   ...
                                   bool allow_actual_base_deduction = true)
{
  ...
  if(pattern_base->kind == Type::TK_NAMED) {
    ...
    if(!parameter) {
      ...
      {
        raw = strip_elaborated_type_prefix(trim_space(raw));
        ...
        for(std::size_t i = 0; i < sizeof(prefixes) / sizeof(prefixes[0]); ++i) {
          ...
          if(raw.compare(0, prefix.size(), prefix) == 0) {
            raw = trim_space(raw.substr(prefix.size()));
            break;
          }
        }
        return strip_elaborated_type_prefix(trim_space(raw));
      };
      const auto record_direct_type_parameter_deduction =
          [&](const std::string & raw_name, const TypePtr & deduced_type) -> bool
      {
        const std::string name = normalize_direct_type_parameter_name(raw_name);
        const TemplateParameterInfo * direct =
            find_template_parameter_by_name(parameters, name);
        if(!direct) {
          direct = find_template_parameter(parameters, name);
        }
        if(!direct || direct->kind != TemplateParameterInfo::TP_TYPE) {
          return false;
        }
        DeducedTypeMap::iterator found = deduced_types.find(direct->name);
        if(found == deduced_types.end()) {
          deduced_types[direct->name] = deduced_type;
          return true;
        }
        return type_equals(found->second, deduced_type);
      };
      const auto try_deduce_dependent_named_declarator_shell =
          [&](std::string text) -> bool
      {
        text = normalize_direct_type_parameter_name(text);
        if(text.size() >= 2 &&
           text.compare(text.size() - 2, 2, "&&") == 0) {
          if(actual_base->kind != Type::TK_RVALUE_REFERENCE) {
            return false;
          }
          return record_direct_type_parameter_deduction(
        ...
        if(!text.empty() && text[text.size() - 1] == '*') {
          if(actual_base->kind != Type::TK_POINTER) {
            return false;
          }
          return record_direct_type_parameter_deduction(
              text.substr(0, text.size() - 1),
    ...
    if(deduction_scope) {
          ...
          [&parameters](const std::string & raw_arg) -> DirectTemplateParameterMatch
      {
        ...
        if(direct &&
           (direct->kind == TemplateParameterInfo::TP_TYPE ||
            ...
            direct->kind == TemplateParameterInfo::TP_TEMPLATE_TEMPLATE)) {
              ...
              TypePtr & actual_arg_type) -> bool
      {
        ...
        if(structured_actual) {
          ...
          actual_arg_type = structured_actual->type;
          return true;
        }
              ...
              TemplateArgument & resolved_arg) -> bool
      {
        ...
        if(parameter.kind == TemplateParameterInfo::TP_TYPE) {
          ...
          if(!lookup_actual_arg_type(actual_arg,
                                     structured_actual,
                                     actual_arg_type)) {
            return false;
          }
        ...
        if(parameter.kind == TemplateParameterInfo::TP_NON_TYPE) {
          if(structured_actual) {
            ...
            if(structured_actual->dependent &&
               !is_partial_order_dependent_non_type_argument(*structured_actual)) {
              return false;
            }
            resolved_arg = *structured_actual;
            return true;
          ...
          if(!resolve_template_argument_for_deduction(ctx,
                                                      actual_scope,
                                                      *deduction_scope,
                                                      parameter,
                                                      actual_arg,
          ...
          return !resolved_arg.dependent ||
                 is_partial_order_dependent_non_type_argument(resolved_arg);
        }

        if(parameter.kind == TemplateParameterInfo::TP_TEMPLATE_TEMPLATE) {
          if(structured_actual) {
            if(structured_actual->kind != TemplateArgument::TA_CLASS_TEMPLATE &&
               structured_actual->kind != TemplateArgument::TA_ALIAS_TEMPLATE) {
              return false;
            }
            resolved_arg = *structured_actual;
            return true;
          ...
          return resolved_arg.kind == TemplateArgument::TA_CLASS_TEMPLATE ||
                 resolved_arg.kind == TemplateArgument::TA_ALIAS_TEMPLATE;
        }
      ...
      const auto type_parameter_name_matches =
          [](const TemplateParameterInfo & parameter,
             const std::string & name) -> bool
      {
        if(name == parameter.name || name == parameter.placeholder_key) {
          return true;
        }
        return std::find(parameter.alternate_names.begin(),
                         parameter.alternate_names.end(),
                         name) != parameter.alternate_names.end();
      };
      const auto syntax_is_direct_type_parameter =
          [&](const TemplateArgumentSyntax & syntax,
              const TemplateParameterInfo & parameter,
              bool pack_expansion) -> bool
      {
        if(parameter.kind != TemplateParameterInfo::TP_TYPE) {
          return true;
        }
        if(syntax.type_id) {
          ...
          if(type_id.kind == CppAstKind::type_id &&
            ...
            if(specifiers.kind == CppAstKind::type_specifier_seq &&
               specifiers.children.size() == 1 &&
               specifiers.children[0].kind == CppAstKind::type_name &&
               !specifiers.children[0].template_id_syntax) {
              const QualifiedName * qualified =
                  cppast_qualified_name_syntax(specifiers.children[0]);
              if(!qualified || (!qualified->rooted && qualified->qualifiers.empty())) {
                const std::string name = strip_elaborated_type_prefix(
                    trim_space(specifiers.children[0].value.empty() ?
                                   node_text(specifiers.children[0]) :
                                   specifiers.children[0].value));
                return type_parameter_name_matches(parameter, name) &&
                       syntax.pack_expansion == pack_expansion;
              }
            }
          }
          if(!pack_expansion) {
        ...
        std::string text =
            !syntax.source_text.empty() ? syntax.source_text : syntax.text;
        ...
        return type_parameter_name_matches(parameter, text);
      };
      const auto direct_match_is_structurally_direct =
          [&](const DirectTemplateParameterMatch & direct_match,
              const TemplateArgument * structured_pattern) -> bool
      {
      ...
      const auto resolve_bound_dependent_type_for_deduction =
          [&](const TypePtr & type, Scope * scope) -> TypePtr
      {
        if(!type || !scope) {
          return type;
        }
        TypePtr resolved;
        if(deduction_ops.resolve_instantiated_dependent_type(*scope,
                                                             type,
                                                             resolved) &&
           resolved) {
          return resolved;
        }
        const auto normalize_bound_type_name =
            [](std::string raw) -> std::string
        {
          raw = strip_elaborated_type_prefix(trim_space(raw));
          static const char * prefixes[] = {
        ...
        const auto lookup_exact_bound_type =
            [&](const std::string & raw) -> TypePtr
        {
          const std::string name = normalize_bound_type_name(raw);
          if(name.empty() ||
             !callsemantic_internal::is_identifier_text(name)) {
            return TypePtr();
          }
          for(Scope * current = scope; current; current = current->parent) {
            if(current->namespace_scope || current->parent == nullptr) {
          ...
          TypePtr looked_up = deduction_ops.lookup_type(*scope, name, true);
          if(looked_up) {
            return looked_up;
          }
          return TypePtr();
        ...
        const bool had_top_cv =
            top_level_cv_flags(type, cv_inner, top_const, top_volatile) &&
            (top_const || top_volatile) &&
            cv_inner;
        TypePtr named = had_top_cv ? cv_inner : strip_top_level_cv(type);
        if(named && named->kind == Type::TK_NAMED) {
          std::vector<std::string> candidate_names;
          const auto append_candidate =
              [&](const std::string & raw) -> void
          {
            const std::string name = normalize_bound_type_name(raw);
            if(name.empty() ||
               std::find(candidate_names.begin(),
                         candidate_names.end(),
                         name) != candidate_names.end()) {
              return;
            }
          ...
          for(std::size_t i = 0; i < candidate_names.size(); ++i) {
            TypePtr bound = lookup_exact_bound_type(candidate_names[i]);
            if(bound) {
              return had_top_cv ? apply_cv(bound, top_const, top_volatile) :
                                  bound;
            }
          }
        }
      ...
      const auto apply_top_level_cv_from_syntax =
          [](const TemplateArgumentSyntax & syntax,
             const TypePtr & type) -> TypePtr
      {
        ...
        bool syntax_const = false;
        bool syntax_volatile = false;
        const auto scan_type_id =
            [&](const std::shared_ptr<CppAstNode> & type_id) -> void
        {
          if(!type_id ||
             type_id->kind != CppAstKind::type_id) {
            return;
          ...
          for(std::size_t child_i = 0;
              child_i < type_id->children.size();
              ++child_i) {
            const CppAstNode & container = type_id->children[child_i];
            if(container.kind != CppAstKind::type_specifier_seq &&
               container.kind != CppAstKind::abstract_declarator) {
              continue;
            }
            for(std::size_t i = 0; i < container.children.size(); ++i) {
              const CppAstNode & child = container.children[i];
              if(child.kind != CppAstKind::cv_qualifier || !child.has_token) {
                continue;
              }
              if(child.simple_type == KW_CONST) {
                syntax_const = true;
              } else if(child.simple_type == KW_VOLATILE) {
                syntax_volatile = true;
              }
            }
          }
        };
        scan_type_id(syntax.type_id);
        scan_type_id(syntax.source_type_id);
        if(!syntax_const && !syntax_volatile) {
          std::string text =
              !syntax.source_text.empty() ? syntax.source_text : syntax.text;
          text = trim_space(text);
          const auto is_identifier_char =
              [](char ch) -> bool
          {
          ...
          while(consumed) {
            consumed = false;
            if(text.compare(0, 5, "const") == 0 &&
               (text.size() == 5 ||
                !is_identifier_char(text[5]))) {
              syntax_const = true;
              text = trim_space(text.substr(5));
              consumed = true;
            } else if(text.compare(0, 8, "volatile") == 0 &&
                      (text.size() == 8 ||
                       !is_identifier_char(text[8]))) {
              syntax_volatile = true;
              text = trim_space(text.substr(8));
              consumed = true;
            }
          }
        }
        ...
        TypePtr base = type;
        TypePtr cv_inner;
        bool existing_const = false;
        bool existing_volatile = false;
        if(top_level_cv_flags(type, cv_inner, existing_const, existing_volatile) &&
           (existing_const || existing_volatile) &&
           cv_inner) {
          base = cv_inner;
        }
        return apply_cv(base,
                        existing_const || syntax_const,
                        existing_volatile || syntax_volatile);
      };
      const auto deduction_type_for_structured_argument =
      ...
      if(parsed_matching_template_ids || parsed_template_template_deduction) {
        ...
        std::vector<std::string> pattern_canonical_args_storage;
        template_api::TemplateNamedTypeMetadata pattern_canonical_metadata;
        bool pattern_uses_canonical_bound_pack_arguments = false;
                ...
                bool require_matching_default_provenance) -> bool
        {
          ...
          while(keep > min_keep) {
            ...
            if(!matches_default &&
               ...
               !parameter.default_argument->children.empty()) {
              std::vector<TemplateParameterInfo> prefix_parameters(
                  source_template->parameters.begin(),
                  source_template->parameters.begin() + keep - 1);
              std::vector<TemplateArgument> prefix_arguments;
                prefix_arguments.reserve(keep - 1);
                bool prefix_ok = true;
                for(std::size_t prefix_i = 0; prefix_i + 1 < keep; ++prefix_i) {
                  if(have_structured_args) {
                    prefix_arguments.push_back((*structured_args)[prefix_i]);
                    continue;
                  }
                  TemplateArgument resolved_prefix;
                  bool resolved = false;
                  try {
                    resolved =
                        resolve_template_argument_for_deduction(
                            ...
                            *deduction_scope,
                            *deduction_scope,
                            source_template->parameters[prefix_i],
                            args[prefix_i],
                            resolved_prefix);
                  } catch(const TemplateSubstitutionFailure &) {
                    resolved = false;
                  ...
                  prefix_arguments.push_back(resolved_prefix);
                }
              if(prefix_ok) {
                ...
                const std::string default_text =
                    parameter.kind == TemplateParameterInfo::TP_NON_TYPE ?
                        default_argument_expression_text(child) :
                        default_type_argument_text_from_ast(parameter, child);
                TemplateArgumentSyntax substituted_default;
                if(make_substituted_default_template_argument_syntax(
                       ctx,
                       *deduction_scope,
                       parameter,
                  ...
                  const std::string normalized_arg =
                      strip_elaborated_type_prefix(trim_space(args[keep - 1]));
                  const std::string normalized_default =
                      strip_elaborated_type_prefix(
                          trim_space(substituted_default.text));
                  matches_default =
                      !normalized_arg.empty() &&
                  ...
                  if(!matches_default) {
                    ...
                    if(structured_arg) {
                      current_argument = *structured_arg;
                    ...
                    matches_default =
                        resolved_default &&
                        resolved_current &&
                        template_arguments_equivalent(default_argument,
                                                      current_argument);
                  }
                }
              }
        ...
        const std::vector<TemplateArgument> * pattern_structured_args =
            ...
            pattern_instantiation.arguments.size() == pattern_args_ptr->size() ?
                &pattern_instantiation.arguments :
                nullptr;
        const bool pattern_trimmed_source_defaulted_tail =
        ...
        bool source_has_concrete_bound_type_pack = false;
        for(std::size_t i = 0; i < pattern_args_ptr->size(); ++i) {
          const DirectTemplateParameterMatch source_match =
              find_direct_template_parameter_from_arg((*pattern_args_ptr)[i]);
          if(source_match.parameter ||
             !source_match.pack_expansion ||
             source_match.name.empty()) {
            continue;
          }
          const std::vector<TypePtr> * bound_types = nullptr;
          for(Scope * current = deduction_scope;
              current;
              current = current->parent) {
            std::map<std::string, std::vector<TypePtr> >::const_iterator found =
                current->named_type_packs.find(source_match.name);
            if(found != current->named_type_packs.end()) {
              bound_types = &found->second;
              break;
            }
            if(current->namespace_scope || current->parent == nullptr) {
              break;
          ...
          bool concrete = bound_types != nullptr;
          for(std::size_t j = 0; concrete && j < bound_types->size(); ++j) {
            concrete = (*bound_types)[j] &&
                       !deduction_ops.type_depends((*bound_types)[j]);
          }
          if(concrete) {
            source_has_concrete_bound_type_pack = true;
            break;
          }
        }
        if(source_has_concrete_bound_type_pack &&
           deduction_ops.describe_named_type(pattern_base,
                                             pattern_canonical_metadata) &&
           pattern_canonical_metadata.source_template ==
               pattern_instantiation.source_template &&
           template_arguments_are_all_types(
               pattern_canonical_metadata.instantiation_arguments)) {
          std::size_t canonical_function_pack_count = 0;
          pattern_canonical_args_storage.reserve(
              pattern_canonical_metadata.instantiation_arguments.size());
          for(std::size_t i = 0;
              i < pattern_canonical_metadata.instantiation_arguments.size();
              ++i) {
            const TemplateArgument & argument =
                pattern_canonical_metadata.instantiation_arguments[i];
            std::string argument_text =
                trim_space(deduction_ops.type_argument_text(argument.type));
            DirectTemplateParameterMatch typed_match =
                find_direct_template_parameter_from_arg(argument_text);
            if(typed_match.parameter &&
               typed_match.parameter->kind == TemplateParameterInfo::TP_TYPE &&
               typed_match.parameter->parameter_pack) {
              ++canonical_function_pack_count;
              if(argument_text.size() < 3 ||
                 argument_text.compare(argument_text.size() - 3, 3, "...") != 0) {
                argument_text += "...";
              }
            }
            pattern_canonical_args_storage.push_back(argument_text);
          }
        ...
        if(trim_source_defaulted_tail(actual_template_decl,
                                      *actual_match_args,
        ...
        if(actual_template_decl &&
          ...
          const std::vector<std::string> & visible_actual_args =
              *actual_match_args;
        ...
        const auto template_entities_match =
            [](const TemplateArgument & lhs, const TemplateArgument & rhs) -> bool
        {
          if(lhs.kind != rhs.kind) {
            return false;
        ...
        const auto structured_template_arguments_match =
            [&](const TemplateArgument & pattern_argument,
                const TemplateArgument & actual_argument) -> bool
        {
          if(pattern_argument.kind == TemplateArgument::TA_TYPE &&
             actual_argument.kind == TemplateArgument::TA_TYPE &&
             pattern_argument.type &&
             actual_argument.type) {
            const TypePtr pattern_arg_type =
                deduction_type_for_structured_argument(pattern_argument,
                                                       deduction_scope);
            const TypePtr actual_arg_type =
          ...
          if(pattern_argument.kind == TemplateArgument::TA_VALUE &&
            ...
            if(pattern_argument.dependent || actual_argument.dependent) {
              return pattern_argument.partial_order_placeholder &&
                     actual_argument.partial_order_placeholder;
            }
            const bool types_match =
                (!pattern_argument.type && !actual_argument.type) ||
                (pattern_argument.type &&
                 actual_argument.type &&
                 type_equals(pattern_argument.type, actual_argument.type));
            return types_match && pattern_argument.value == actual_argument.value;
          }
          if(pattern_argument.kind == TemplateArgument::TA_CLASS_TEMPLATE ||
            ...
            return template_entities_match(pattern_argument, actual_argument);
          }
          return false;
        };
        ...
        for(std::size_t i = 0; i < pattern_args.size(); ++i) {
          ...
          if(structured_pattern &&
             ...
             structured_pattern->type) {
            const TypePtr pattern_arg_type =
                deduction_type_for_structured_argument(*structured_pattern,
                                                       deduction_scope);
            TypePtr actual_arg_type =
                structured_actual &&
            ...
            if(!deduce_template_argument_impl(ctx,
              ...
              if(parser_trace::enabled("template.resolve")) {
                std::ostringstream trace;
                trace << "deduce-template-arg-structured-type-fail pattern="
                      << pattern_arg
                      << " resolved-pattern="
                      << deduction_ops.type_argument_text(pattern_arg_type)
                      << " actual=" << actual_arg
                      << " actual-type="
                      << deduction_ops.type_argument_text(actual_arg_type);
                parser_trace::note("template.resolve", std::string(), trace.str());
              }
              return false;
            }
          ...
          if(direct_match.parameter &&
             direct_match_is_structurally_direct(direct_match, structured_pattern)) {
            ...
            } else if(direct_match.parameter->kind ==
                          TemplateParameterInfo::TP_TEMPLATE_TEMPLATE) {
              if(!record_deduced_template_pack_arguments(
                     *direct_match.parameter,
                     std::vector<TemplateArgument>(1, actual_direct_argument))) {
                return false;
              }
...

          {
            ...
            if(pattern_arg_type) {
              pattern_arg_type =
                  resolve_bound_dependent_type_for_deduction(pattern_arg_type,
                                                             deduction_scope);
              TypePtr actual_arg_type =
                  structured_actual &&
                  structured_actual->kind == TemplateArgument::TA_TYPE ?
              ...
              if(!deduce_template_argument_impl(ctx,
                ...
                if(parser_trace::enabled("template.resolve")) {
                  std::ostringstream trace;
                  trace << "deduce-template-arg-scope-type-fail pattern="
                        << pattern_arg
                        << " resolved-pattern="
                        << deduction_ops.type_argument_text(pattern_arg_type)
                        << " actual=" << actual_arg
          ...
          if(structured_pattern &&
             structured_actual &&
             structured_template_arguments_match(*structured_pattern,
                                                 *structured_actual)) {
            ++actual_index;
            continue;
          }
          if(parser_trace::enabled("template.resolve")) {
            std::ostringstream trace;
            trace << "deduce-template-arg-structured-match-fail pattern="
                  << pattern_arg
                  << " actual=" << actual_arg;
            parser_trace::note("template.resolve", std::string(), trace.str());
          }
...
bool deduce_template_argument(
    ...
    std::map<std::string, std::vector<TemplateArgument> > & deduced_pack_arguments,
...
{
  DeducedValueMap deduced_values;
  return deduce_template_argument(services,
                                  parameters,
                                  pattern,
                                  actual,
                                  deduced,
