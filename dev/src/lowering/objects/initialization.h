...
protected:
	...
	{
		Derived& derived = static_cast<Derived&>(*this);
		// An empty type carries no value, so copying one has nothing to
		// write.  Writing its padding byte anyway is not merely wasted work:
		// an empty member declared [[no_unique_address]] shares its address
		// with the member it overlaps, and the byte lands on that member.
		if (derived.IsClassObjectType(type))
		{
			const TypeRecord& object = derived.program_.types.Get(
				derived.ExpressionObjectType(type));
			if (object.entity != kNoEntity &&
