...
std::string SourceLocationTokenView::source_location_for_last_qualified_member_start_before(
    const std::string & location,
    ...
    std::string & member_name) const
{
  ...
  return source_location_for_last_qualified_member_start_before_in_range(
      location, before_location, 0, token_sequence_->size(), member_name);
}

std::string
SourceLocationTokenView::source_location_for_last_qualified_member_start_before_in_range(
    const std::string & location,
    const std::string & before_location,
    std::size_t begin,
    std::size_t end,
...
{
  member_name.clear();
  if(!(token_sequence_ && source_locations_)) {
    return std::string();
  }
  const ParsedSourceLocation base = parse_source_location(
      template_api::normalize_template_witness_source_location(location));
  const ParsedSourceLocation before = parse_source_location(
      template_api::normalize_template_witness_source_location(before_location));
  if(!base.valid ||
     !before.valid ||
     base.file != before.file ||
     base.line != before.line) {
    return std::string();
  }
  ...
  for(std::size_t i = begin; i + 1 < end; ++i) {
    ...
    if(scope_location.column < base.column) {
      continue;
    }
    if(scope_location.column >= before.column) {
      break;
    }
    if(scope_token.source == "<") {
      ++angle_depth;
      continue;
    }
    if(scope_token.source == ">") {
    ...
    if(scope_token.source != "::" || angle_depth != 0) {
      continue;
    }
    const RecogToken & member_token = token_sequence_->peek(i + 1);
    if(member_token.is_eof() ||
       member_token.location_id == 0 ||
       !member_token.is_identifier()) {
      continue;
    }
    const ParsedSourceLocation member_location = parse_source_location(
        template_api::normalize_template_witness_source_location(
            source_locations_->describe(member_token.location_id)));
    if(member_location.valid &&
       member_location.file == base.file &&
       scope_location.line == base.line &&
       member_location.line == base.line &&
       member_location.column == scope_location.column + 2 &&
       member_location.column < before.column) {
      member_name = member_token.source;
    }
  }
  return member_name.empty() ?
      std::string() :
      source_location_for_qualified_member_start_on_line_in_range(
          location, member_name, begin, end);
}
...
void SourceLocationTokenView::collect_nested_template_id_source_occurrences(
    ...
    std::vector<NestedSourceTemplateIdOccurrence> & out) const
{
  ...
  if(!skip_this) {
    NestedSourceTemplateIdOccurrence occurrence;
    occurrence.text =
        trim_space(compact_template_id_token_text(name_begin,
                                                  close_index,
                                                  close_chars));
    occurrence.template_name =
        trim_space(compact_token_range_text(name_begin, open_index));
    occurrence.location =
        template_api::normalize_template_witness_source_location(
            source_location_for_token_index(anchor_index));
    out.push_back(occurrence);
  }

  for(std::size_t i = open_index + 1; i < close_index; ++i) {
    if(token_sequence_->peek(i).source != "<") {
      continue;
    }
    std::size_t nested_close = 0;
    int nested_close_chars = 0;
    collect_nested_template_id_source_occurrences(i,
                                                 false,
                                                 same_line_only,
                                                 out);
    if(matching_template_close_token(i,
...
SourceLocationTokenView::nested_template_id_source_occurrences_at_location(
    const std::string & location,
    bool same_line_only) const
{
  std::vector<NestedSourceTemplateIdOccurrence> out;
  std::size_t start_index = 0;
  if(!token_index_at_source_location(location, start_index)) {
    return out;
  }
  std::size_t open_index = 0;
  if(!find_next_token_source_on_same_line(start_index, "<", open_index)) {
    return out;
  }
  collect_nested_template_id_source_occurrences(open_index,
                                                true,
                                                same_line_only,
                                                out);
  return out;
...
std::vector<NestedSourceTemplateIdOccurrence>
SourceLocationTokenView::template_id_source_occurrences_after_location(
    const std::string & location,
    bool same_line_only) const
{
  std::vector<NestedSourceTemplateIdOccurrence> out;
  ...
  if(token_index_at_source_location(location, start_index) &&
    ...
    for(std::size_t i = 0; i < token_count; ++i) {
      ...
      if(parsed.line < target.line) {
        continue;
      }
      if(parsed.line == target.line && parsed.column < target.column) {
        continue;
      }
      start_location = parsed;
      scan_from = i;
      found = true;
      break;
    }
  ...
  const std::size_t token_count = token_sequence_->size();
  for(std::size_t i = scan_from; i < token_count; ++i) {
    const RecogToken & token = token_sequence_->peek(i);
    if(token.is_eof()) {
      break;
    }
    ...
    if(same_line_only && parsed.line > start_location.line) {
      break;
    }
    if(same_line_only && parsed.line != start_location.line) {
      continue;
