...
public:
  ...
  bool value_argument_template_parameter_witness_name(
      const TemplateArgument & argument,
      const std::vector<TemplateParameterInfo> & canonical_parameters,
      std::string & out) const
  {
    ...
    const std::string text = trim_space(argument.text);
    if(text.empty()) {
      return false;
    }
    for(std::size_t i = 0; i < canonical_parameters.size(); ++i) {
      ...
      if(text == parameter.name ||
         std::find(parameter.alternate_names.begin(),
                   ...
                   text) != parameter.alternate_names.end()) {
        out = stable_template_parameter_witness_name(parameter, i);
        return true;
      }
    }
  ...
  bool witness_identifier_char(char ch) const
  {
    return std::isalnum(static_cast<unsigned char>(ch)) || ch == '_';
  }

  bool replace_identifier_token(std::string & text,
                                ...
                                const std::string & to) const
  {
    ...
    while((pos = text.find(from, pos)) != std::string::npos) {
      const bool left_boundary =
          pos == 0 || !witness_identifier_char(text[pos - 1]);
      const std::size_t end = pos + from.size();
      const bool right_boundary =
          end >= text.size() || !witness_identifier_char(text[end]);
  ...
  bool canonicalize_template_parameter_names_in_text(
      std::string & text,
      const std::vector<TemplateParameterInfo> & canonical_parameters) const
  {
    ...
    for(std::size_t i = 0; i < canonical_parameters.size(); ++i) {
      ...
      const std::string stable_name =
          stable_template_parameter_witness_name(parameter, i);
      changed =
          replace_identifier_token(text, parameter.name, stable_name) || changed;
      for(std::size_t j = 0; j < parameter.alternate_names.size(); ++j) {
        changed =
            replace_identifier_token(text,
      ...
      const std::vector<TemplateParameterInfo> & canonical_parameters) const
  {
    ...
    for(std::size_t i = 0; i < count; ++i) {
      ...
      if(arguments[i].kind != TemplateArgument::TA_TYPE) {
        if(arguments[i].kind == TemplateArgument::TA_VALUE) {
          ...
          if(value_argument_template_parameter_witness_name(
                 arguments[i],
                 canonical_parameters,
                 canonical)) {
    ...
    for(std::size_t i = 0; i < bindings.size(); ++i) {
      for(std::size_t j = 0; j < bindings[i].pack_arguments.size(); ++j) {
        canonicalize_template_parameter_names_in_text(
            bindings[i].pack_arguments[j],
            canonical_parameters);
      }
      if(bindings[i].pack_binding && !bindings[i].pack_arguments.empty()) {
        ...
        for(std::size_t j = 0; j < bindings[i].pack_arguments.size(); ++j) {
          ...
          pack_text << bindings[i].pack_arguments[j];
        }
        pack_text << ">";
        bindings[i].arg = pack_text.str();
      } else {
        canonicalize_template_parameter_names_in_text(bindings[i].arg,
                                                      canonical_parameters);
      }
    }
  }
        ...
        if(source_capture_enabled &&
          ...
          for(std::size_t i = 0; i < decl.partial_specializations.size(); ++i) {
            ...
            if(partial.class_node &&
               (partial.class_node == specialization.class_node ||
                (resolved_info &&
                 partial.class_node == resolved_info->template_output_node))) {
          ...
          if(selected_partial) {
            const witness::TemplateWitnessSourceAnchor selected_decl_anchor =
                class_use_selected_decl_anchor(ctx, &decl, specialization);
            const std::string chosen_use_location =
                template_api::normalize_template_witness_source_location(
                    use_location);
            if(!chosen_use_location.empty() &&
               chosen_use_location != selected_decl_anchor.location) {
              const std::string qualified_template_name =
                  resolved_info && !resolved_info->qualified_name.empty() ?
                      strip_trailing_top_level_template_arguments(
                          resolved_info->qualified_name) :
                      decl.name;
