...
// the enclosing class template parameters when it is probed through void_t
// expression SFINAE. The use-site may have unrelated template parameters with
// the same names.
...
static_assert(can_query<const executor&, const context_t&>::value,
    "plain context query must be valid");
static_assert(query_free<executor, context_as_t<int&>>::value,
    "hidden friend query must be found by expression SFINAE");
static_assert(can_query<executor, context_as_t<int&>>::value,
    "context_as query must be valid");
