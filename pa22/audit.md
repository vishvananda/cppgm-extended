...
|---|---|---|
| Requirements | PA22 requires nonvirtual multi-base layout/access/lifecycle, member-pointer target plus adjustment, target-based bool semantics, ambiguity rejection, and single-vptr `dynamic_cast<void*>`. | closed |
...
file-audit findings remain for PA22. Checked-fixture compatibility intentionally
...
- Added a compile-fail regression for ambiguous member-pointer object
  application and isolated base-path ownership in a PA22 source module.
...
measured PA22 ownership surface.

## Validation

- `perl scripts/cppgm_file_audit.pl --stage pa22 --paths dev/src`: pass, no
  fatal issues (20 inherited advisory warnings).
- `make test-pa22`: pass, 97/97.
- `make test-report-through-pa22`: pass, 3,814/3,814 tests and 27/27 stages.
- `git diff --check`: pass.
