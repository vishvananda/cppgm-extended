# PA38 Ladder Fix Process

This document defines the active maintainer workflow for the current PA38 fix
phase after the base scaffold landed.

It is intentionally similar to the bootstrap frontier process, but the active
frontier here is the public `pa1` through `pa9` PA38 self-host ladder rather
than the full compiler-source bootstrap set.

Use this process to close the current PA38 ladder issues without turning PA38
into a dumping ground for unrelated large reproductions.

## Goal

Turn each PA38 ladder failure into:
...
- Keep PA38 integration proofs small. PA38 should prove the ladder boundary,
...
Use the `pa38` wrapper directly.

Build or test one checkpoint:

```sh
make -C pa38 pptoken-self CXX=../dev/cppgm++ CPPGM_HOST_CXX=/path/to/host-cxx
make -C pa38 test-pptoken CXX=../dev/cppgm++ CPPGM_HOST_CXX=/path/to/host-cxx
```

Expand the ladder through a checkpoint:

```sh
make -C pa38 through-pa1 CXX=../dev/cppgm++ CPPGM_HOST_CXX=/path/to/host-cxx
make -C pa38 test-through-pa1 CXX=../dev/cppgm++ CPPGM_HOST_CXX=/path/to/host-cxx
make -C pa38 test-through-pa2 CXX=../dev/cppgm++ CPPGM_HOST_CXX=/path/to/host-cxx
...
make -C pa38 test-through-pa9 CXX=../dev/cppgm++ CPPGM_HOST_CXX=/path/to/host-cxx
```
...
our compiler output rather than in the PA38 wrapper or assignment harness.

The PA38 wrapper now defaults to separate object roots for the two modes:

- host-control builds use `../obj/pa38/host/...`
- self-host builds use `../obj/pa38/selfhost/...`
...
If the self-host ladder has already warmed `obj/pa38/selfhost`, reuse that same object tree when
...
For reduced `PA32`-style link or runtime-contract failures, use the link-root-cause mode before
...
- host object/link/runtime interop belongs in `PA32`
- hosted-header and vendor-compatibility belongs in `PA34`
- only true ladder-only integration proofs should remain in `pa38`
...
- a bug first exposed by `test-through-pa4` may still belong in `pa32` if the
  reduced reproducer is a host object/link/runtime interop failure
- a bug first exposed while self-hosting `macro-self` may belong in `pa34` if
  the reduced reproducer is a hosted compile-compatibility failure, or in
  `pa35` if the hosted code only breaks after emitted definitions have to link
  and run
...
Do not leave a giant PA38-only proof as the main regression unless there is no
...
shape moved the durable regression into `pa18` instead of leaving a large PA38-only bootstrap
proof.
...
3. rerun `make -C pa38 test-through-paN ...` for the active ladder rung
...
Stay in this PA38 ladder process while the failure is still honestly exposed by
...
- the full `pa1` through `pa9` PA38 ladder is green through
  `make -C pa38 test-through-pa9 ...`
