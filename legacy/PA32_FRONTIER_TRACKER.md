# PA32 Frontier Tracker

Current checkpoint:

- root `make test-report` is now at `1502 / 1503`
- all `< pa32` suites are green again
- targeted PA32 compile fixes now landed for:
...
4. add earlier-PA regression tests if the root cause is not PA32-specific
...
- prefer PA32 batch harnesses for the current frontier
