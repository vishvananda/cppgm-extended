# PA12 Full-Stage Audit

## Findings

The independent audit started from a clean 241/241 PA12 and 1,677/1,677
through-stage baseline. Review of `spec.md`, the PA12 contract, all 33 PA12
...
1. A fixed non-trivial class array had O(bound) PA7 destructor nodes and
...
- PA7 now represents a large fixed member-array destructor obligation with one
...
- PA11 constructor-array lowering emits a forward index loop above the inline
...
- PA11 destructor lowering flattens bounded multidimensional arrays and emits a
  reverse index loop above the same limit. Local, temporary, member, normal,
  and unwind destructor actions all use that owner.
- PA7 now memoizes the automatic-destructor lowering decision by canonical
...
destruction, preprocessing retains source-offset tokens and PA5 parses the
declarations once into its assignment-mandated `SyntaxArena`. PA6 interns the
...
`TypeId`. PA7 class completion selects the canonical `Elem` special members
...
`SemanticGraphView`. PA10-PA12 creates function-owned typed slots, operands,
...
parsed back by the production PA12 path.

At the boundaries, immutable source plus PA5 tokens/syntax are live while the
single PA6/PA7 graph is built; during synchronous lowering that graph and the
growing typed program overlap. The first overlap is required by the staged
PA5 contract and is the documented adaptation from `spec.md`'s future
...
instantiation and template-aware PA12 class-value semantics are outside the
...
- `make test-pa12`: 241/241 pass (228 handout plus 13 course audit tests).
- `perl scripts/cppgm_file_audit.pl --stage pa12 --paths dev/src`: pass with
  11 non-fatal established header-division advisories.
- `make test-report-through-pa12`: 1,677/1,677 tests and 17/17 stages pass.
