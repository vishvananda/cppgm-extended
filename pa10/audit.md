...
3. Recursive PA10 top-level, statement/control, and switch walks overflowed on valid
   deep inputs.
4. PA10 retained `Symbol&` across nested lowering that could append string-literal
   symbols; ASan proved a heap use-after-free.
5. PA7 retained type-table references and parameter spans across recursive type
...
7. PA7 eagerly interned a full qualified prefix for every nested namespace, retaining
...
- Symbol mutation is performed by stable ID after nested lowering. PA7 snapshots
  compact type records and copies function parameter IDs before recursive analysis.
- PA6 owns a revisioned derived lookup cache invalidated by bindings, namespaces,
...
- PA10 is divided into typed records, model/index implementation, graph lowering, and
...
- Combined ASan+UBSan fixture runs: PA6 68+2, PA7 166+8, PA10 108; all pass.
- Process-only `strace`: initial compiler `execve`, `exit_group(0)`, no child process.
- `make test-report-through-pa10`: pass, 1,145/1,145 tests and 15/15 stages.
...
| Procedural lowering | Pass after audit fixes | Complete PA10 contract, explicit CFG worklist, flat side tables, no recursive PA10 depth failure |
| Full-stage final audit | Pass | UAFs removed across PA7/PA10 ownership boundaries; semantic quadratics removed; file audit, sanitizers, self-containment trace, and 1,145-test report clean |
